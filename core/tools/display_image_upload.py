...
# CLI line buffer is 8192 bytes; each raw byte → 2 hex chars; overhead = 22 bytes
# ("display-image chunk \r\n").  Keep a margin so near-boundary USB packet framing
# on macOS cannot push the line over the buffer edge.
_MAX_CHUNK_BYTES = 4000
...
@click.option(
    ...
    default=None,
    metavar="LEVEL",
    help="Set display backlight level (0-255) before the image is shown",
)
def main(
    port: str,
    image: str,
    ...
    <serial-port>  Linux: /dev/ttyACM1, macOS: /dev/cu.usbmodemXXXX, Windows: COM3
    ...
    click.echo(f"Image: {len(raw)} bytes → {port}")

    try:
        upload_image(port, raw, chunk_size, timings, backlight)
