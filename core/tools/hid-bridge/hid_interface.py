...
class HIDInterface:
    ...
    def create_device(self):
        ...
        logger.log_uhid_event(
            "UHID_CREATE2",
            f"name='{name.decode('ascii')}' phys='{phys.decode('ascii')}' uniq=0x{uniq.hex()} rd_size={len(rd_data)} bus=0x{bus:04x} vendor=0x{vendor:04x} product=0x{product:04x} version=0x{version:04x} country=0x{country:04x} rd_data=0x{rd_data.hex()}",
        )
