...
JSON_FILE = ROOT / "core" / "embed" / "sec" / "tropic" / "config" / "tropic_configs.json"
MD_FILE = ROOT / "docs" / "core" / "misc" / "tropic_configs.md"

HEADER = f"""This file is auto-generated from the JSON configuration in `{JSON_FILE.name}`.
Do not edit this file directly. Instead, edit the JSON and run `{Path(__file__).name}`.

TODO: Adjust the configuration to match the revision of the provisioned tropics.
"""

def print_value_lines(value_lines, md_file):
    col_count = max(len(line) for line in value_lines)
    col_len = [max(len(line[i]) for line in value_lines) for i in range(col_count)]

    line = value_lines[0]
    print(f"| {' | '.join(line[i].ljust(col_len[i]) for i in range(col_count))} |", file=md_file)
    print(f"|{'|'.join('-'*(col_len[i]+2) for i in range(col_count))}|", file=md_file)
    for line in value_lines[1:]:
        print(f"| {' | '.join(line[i].ljust(col_len[i]) for i in range(col_count))} |", file=md_file)
    print("", file=md_file)


def json_to_markdown(json_filename, md_filename):
            ...
            for category, details in config.items():
                if category != 'uap':
                    ...
                    for value_name, values in details['setting'].items():
                        ...
                        value_lines.append([f"{value_name.upper()} (bit {bit})", f"{val} {comment.upper()}"])
                    print_value_lines(value_lines, md_file)


                else:
                    for category, info in details.get('pairing_key_0', {}).items():
                        print(f"## {category.upper()} ({info['address']})", file=md_file)
                        value_lines = [['Setting'] + [f"Pairing Key {i}" for i in range(4)]]
                        for value_name in info['setting'].keys():
                            value_line = [f"{value_name.upper()}"]
                            for key in range(4):
                                values = details[f'pairing_key_{key}'][category]['setting'][value_name]
                                bit = values['bit']
