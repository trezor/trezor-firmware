...
def print_config(config: dict, md_file: TextIOWrapper) -> None:
...
def json_to_markdown(json_filename: Path, md_filename: Path) -> None:
    ...
    with open(md_filename, "w") as md_file:
        ...
        for config_name, configs in (
            ("irreversible_configurations", data["irreversible_configurations"]),
            ("reversible_configurations", data["reversible_configurations"]),
        ):
            print(f"# {config_name.replace('_', ' ').upper()}", file=md_file)
            for config_version in configs:
                print(f"## VERSION {config_version['version']}", file=md_file)
                print_config(config_version["config"], md_file)
            print("", file=md_file)
