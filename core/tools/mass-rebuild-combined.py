...
class EnvInfo:
    ...
    def __init__(self, model: str, repo_dir: Path) -> None:
        ...
        if (repo_dir / "core" / "embed" / "xtask").exists():
            ...
            self.firmware_extra = [
                Path(f"core/build-xtask/artifacts/{model.upper()}/firmware.elf"),
                Path(f"core/build-xtask/artifacts/{model.upper()}/kernel.elf"),
            ]
        else:
            ...
            self.firmware_extra = [
                Path("core/build/firmware/firmware.elf"),
                Path("core/build/kernel/kernel.elf"),
            ]
...
def copy_renamed(source: Path, target_dir: Path, version: str) -> None:
    if not source.is_file():
        LOG.error(f"Does not exist: {source}")
        return

    dest_file = target_dir / f"{source.stem}-{version}{''.join(source.suffixes)}"
    source.copy(dest_file)
    LOG.debug(f"Saved {dest_file}")
...
class Config:
    def __init__(
        ...
        LOG.info(f"Model: {self.model.upper()}")

        self.output_dir = Path(output_dir).resolve() / self.model
        LOG.info(f"Output dir: {self.output_dir}")
        self.log_dir = self.output_dir / "logs"
        self.log_dir.mkdir(exist_ok=True, parents=True)

        self.boardloader_options = boardloader_options
    ...
    def build_firmware(
        ...
        with log_commands(log_file), in_worktree(git_tag) as workdir:
            ...
            if self.keep_elf:
                for f in t.firmware_extra:
                    copy_renamed(f, self.output_dir, fw_version)
    ...
    def build_combined(
        ...
        artifacts_dir = (
            repo_root / "core" / "build-xtask" / "artifacts" / self.model.upper()
        )
        ...
        with contextlib.chdir(repo_root), log_commands(log_file):
            sh(f'nix-shell --run "uv run xtask combine -m {self.model} firmware"')
        (artifacts_dir / "combined-firmware.bin").copy(combined_bin)

    def rebuild_boardloader(self) -> Path:
        boardloader_options = ensure_options(
            self.boardloader_options,
            self.model,
            self.output_dir / "boardloader-options.txt",
        )
        boardloader_version = BOARDLOADER_VERSIONS[self.model]
        boardloader_bin = self.output_dir / f"boardloader-{boardloader_version}.bin"
        if not boardloader_bin.exists():
            log_file = self.log_dir / f"boardloader-{boardloader_version}.txt"
            self.build_boardloader(
                boardloader_version, boardloader_options, boardloader_bin, log_file
            )
        else:
    ...
    def rebuild_bootloaders(
        ...
        for bl_version in blv:
            ...
            if bl_version in BLACKLIST_BOOTLOADERS:
                ...
                continue
            log_file = self.log_dir / f"bootloader-{bl_version}.txt"
            self.build_bootloader(
                bl_version, bootloader_options, bootloader_bin, log_file
            )
    ...
    def rebuild_firmware(
        ...
        firmware_options = ensure_options(
            self.firmware_options, self.model, self.output_dir / "firmware-options.txt"
        )
        ...
        for fw_version, bl_version in bfw:
            firmware_bin = self.output_dir / f"firmware-{fw_version}.bin"
            bootloader_bin = self.output_dir / f"bootloader-{bl_version}.bin"
            if bl_version in BLACKLIST_BOOTLOADERS:
                LOG.warning(
                ...
                continue
            elif (self.model, fw_version) in BLACKLIST_FIRMWARE:
                LOG.warning(f"Skipping blacklisted firmware {fw_version}")
                continue
            elif not firmware_bin.exists():
                log_file = self.log_dir / f"firmware-{fw_version}.txt"
                self.build_firmware(
                    fw_version,
                    firmware_options,
                    bootloader_bin,
            ...
            combined_bin = self.output_dir / f"combined-firmware-{fw_version}.bin"
            if not combined_bin.exists():
                log_file = self.log_dir / f"combined-firmware-{fw_version}.txt"
                self.build_combined(
                    boardloader_bin,
                    bootloader_bin,
                    firmware_bin,
...
def main(
    ...
    config.rebuild_bootloaders(releases)
    config.rebuild_firmware(releases, boardloader_bin)
