...
class FaceProcessor:
    ...
    def write_foreign_json(self, upper_cased=False) -> None:
        for lang, language_chars in all_languages.items():
            ...
            all_lang_chars = list(language_chars) + [chr(i) for i in range(MIN_GLYPH, MAX_GLYPH + 1)]
            ...
            for left_char in all_lang_chars:
                ...
                for right_char in all_lang_chars:
                    ...
                    if left_idx in range(MIN_GLYPH, MAX_GLYPH + 1) and right_idx in range(MIN_GLYPH, MAX_GLYPH + 1):
                        continue
                    ...
                    if kerning.x != 0:
                        ...
                        fontdata["kernings"].append((left_char, right_char, kerning.x // 64))
