...
def _escaped_bytes(data: bytes) -> str:
    r"""Render `data` as a `\xNN`-escaped string literal."""
...
@click.option(
    ...
    type=int,
    default=16,
    help="Bytes per line of the wrapped output.",
)
def print_keys(width: int) -> None:
    r"""Print the dev ML-DSA-44 public keys as hex and as `\x`-escaped bytes."""
    if width < 1:
        raise click.ClickException("--width must be at least 1")

    public_keys = [
        mldsa.MLDSA44PrivateKey.from_seed_bytes(seed).public_key()
        for seed in DEV_SIGNING_SEEDS
    ]
    for slot, key in enumerate(public_keys):
        raw = key.public_bytes_raw()
        chunks = [raw[i : i + width] for i in range(0, len(raw), width)]

        print(f"Public key {slot} ({len(raw)} bytes):")
        print("  hex:")
        for chunk in chunks:
            print(f"    {chunk.hex()}")
        print("  escaped:")
        for chunk in chunks:
            print(f'    "{_escaped_bytes(chunk)}"')
        print()
...
@click.option(
    ...
    help=(
        "Directory into which the apps are copied (renamed to "
        "'{app_id}_{version}.tapp'), and the RootPacket(s) and proofs are written."
    ),
)
def build_rootpackets(apps: tuple[Path, ...], out_dir: Path) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    print_apps(*apps)
    # Parse and copy each app into `out_dir` under its canonical name; the returned
    # AppInfo.path points at the copy, so proofs land next to it in `out_dir`.
    app_infos = _get_app_info_list(out_dir, *apps)

    tree_0 = _get_tree(_filter_apps(app_infos, 0), app_ring=0)
    tree_1 = _get_tree(_filter_apps(app_infos, 1), app_ring=1)
    tree_2 = _get_tree(_filter_apps(app_infos, 2), app_ring=2)

    if tree_0 is not None:
        _create_proofs(_filter_apps(app_infos, 0), tree_0, store=True)
    ...
    if tree_1 or tree_2:
        ...
        if tree_1:
            ...
            _create_proofs(_filter_apps(app_infos, 1), tree_1, store=True)

        if tree_2:
            root_2 = tree_2.get_root_hash()
            _create_proofs(_filter_apps(app_infos, 2), tree_2, store=True)
...
def _app_filename(header: AppHeader) -> str:
...
def _get_app_info_list(
    ...
    *app_paths: Path,
    ...
    for path, data in _get_files_iterator(*app_paths):
        ...
        print(f"App {path} copied to {out_path}")
        app_info_list.append(AppInfo(out_path, digest, app_ring))
...
def generate_apps(out_dir: Path | None, test: int, app_ring: int) -> None:
    ...
    for i in range(test):
        ...
        app_image = _generate_app_image(
            app_ring=app_ring,
            id=f"mockapp_r{app_ring}_{idx_str}.trezor.com",
