...
def get_binding_context(descriptor):
    ...
    pairs = ", ".join(
        "(%d, unhexlify(%r))" % (d.chainId, normalize_address(d.address))
...
def get_formatter(field, func_inputs):
    ...
    if fmt == FieldFormat.ADDRESS_NAME:
        ...
        if params and params.tokenPath:
            ...
            token_path = token_path if isinstance(token_path, str) else repr(token_path)
            return "TokenAmountFormatter(token_path=%s)" % token_path
        else:
...
from apps.ethereum.clear_signing import (
    ...
    for sig, display_format in descriptor.display.formats.items():
        ...
        print(
            '        func_sig=unhexlify("%s"),%s'
            % (sig, "  # %s" % func_sig if func_sig else "")
        )
