...
class TrezorEmulator:
    ...
    @classmethod
    async def create(cls, emulator_port, char_tx, char_rx):
        localhost = "127.0.0.1"
        data_transport, data_protocol = await TrezorUDP.create(localhost, emulator_port)

        char_rx.write_callback = data_protocol.write
        ...
        event_read_task = asyncio.create_task(obj.print_commands())
...
async def emulator_main(bus_address: str, emulator_port: int):
    ...
    emulator = await TrezorEmulator.create(emulator_port, char_tx, char_rx)
