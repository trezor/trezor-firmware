...
pub unsafe fn call_protected<F, T>(
    mut func: F,
) -> Result<T, Error>
where
    F: FnMut() -> T,
...
macro_rules! catch_exception {
    ...
    (@real safe $func:expr => { $($name:ident: $arg:expr,)* }) => {{
        ...
        // call the closure
        #[allow(unused_unsafe)]
        unsafe { $crate::runtime::call_protected(closure) }
    }};
    ...
    (@zip $kind:ident $func:expr => { $arg:expr, $($nextarg:tt)* } { $name:ident $($nextname:ident)* } { $($collected:tt)* }) => {
        $crate::runtime::catch_exception!(@zip $kind $func => { $($nextarg)* } { $($nextname)* } { $($collected)* $name: $arg, })
    };
    // zip terminator: {}
    (@zip $kind:ident $func:expr => { } { $name:ident $($nextname:ident)* } { $($collected:tt)* }) => {
        $crate::runtime::catch_exception!(@real $kind $func => { $($collected)* })
    };

    // start of the zip, with a list of new names
    (@start $kind:ident $func:expr => { $($arg:expr,)* }) => {
        $crate::runtime::catch_exception!(
            @zip $kind $func =>
                { $($arg,)* }
    ...
    (unsafe { $func:expr } => { $($arg:expr),* }) => {
        $crate::runtime::catch_exception!(@start not_safe $func => { $($arg,)* })
    };
    ...
    ($func:expr => { $($arg:expr),* }) => {
        $crate::runtime::catch_exception!(@start safe $func => { $($arg,)* })
    };
    ...
    (unsafe { $func:expr }) => { $crate::runtime::catch_exception!(@real not_safe $func => { }) };
    ($func:expr) => { $crate::runtime::catch_exception!(@real safe $func => { }) };
