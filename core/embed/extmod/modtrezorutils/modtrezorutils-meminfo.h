...
static void print_type(
    FILE *out, const char *typename, const char *shortval, const void *ptr, bool end)
{
    ...
    fprintf(out, "{\"type\": \"%s\", \"alloc\": %ld, \"ptr\": \"%p\"", typename, size, ptr);
    if (shortval) {
...
void dump_map_as_values(FILE *out, const void *const owner, const mp_map_t *map)
{
...
void dump_function(FILE *out, const mp_obj_fun_bc_t *func)
{
    ...
    fprintf(out, ",\n\"qstr_table_ptr\": \"%p\"", func->context->constants.qstr_table);
    fprintf(
        out, ",\n\"qstr_table_alloc\": \"%ld\"",
...
void dump_value_opt(FILE *out, mp_const_obj_t value, bool eval_short)
{
    ...
    if (mp_obj_is_str_or_bytes(value)) {
    ...
    else if (mp_obj_is_type(value, &mp_type_fun_bc) || mp_obj_is_type(value, &mp_type_gen_wrap)) {
        ...
        mp_obj_is_type(value, &mp_type_staticmethod) ||
...
void dump_qstr_pool(FILE *out, const qstr_pool_t *pool)
{
    ...
    fprintf(out, ", \"qstrs\": [\n");
    for (const char *const *q = pool->qstrs, *const *q_top = pool->qstrs + pool->len; q < q_top;
         q++) {
        if (q < (q_top - 1))
            fprintf(out, "\"%s\",\n", Q_GET_DATA(*q));
        else
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mod_trezorutils_meminfo_obj, mod_trezorutils_meminfo);
