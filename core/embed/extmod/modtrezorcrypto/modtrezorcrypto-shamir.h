...
mp_obj_t mod_trezorcrypto_shamir_interpolate(mp_obj_t shares, mp_obj_t x)
{
    ...
    if (shamir_interpolate(
            (uint8_t *)vstr.buf, x_uint8, share_indices, share_values, share_count, value_len) !=
        true) {
        vstr_clear(&vstr);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(
    mod_trezorcrypto_shamir_interpolate_obj, mod_trezorcrypto_shamir_interpolate);
...
STATIC MP_DEFINE_CONST_DICT(mod_trezorcrypto_shamir_globals, mod_trezorcrypto_shamir_globals_table);
