...
///     CIP-3).
///     """
STATIC mp_obj_t mod_trezorcrypto_cardano_derive_icarus(size_t n_args, const mp_obj_t *args)
{
    ...
    const int res = secret_from_entropy_cardano_icarus(
        (const uint8_t *)ppassphrase, phrase.len, mnemonic_bits, mnemonic_bytes_used,
        (uint8_t *)vstr.buf, callback);

    ui_wait_callback = mp_const_none;

    if (res != 1) {
        mp_raise_msg(&mp_type_RuntimeError, "Unexpected failure in Icarus derivation.");
    }
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(
    mod_trezorcrypto_cardano_derive_icarus_obj, 3, 4, mod_trezorcrypto_cardano_derive_icarus);
...
STATIC mp_obj_t mod_trezorcrypto_from_secret(mp_obj_t secret)
{
    ...
    if (res != 1) {
        ...
        mp_raise_msg(&mp_type_RuntimeError, "Unexpected failure in constructing Cardano node.");
    }
    o->fingerprint = hdnode_fingerprint(&o->hdnode);
    return MP_OBJ_FROM_PTR(o);
}

STATIC MP_DEFINE_CONST_FUN_OBJ_1(mod_trezorcrypto_from_secret_obj, mod_trezorcrypto_from_secret);
...
STATIC mp_obj_t mod_trezorcrypto_from_seed_slip23(mp_obj_t seed)
{
    ...
    if (res != 1) {
        mp_raise_msg(&mp_type_RuntimeError, "Unexpected failure in SLIP-23 derivation.");
    }
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(
    mod_trezorcrypto_from_seed_slip23_obj, mod_trezorcrypto_from_seed_slip23);
...
STATIC mp_obj_t mod_trezorcrypto_from_seed_ledger(mp_obj_t seed)
{
    ...
    if (res != 1) {
        mp_raise_msg(&mp_type_RuntimeError, "Unexpected failure in Ledger derivation.");
    }
    res = hdnode_from_secret_cardano(secret, &hdnode);
    if (res != 1) {
        mp_raise_msg(&mp_type_RuntimeError, "Unexpected failure in constructing Cardano node.");
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(
    mod_trezorcrypto_from_seed_ledger_obj, mod_trezorcrypto_from_seed_ledger);
...
STATIC const mp_rom_map_elem_t mod_trezorcrypto_cardano_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_from_secret), MP_ROM_PTR(&mod_trezorcrypto_from_secret_obj)},
    {MP_ROM_QSTR(MP_QSTR_from_seed_slip23), MP_ROM_PTR(&mod_trezorcrypto_from_seed_slip23_obj)},
    {MP_ROM_QSTR(MP_QSTR_from_seed_ledger), MP_ROM_PTR(&mod_trezorcrypto_from_seed_ledger_obj)},
};
STATIC MP_DEFINE_CONST_DICT(
    mod_trezorcrypto_cardano_globals, mod_trezorcrypto_cardano_globals_table);
