...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(
    mod_trezorcrypto_random_uniform_obj, mod_trezorcrypto_random_uniform);
...
///     """
STATIC mp_obj_t mod_trezorcrypto_random_bytes(size_t n_args, const mp_obj_t *args)
{
    ...
    if (n_args > 1 && mp_obj_is_true(args[1])) {
        if (!optiga_random_buffer((uint8_t *)vstr.buf, len)) {
            vstr_clear(&vstr);
            mp_raise_msg(&mp_type_RuntimeError, "Failed to get randomness from Optiga.");
        }
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(
    mod_trezorcrypto_random_bytes_obj, 1, 2, mod_trezorcrypto_random_bytes);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(
    mod_trezorcrypto_random_shuffle_obj, mod_trezorcrypto_random_shuffle);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(
    mod_trezorcrypto_random_reseed_obj, mod_trezorcrypto_random_reseed);
#endif
...
#ifdef TREZOR_EMULATOR
    {MP_ROM_QSTR(MP_QSTR_reseed), MP_ROM_PTR(&mod_trezorcrypto_random_reseed_obj)},
#endif
};
