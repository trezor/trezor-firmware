...
STATIC MP_DEFINE_CONST_FUN_OBJ_0(
    mod_trezorcrypto_nist256p1_generate_secret_obj, mod_trezorcrypto_nist256p1_generate_secret);
...
STATIC mp_obj_t mod_trezorcrypto_nist256p1_publickey(size_t n_args, const mp_obj_t *args)
{
    ...
    if (compressed) {
        vstr_init_len(&pk, 33);
        if (ecdsa_get_public_key33(&nist256p1, (const uint8_t *)sk.buf, (uint8_t *)pk.buf) != 0) {
            mp_raise_ValueError("Invalid secret key");
        }
    } else {
        vstr_init_len(&pk, 65);
        if (ecdsa_get_public_key65(&nist256p1, (const uint8_t *)sk.buf, (uint8_t *)pk.buf) != 0) {
            mp_raise_ValueError("Invalid secret key");
        }
    }
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(
    mod_trezorcrypto_nist256p1_publickey_obj, 1, 2, mod_trezorcrypto_nist256p1_publickey);
...
STATIC mp_obj_t mod_trezorcrypto_nist256p1_sign(size_t n_args, const mp_obj_t *args)
{
    ...
    if (0 != ecdsa_sign_digest(
                 &nist256p1, (const uint8_t *)sk.buf, (const uint8_t *)dig.buf,
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(
    mod_trezorcrypto_nist256p1_sign_obj, 2, 3, mod_trezorcrypto_nist256p1_sign);
...
///     """
STATIC mp_obj_t
mod_trezorcrypto_nist256p1_verify(mp_obj_t public_key, mp_obj_t signature, mp_obj_t digest)
{
    ...
    return mp_obj_new_bool(
        0 == ecdsa_verify_digest(
                 &nist256p1, (const uint8_t *)pk.buf, (const uint8_t *)sig.buf + offset,
                 (const uint8_t *)dig.buf));
}
STATIC MP_DEFINE_CONST_FUN_OBJ_3(
    mod_trezorcrypto_nist256p1_verify_obj, mod_trezorcrypto_nist256p1_verify);
...
STATIC mp_obj_t mod_trezorcrypto_nist256p1_verify_recover(mp_obj_t signature, mp_obj_t digest)
{
    ...
    vstr_t pk = {0};
    vstr_init_len(&pk, 65);
    if (0 == ecdsa_recover_pub_from_sig(
                 &nist256p1, (uint8_t *)pk.buf, (const uint8_t *)sig.buf + 1,
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(
    mod_trezorcrypto_nist256p1_verify_recover_obj, mod_trezorcrypto_nist256p1_verify_recover);
...
STATIC mp_obj_t mod_trezorcrypto_nist256p1_multiply(mp_obj_t secret_key, mp_obj_t public_key)
{
    ...
    if (0 !=
        ecdh_multiply(
            &nist256p1, (const uint8_t *)sk.buf, (const uint8_t *)pk.buf, (uint8_t *)out.buf)) {
        vstr_clear(&out);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(
    mod_trezorcrypto_nist256p1_multiply_obj, mod_trezorcrypto_nist256p1_multiply);
...
STATIC MP_DEFINE_CONST_DICT(
    mod_trezorcrypto_nist256p1_globals, mod_trezorcrypto_nist256p1_globals_table);
