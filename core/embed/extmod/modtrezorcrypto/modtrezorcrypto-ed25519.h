...
STATIC MP_DEFINE_CONST_FUN_OBJ_0(
    mod_trezorcrypto_ed25519_generate_secret_obj, mod_trezorcrypto_ed25519_generate_secret);
...
STATIC mp_obj_t mod_trezorcrypto_ed25519_publickey(mp_obj_t secret_key)
{
    ...
    ed25519_publickey(*(const ed25519_secret_key *)sk.buf, *(ed25519_public_key *)pk.buf);
    return mp_obj_new_str_from_vstr(&mp_type_bytes, &pk);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(
    mod_trezorcrypto_ed25519_publickey_obj, mod_trezorcrypto_ed25519_publickey);
...
STATIC mp_obj_t mod_trezorcrypto_ed25519_sign(size_t n_args, const mp_obj_t *args)
{
    ...
    if (n_args == 3) {
        ...
        if (memcmp(hash_func.buf, "keccak", sizeof("keccak")) == 0) {
            ed25519_sign_keccak(
                msg.buf, msg.len, *(const ed25519_secret_key *)sk.buf,
        ...
        ed25519_sign(
            msg.buf, msg.len, *(const ed25519_secret_key *)sk.buf, *(ed25519_signature *)sig.buf);
    }
...
///     """
STATIC mp_obj_t
mod_trezorcrypto_ed25519_sign_ext(mp_obj_t secret_key, mp_obj_t secret_extension, mp_obj_t message)
{
    ...
    ed25519_sign_ext(
        msg.buf, msg.len, *(const ed25519_secret_key *)sk.buf,
        *(const ed25519_secret_key *)skext.buf, *(ed25519_signature *)sig.buf);
    return mp_obj_new_str_from_vstr(&mp_type_bytes, &sig);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_3(
    mod_trezorcrypto_ed25519_sign_ext_obj, mod_trezorcrypto_ed25519_sign_ext);
...
///     """
STATIC mp_obj_t
mod_trezorcrypto_ed25519_verify(mp_obj_t public_key, mp_obj_t signature, mp_obj_t message)
{
    ...
    return (0 == ed25519_sign_open(
                     msg.buf, msg.len, *(const ed25519_public_key *)pk.buf,
                     *(const ed25519_signature *)sig.buf))
               ? mp_const_true
               : mp_const_false;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_3(
    mod_trezorcrypto_ed25519_verify_obj, mod_trezorcrypto_ed25519_verify);
...
STATIC mp_obj_t mod_trezorcrypto_ed25519_cosi_combine_publickeys(mp_obj_t public_keys)
{
    ...
    vstr_t pk = {0};
    vstr_init_len(&pk, sizeof(ed25519_public_key));
    if (0 != ed25519_cosi_combine_publickeys(*(ed25519_public_key *)pk.buf, pks, pklen)) {
        vstr_clear(&pk);
...
STATIC mp_obj_t mod_trezorcrypto_ed25519_cosi_combine_signatures(mp_obj_t R, mp_obj_t signatures)
{
    ...
    ed25519_cosi_combine_signatures(
        *(ed25519_signature *)sig.buf, *(const ed25519_public_key *)sigR.buf, sigs, siglen);
    return mp_obj_new_str_from_vstr(&mp_type_bytes, &sig);
}
...
STATIC mp_obj_t mod_trezorcrypto_ed25519_cosi_commit()
{
    ...
    ed25519_cosi_commit(*(ed25519_secret_key *)nonce.buf, *(ed25519_public_key *)commitment.buf);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_0(
    mod_trezorcrypto_ed25519_cosi_commit_obj, mod_trezorcrypto_ed25519_cosi_commit);
...
STATIC mp_obj_t mod_trezorcrypto_ed25519_cosi_sign(size_t n_args, const mp_obj_t *args)
{
    ...
    if (0 != ed25519_cosi_sign(
                 msg.buf, msg.len, *(const ed25519_secret_key *)sk.buf,
                 *(const ed25519_secret_key *)nonce.buf, *(const ed25519_public_key *)sigR.buf,
                 *(const ed25519_secret_key *)pk.buf, *(ed25519_cosi_signature *)sig.buf)) {
        vstr_clear(&sig);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(
    mod_trezorcrypto_ed25519_cosi_sign_obj, 5, 5, mod_trezorcrypto_ed25519_cosi_sign);
...
STATIC const mp_rom_map_elem_t mod_trezorcrypto_ed25519_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_cosi_commit), MP_ROM_PTR(&mod_trezorcrypto_ed25519_cosi_commit_obj)},
    {MP_ROM_QSTR(MP_QSTR_cosi_sign), MP_ROM_PTR(&mod_trezorcrypto_ed25519_cosi_sign_obj)},
};
STATIC MP_DEFINE_CONST_DICT(
    mod_trezorcrypto_ed25519_globals, mod_trezorcrypto_ed25519_globals_table);
