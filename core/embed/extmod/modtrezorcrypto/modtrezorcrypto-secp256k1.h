...
STATIC MP_DEFINE_CONST_FUN_OBJ_0(
    mod_trezorcrypto_secp256k1_generate_secret_obj, mod_trezorcrypto_secp256k1_generate_secret);
...
STATIC mp_obj_t mod_trezorcrypto_secp256k1_publickey(size_t n_args, const mp_obj_t *args)
{
    ...
    if (compressed) {
        vstr_init_len(&pk, 33);
        ret = ecdsa_get_public_key33(&secp256k1, (const uint8_t *)sk.buf, (uint8_t *)pk.buf);
    } else {
        vstr_init_len(&pk, 65);
        ret = ecdsa_get_public_key65(&secp256k1, (const uint8_t *)sk.buf, (uint8_t *)pk.buf);
    }
    if (0 != ret) {
        vstr_clear(&pk);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(
    mod_trezorcrypto_secp256k1_publickey_obj, 1, 2, mod_trezorcrypto_secp256k1_publickey);
...
{
    (void)v;
    return !(signature[0] & 0x80) && !(signature[0] == 0 && !(signature[1] & 0x80)) &&
           !(signature[32] & 0x80) && !(signature[32] == 0 && !(signature[33] & 0x80));
}
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(
    mod_trezorcrypto_secp256k1_sign_obj, 2, 4, mod_trezorcrypto_secp256k1_sign);
...
///     """
STATIC mp_obj_t
mod_trezorcrypto_secp256k1_verify(mp_obj_t public_key, mp_obj_t signature, mp_obj_t digest)
{
    ...
    int ret = ecdsa_verify_digest(
        &secp256k1, (const uint8_t *)pk.buf, (const uint8_t *)sig.buf + offset,
        (const uint8_t *)dig.buf);
    return mp_obj_new_bool(ret == 0);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_3(
    mod_trezorcrypto_secp256k1_verify_obj, mod_trezorcrypto_secp256k1_verify);
...
STATIC mp_obj_t mod_trezorcrypto_secp256k1_verify_recover(mp_obj_t signature, mp_obj_t digest)
{
    ...
    if (ecdsa_recover_pub_from_sig(
            &secp256k1, (uint8_t *)pk.buf, (const uint8_t *)sig.buf + 1, (const uint8_t *)dig.buf,
            recid) == 0) {
        if (compressed) {
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(
    mod_trezorcrypto_secp256k1_verify_recover_obj, mod_trezorcrypto_secp256k1_verify_recover);
...
STATIC mp_obj_t mod_trezorcrypto_secp256k1_multiply(mp_obj_t secret_key, mp_obj_t public_key)
{
    ...
    if (0 !=
        ecdh_multiply(
            &secp256k1, (const uint8_t *)sk.buf, (const uint8_t *)pk.buf, (uint8_t *)out.buf)) {
        vstr_clear(&out);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(
    mod_trezorcrypto_secp256k1_multiply_obj, mod_trezorcrypto_secp256k1_multiply);
...
#if !BITCOIN_ONLY
    {MP_ROM_QSTR(MP_QSTR_CANONICAL_SIG_ETHEREUM), MP_ROM_INT(CANONICAL_SIG_ETHEREUM)},
    {MP_ROM_QSTR(MP_QSTR_CANONICAL_SIG_EOS), MP_ROM_INT(CANONICAL_SIG_EOS)},
#endif
};
STATIC MP_DEFINE_CONST_DICT(
    mod_trezorcrypto_secp256k1_globals, mod_trezorcrypto_secp256k1_globals_table);
