    ...
    switch (o->mode) {
        ...
        case CFB:
            if (encrypt) {
                ...
                aes_cfb_decrypt(
                    buf.buf, (uint8_t *)vstr.buf, buf.len, o->iv,
        ...
        case CTR:  // (encrypt == decrypt)
            aes_ctr_crypt(
                buf.buf, (uint8_t *)vstr.buf, buf.len, o->iv, aes_ctr_cbuf_inc, &(o->encrypt_ctx));
            break;
    }
...
STATIC MP_DEFINE_CONST_DICT(
    mod_trezorcrypto_AES_locals_dict, mod_trezorcrypto_AES_locals_dict_table);
