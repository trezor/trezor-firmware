...
///     """
///     """
STATIC mp_obj_t mod_trezorio_HID_make_new(
    const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args)
{
    STATIC const mp_arg_t allowed_args[] = {
        ...
        {MP_QSTR_emu_port, MP_ARG_REQUIRED | MP_ARG_KW_ONLY | MP_ARG_INT, {.u_int = 0}},
...
///     Sends message using USB HID (device) or UDP (emulator).
///     """
STATIC mp_obj_t mod_trezorio_HID_write_blocking(mp_obj_t self, mp_obj_t msg, mp_obj_t timeout_ms)
{
    mp_obj_HID_t *o = MP_OBJ_TO_PTR(self);
    mp_buffer_info_t buf = {0};
    mp_get_buffer_raise(msg, &buf, MP_BUFFER_READ);
    uint32_t timeout = trezor_obj_get_uint(timeout_ms);
    ssize_t r = usb_hid_write_blocking(o->info.iface_num, buf.buf, buf.len, timeout);
    return MP_OBJ_NEW_SMALL_INT(r);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_3(
    mod_trezorio_HID_write_blocking_obj, mod_trezorio_HID_write_blocking);
...
STATIC const mp_rom_map_elem_t mod_trezorio_HID_locals_dict_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_write_blocking), MP_ROM_PTR(&mod_trezorio_HID_write_blocking_obj)},
};
