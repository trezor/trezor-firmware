...
/*  variables for DiagMeasurePrefix/Infix/Postfix */
static uint8_t  diagCeMem[ST25R500_DIAG_MEAS_CMD_LEN];
static uint8_t  diagReg;
static uint8_t  diagTMode;
...
ReturnCode st25r500Initialize( void )
{
    ...
    {
        /* Unable to retrieve back the written pattern from ST25R register - serial communication (SPI) not operational */
        platformErrorHandle();
        return RFAL_ERR_IO;
    }
...
ReturnCode st25r500AdjustRegulators( uint8_t drop, uint16_t* result )
{
    ...
    if( result != NULL )
    {
        *result = (uint16_t)(((((uint32_t)res + 1U) * 46875U)+500U)/1000U);  /* 46.875mV steps */
    }
    return RFAL_ERR_NONE;
}
...
ReturnCode st25r500DiagMeasure( uint8_t meas, uint16_t *res )
{
    ...
    ret = st25r500DiagMeasurePrefix(meas);

    if (RFAL_ERR_NONE == ret)
    {
        ret = st25r500DiagMeasureInfix(res);
...
/*******************************************************************************/
ReturnCode st25r500DiagMeasurePrefix( uint8_t meas )
{
    const uint8_t  command[ST25R500_DIAG_MEAS_CMD_LEN] = { meas, 0x00 };

    st25r500ReadRegister( ST25R500_REG_OPERATION, &diagReg );

    /* Check proper operation mode, device must be in RD mode */
    if( (ST25R500_REG_OPERATION_en & diagReg) == 0U )
    {
        return RFAL_ERR_WRONG_STATE;
    }

    /* Ensure no test mode being used and ensure restoring CE memory */
    st25r500TestModeSaveDisable( diagTMode );
    st25r500ReadMultipleRegisters( ST25R500_REG_CEM_A, diagCeMem, sizeof(diagCeMem) );

    /* Diagnostic Measurements require Rx enabled.                   *
     * For convenience, enabled it temporarily if currently disabled */
    if( (ST25R500_REG_OPERATION_rx_en & diagReg) == 0U )
    {
        st25r500WriteRegister( ST25R500_REG_OPERATION, (diagReg | ST25R500_REG_OPERATION_rx_en) );
    }

    /* Write command to CE mem */
...
/*******************************************************************************/
ReturnCode st25r500DiagMeasureInfix( uint16_t *res )
{
    uint16_t measurement;
    uint8_t  result[ST25R500_DIAG_MEAS_RES_LEN];

    /* Execute Diagnostic measurement */
    st25r500ExecuteCommand( ST25R500_CMD_TRIGGER_DIAG );

    st25r500WaitForInterruptsTimed( ST25R500_IRQ_MASK_DCT, 2 );
...
/*******************************************************************************/
void st25r500DiagMeasurePostfix( void )
{
    st25r500DisableInterrupts( ST25R500_IRQ_MASK_DCT );
    ...
    if( (ST25R500_REG_OPERATION_rx_en & diagReg) == 0U )
    {
        st25r500WriteRegister( ST25R500_REG_OPERATION, diagReg );
    }

    /* Restore previous Test modes and CE memory */
    st25r500WriteMultipleRegisters( ST25R500_REG_CEM_A, diagCeMem, sizeof(diagCeMem) );
    st25r500TestModeRestore( diagTMode );
}
...
{
    if( (cmd < ST25R500_CMD_SET_DEFAULT) || (cmd > ST25R500_CMD_TEST_ACCESS) )
    {
        return false;
    }
    return true;
