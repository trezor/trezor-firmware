...
#define RFAL_FEATURE_ISO_DEP_IBLOCK_MAX_LEN \
  (1U) /*!< ISO-DEP I-Block max length, set to "none" */
#define RFAL_FEATURE_ISO_DEP_APDU_MAX_LEN \
  (1U) /*!< ISO-DEP APDU max length, set to "none"    */
...
#define RFAL_ISODEP_NO_DID \
  (0x10U) /*!< DID value indicating the ISO-DEP layer not to use DID */
...
#define RFAL_ISODEP_FSX_KEEP \
  (0xFFU) /*!< Flag to keep FSX from activation                     */
...
#define RFAL_ISODEP_DEFAULT_FSC                       \
  RFAL_ISODEP_FSX_256 /*!< FSC default value (aligned \
                         RFAL_ISODEP_DEFAULT_FSCI) */
#define RFAL_ISODEP_DEFAULT_SFGI \
  (0U) /*!< SFGI Default value to be used  in Listen Mode        */
...
#define RFAL_ISODEP_MAX_R_RETRYS                                        \
  (3U) /*!< Number of retries for a R-Block     Digital 2.0 B9 - nRETRY \
          ACK/NAK: [2,5]  */
#define RFAL_ISODEP_MAX_WTX_NACK_RETRYS                                 \
  (3U) /*!< Number of S(WTX) replied with NACK  Digital 2.0 B9 - nRETRY \
          WTX[2,5]        */
...
#define RFAL_ISODEP_MAX_DSL_RETRYS                                      \
  (0U) /*!< Number of retries for a S(DESELECT) Digital 2.0 B9 - nRETRY \
          DESELECT: [0,5] */
#define RFAL_ISODEP_RATS_RETRIES                                             \
  (1U) /*!< RATS retries upon fail              Digital 2.0 B7 - nRETRY RATS \
          [0,1]      */
...
typedef enum {
  ...
  RFAL_ISODEP_FSXI_512 = 9, /*!< Frame Size for Proximity Card Integer with 512
                               bytes   ISO14443-3 Amd2 2012 */
  RFAL_ISODEP_FSXI_1024 = 10, /*!< Frame Size for Proximity Card Integer with
                                 1024 bytes  ISO14443-3 Amd2 2012 */
  RFAL_ISODEP_FSXI_2048 = 11, /*!< Frame Size for Proximity Card Integer with
                                 2048 bytes  ISO14443-3 Amd2 2012 */
  RFAL_ISODEP_FSXI_4096 = 12  /*!< Frame Size for Proximity Card Integer with
                                 4096 bytes  ISO14443-3 Amd2 2012 */
} rfalIsoDepFSxI;
...
typedef enum {
  ...
  RFAL_ISODEP_FSX_512 = 512,   /*!< Frame Size for Proximity Card with 512 bytes
                                  ISO14443-3 Amd2 2012 */
  RFAL_ISODEP_FSX_1024 = 1024, /*!< Frame Size for Proximity Card with 1024
                                  bytes  ISO14443-3 Amd2 2012 */
  RFAL_ISODEP_FSX_2048 = 2048, /*!< Frame Size for Proximity Card with 2048
                                  bytes  ISO14443-3 Amd2 2012 */
  RFAL_ISODEP_FSX_4096 = 4096, /*!< Frame Size for Proximity Card with 4096
                                  bytes  ISO14443-3 Amd2 2012 */
} rfalIsoDepFSx;
...
typedef struct {
  ...
  uint8_t
      nfcid0[RFAL_NFCB_NFCID0_LEN]; /*!< NFCID0 of the card to be selected */
  struct {
  ...
  uint8_t
      HLInfo[RFAL_ISODEP_ATTRIB_HLINFO_LEN]; /*!< Higher Layer Information */
} rfalIsoDepAttribCmd;
...
typedef struct {
  ...
  uint8_t value[RFAL_ISODEP_SPARAM_VALUES_MAX_LEN]; /*!< S(PARAMETERS) Value
                                                       field      */
...
typedef union { /*  PRQA S 0750 # MISRA 19.2 - Both members of the union will
                   not be used concurrently, device is only of type A or B at a
                   time. Thus no problem can occur. */
  ...
  union {
    struct {
      rfalIsoDepAttribRes
          ATTRIB_RES;        /*!< ATTRIB_RES              (Poller mode)    */
 ...
 *                                Use RFAL_ISODEP_MAX_WTX_RETRYS_ULTD for
 disabling
 ...
 *
 *  Pass in RATS for T4AT, or ATTRIB for T4BT, to handle ATS or ATTRIB Response
 *respectively The Activation Handling handles ATS and ATTRIB Response; and
 *additionally PPS Response if a PPS is received for T4AT. The method uses the
 *current RFAL state machine to determine if it is expecting RATS or ATTRIB
 *
 *  Activation is completed if PPS Response is sent or if first PDU is received
 *in T4T-A Activation is completed if ATTRIB Response is sent in T4T-B
 *
 *  \ref rfalIsoDepListenGetActivationStatus provide status if activation is
 *completed. \ref rfalIsoDepStartTransceive shall be called right after
 ...
 *  \param[in] actParam       : reference to incoming reception information will
 *be placed
 *
 *
 ...
 *  buffer owned by the caller and passed on actParam must still contain this
 *data. The first data will be processed (I-Block or S-DSL) by
 *rfalIsoDepGetTransceiveStatus() inform the caller and then for the next
 *transaction use rfalIsoDepStartTransceive()
 *
 ...
 *  \param[in]  DID    : Device ID to be used or RFAL_ISODEP_NO_DID for not use
 *DID \param[out] ats    : pointer to place the ATS Response \param[out] atsLen
 *: pointer to place the ATS length
 *
 ...
 *  \param[in]  DSI    : DSI code the divisor from Listener (PICC) to Poller
 *(PCD) \param[in]  DRI    : DRI code the divisor from Poller (PCD) to Listener
 *(PICC) \param[out] ppsRes : pointer to place the PPS Response
 *
 *  \return RFAL_ERR_WRONG_STATE  : RFAL not initialized or incorrect mode
 *  \return RFAL_ERR_PARAM        : Invalid parameters
 *  \return RFAL_ERR_IO           : Generic internal error
 ...
 *  ISO-DEP layer (ISO14443-4) and checks if the received ATTRIB Response is
 *valid
 *
 *  \param[in]  nfcid0    : NFCID0 to be used for the ATTRIB
 *  \param[in]  PARAM1    : ATTRIB PARAM1 byte (communication parameters)
 *  \param[in]  DSI       : DSI code the divisor from Listener (PICC) to Poller
 *(PCD) \param[in]  DRI       : DRI code the divisor from Poller (PCD) to
 *Listener (PICC) \param[in]  FSDI      : PCD's Frame Size to be announced on
 *the ATTRIB \param[in]  PARAM3    : ATTRIB PARAM1 byte (protocol type)
 *  \param[in]  DID       : Device ID to be used or RFAL_ISODEP_NO_DID for not
 *use DID \param[in]  HLInfo    : pointer to Higher layer INF (NULL if none)
 ...
 *
 *  \return RFAL_ERR_NONE    : Deselect successfully sent and acknowledged by
 *PICC \return RFAL_ERR_PROTO   : Protocol error occurred \return
 *RFAL_ERR_TIMEOUT : No response rcvd from PICC
 *
 *****************************************************************************
 */
 ...
 *
 *  This performs a NFC-A Activation into ISO-DEP layer (ISO14443-4) with the
 *given parameters. It sends RATS and if the higher bit rates are supported by
 *  both devices it additionally sends PPS
 *  Once Activated all details of the device are provided on isoDepDev
 *
 *  \param[in]  FSDI          : Frame Size Device Integer to be used
 *  \param[in]  DID           : Device ID to be used or RFAL_ISODEP_NO_DID for
 *not use DID \param[in]  maxBR         : Max bit rate supported by the Poller
 *  \param[out] rfalIsoDepDev : ISO-DEP information of the activated Listen
 *device
 *
 *  \return RFAL_ERR_WRONG_STATE  : RFAL not initialized or incorrect mode
 *  \return RFAL_ERR_PARAM        : Invalid parameters
 ...
 *
 *  This performs a NFC-B Activation into ISO-DEP layer (ISO14443-4) with the
 *given parameters. It sends ATTRIB and calculates supported higher bit rates of
 *both devices and performs activation. Once Activated all details of the device
 *are provided on isoDepDev
 *
 *  \param[in]  FSDI           : Frame Size Device Integer to be used
 *  \param[in]  DID            : Device ID to be used or RFAL_ISODEP_NO_DID for
 *not use DID \param[in]  maxBR          : Max bit rate supported by the Poller
 *  \param[in]  PARAM1         : ATTRIB PARAM1 byte (communication parameters)
 *  \param[in]  nfcbDev        : pointer to the NFC-B Device containing the
 *SENSB_RES \param[in]  HLInfo         : pointer to Higher layer INF (NULL if
 *none) \param[in]  HLInfoLen      : Length HLInfo \param[out] rfalIsoDepDev  :
 *ISO-DEP information of the activated Listen device
 *
 *  \return RFAL_ERR_WRONG_STATE  : RFAL not initialized or incorrect mode
 *  \return RFAL_ERR_PARAM        : Invalid parameters
 *  \return RFAL_ERR_IO           : Generic internal error
 ...
 *
 *  \param[out] rfalIsoDepDev  : ISO-DEP information of the activated Listen
 *device \param[in]  maxTxBR        : Maximum Tx bit rate supported by PCD
 ...
 *
 *  This starts a NFC-A Activation into ISO-DEP layer (ISO14443-4) with the
 *given parameters. It sends RATS and if the higher bit rates are supported by
 *  both devices it additionally sends PPS
 *  Once Activated all details of the device are provided on isoDepDev
 *
 ...
 *  \return RFAL_ERR_FRAMING      : Framing error detected
 *  \return RFAL_ERR_PROTO        : Protocol error detected
 *  \return RFAL_ERR_NONE         : No error, start of asynchronous operation
 *successful
 *****************************************************************************
 */
 ...
 *
 *  This starts a NFC-B Activation into ISO-DEP layer (ISO14443-4) with the
 *given parameters. It will send ATTRIB and calculate supported higher bit rates
 *of both devices and perform activation. Once Activated all details of the
 *device are provided on isoDepDev
 *
