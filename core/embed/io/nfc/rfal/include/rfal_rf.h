 ...
 *
 *  It provides interfaces to configure the RF IC, set/get timings, modes, bit
 * rates, specific handlings, execute listen mode, etc.
 *
 *  Furthermore it provides a common interface to perform a Transceive
 * operations. The Transceive can be executed in a blocking or non blocking
 * way.<br> Additionally few specific Transceive methods are available to cope
 * with the specifics of these particular operations.
 *
...
*/
#define RFAL_VERSION \
  0x030001U /*!< RFAL Current Version: v3.0.1                      */

#define RFAL_FWT_NONE \
  0xFFFFFFFFU /*!< Disabled FWT: Wait forever for a response         */
#define RFAL_GT_NONE \
  RFAL_TIMING_NONE /*!< Disabled GT: No GT will be applied after Field On */

#define RFAL_TIMING_NONE \
  0x00U /*!< Timing disabled | Don't apply                     */

#define RFAL_1FC_IN_4096FC \
  (uint32_t)4096U /*!< Number of 1/fc cycles in one 4096/fc              */
#define RFAL_1FC_IN_2048FC \
  (uint32_t)2048U /*!< Number of 1/fc cycles in one 2048/fc              */
#define RFAL_1FC_IN_512FC \
  (uint32_t)512U /*!< Number of 1/fc cycles in one 512/fc               */
#define RFAL_1FC_IN_64FC \
  (uint32_t)64U /*!< Number of 1/fc cycles in one 64/fc                */
#define RFAL_1FC_IN_8FC \
  (uint32_t)8U /*!< Number of 1/fc cycles in one 8/fc                 */
#define RFAL_US_IN_MS \
  (uint32_t)1000U /*!< Number of us in one ms                            */
#define RFAL_1MS_IN_1FC \
  (uint32_t)13560U /*!< Number of 1/fc cycles in 1ms                      */
#define RFAL_BITS_IN_BYTE \
  (uint16_t)8U /*!< Number of bits in one byte                        */

#define RFAL_CRC_LEN \
  2U /*!< RF CRC LEN                                        */

/*! Default TxRx flags: Tx CRC automatic, Rx CRC removed, NFCIP1 mode off, AGC
 * On, Tx Parity automatic, Rx Parity removed */
...
#define RFAL_LM_MASK_NFCA    \
  ((uint32_t)1U << (uint8_t) \
       RFAL_MODE_LISTEN_NFCA) /*!< Bitmask for Listen Mode enabling NFCA    */
#define RFAL_LM_MASK_NFCB    \
  ((uint32_t)1U << (uint8_t) \
       RFAL_MODE_LISTEN_NFCB) /*!< Bitmask for Listen Mode enabling NFCB    */
#define RFAL_LM_MASK_NFCF    \
  ((uint32_t)1U << (uint8_t) \
       RFAL_MODE_LISTEN_NFCF) /*!< Bitmask for Listen Mode enabling NFCF    */
#define RFAL_LM_MASK_ACTIVE_P2P                                                \
  ((uint32_t)1U << (uint8_t)                                                   \
       RFAL_MODE_LISTEN_ACTIVE_P2P) /*!< Bitmask for Listen Mode enabling AP2P \
                                     */

#define RFAL_LM_SENS_RES_LEN \
  2U /*!< NFC-A SENS_RES (ATQA) length                      */
#define RFAL_LM_SENSB_RES_LEN \
  13U /*!< NFC-B SENSB_RES (ATQB) length                     */
#define RFAL_LM_SENSF_RES_LEN \
  19U /*!< NFC-F SENSF_RES  length                           */
#define RFAL_LM_SENSF_SC_LEN \
  2U /*!< NFC-F System Code length                          */

#define RFAL_NFCID3_LEN \
  10U /*!< NFCID3 length                                     */
#define RFAL_NFCID2_LEN \
  8U /*!< NFCID2 length                                     */
#define RFAL_NFCID1_TRIPLE_LEN \
  10U /*!< NFCID1 length                                     */
#define RFAL_NFCID1_DOUBLE_LEN \
  7U /*!< NFCID1 length                                     */
#define RFAL_NFCID1_SINGLE_LEN \
  4U /*!< NFCID1 length                                     */
...
/*! Returns the maximum supported bit rate for RW mode. Caller must check if
 * mode is supported before, as even if mode is not supported will return the
 * min  */
               ...
               : ((RFAL_SUPPORT_BR_RW_1695)                     \
                      ...
                      : ((RFAL_SUPPORT_BR_RW_848)               \
                             ...
                             : ((RFAL_SUPPORT_BR_RW_424)        \
                                    ? RFAL_BR_424               \
                                    : ((RFAL_SUPPORT_BR_RW_212) \
                                           ? RFAL_BR_212        \
                                           : RFAL_BR_106)))))))

/*! Returns the maximum supported bit rate for AP2P mode. Caller must check if
 * mode is supported before, as even if mode is not supported will return the
 * min  */
...
/*! Returns the maximum supported bit rate for CE-A mode. Caller must check if
 * mode is supported before, as even if mode is not supported will return the
 * min  */
...
/*! Returns the maximum supported bit rate for CE-B mode. Caller must check if
 * mode is supported before, as even if mode is not supported will return the
 * min  */
...
/*! Returns the maximum supported bit rate for CE-F mode. Caller must check if
 * mode is supported before, as even if mode is not supported will return the
 * min  */
#define rfalGetMaxBrCEF() \
  (((RFAL_SUPPORT_BR_CE_F_424) ? RFAL_BR_424 : RFAL_BR_212))

#define rfalIsModeActiveComm(md)                                           \
  (((md) == RFAL_MODE_POLL_ACTIVE_P2P) ||                                  \
   ((md) == RFAL_MODE_LISTEN_ACTIVE_P2P)) /*!< Checks if mode md is Active \
                                             Communication  */
...
#define rfalIsModePassiveListen(md)                                      \
  (((md) == RFAL_MODE_LISTEN_NFCA) || ((md) == RFAL_MODE_LISTEN_NFCB) || \
   ((md) ==                                                              \
    RFAL_MODE_LISTEN_NFCF)) /*!< Checks if mode md is Passive Listen        */
...
#define rfalConv1fcTo8fc(t)  \
  (uint32_t)((uint32_t)(t) / \
             RFAL_1FC_IN_8FC) /*!< Converts the given t from 1/fc to 8/fc */
#define rfalConv8fcTo1fc(t)  \
  (uint32_t)((uint32_t)(t) * \
             RFAL_1FC_IN_8FC) /*!< Converts the given t from 8/fc to 1/fc */

#define rfalConv1fcTo64fc(t) \
  (uint32_t)(                \
      (uint32_t)(t) /        \
      RFAL_1FC_IN_64FC) /*!< Converts the given t from 1/fc  to 64/fc   */
#define rfalConv64fcTo1fc(t) \
  (uint32_t)((uint32_t)(t) * \
             RFAL_1FC_IN_64FC) /*!< Converts the given t from 64/fc to 1/fc */

#define rfalConv1fcTo512fc(t) \
  (uint32_t)(                 \
      (uint32_t)(t) /         \
      RFAL_1FC_IN_512FC) /*!< Converts the given t from 1/fc  to 512/fc  */
#define rfalConv512fcTo1fc(t) \
  (uint32_t)(                 \
      (uint32_t)(t) *         \
      RFAL_1FC_IN_512FC) /*!< Converts the given t from 512/fc to 1/fc   */

#define rfalConv1fcTo2018fc(t) \
  (uint32_t)(                  \
      (uint32_t)(t) /          \
      RFAL_1FC_IN_2048FC) /*!< Converts the given t from 1/fc to 2048/fc  */
#define rfalConv2048fcTo1fc(t) \
  (uint32_t)(                  \
      (uint32_t)(t) *          \
      RFAL_1FC_IN_2048FC) /*!< Converts the given t from 2048/fc to 1/fc  */

#define rfalConv1fcTo4096fc(t) \
  (uint32_t)(                  \
      (uint32_t)(t) /          \
      RFAL_1FC_IN_4096FC) /*!< Converts the given t from 1/fc to 4096/fc  */
#define rfalConv4096fcTo1fc(t) \
  (uint32_t)(                  \
      (uint32_t)(t) *          \
      RFAL_1FC_IN_4096FC) /*!< Converts the given t from 4096/fc to 1/fc  */

#define rfalConv1fcToMs(t)   \
  (uint32_t)((uint32_t)(t) / \
             RFAL_1MS_IN_1FC) /*!< Converts the given t from 1/fc to ms */
#define rfalConvMsTo1fc(t)   \
  (uint32_t)((uint32_t)(t) * \
             RFAL_1MS_IN_1FC) /*!< Converts the given t from ms to 1/fc */

#define rfalConv1fcToUs(t)                     \
  (uint32_t)(((uint32_t)(t) * RFAL_US_IN_MS) / \
             RFAL_1MS_IN_1FC) /*!< Converts the given t from 1/fc to us */
#define rfalConvUsTo1fc(t)                       \
  (uint32_t)(((uint32_t)(t) * RFAL_1MS_IN_1FC) / \
             RFAL_US_IN_MS) /*!< Converts the given t from us to 1/fc       */

#define rfalConv64fcToMs(t)     \
  (uint32_t)((uint32_t)(t) /    \
             (RFAL_1MS_IN_1FC / \
              RFAL_1FC_IN_64FC)) /*!< Converts the given t from 64/fc to ms */
#define rfalConvMsTo64fc(t)     \
  (uint32_t)((uint32_t)(t) *    \
             (RFAL_1MS_IN_1FC / \
              RFAL_1FC_IN_64FC)) /*!< Converts the given t from ms to 64/fc */
...
#define rfalConvBytesToBits(n) \
  (uint32_t)(                  \
      (uint32_t)(n) *          \
      (RFAL_BITS_IN_BYTE)) /*!< Converts the given n from bytes to bits    */

#define rfalRunBlocking(e, fn) \
  do {                         \
    (e) = (fn);                \
    rfalWorker();              \
  } while ((e) == RFAL_ERR_BUSY) /*!< Macro used for the blocking methods */
...
#define RFAL_GT_NFCA \
  rfalConvMsTo1fc(5U) /*!< GTA  Digital 2.0  6.10.4.1 & B.2 */
#define RFAL_GT_NFCB \
  rfalConvMsTo1fc(5U) /*!< GTB  Digital 2.0  7.9.4.1  & B.3 */
#define RFAL_GT_NFCF \
...
#define RFAL_FDT_POLL_NFCB_POLLER                                        \
  6780U /*!< FDTB,POLL,MIN = TR2B,MIN,DEFAULT Digital 1.1 7.9.3 & A.3  ; \
           EMVCo 3.0 FDTB,PCD,MIN  Table A.5                  */
...
typedef enum {
  ...
  RFAL_BR_211p88 =
      0xE9, /*!< Bit Rate 211,88 kbit/s (fc/64) Fast Mode VICC->VCD */
  RFAL_BR_105p94 =
      0xEA, /*!< Bit Rate 105,94 kbit/s (fc/128) Fast Mode VICC->VCD */
  RFAL_BR_52p97 =
      0xEB, /*!< Bit Rate 52.97 kbit/s (fc/256) Fast Mode VICC->VCD */
  RFAL_BR_26p48 = 0xEC, /*!< Bit Rate 26,48 kbit/s (fc/512) NFCV VICC->VCD &
                           VCD->VICC 1of4   */
  RFAL_BR_1p66 =
      0xED, /*!< Bit Rate 1,66 kbit/s (fc/8192) NFCV VCD->VICC 1of256 */
  RFAL_BR_KEEP =
      0xFF /*!< Value indicating to keep the same previous bit rate */
} rfalBitRate;
...
enum {
  RFAL_TXRX_FLAGS_CRC_TX_AUTO =
      (0U << 0), /*!< CRC will be generated automatic upon transmission */
  RFAL_TXRX_FLAGS_CRC_TX_MANUAL =
      (1U << 0), /*!< CRC was calculated manually, included in txBuffer */
  RFAL_TXRX_FLAGS_CRC_RX_KEEP =
      (1U << 1), /*!< Upon Reception keep the CRC in rxBuffer (reflected on rcvd
                    length)                    */
  RFAL_TXRX_FLAGS_CRC_RX_REMV = (0U << 1), /*!< Remove the CRC from rxBuffer */
  RFAL_TXRX_FLAGS_NFCIP1_ON =
      (1U << 2), /*!< Enable NFCIP1 mode: Add SB(F0) and LEN bytes during Tx and
                    skip SB(F0) byte during Rx */
  RFAL_TXRX_FLAGS_NFCIP1_OFF =
      (0U << 2), /*!< Disable NFCIP1 mode: do not append protocol bytes while Tx
                    nor skip while Rx          */
  RFAL_TXRX_FLAGS_AGC_OFF =
      (1U << 3), /*!< Disable Automatic Gain Control, improving multiple devices
                    collision detection. \b DEPRECATED: flag is depreacted,
                    usage of Anticollision APIs based on Analog Config table
                    with RFAL_ANALOG_CONFIG_ANTICOL settings */
  RFAL_TXRX_FLAGS_AGC_ON =
      (0U << 3), /*!< Enable Automatic Gain Control, improving single device
                    reception                \b DEPRECATED: flag is deprecated,
                    usage of Anticollision APIs based on Analog Config table
                    with RFAL_ANALOG_CONFIG_ANTICOL settings */
  RFAL_TXRX_FLAGS_PAR_RX_KEEP =
      (1U << 4), /*!< Disable Parity check and keep the Parity and CRC bits in
                    the received buffer          */
  RFAL_TXRX_FLAGS_PAR_RX_REMV =
      (0U << 4), /*!< Enable Parity check and remove the parity bits from the
                    received buffer               */
  RFAL_TXRX_FLAGS_PAR_TX_NONE =
      (1U << 5), /*!< Disable automatic Parity generation (ISO14443A) and use
                    the one provided in the buffer*/
  RFAL_TXRX_FLAGS_PAR_TX_AUTO =
      (0U << 5), /*!< Enable automatic Parity generation (ISO14443A) */
  RFAL_TXRX_FLAGS_NFCV_FLAG_MANUAL =
      (1U << 6), /*!< Disable automatic adaption of flag byte (ISO15693)
...
typedef enum {
  ...
  RFAL_ERRORHANDLING_EMD = 1 /*!< EMD suppression enabled  Digital 2.1  4.1.1.1
...
typedef struct {
  ...
  uint16_t
      rxBufLen; /*!< (In)  Maximum length of the incoming message in bits */
...
#define RFAL_FELICA_POLL_RES_LEN                                            \
  (RFAL_FELICA_LEN_LEN + 1U + 8U + 8U +                                     \
   2U) /*!< Maximum FeliCa Poll Response length (LEN + CMD + NFCID2 + PAD + \
          RD) */
...
enum {
  RFAL_FELICA_POLL_RC_NO_REQUEST =
      0x00U, /*!< RC: No System Code information requested */
  RFAL_FELICA_POLL_RC_SYSTEM_CODE =
      0x01U, /*!< RC: System Code information requested */
  RFAL_FELICA_POLL_RC_COM_PERFORMANCE =
      0x02U /*!< RC: Advanced protocol features supported */
};
...
typedef enum {
  RFAL_LM_NFCID_LEN_04 =
      RFAL_NFCID1_SINGLE_LEN, /*!< Listen mode indicates  4 byte NFCID */
  RFAL_LM_NFCID_LEN_07 =
      RFAL_NFCID1_DOUBLE_LEN, /*!< Listen mode indicates  7 byte NFCID */
  RFAL_LM_NFCID_LEN_10 =
      RFAL_NFCID1_TRIPLE_LEN, /*!< Listen mode indicates 10 byte NFCID */
} rfalLmNfcidLen;
...
typedef enum {
  ...
  RFAL_LM_STATE_READY_A =
      0x03, /*!< Ready A state  Activity 1.1  5.3 5.4 & 5.5  */
  ...
  RFAL_LM_STATE_CARDEMU_4A =
      0x07, /*!< Card Emulation 4A state  Activity 1.1  5.10 */
  RFAL_LM_STATE_CARDEMU_4B =
      0x08, /*!< Card Emulation 4B state  Activity 1.1  5.14 */
  RFAL_LM_STATE_CARDEMU_3 =
      0x09, /*!< Card Emulation 3 state  Activity 1.1  5.18  */
  ...
  RFAL_LM_STATE_READY_Ax =
      0x0E, /*!< Ready A* state  Activity 1.1  5.3 5.4 & 5.5 */
...
typedef enum {
  ...
  RFAL_LP_MODE_HR =
      1 /*!< Set RF Chip in Hold Reset state (available for specific devices) */
} rfalLpMode;
...
#define RFAL_WUM_REFERENCE_AUTO \
  0xFFU /*!< Indicates new reference is set by the driver */
...
typedef enum {
  ...
  RFAL_WUM_STATE_ENABLED_WOKE =
      0x03, /*!< Wake-Up mode enabled and has received IRQ(s)*/
} rfalWumState;
 ...
 *error \return RFAL_ERR_NONE         : No error
 *****************************************************************************
 */
...
ReturnCode rfalISO14443ATransceiveShortFrame(rfal14443AShortFrameCmd txCmd,
                                             ...
                                             uint16_t *rxRcvdLen, uint32_t fwt);
 ...
 *
 *
 * \param[in,out] buf        : reference to ANTICOLLISION command (with known
 *UID if any) to be sent (also out param) reception will be place on this buf
 *after bytesToSend buffer must be capable of holding a whole Anticollison frame
 *(rfalNfcaSelReq) \param[in,out] bytesToSend: reference number of full bytes to
 *be sent (including CMD byte and SEL_PAR) if a collision occurs will contain
 *the number of clear bytes \param[in,out] bitsToSend : reference to number of
 *bits (0-7) to be sent; and received (also out param) if a collision occurs
 *will indicate the number of clear bits (also out param) \param[out] rxLength
 *: reference to the return the received length in bits \param[in]     fwt :
 *Frame Waiting Time in 1/fc
 *
 ...
 * \param[in]   pollResListSize   : number of responses that can be placed in
 *pollResList \param[out]  devicesDetected   : number of cards found \param[out]
 *collisionsDetected: number of collisions detected
 *
 * \return RFAL_ERR_NONE        : If there is no error
 * \return RFAL_ERR_PARAM       : Invalid parameter
 * \return RFAL_ERR_WRONG_STATE : RFAL not initialized or mode not set
 ...
 * \param[in]  rxLen:     pointer to write the data length in bits placed into
 *rxBuf
 *
 *
 * \return RFAL_ERR_WRONG_STATE : Not initialized properly
 ...
 *
 * \warning the listen mode will be disabled immediately on the RFchip
 *regardless of any ongoing operations like Transceive
 *
 * \return RFAL_ERR_NONE : Done with no error
 *
 *****************************************************************************
 ...
 *  sequence takes longer. Otherwise WU mode is running after
 *rfalWakeUpModeStart
 *
