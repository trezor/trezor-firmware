...
#if ((RFAL_FEATURE_NFC_DEP_BLOCK_MAX_LEN != 64) &&  \
     (RFAL_FEATURE_NFC_DEP_BLOCK_MAX_LEN != 128) && \
     (RFAL_FEATURE_NFC_DEP_BLOCK_MAX_LEN != 192) && \
     (RFAL_FEATURE_NFC_DEP_BLOCK_MAX_LEN != 254))
#error \
...
#error \
    " RFAL: Invalid NFC-DEP PDU Max length. Please change RFAL_FEATURE_NFC_DEP_PDU_MAX_LEN. "
#endif
 ...
 ******************************************************************************
 */
#define NFCIP_ATR_RETRY_MAX \
  2U /*!< Max consecutive retrys of an ATR REQ with transm error*/

#define NFCIP_PSLPAY_LEN \
  (2U) /*!< PSL Payload length (BRS + FSL)                        */
...
#define NFCIP_ATRREQ_BUF_LEN    \
  (RFAL_NFCDEP_ATRREQ_MAX_LEN + \
   RFAL_NFCDEP_LEN_LEN) /*!< ATR REQ max length (incl LEN)     */
#define NFCIP_ATRRES_BUF_LEN    \
  (RFAL_NFCDEP_ATRRES_MAX_LEN + \
   RFAL_NFCDEP_LEN_LEN) /*!< ATR RES max length (incl LEN)     */
...
#define NFCIP_INIT_MAX_RTOX \
  59U /*!< Maximum RTOX value  Digital 1.0  14.8.4.1             */

#define NFCIP_TARG_MIN_RTOX \
  1U /*!< Minimum target RTOX value  Digital 1.0  14.8.4.1      */
#define NFCIP_TARG_MAX_RTOX \
  59U /*!< Maximum target RTOX value  Digital 1.0  14.8.4.1      */

#define NFCIP_TRECOV \
  1280U /*!< Digital 1.0  A.10  Trecov                             */

#define NFCIP_TIMEOUT_ADJUSTMENT                                              \
  3072U /*!< Timeout Adjustment to compensate timing from end of Tx to end of \
           frame  */
#define NFCIP_RWT_ACTIVATION                                                \
  (0x1000001U +                                                             \
   NFCIP_TIMEOUT_ADJUSTMENT) /*!< Digital 2.2  B.11  RWT ACTIVATION  2^24 + \
                                RWT Delta + Adjustment*/
#define NFCIP_RWT_ACM_ACTIVATION                                            \
  (0x200001U +                                                              \
   NFCIP_TIMEOUT_ADJUSTMENT) /*!< Digital 2.2  B.11  RWT ACTIVATION  2^21 + \
                                RWT Delta + Adjustment*/

#define RFAL_NFCDEP_HEADER_PAD                                               \
  (RFAL_NFCDEP_DEPREQ_HEADER_LEN -                                           \
   RFAL_NFCDEP_LEN_MIN) /*!< Difference between expected rcvd header len and \
                           max foreseen */
...
#define NFCIP_BR_MASK \
  NFCIP_BS_MASK /*!< Bit mask for BR value on a ATR REQ/RES                */

#define NFCIP_PP_GB_MASK \
  0x02U /*!< Bit mask for GB value in PP byte on a ATR REQ/RES     */
#define NFCIP_PP_NAD_MASK \
  0x01U /*!< Bit mask for NAD value in PP byte on a ATR REQ/RES    */

#define NFCIP_PFB_xPDU_MASK \
  0xE0U /*!< Bit mask for PDU type                                 */
#define NFCIP_PFB_IPDU \
  0x00U /*!< Bit mask indicating a Information PDU                 */
#define NFCIP_PFB_RPDU \
  0x40U /*!< Bit mask indicating a Response PDU                    */
#define NFCIP_PFB_SPDU \
  0x80U /*!< Bit mask indicating a Supervisory PDU                 */

#define NFCIP_PFB_MI_BIT \
  0x10U /*!< Bit mask for the chaining bit (MI) of PFB             */
#define NFCIP_PFB_DID_BIT \
  0x04U /*!< Bit mask for the DID presence bit of PFB              */
#define NFCIP_PFB_NAD_BIT \
  0x08U /*!< Bit mask for the NAD presence bit of PFB              */
#define NFCIP_PFB_PNI_MASK \
  0x03U /*!< Bit mask for the Packet Number Information            */

#define NFCIP_PFB_Rx_MASK \
  0x10U /*!< Bit mask for the R-PDU type                           */
#define NFCIP_PFB_ACK \
  0x00U /*!< Bit mask for R-PDU indicating ACK                     */
#define NFCIP_PFB_NACK \
  0x10U /*!< Bit mask for R-PDU indicating NAK                     */

#define NFCIP_PFB_Sx_MASK \
  0x10U /*!< Bit mask for the R-PDU type                           */
#define NFCIP_PFB_ATN \
  0x00U /*!< Bit mask for R-PDU indicating ACK                     */
#define NFCIP_PFB_TO \
  0x10U /*!< Bit mask for R-PDU indicating NAK                     */

#define NFCIP_PFB_INVALID \
  0xFFU /*!< Invalid PFB value                                     */
...
#define nfcipIsTransmissionError(e)                      \
  (((e) == RFAL_ERR_CRC) || ((e) == RFAL_ERR_FRAMING) || \
   ((e) == RFAL_ERR_PAR)) /*!< Checks if is a Trasmission error */

#define nfcipConv1FcToMs(v) \
  (rfalConv1fcToMs((v)) +   \
   1U) /*!< Converts value v 1fc into milliseconds (fc=13.56)     */
...
#define nfcip_PFBisIMI(pfb)                                                  \
  (nfcip_PFBisIPDU(pfb) &&                                                   \
   (((pfb) & NFCIP_PFB_MI_BIT) ==                                            \
    NFCIP_PFB_MI_BIT)) /*!< Checks if pfb is a Information PDU indicating MI \
                          chaining */

#define nfcip_PFBisRNACK(pfb)      \
  (nfcip_PFBisRPDU(pfb) &&         \
   (((pfb) & NFCIP_PFB_Rx_MASK) == \
...
#define nfcip_PFBIPDU(pni)                                                    \
  ((uint8_t)(0x00U | NFCIP_PFB_IPDU |                                         \
             ((pni) & NFCIP_PFB_PNI_MASK))) /*!< Returns a PFB I-PDU with the \
                                               given packet number (pni) */
#define nfcip_PFBIPDU_MI(pni)                                                  \
  ((uint8_t)(isoDep_PCBIBlock(pni) |                                           \
             NFCIP_PFB_MI_BIT)) /*!< Returns a PFB I-PDU with the given packet \
                                   number (pni) indicating chaing */

#define nfcip_PFBRPDU(pni)                                                    \
  ((uint8_t)(0x00U | NFCIP_PFB_RPDU |                                         \
             ((pni) & NFCIP_PFB_PNI_MASK))) /*!< Returns a PFB R-PDU with the \
                                               given packet number (pni) */
#define nfcip_PFBRPDU_NACK(pni)                                              \
  ((uint8_t)(nfcip_PFBRPDU(pni) |                                            \
             NFCIP_PFB_NACK)) /*!< Returns a PFB R-PDU with the given packet \
                                 number (pni) indicating NACK   */
#define nfcip_PFBRPDU_ACK(pni)                                              \
  ((uint8_t)(nfcip_PFBRPDU(pni) |                                           \
             NFCIP_PFB_ACK)) /*!< Returns a PFB R-PDU with the given packet \
                                number (pni) indicating ACK    */
...
#define nfcip_PNIInc(pni)                                                 \
  ((uint8_t)(((pni) + 1U) &                                               \
             NFCIP_PFB_PNI_MASK)) /*!< Returns a incremented PNI from the \
                                     given (pni)        */
#define nfcip_PNIDec(pni)                                                     \
  ((uint8_t)(((pni)-1U) & NFCIP_PFB_PNI_MASK)) /*!< Returns a decremented PNI \
                                                  from the given (pni) */

#define nfcip_PBF_PNI(pfb)                                               \
  ((uint8_t)((pfb) & NFCIP_PFB_PNI_MASK)) /*!< Returns the Packet Number \
                                             Information (pni)           */

#define nfcip_PPwGB(lr)                                                   \
  (rfalNfcDepLR2PP(lr) |                                                  \
   NFCIP_PP_GB_MASK) /*!< Returns a PP byte containing the given PP value \
                        indicating GB                  */

#define nfcip_DIDMax(did)                                                    \
  (RFAL_MIN((did),                                                           \
            RFAL_NFCDEP_DID_MAX)) /*!< Ensures that the given did has proper \
                                     value  Digital 14.6.2.3 DID [0 14] */
#define nfcip_RTOXTargMax(wt)                                               \
  (uint8_t)(                                                                \
      RFAL_MIN((RFAL_NFCDEP_RWT_TRG_MAX / rfalNfcDepWT2RWT(wt)),            \
               NFCIP_TARG_MAX_RTOX)) /*!< Calculates the Maximum RTOX value \
                                        for the given wt as a Target */

#define nfcipIsInitiator(st)       \
  (((st) >= NFCIP_ST_INIT_IDLE) && \
   ((st) <= NFCIP_ST_INIT_RLS)) /*!< Checks if module is set as Initiator */
#define nfcipIsTarget(st) \
  (!nfcipIsInitiator(st)) /*!< Checks if module is set as Target */
...
#define nfcipIsEmptyDEPEnabled(op)                                          \
  (!nfcipIsEmptyDEPDisabled(op)) /*!< Checks if empty payload is allowed by \
                                    operation config  NCI 1.0 Table 81 */
#define nfcipIsEmptyDEPDisabled(op)                                            \
  (((op) & RFAL_NFCDEP_OPER_EMPTY_DEP_DIS) !=                                  \
   0U) /*!< Checks if empty payload is not allowed by operation config NCI 1.0 \
          Table 81   */

#define nfcipIsRTOXReqEnabled(op)                                            \
  (!nfcipIsRTOXReqDisabled(op)) /*!< Checks if send a RTOX_REQ is allowed by \
                                   operation config  NCI 1.0 Table 81     */
#define nfcipIsRTOXReqDisabled(op)                                       \
  (((op) & RFAL_NFCDEP_OPER_RTOX_REQ_DIS) !=                             \
   0U) /*!< Checks if send a RTOX_REQ is not allowed by operation config \
          NCI 1.0 Table 81 */

/*! Checks if isDeactivating callback is set and calls it, otherwise returns
 * false */
...
#define nfcipRTOXAdjust(v)                                               \
  ((v) - ((v) >> 3)) /*!< Adjust RTOX timer value to a percentage of the \
                        total, current 88% */

/*******************************************************************************/

// timerPollTimeoutValue is necessary after timerCalculateTimeout so that system
// will wake up upon timer timeout.
...
/*! Digital 1.1 - 16.12.5.2  The Target SHALL NOT attempt any error recovery and
...
/*! Struct that holds all DEP parameters/configs for the following
 * communications */
typedef struct {
...
typedef struct {
  ...
  rfalNfcDepDeactCallback
      isDeactivating; /*!< Deactivating flag check callback              */
...
static ReturnCode nfcipTxRx(rfalNfcDepCmd cmd, uint8_t *txBuf, uint32_t fwt,
                            uint8_t *paylBuf, uint8_t paylBufLen,
                            uint8_t *rxBuf, uint16_t rxBufLen,
                            uint16_t *rxActLen);
static ReturnCode nfcipTx(rfalNfcDepCmd cmd, uint8_t *txBuf, uint8_t *paylBuf,
                          uint16_t paylLen, uint8_t pfbData, uint32_t fwt);
static ReturnCode nfcipDEPControlMsg(uint8_t pfb, uint8_t RTOX);
static ReturnCode nfcipInitiatorHandleDEP(ReturnCode rxRes, uint16_t rxLen,
                                          uint16_t *outActRxLen,
                                          bool *outIsChaining);
static ReturnCode nfcipTargetHandleRX(ReturnCode rxRes, uint16_t *outActRxLen,
                                      bool *outIsChaining);
static ReturnCode nfcipTargetHandleActivation(rfalNfcDepDevice *nfcDepDev,
                                              uint8_t *outBRS);
...
static bool nfcipDxIsSupported(uint8_t Dx, uint8_t BRx, uint8_t BSx) {
  ...
  if ((Dx == (uint8_t)RFAL_NFCDEP_Dx_08_848) &&
      (Bx >= (uint8_t)RFAL_NFCDEP_Bx_08_848)) {
    return true;
  }
...
static ReturnCode nfcipDEPControlMsg(uint8_t pfb, uint8_t RTOX) {
  ...
  /*******************************************************************************/
  const rfalNfcDepCmd depCmd =
      ((gNfcip.cfg.role == RFAL_NFCDEP_ROLE_TARGET) ? NFCIP_CMD_DEP_RES
                                                    : NFCIP_CMD_DEP_REQ);
  fwt =
  ...
  if (nfcip_PFBisSTO(pfb)) {
    ...
    return nfcipTx(depCmd, ctrlMsg, &ctrlMsg[RFAL_NFCDEP_DEPREQ_HEADER_LEN],
                   sizeof(uint8_t), pfb, fwt);
  } else {
...
static ReturnCode nfcipInitiatorHandleDEP(ReturnCode rxRes, uint16_t rxLen,
  ...
  *outIsChaining = false;
    ...
    case RFAL_ERR_TIMEOUT:
      ...
      if (nfcip_PFBisRNACK(gNfcip.lastPFB)) {
        ...
        RFAL_EXIT_ON_ERR(ret,
                         nfcipDEPControlMsg(nfcip_PFBRPDU_NACK(gNfcip.pni), 0));
  ...
  /* Due to different modes on ST25R391x (with/without CRC) use NFC-DEP LEN
   * instead of bytes retrieved */
  ...
  if (!(nfcip_PFBisSPDU(rxPFB) || nfcip_PFBisRPDU(rxPFB) ||
        nfcip_PFBisIPDU(rxPFB))) {
    return RFAL_ERR_PROTO;
  }
  ...
  if (gNfcip.cfg.nad != RFAL_NFCDEP_NAD_NO) {
    if ((gNfcip.rxBuf[rxMsgIt++] != gNfcip.cfg.nad) ||
        (!nfcip_PFBhasNAD(rxPFB))) {
      return RFAL_ERR_PROTO;
    }
    optHdrLen++;                     /* Inc header optional field cnt*/
  ...
  if (nfcip_PFBisRPDU(rxPFB)) {
    ...
    if (nfcip_PFBisRACK(rxPFB)) {
      ...
      } else /* Digital 1.0 14.12.4.5 ACK with wrong PNI Initiator may
                retransmit */
      {
        ...
        /* Extended the MAY in Digital 1.0 14.12.4.5 to only reTransmit if the
         * ACK is for the previous DEP, otherwise raise Protocol immediately If
         * the PNI difference is more than 1 it is worthless to reTransmit 3x
  ...
  if (nfcip_PFBisSPDU(rxPFB)) {
    ...
    if (nfcip_PFBisSATN(rxPFB)) /* If is a S-ATN        */
    {
      ...
      if (nfcip_PFBisSATN(gNfcip.lastPFB)) /* Check if is expected */
      {
        ...
        if (nfcip_PFBisSTO(gNfcip.lastPFBnATN)) {
          ...
          RFAL_EXIT_ON_ERR(
              ret, nfcipDEPControlMsg(nfcip_PFBSPDU_TO(), gNfcip.lastRTOX));
        } else {
          /* ReTransmit ? */
          if (gNfcip.cntTxRetrys++ >= RFAL_NFCDEP_MAX_TX_RETRYS) {
            return RFAL_ERR_PROTO;
    ...
    else if (nfcip_PFBisSTO(rxPFB)) /* If is a S-TO (RTOX)  */
    {
      ...
      /* Digital 1.1 16.12.4.3 - Initiator MAY stop accepting subsequent RTOX
       * Req   *
  ...
  if (nfcip_PFBisIPDU(rxPFB)) {
    ...
    nfcipClearCounters();
    *outActRxLen =
        ((uint16_t)nfcDepLen - RFAL_NFCDEP_DEP_HEADER - (uint16_t)optHdrLen);

    if ((&gNfcip.rxBuf[gNfcip.rxBufPaylPos] !=
         &gNfcip.rxBuf[RFAL_NFCDEP_DEP_HEADER + optHdrLen]) &&
        (*outActRxLen > 0U)) {
      RFAL_MEMMOVE(&gNfcip.rxBuf[gNfcip.rxBufPaylPos],
                   &gNfcip.rxBuf[RFAL_NFCDEP_DEP_HEADER + optHdrLen],
                   *outActRxLen);
    }
    ...
    if (nfcip_PFBisIMI(rxPFB)) {
      ...
      *outIsChaining = true;
      ...
      RFAL_EXIT_ON_ERR(ret, nfcipDEPControlMsg(nfcip_PFBRPDU_ACK(gNfcip.pni),
  ...
  *outIsChaining = false;
  ...
  switch (rxRes) {
    ...
    default:
       ...
       * * The Target MUST always stay in receive mode when a * Transmission
       * Error or a Protocol Error occurs.                              *
       *                                                                             *
       * Do not push Transmission/Protocol Errors to upper layer in Listen Mode
       * #766 */
  ...
  switch (gNfcip.rxBuf[rxMsgIt++]) {
    ...
    case (uint8_t)NFCIP_CMD_DSL_REQ:
      ...
      /* [Digital 1.0, 16.9.1.2]: If DID == 0, Target SHALL ignore DSL_REQ with
       * DID */
      if ((((gNfcip.rxBuf[rxMsgIt++] != gNfcip.cfg.did) ||
    ...
    case (uint8_t)NFCIP_CMD_RLS_REQ:
      ...
      /* [Digital 1.0, 16.10.2.2]: If DID == 0, Target SHALL ignore DSL_REQ with
       * DID */
      if ((((gNfcip.rxBuf[rxMsgIt++] != gNfcip.cfg.did) ||
            (nfcDepLen != RFAL_NFCDEP_DSL_RLS_LEN_DID)) &&
           (gNfcip.cfg.did != RFAL_NFCDEP_DID_NO)) ||
    ...
    /*******************************************************************************/
    /*case NFCIP_CMD_PSL_REQ:              PSL must be handled in Activation
     * only */
  ...
  if (nfcip_PFBisRPDU(rxPFB)) {
    ...
    if (nfcip_PFBisRACK(rxPFB)) {
      ...
      if (gNfcip.pni == nfcip_PBF_PNI(rxPFB)) {
        ...
        /* This block has been transmitted and acknowledged, perform RTOX until
         * next data is provided  */

        /* Digital 1.1  16.12.4.7 - If ACK rcvd continue with chaining or an
         * RTOX */
        nfcipTimerStart(
            gNfcip.RTOXTimer,
      ...
      else if (nfcip_PFBisSATN(gNfcip.lastPFB) &&
               (nfcip_PNIDec(gNfcip.pni) == nfcip_PBF_PNI(rxPFB))) {
        nfcipLogI(" NFCIP(T) wrong PNI, last was ATN reTx  \r\n");
        /* Spec says to leave current PNI as is, but will be Inc after Tx,
         * remaining the same */
    ...
    else if (nfcip_PFBisRNACK(rxPFB) &&
             (nfcip_PNIDec(gNfcip.pni) == nfcip_PBF_PNI(rxPFB))) {
  ...
  if (nfcip_PFBisSPDU(rxPFB)) {
    ...
    else if (nfcip_PFBisSTO(rxPFB)) /* If is a S-TO (RTOX)  */
    {
      if (nfcip_PFBisSTO(gNfcip.lastPFBnATN)) {
        ...
        /* Digital 1.1  16.8.4.6  RTOX value in RES different that in REQ ->
         * Protocol Error */
        ...
        nfcipTimerStart(
            gNfcip.RTOXTimer,
            nfcipRTOXAdjust(nfcipConv1FcToMs(gNfcip.lastRTOX *
                                             rfalNfcDepWT2RWT(gNfcip.cfg.to))));
        gNfcip.state = NFCIP_ST_TARG_DEP_RTOX;
  ...
  if (nfcip_PFBisIPDU(rxPFB)) {
    if (gNfcip.pni != nfcip_PBF_PNI(rxPFB)) {
      ...
      if (nfcip_PFBisSATN(gNfcip.lastPFB) &&
          (nfcip_PNIDec(gNfcip.pni) == nfcip_PBF_PNI(rxPFB))) {
        /* Spec says to leave current PNI as is, but will be Inc after Data Tx,
         * remaining the same */
        gNfcip.pni = nfcip_PNIDec(gNfcip.pni);
        ...
        if (nfcip_PFBisIMI(rxPFB)) {
          nfcipLogI(
              ...
              "\r\n");
          RFAL_EXIT_ON_ERR(ret,
                           nfcipDEPControlMsg(nfcip_PFBRPDU_ACK(gNfcip.pni),
                                              gNfcip.rxBuf[rxMsgIt++]));

          /* Digital 1.1 16.12.3.4 (...) leave the current PNI unchanged
           * afterwards */
          gNfcip.pni = nfcip_PNIInc(gNfcip.pni);
        } else {
          nfcipLogI(
              " NFCIP(T) PNI = prevPNI && ATN before -> reTx last I-PDU  \r\n");
          gNfcip.state = NFCIP_ST_TARG_DEP_TX;
        }
...
static ReturnCode nfcipTx(rfalNfcDepCmd cmd, uint8_t *txBuf, uint8_t *paylBuf,
  ...
  if ((paylLen == 0U) || (payloadBuf == NULL)) {
    payloadBuf =
        (uint8_t *)&txBuf[RFAL_NFCDEP_DEPREQ_HEADER_LEN]; /* If not a DEP (no
                                                             Data) ensure enough
                                                             space for header */
  }
  ...
  switch (cmd) {
    ...
    case NFCIP_CMD_ATR_REQ:
      ...
      rfalNfcDepSetNFCID(payloadBuf, gNfcip.cfg.nfcid,
                         gNfcip.cfg.nfcidLen); /* NFCID */
      ...
      if (gNfcip.cfg.gbLen > 0U) {
        payloadBuf[txBufIt++] =
            nfcip_PPwGB(gNfcip.cfg.lr); /* PP signalling GB  */
        RFAL_MEMCPY(&payloadBuf[txBufIt], gNfcip.cfg.gb,
                    gNfcip.cfg.gbLen); /* set General Bytes */
        txBufIt += gNfcip.cfg.gbLen;
      } else {
        payloadBuf[txBufIt++] =
            rfalNfcDepLR2PP(gNfcip.cfg.lr); /* PP without GB     */
      }

      if ((txBufIt + RFAL_NFCDEP_CMDTYPE_LEN + RFAL_NFCDEP_CMD_LEN) >
          RFAL_NFCDEP_ATRREQ_MAX_LEN) /* Check max ATR length (ATR_REQ =
                                         ATR_RES)*/
      {
        return RFAL_ERR_PARAM;
      }
    ...
    case NFCIP_CMD_DEP_RES:
      ...
      *(--txBlock) = pfb; /* PFB */
      ...
      if ((paylLen == 0U) && nfcipIsEmptyDEPDisabled(gNfcip.cfg.oper) &&
          nfcip_PFBisIPDU(pfb)) {
        return RFAL_ERR_PARAM;
      }
      break;
  ...
  *(--txBlock) =
      (uint8_t)(nfcipCmdIsReq(cmd) ? NFCIP_REQ : NFCIP_RES); /* CMDType */

  txBufIt += paylLen +
             (uint16_t)((uintptr_t)payloadBuf -
                        (uintptr_t)txBlock); /* Calculate overall buffer size */

  if (txBufIt >
      gNfcip
          .fsc) /* Check if msg length violates the maximum payload size FSC */
  {
...
static void nfcipConfig(const rfalNfcDepConfigs *cfg) {
  ...
  RFAL_MEMCPY(&gNfcip.cfg, cfg,
              sizeof(rfalNfcDepConfigs)); /* Copy given config to local       */

  gNfcip.cfg.to = RFAL_MIN(RFAL_NFCDEP_WT_TRG_MAX,
                           gNfcip.cfg.to); /* Ensure proper WT value       */
  gNfcip.cfg.did = nfcip_DIDMax(gNfcip.cfg.did); /* Ensure proper DID value */
  gNfcip.fsc =
      rfalNfcDepLR2FS(gNfcip.cfg.lr); /* Calculate FSC based on given LR  */
  ...
  switch (gNfcip.state) {
    ...
    case NFCIP_ST_INIT_DEP_TX:
      ...
      nfcipLogD(" NFCIP(I) Tx PNI: %d txLen: %d \r\n", gNfcip.pni,
                gNfcip.txBufLen);
      ret = nfcipTx(NFCIP_CMD_DEP_REQ, gNfcip.txBuf,
                    &gNfcip.txBuf[gNfcip.txBufPaylPos], gNfcip.txBufLen,
    ...
    /*******************************************************************************/
    case NFCIP_ST_INIT_DEP_RX: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall
                                  through */

      ret = nfcipDataRx(false);

      if (ret != RFAL_ERR_BUSY) {
        ret = nfcipInitiatorHandleDEP(
            ret, ((gNfcip.rxRcvdLen != NULL) ? *gNfcip.rxRcvdLen : 0U),
            outActRxLen, outIsChaining);
      }
    ...
    case NFCIP_ST_TARG_DEP_RTOX:
      ...
      if (!nfcipTimerisExpired(
              gNfcip.RTOXTimer)) /* Do nothing until RTOX timer has expired */
      {
        return RFAL_ERR_BUSY;
      }
      ...
      if (nfcipIsRTOXReqDisabled(gNfcip.cfg.oper)) {
        ...
        nfcipLogI(
            " NFCIP(T) RTOX not sent due to config, NOT reenabling Rx \r\n");
        return RFAL_ERR_TIMEOUT;
      }

      if (gNfcip.cntRTOXRetrys++ >
          RFAL_NFCDEP_MAX_RTOX_RETRYS) /* Check maximum consecutive RTOX
                                          requests */
      {
        return RFAL_ERR_PROTO;
      }
      ...
      gNfcip.lastRTOX = nfcip_RTOXTargMax(
          gNfcip.cfg.to); /* Calculate requested RTOX value, and send it */
      RFAL_EXIT_ON_ERR(ret,
                       nfcipDEPControlMsg(nfcip_PFBSPDU_TO(), gNfcip.lastRTOX));

      /* Set waiting for RTOX Ack Flag */
      gNfcip.isWait4RTOX = true;

      gNfcip.state =
          NFCIP_ST_TARG_DEP_RX; /* Go back to Rx to process RTOX ack       */
      return RFAL_ERR_BUSY;
    ...
    case NFCIP_ST_TARG_DEP_TX:
      ...
      nfcipLogD(" NFCIP(T) Tx PNI: %d txLen: %d \r\n", gNfcip.pni,
                gNfcip.txBufLen);
      ret = nfcipTx(NFCIP_CMD_DEP_RES, gNfcip.txBuf,
                    &gNfcip.txBuf[gNfcip.txBufPaylPos], gNfcip.txBufLen,
                    nfcip_PFBIPDU(gNfcip.pni), NFCIP_NO_FWT);
      ...
      switch (ret) {
        ...
        default:
          gNfcip.state =
              NFCIP_ST_TARG_DEP_IDLE; /* Upon Tx error, goto IDLE state */
          return ret;
      }
      /* fall through */

    /*******************************************************************************/
    case NFCIP_ST_TARG_DEP_RX: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall
                                  through */

      if (gNfcip.isReqPending) /* if already has Data should be from a DEP from
...
static void nfcipSetDEPParams(const rfalNfcDepDEPParams *DEPParams) {
  ...
  if (gNfcip.cfg.role == RFAL_NFCDEP_ROLE_TARGET) {
    ...
    /*Digital 1.0  14.12.4.1 In target mode the first PDU MUST be sent by the
     * Initiator */
    gNfcip.state = NFCIP_ST_TARG_DEP_RX;
    return;
  }

  /* New data TxRx request clear previous error counters for consecutive TxRx
   * without reseting communication/protocol layer*/
  nfcipClearCounters();
...
bool rfalNfcDepTargetRcvdATR(void) {
  return ((gNfcip.cfg.role == RFAL_NFCDEP_ROLE_TARGET) &&
          nfcipIsTarget(gNfcip.state) &&
...
bool rfalNfcDepIsAtrReq(const uint8_t *buf, uint16_t bufLen, uint8_t *nfcid3) {
  ...
  if ((bufLen < RFAL_NFCDEP_ATRREQ_MIN_LEN) ||
      (bufLen > RFAL_NFCDEP_ATRREQ_MAX_LEN)) {
    return false;
  }
  ...
  if (nfcid3 != NULL) {
    RFAL_MEMCPY(nfcid3, &buf[RFAL_NFCDEP_ATR_REQ_NFCID3_POS],
  ...
  if (gNfcip.rxBuf[msgIt] == (uint8_t)NFCIP_CMD_PSL_REQ) {
    ...
    *outBRS = gNfcip.rxBuf[msgIt++]; /* assign output BRS value      */
    ...
    if (nfcDepDev != NULL) {
      ...
      nfcDepDev->activation.Initiator.ATR_REQ.PPi |=
    ...
    if (gNfcip.rxBuf[msgIt] == (uint8_t)NFCIP_CMD_DEP_REQ) {
      ...
      /*******************************************************************************/
      /* Digital 1.0 14.8.2.1  check if DID is expected and match -> Protocol
       * Error  */
      if (nfcip_PFBhasDID(gNfcip.rxBuf[msgIt])) {
        if (gNfcip.rxBuf[++msgIt] != gNfcip.cfg.did) {
          return RFAL_ERR_PROTO;
        }
      } else if (gNfcip.cfg.did !=
                 RFAL_NFCDEP_DID_NO) /* DID expected but not rcv */
      {
        return RFAL_ERR_PROTO;
      } else {
        /* MISRA 15.7 - Empty else */
    ...
    /* Signal Request pending to be digested on normal Handling (DEP_REQ,
     * DSL_REQ, RLS_REQ) */
...
ReturnCode rfalNfcDepATR(const rfalNfcDepAtrParam *param,
  ...
  RFAL_EXIT_ON_ERR(
      ret, nfcipTxRx(NFCIP_CMD_ATR_REQ, txBuf, nfcipRWTActivation(), NULL, 0,
                     rxBuf, NFCIP_ATRRES_BUF_LEN, &rxLen));
  ...
  if (rxBuf[msgIt++] !=
      (uint8_t)NFCIP_CMD_ATR_RES) /* Checking if is a ATR RES */
  {
    return RFAL_ERR_PROTO;
  }
...
ReturnCode rfalNfcDepPSL(uint8_t BRS, uint8_t FSL) {
  ...
  RFAL_EXIT_ON_ERR(
      ret,
      nfcipTxRx(NFCIP_CMD_PSL_REQ, txBuf, (gNfcip.cfg.fwt + gNfcip.cfg.dFwt),
                &txBuf[NFCIP_PSLREQ_LEN], (msgIt - NFCIP_PSLREQ_LEN), rxBuf,
                NFCIP_PSLRES_LEN, &rxLen));
  ...
  if (rxBuf[msgIt++] !=
      (uint8_t)NFCIP_CMD_PSL_RES) /* Checking if is a PSL RES       */
  {
    return RFAL_ERR_PROTO;
  }
...
ReturnCode rfalNfcDepDSL(void) {
  ...
  RFAL_EXIT_ON_ERR(ret, nfcipTxRx(NFCIP_CMD_DSL_REQ, txBuf,
                                  (gNfcip.cfg.fwt + gNfcip.cfg.dFwt), NULL, 0,
                                  rxBuf, (uint16_t)sizeof(rxBuf), &rxLen));
  ...
  if (rxBuf[rxMsgIt++] !=
      (uint8_t)NFCIP_CMD_DSL_RES) /* Checking if is DSL RES          */
  {
    return RFAL_ERR_PROTO;
  }
...
ReturnCode rfalNfcDepRLS(void) {
  ...
  if (gNfcip.cfg.role ==
      RFAL_NFCDEP_ROLE_TARGET) /* Target has no release procedure */
  {
    return RFAL_ERR_NONE;
  }

  /* Repeating a RLS REQ is optional, not doing it */
  RFAL_EXIT_ON_ERR(ret, nfcipTxRx(NFCIP_CMD_RLS_REQ, txBuf,
                                  (gNfcip.cfg.fwt + gNfcip.cfg.dFwt), NULL, 0,
                                  rxBuf, (uint16_t)sizeof(rxBuf), &rxLen));
  ...
  if (rxBuf[rxMsgIt++] !=
      (uint8_t)NFCIP_CMD_RLS_RES) /* Checking if is RLS RES         */
  {
    return RFAL_ERR_PROTO;
  }
...
ReturnCode rfalNfcDepInitiatorHandleActivation(rfalNfcDepAtrParam *param,
  ...
  param->NAD = RFAL_NFCDEP_NAD_NO; /* Digital 1.1  16.6.2.9  Initiator SHALL NOT
                                      use NAD */
  ...
  do { /* Upon transmission error ATR REQ should be retried */
    ...
    ret = rfalNfcDepATR(param, &nfcDepDev->activation.Target.ATR_RES,
  ...
  /*******************************************************************************/
  nfcDepDev->info.GBLen =
      (nfcDepDev->activation.Target.ATR_RESLen - RFAL_NFCDEP_ATRRES_MIN_LEN);
  nfcDepDev->info.DID = nfcDepDev->activation.Target.ATR_RES.DID;
  nfcDepDev->info.NAD = RFAL_NFCDEP_NAD_NO; /* Digital 1.1  16.6.3.11 Initiator
                                               SHALL ignore b1 of PPt */
  nfcDepDev->info.LR =
      rfalNfcDepPP2LR(nfcDepDev->activation.Target.ATR_RES.PPt);
  nfcDepDev->info.FS = rfalNfcDepLR2FS(nfcDepDev->info.LR);
  nfcDepDev->info.WT =
      (nfcDepDev->activation.Target.ATR_RES.TO & RFAL_NFCDEP_WT_MASK);
   ...
   * (Activity 1.0) PSL_REQ shall be sent to update LR or bit rate
   * (Activity 2.0)
   * */
      ...
      (desiredBR != RFAL_BR_KEEP)) /* if desired BR is different    */
  {
    if (nfcipDxIsSupported(
            (uint8_t)desiredBR, nfcDepDev->activation.Target.ATR_RES.BRt,
...
static ReturnCode nfcipDataTx(uint8_t *txBuf, uint16_t txBufLen, uint32_t fwt) {
  return rfalTransceiveBlockingTx(
      txBuf, txBufLen, gNfcip.rxBuf, gNfcip.rxBufLen, gNfcip.rxRcvdLen,
      (RFAL_TXRX_FLAGS_DEFAULT | (uint32_t)RFAL_TXRX_FLAGS_NFCIP1_ON),
  ...
  if (ret != RFAL_ERR_BUSY) {
    if (gNfcip.rxRcvdLen != NULL) {
      ...
      if ((ret == RFAL_ERR_NONE) && (gNfcip.rxBuf != NULL)) {
        /* Digital 1.1  16.4.1.3 - Length byte LEN SHALL have a value between 3
         * and 255 -> otherwise treat as Transmission Error *
         *                       - Ensure that actual received and frame length
         * do match, otherwise treat as Transmission error  */
...
ReturnCode rfalNfcDepListenStartActivation(const rfalNfcDepTargetParam *param,
  ...
  rxParam.nfcDepDev->activation.Initiator.ATR_REQLen =
      (uint8_t)atrReqLength; /* nfcipIsAtrReq() is already checking Min and Max
                                buffer lengths */
  if (atrReqLength > 0U)     /* MISRA 21.18 */
  {
    RFAL_MEMCPY((uint8_t *)&rxParam.nfcDepDev->activation.Initiator.ATR_REQ,
                atrReq, atrReqLength);
  }

  rxParam.nfcDepDev->info.GBLen =
      (uint8_t)(atrReqLength - RFAL_NFCDEP_ATRREQ_MIN_LEN);
  rxParam.nfcDepDev->info.DID =
      rxParam.nfcDepDev->activation.Initiator.ATR_REQ.DID;
  ...
  RFAL_EXIT_ON_ERR(ret, nfcipTx(NFCIP_CMD_ATR_RES, (uint8_t *)gNfcip.rxBuf,
                                NULL, 0, 0, NFCIP_NO_FWT));
...
ReturnCode rfalNfcDepStartTransceive(const rfalNfcDepTxRxParam *param) {
  ...
  nfcDepParams.txBufPaylPos =
      RFAL_NFCDEP_DEPREQ_HEADER_LEN; /* position in txBuf where actual outgoing
                                        data is located */
...
/*******************************************************************************/
static void rfalNfcDepPdu2BLockParam(rfalNfcDepPduTxRxParam pduParam,
  ...
  maxInfLen =
      (blockParam->FSx - (RFAL_NFCDEP_HEADER + RFAL_NFCDEP_DEP_PFB_LEN));
  maxInfLen +=
      ((blockParam->DID != RFAL_NFCDEP_DID_NO) ? RFAL_NFCDEP_DID_LEN : 0U);
  ...
  blockParam->txBuf =
      (rfalNfcDepBufFormat *)
          pduParam.txBuf; /*  PRQA S 0310 # MISRA 11.3 - Intentional safe cast
                             to avoiding large buffer duplication */
  blockParam->rxBuf =
      pduParam.tmpBuf; /* Simply using the pdu buffer is not possible because of
                          current ACK handling */
...
ReturnCode rfalNfcDepStartPduTransceive(rfalNfcDepPduTxRxParam param) {
  ...
  rfalNfcDepPdu2BLockParam(gNfcip.PDUParam, &txRxParam, gNfcip.PDUTxPos,
                           gNfcip.PDURxPos);
...
ReturnCode rfalNfcDepGetPduTransceiveStatus(void) {
  ...
  switch (ret) {
    ...
    case RFAL_ERR_NONE:
      ...
      if (gNfcip.isTxChaining) {
        ...
        if (txRxParam.txBufLen > 0U) /* MISRA 21.18 */
        {
          ...
          RFAL_MEMCPY(gNfcip.PDUParam.txBuf->pdu,
    ...
    case RFAL_ERR_AGAIN: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall through
      ...
      /* Check if no PDU transceive has been started before (data from
       * rfalNfcDepListenStartActivation) */
      ...
      if ((*gNfcip.PDUParam.rxLen) > 0U) /* MISRA 21.18 */
      {
        ...
        RFAL_MEMCPY(&gNfcip.PDUParam.rxBuf->pdu[gNfcip.PDURxPos],
                    gNfcip.PDUParam.tmpBuf->inf, *gNfcip.PDUParam.rxLen);
