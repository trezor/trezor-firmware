...
/*! Struct that holds all involved on a Transceive including the context passed
 * by the caller     */
typedef struct {
...
/*! Struct that holds certain WU mode information to be retrieved by
 * rfalWakeUpModeGetInfo        */
typedef struct {
...
typedef struct {
  uint16_t expWL; /*!< The amount of bytes expected to be Tx when a WL interrupt
                     occours                          */
  uint16_t bytesTotal;   /*!< Total bytes to be transmitted OR the total bytes
                            received                                  */
  uint16_t bytesWritten; /*!< Amount of bytes already written on FIFO (Tx) OR
                            read (RX) from FIFO and written on rxBuffer*/
  uint8_t status[ST25R3916_FIFO_STATUS_LEN]; /*!< FIFO Status Registers */
} rfalFIFO;

/*! Struct that holds RFAL's configuration settings */
typedef struct {
  uint8_t obsvModeTx; /*!< RFAL's config of the ST25R3916's observation mode
                         while Tx */
  uint8_t obsvModeRx; /*!< RFAL's config of the ST25R3916's observation mode
                         while Rx */
  rfalEHandling eHandling; /*!< RFAL's error handling config/mode */
} rfalConfigs;

/*! Struct that holds NFC-A data - Used only inside
...
typedef struct {
  ...
  rfalFeliCaPollRes
      pollResponses[RFAL_FELICA_POLL_MAX_SLOTS]; /* FeliCa Poll response buffer
                                                    (16 slots)  */
} rfalNfcfWorkingData;

/*! Struct that holds NFC-V current context
 *
 * This buffer has to be big enough for coping with maximum response size
 * (hamming coded)
...
typedef struct {
  uint8_t codingBuffer[((2 + 255 + 3) * 2)]; /*!< Coding buffer,   length MUST
                                                be above 257: [257; ...]    */
  uint16_t nfcvOffset; /*!< Offset needed for ISO15693 coding function */
  rfalTransceiveContext origCtx; /*!< context provided by user */
  uint16_t ignoreBits; /*!< Number of bits at the beginning of a frame to be
...
typedef struct {
  ...
  rfalNfcaWorkingData
      nfcaData; /*!< RFAL's working data when supporting NFC-A     */
#endif          /* RFAL_FEATURE_NFCA */

#if RFAL_FEATURE_NFCF
  rfalNfcfWorkingData
      nfcfData; /*!< RFAL's working data when supporting NFC-F     */
#endif          /* RFAL_FEATURE_NFCF */

#if RFAL_FEATURE_NFCV
  rfalNfcvWorkingData
      nfcvData; /*!< RFAL's working data when performing NFC-V     */
...
typedef union { /*  PRQA S 0750 # MISRA 19.2 - Both members are of the same
                   type, just different names.  Thus no problem can occur. */
  uint8_t PTMem_A[ST25R3916_PTM_A_LEN]; /*!< PT_Memory area allocated for NFC-A
                                           configuration               */
  uint8_t PTMem_F[ST25R3916_PTM_F_LEN]; /*!< PT_Memory area allocated for NFC-F
                                           configuration               */
  uint8_t TSN[ST25R3916_PTM_TSN_LEN];   /*!< PT_Memory area allocated for TSN -
                                           Random numbers              */
} t_rfalPTMem;
...
#define RFAL_FIFO_OUT_WL                                              \
  (ST25R3916_FIFO_DEPTH -                                             \
   RFAL_FIFO_IN_WL) /*!< Number of bytes sent/out of the FIFO when WL \
                       interrupt occurs while Tx          */
...
#define RFAL_ST25R3916_GPT_MAX_1FC                                             \
  rfalConv8fcTo1fc(0xFFFFU) /*!< Max GPT steps in 1fc (0xFFFF steps of 8/fc => \
                               0xFFFF * 590ns  = 38,7ms)      */
#define RFAL_ST25R3916_NRT_MAX_1FC                                        \
  rfalConv4096fcTo1fc(0xFFFFU) /*!< Max NRT steps in 1fc (0xFFFF steps of \
                                  4096/fc => 0xFFFF * 302us  = 19.8s )      */
...
#define RFAL_ST25R3916_MRT_MAX_1FC                                            \
  rfalConv64fcTo1fc(0x00FFU) /*!< Max MRT steps in 1fc (0x00FF steps of 64/fc \
                                => 0x00FF * 4.72us = 1.2ms )      */
#define RFAL_ST25R3916_MRT_MIN_1FC                                             \
  rfalConv64fcTo1fc(0x0004U) /*!< Min MRT steps in 1fc ( 0<=mrt<=4 ; 4 (64/fc) \
                                => 0x0004 * 4.72us = 18.88us )    */
#define RFAL_ST25R3916_GT_MAX_1FC                                             \
  rfalConvMsTo1fc(6000U) /*!< Max GT value allowed in 1/fc (SFGI=14 => SFGT + \
                            dSFGT = 5.4s)                   */
...
#define RFAL_RX_INC_BYTE_LEN                                                  \
  (uint8_t)1U /*!< Threshold where incoming rx shall be considered incomplete \
                 byte NFC - T2T       */
...
#define RFAL_FELICA_POLL_SLOT_TIME                                             \
  256U /*!<  FeliCa Poll Time Slot duration is 1.208 ms ~256*64/fc Digital 1.1 \
          A4           */
...
#define RFAL_LM_GT                                                          \
  rfalConvUsTo1fc(100U) /*!< Listen Mode Guard Time enforced (GT - Passive; \
                           TIRFG - Active)                  */
#define RFAL_FDT_POLL_ADJUSTMENT                                             \
  rfalConvUsTo1fc(80U) /*!< FDT Poll adjustment: Time between the expiration \
                          of GPT to the actual Tx        */
...
#define RFAL_ST25R3916B_AAT_SETTLE                                           \
  ST25R3916_REG_MEAS_TX_DELAY_meas_tx_del_4_83ms /*!< Time between Oscilator \
                                                    stable and TX On in      \
                                                    meas_tx_del steps */
 ...
 *    SOF (14etu) + 1Byte (10etu) + 1etu (IRQ comes 1etu after first byte) -
 * 3etu (ST25R3916 sends TXE 3etu after) */
...
/*! FDT Listen adjustment for ISO14443A   EMVCo 2.6  4.8.1.3  ;
 * Digital 1.1  6.10
 *
 *  276: Time from the rising pulse of the pause of the logic '1' (i.e. the time
 * point to measure the deaftime from), to the actual end of the EOF sequence
 * (the point where the MRT starts). Please note that the ST25R391x uses the
 *       ISO14443-2 definition where the EOF consists of logic '0' followed by
 * sequence Y. -64: Further adjustment for receiver to be ready just before
 * first bit
 */
 ...
 * Set FDT Listen one step earlier than on the more recent spec versions for
 * greater interoprability
 */
...
/*! Calculates Transceive Sanity Timer. It accounts for the slowest bit rate and
 * the longest data format 1s for transmission and reception of a 4K message at
 * 106kpbs (~425ms each direction) plus TxRx preparation and FIFO load over
 * Serial Interface                                      */
#define rfalCalcSanityTmr(fwt) (uint16_t)(1000U + rfalConv1fcToMs((fwt)))

#define rfalGennTRFW(n)                                                        \
  ((uint8_t)(((n) + 1U) % 7U)) /*!< Generate next n*TRFW used for RFCA: modulo \
                                  a prime to avoid alias effects */

#define rfalCalcNumBytes(nBits)                                                \
  (((uint32_t)(nBits) + 7U) / 8U) /*!< Returns the number of bytes required to \
                                     fit given the number of bits */
...
#define rfalTimerisExpired(timer) \
  platformTimerIsExpired(timer) /*!< Checks if timer has expired */
#define rfalTimerDestroy(timer) \
  platformTimerDestroy(timer) /*!< Destroys timer */

#define rfalST25R3916ObsModeDisable() \
  st25r3916WriteTestRegister(         \
      0x01U, (0x40U)) /*!< Disable ST25R3916 Observation mode */
#define rfalST25R3916ObsModeTx() \
  st25r3916WriteTestRegister(    \
      0x01U,                     \
      (0x40U | gRFAL.conf.obsvModeTx)) /*!< Enable Tx Observation mode */
#define rfalST25R3916ObsModeRx() \
  st25r3916WriteTestRegister(    \
      0x01U,                     \
      (0x40U | gRFAL.conf.obsvModeRx)) /*!< Enable Rx Observation mode */
...
#define rfalIsIncompleteByteError(error)                                     \
  (((error) >= RFAL_ERR_INCOMPLETE_BYTE) &&                                  \
   ((error) <= RFAL_ERR_INCOMPLETE_BYTE_07)) /*!< Checks if given error is a \
                                                Incomplete error */
...
#define rfalConvBR2ACBR(b)                                                     \
  (((rfalAdjACBR((b))) << RFAL_ANALOG_CONFIG_BITRATE_SHIFT) &                  \
   RFAL_ANALOG_CONFIG_BITRATE_MASK) /*!< Converts ST25R391x Bit rate to Analog \
                                       Configuration bit rate id */

#define rfalConvTDFormat(v) \
  ((uint16_t)(v)            \
   << 8U) /*!< Converts a uint8_t to the format used in SW Tag Detection */
#define rfalAddFracTDFormat(fd) ((((uint16_t)(fd)) & 0x03U) * 64U)

#define rfalRunBlocking(e, fn) \
  do {                         \
    (e) = (fn);                \
    rfalWorker();              \
  } while ((e) ==              \
           RFAL_ERR_BUSY) /*!< Macro used for the blocking operations      */
...
static uint16_t rfalWakeUpModeFilter(uint16_t curRef, uint16_t curVal,
                                     uint8_t weight);
#endif /* RFAL_FEATURE_WAKEUP_MODE */
...
ReturnCode rfalInitialize(void) {
  ...
  /*******************************************************************************/
  /* Perform Automatic Calibration (if configured to do so). * Registers set by
   * rfalSetAnalogConfig will tell rfalCalibrate what to perform*/
  rfalCalibrate();
...
ReturnCode rfalCalibrate(void) {
  ...
  /* Automatic regulator adjustment only performed if not set manually on Analog
   * Configs */
  if (st25r3916CheckReg(ST25R3916_REG_REGULATOR_CONTROL,
                        ST25R3916_REG_REGULATOR_CONTROL_reg_s, 0x00)) {
    /* Adjust the regulators so that Antenna Calibrate has better Regulator
     * values */
...
ReturnCode rfalAdjustRegulators(uint16_t *result) {
  ...
  st25r3916WriteRegister(ST25R3916_REG_AUX_MOD,
                         (reg_auxmod & ~ST25R3916_REG_AUX_MOD_rgs_am));
#endif /* ST25R3916B */
...
ReturnCode rfalDeinitialize(void) {
  ...
  rfalSetAnalogConfig(
      (RFAL_ANALOG_CONFIG_TECH_CHIP | RFAL_ANALOG_CONFIG_CHIP_DEINIT));
...
ReturnCode rfalSetMode(rfalMode mode, rfalBitRate txBR, rfalBitRate rxBR) {
  ...
  switch (mode) {
    ...
    case RFAL_MODE_POLL_NFCB:
      ...
      st25r3916WriteRegister(ST25R3916_REG_MODE,
                             ST25R3916_REG_MODE_om_iso14443b);

      /* Set the EGT, SOF, EOF and EOF */
      st25r3916ChangeRegisterBits(
          ST25R3916_REG_ISO14443B_1,
          (ST25R3916_REG_ISO14443B_1_egt_mask |
           ST25R3916_REG_ISO14443B_1_sof_mask | ST25R3916_REG_ISO14443B_1_eof),
          ((0U << ST25R3916_REG_ISO14443B_1_egt_shift) |
           ST25R3916_REG_ISO14443B_1_sof_0_10etu |
           ST25R3916_REG_ISO14443B_1_sof_1_2etu |
    ...
    case RFAL_MODE_POLL_ACTIVE_P2P:
      ...
      /* Set NFCIP1 active communication Initiator mode and Automatic Response
       * RF Collision Avoidance to always after EOF */
      ...
      /* Set GPT to start after end of TX, as GPT is used in active
       * communication mode to timeout the field switching off after TXE */
      st25r3916SetStartGPTimer(
          (uint16_t)rfalConv1fcTo8fc(RFAL_AP2P_FIELDOFF_TCMDOFF),
          ST25R3916_REG_TIMER_EMV_CONTROL_gptc_etx_nfc);

      /* Set PPon2 timer with the max time between our field Off and other peer
       * field On : Tadt + (n x Trfw)    */
      st25r3916WriteRegister(
          ST25R3916_REG_PPON2,
          (uint8_t)rfalConv1fcTo64fc(RFAL_AP2P_FIELDON_TADTTRFW));
    ...
    case RFAL_MODE_LISTEN_ACTIVE_P2P:
      ...
      /* Set NFCIP1 active communication Target mode and Automatic Response RF
       * Collision Avoidance to always after EOF */
      st25r3916WriteRegister(
          ST25R3916_REG_MODE,
  ...
  gRFAL.state =
      ((gRFAL.state < RFAL_STATE_MODE_SET) ? RFAL_STATE_MODE_SET : gRFAL.state);
  gRFAL.mode = mode;
...
/*******************************************************************************/
rfalMode rfalGetMode(void) { return gRFAL.mode; }
...
ReturnCode rfalSetBitRate(rfalBitRate txBR, rfalBitRate rxBR) {
  ...
  if ((RFAL_MODE_POLL_NFCV != gRFAL.mode) &&
    ...
    RFAL_EXIT_ON_ERR(
        ret, st25r3916SetBitrate((uint8_t)gRFAL.txBR, (uint8_t)gRFAL.rxBR));
  }
  ...
  switch (gRFAL.mode) {
    ...
    case RFAL_MODE_POLL_NFCA_T1T:
      ...
      rfalSetAnalogConfig((rfalAnalogConfigId)(RFAL_ANALOG_CONFIG_POLL |
                                               RFAL_ANALOG_CONFIG_TECH_NFCA |
                                               rfalConvBR2ACBR(gRFAL.txBR) |
                                               RFAL_ANALOG_CONFIG_TX));
      rfalSetAnalogConfig((rfalAnalogConfigId)(RFAL_ANALOG_CONFIG_POLL |
                                               RFAL_ANALOG_CONFIG_TECH_NFCA |
                                               rfalConvBR2ACBR(gRFAL.rxBR) |
    ...
    case RFAL_MODE_POLL_PICOPASS:
...

      {
        ...
        switch (gRFAL.rxBR) {
          case RFAL_BR_52p97: /*  PRQA S 2880 # MISRA 2.1 - Inconsistently
                                 marked as unreachable code */
            config.speedMode = 1;
            break;
          default:
            config.speedMode = 0;
        ...
        /* MISRA 11.3 - Cannot point directly into different object type, copy
         * to local var */
        st25rStreamConf.din = rfalIso15693StreamConfig->din;
        st25rStreamConf.dout = rfalIso15693StreamConfig->dout;
        st25rStreamConf.report_period_length =
            rfalIso15693StreamConfig->report_period_length;
    ...
    case RFAL_MODE_LISTEN_ACTIVE_P2P:
      ...
      rfalSetAnalogConfig((RFAL_ANALOG_CONFIG_TECH_CHIP |
                           RFAL_ANALOG_CONFIG_CHIP_LISTEN_COMMON));
      rfalSetAnalogConfig((rfalAnalogConfigId)(RFAL_ANALOG_CONFIG_LISTEN |
                                               RFAL_ANALOG_CONFIG_TECH_AP2P |
                                               rfalConvBR2ACBR(gRFAL.txBR) |
                                               RFAL_ANALOG_CONFIG_TX));
      rfalSetAnalogConfig((rfalAnalogConfigId)(RFAL_ANALOG_CONFIG_LISTEN |
...
void rfalSetErrorHandling(rfalEHandling eHandling) {
  switch (eHandling) {
    case RFAL_ERRORHANDLING_NONE:
      st25r3916ClrRegisterBits(ST25R3916_REG_EMD_SUP_CONF,
                               ST25R3916_REG_EMD_SUP_CONF_emd_emv);
      break;
    ...
    case RFAL_ERRORHANDLING_EMD:
      ...
      st25r3916ModifyRegister(
          ...
          (ST25R3916_REG_EMD_SUP_CONF_emd_emv |
           ST25R3916_REG_EMD_SUP_CONF_emd_thld_mask),
...
/*******************************************************************************/
rfalEHandling rfalGetErrorHandling(void) { return gRFAL.conf.eHandling; }
...
/*******************************************************************************/
uint32_t rfalGetFDTPoll(void) { return gRFAL.timings.FDTPoll; }
...
/*******************************************************************************/
uint32_t rfalGetFDTListen(void) { return gRFAL.timings.FDTListen; }
...
/*******************************************************************************/
uint32_t rfalGetGT(void) { return gRFAL.timings.GT; }
...
ReturnCode rfalFieldOnAndStartGT(void) {
  ...
  rfalSetAnalogConfig(
      (RFAL_ANALOG_CONFIG_TECH_CHIP | RFAL_ANALOG_CONFIG_CHIP_FIELD_ON));
  ...
  if ((!st25r3916IsTxEnabled()) || (!gRFAL.field)) {
    ...
    st25r3916ChangeRegisterBits(ST25R3916_REG_OP_CONTROL,
                                ST25R3916_REG_OP_CONTROL_en_fd_mask,
                                ST25R3916_REG_OP_CONTROL_en_fd_manual_efd_ca);

    /* Use Thresholds set by AnalogConfig */
    ret = st25r3916PerformCollisionAvoidance(
        ST25R3916_CMD_INITIAL_RF_COLLISION, ST25R3916_THRESHOLD_DO_NOT_SET,
        ST25R3916_THRESHOLD_DO_NOT_SET,
        (ST25R3916_REG_AUX_nfc_n_mask & gRFAL.timings.nTRFW));
  ...
  if ((gRFAL.timings.GT != RFAL_TIMING_NONE)) {
    ...
    rfalTimerStart(gRFAL.tmr.GT,
                   rfalConv1fcToMs(RFAL_MAX((gRFAL.timings.GT),
                                            RFAL_ST25R3916_GT_MIN_1FC)));
  }
...
ReturnCode rfalFieldOff(void) {
  ...
  rfalSetAnalogConfig(
      (RFAL_ANALOG_CONFIG_TECH_CHIP | RFAL_ANALOG_CONFIG_CHIP_FIELD_OFF));
  gRFAL.field = false;
...
ReturnCode rfalStartTransceive(const rfalTransceiveContext *ctx) {
  ...
  if (((ctx->flags & (uint32_t)RFAL_TXRX_FLAGS_PAR_RX_KEEP) != 0U) &&
      ((ctx->flags & (uint32_t)RFAL_TXRX_FLAGS_CRC_RX_MANUAL) == 0U)) {
    return RFAL_ERR_NOTSUPP;
  }
  ...
  if (gRFAL.state >= RFAL_STATE_MODE_SET) {
    ...
    if (gRFAL.timings.FDTListen != RFAL_TIMING_NONE) {
      ...
      st25r3916ClrRegisterBits(ST25R3916_REG_TIMER_EMV_CONTROL,
                               ST25R3916_REG_TIMER_EMV_CONTROL_mrt_step);

      /* If Correlator is being used further adjustment is required for NFCB */
      if (gRFAL.mode == RFAL_MODE_POLL_NFCB) {
        if (st25r3916CheckReg(ST25R3916_REG_AUX, ST25R3916_REG_AUX_dis_corr,
                              0x00U)) {
          FxTAdj +=
              (uint32_t)RFAL_FDT_LISTEN_B_ADJT_CORR; /* Reduce FDT(Listen) */
          st25r3916SetRegisterBits(
              ST25R3916_REG_CORR_CONF1,
              ST25R3916_REG_CORR_CONF1_corr_s3); /* Ensure BPSK start to 33
                                                    pilot pulses */
          st25r3916ChangeRegisterBits(
      ...
      st25r3916WriteRegister(
          ...
          (uint8_t)rfalConv1fcTo64fc((FxTAdj > gRFAL.timings.FDTListen)
                                         ? RFAL_ST25R3916_MRT_MIN_1FC
                                         : (gRFAL.timings.FDTListen - FxTAdj)));
    }

    /*******************************************************************************/
    /* FDT Poll will be loaded in rfalPrepareTransceive() once the previous was
     * expired */
    ...
    if ((gRFAL.TxRx.ctx.fwt != RFAL_FWT_NONE) && (gRFAL.TxRx.ctx.fwt != 0U)) {
      ...
      if ((gRFAL.mode == RFAL_MODE_POLL_NFCF) ||
          (gRFAL.mode == RFAL_MODE_POLL_ACTIVE_P2P)) {
        FxTAdj +=
            (uint32_t)((gRFAL.txBR == RFAL_BR_212) ? RFAL_FWT_F_212_ADJUSTMENT
                                                   : RFAL_FWT_F_424_ADJUSTMENT);
      }

      /* Ensure that the given FWT doesn't exceed NRT maximum */
      gRFAL.TxRx.ctx.fwt =
          RFAL_MIN((gRFAL.TxRx.ctx.fwt + FxTAdj), RFAL_ST25R3916_NRT_MAX_1FC);
    ...
    if ((RFAL_MODE_POLL_NFCV == gRFAL.mode) ||
      ...
      gRFAL.TxRx.ctx.rxBuf =
          ((gRFAL.nfcvData.origCtx.rxBuf != NULL) ? gRFAL.nfcvData.codingBuffer
                                                  : NULL);
      ...
      gRFAL.TxRx.ctx.flags = (uint32_t)RFAL_TXRX_FLAGS_CRC_TX_MANUAL |
                             ...
                             (uint32_t)(gRFAL.nfcvData.origCtx.flags &
                                        (uint32_t)RFAL_TXRX_FLAGS_AGC_OFF) |
                             (uint32_t)RFAL_TXRX_FLAGS_PAR_RX_KEEP |
                             (uint32_t)RFAL_TXRX_FLAGS_PAR_TX_NONE;

      /* In NFCV a TxRx with a valid txBuf and txBufSize==0 indicates to send an
       * EOF */
    ...
    if (st25r3916CheckReg(ST25R3916_REG_AUX_MOD, ST25R3916_REG_AUX_MOD_rgs_am,
                          ST25R3916_REG_AUX_MOD_rgs_am) &&
        rfalIsModeActiveComm(gRFAL.mode)) {
    ...
    if ((gRFAL.TxRx.ctx.txBuf == NULL) || (gRFAL.TxRx.ctx.txBufLen == 0U)) {
      ...
      /* Start NRT manually, if FWT = 0 (wait endlessly for Rx) chip will ignore
       * anyhow */
...
bool rfalIsTransceiveInTx(void) {
  return ((gRFAL.TxRx.state >= RFAL_TXRX_STATE_TX_IDLE) &&
          (gRFAL.TxRx.state < RFAL_TXRX_STATE_RX_IDLE));
}
...
/*******************************************************************************/
ReturnCode rfalTransceiveBlockingTx(uint8_t *txBuf, uint16_t txBufLen,
  ...
  rfalCreateByteFlagsTxRxContext(ctx, txBuf, txBufLen, rxBuf, rxBufLen, actLen,
                                 flags, fwt);
...
ReturnCode rfalTransceiveBlockingTxRx(uint8_t *txBuf, uint16_t txBufLen,
  ...
  RFAL_EXIT_ON_ERR(ret, rfalTransceiveBlockingTx(txBuf, txBufLen, rxBuf,
                                                 rxBufLen, actLen, flags, fwt));
...
/*******************************************************************************/
rfalTransceiveState rfalGetTransceiveState(void) { return gRFAL.TxRx.state; }

/*******************************************************************************/
ReturnCode rfalGetTransceiveStatus(void) {
  return ((gRFAL.TxRx.state == RFAL_TXRX_STATE_IDLE) ? gRFAL.TxRx.status
                                                     : RFAL_ERR_BUSY);
}
...
ReturnCode rfalGetTransceiveRSSI(uint16_t *rssi) {
  ...
  isSumMode = (st25r3916CheckReg(ST25R3916_REG_CORR_CONF1,
                                 ST25R3916_REG_CORR_CONF1_corr_s4,
                                 ST25R3916_REG_CORR_CONF1_corr_s4)
                   ? st25r3916CheckReg(ST25R3916_REG_AUX,
                                       ST25R3916_REG_AUX_dis_corr, 0x00)
                   : false);
  if (isSumMode) {
    /*******************************************************************************/
    ...
    *rssi = (uint16_t)sqrt(
        ...
        ((double)pmRSSI *
         (double)pmRSSI)); /*  PRQA S 5209 # MISRA 4.9 - External function
                              (sqrt()) requires double */
#else
    ...
    *rssi = (st25r3916CheckReg(ST25R3916_REG_AUX_DISPLAY,
...
static void rfalErrorHandling(void) {
  ...
  if (gRFAL.conf.eHandling == RFAL_ERRORHANDLING_EMD) {
    ...
    rxHasIncParError = (rfalFIFOStatusIsIncompleteByte()
                            ? true
                            : rfalFIFOStatusIsMissingPar()); /* MISRA 13.5 */

    /* In case there are residual bits decrement FIFO bytes */
    /* Ensure FIFO contains some byte as the FIFO might be empty upon Framing
     * errors */
    ...
    if (((gRFAL.fifo.bytesTotal + fifoBytesToRead) < RFAL_EMVCO_RX_MAXLEN) &&
        ((gRFAL.TxRx.status == RFAL_ERR_RF_COLLISION) ||
         ...
         (gRFAL.TxRx.status == RFAL_ERR_CRC) || rxHasIncParError)) {
      /* Ignore this reception, ReEnable receiver which also clears the FIFO */
      st25r3916ExecuteCommand(ST25R3916_CMD_UNMASK_RECEIVE_DATA);

      /* Ensure that the NRT has not expired meanwhile */
      if (st25r3916CheckReg(ST25R3916_REG_NFCIP1_BIT_RATE,
                            ST25R3916_REG_NFCIP1_BIT_RATE_nrt_on, 0x00)) {
        if (st25r3916CheckReg(ST25R3916_REG_AUX_DISPLAY,
                              ST25R3916_REG_AUX_DISPLAY_rx_act, 0x00)) {
          /* Abort reception */
  ...
  if (gRFAL.mode == RFAL_MODE_POLL_NFCA) {
    /*******************************************************************************/
    /* If we received a frame with a incomplete byte we`ll raise a specific
     * error  * ( support for T2T 4 bit ACK / NAK, MIFARE and Kovio ) */
    /*******************************************************************************/
    if ((gRFAL.TxRx.status == RFAL_ERR_PAR) ||
        (gRFAL.TxRx.status == RFAL_ERR_CRC)) {
      if ((rfalFIFOStatusIsIncompleteByte()) &&
          (fifoBytesToRead == RFAL_RX_INC_BYTE_LEN)) {
...
static void rfalCleanupTransceive(void) {
  ...
  /* Restore default settings for listen mode, Receiving parity + CRC bits and
   * manual Tx Parity*/
  ...
  /* Restore AGC enabled */
  st25r3916SetRegisterBits(ST25R3916_REG_RX_CONF2,
...
static void rfalPrepareTransceive(void) {
  ...
  if (gRFAL.timings.FDTPoll != RFAL_TIMING_NONE) {
    /* In Passive communications General Purpose Timer is used to measure FDT
     * Poll */
    if (rfalIsModePassiveComm(gRFAL.mode)) /* Passive Comms */
    {
      ...
      st25r3916SetStartGPTimer(
          (uint16_t)rfalConv1fcTo8fc(
              ((gRFAL.timings.FDTPoll < RFAL_FDT_POLL_ADJUSTMENT)
                   ? gRFAL.timings.FDTPoll
                   : (gRFAL.timings.FDTPoll - RFAL_FDT_POLL_ADJUSTMENT))),
    ...
    /* In Active Poller mode GT PPON1 is used to ensure FDT Poll */
      ...
      st25r3916WriteRegister(
          ST25R3916_REG_FIELD_ON_GT,
          (uint8_t)rfalConv1fcTo2018fc(gRFAL.timings.FDTPoll));
    } else {
  ...
  reg = (ST25R3916_REG_ISO14443A_NFC_no_tx_par_off |
  ...
  /* Check if Parity check is to be skipped and to keep the parity + CRC bits in
   * FIFO */
  ...
  st25r3916ChangeRegisterBits(ST25R3916_REG_ISO14443A_NFC,
                              (ST25R3916_REG_ISO14443A_NFC_no_tx_par |
                               ST25R3916_REG_ISO14443A_NFC_no_rx_par |
                               ST25R3916_REG_ISO14443A_NFC_nfc_f0),
                              reg);
  ...
  if ((gRFAL.TxRx.ctx.flags & (uint32_t)RFAL_TXRX_FLAGS_AGC_OFF) != 0U) {
    st25r3916ClrRegisterBits(ST25R3916_REG_RX_CONF2,
                             ST25R3916_REG_RX_CONF2_agc_en);
  } else {
    st25r3916SetRegisterBits(ST25R3916_REG_RX_CONF2,
                             ST25R3916_REG_RX_CONF2_agc_en);
  ...
  if (rfalIsModePassiveListen(gRFAL.mode)) {
    maskInterrupts |=
        (ST25R3916_IRQ_MASK_EOF |
         ST25R3916_IRQ_MASK_WU_F); /* Enable external Field interrupts to detect
                                      Link Loss and SENF_REQ auto responses */
  }

  /* In Active comms enable also External Field interrupts and set RF Collsion
   * Avoidance */
  if (rfalIsModeActiveComm(gRFAL.mode)) {
    maskInterrupts |= (ST25R3916_IRQ_MASK_EOF | ST25R3916_IRQ_MASK_EON |
                       ST25R3916_IRQ_MASK_PPON2 | ST25R3916_IRQ_MASK_CAT |
...
static void rfalTransceiveTx(void) {
  ...
  switch (gRFAL.TxRx.state) {
    ...
    /*******************************************************************************/
    case RFAL_TXRX_STATE_TX_WAIT_GT: /*  PRQA S 2003 # MISRA 16.3 - Intentional
                                        fall through */
    ...
    /*******************************************************************************/
    case RFAL_TXRX_STATE_TX_WAIT_FDT: /*  PRQA S 2003 # MISRA 16.3 - Intentional
                                         fall through */
    ...
    /*******************************************************************************/
    case RFAL_TXRX_STATE_TX_PREP_TX: /*  PRQA S 2003 # MISRA 16.3 - Intentional
                                        fall through */
      ...
      if ((RFAL_MODE_POLL_NFCV == gRFAL.mode) ||
          (RFAL_MODE_POLL_PICOPASS == gRFAL.mode)) {
                ...
                st25r3916ExecuteCommand( ST25R3916_CMD_CLEAR_FIFO );
#endif
        /* Calculate the bytes needed to be Written into FIFO (a incomplete byte
         * will be added as 1byte) */
        gRFAL.nfcvData.nfcvOffset = 0;
        ret = rfalIso15693VCDCode(
            gRFAL.TxRx.ctx.txBuf, rfalConvBitsToBytes(gRFAL.TxRx.ctx.txBufLen),
            (((gRFAL.nfcvData.origCtx.flags &
               (uint32_t)RFAL_TXRX_FLAGS_CRC_TX_MANUAL) != 0U)
                 ? false
                 : true),
            (((gRFAL.nfcvData.origCtx.flags &
               (uint32_t)RFAL_TXRX_FLAGS_NFCV_FLAG_MANUAL) != 0U)
                 ? false
                 : true),
            (RFAL_MODE_POLL_PICOPASS == gRFAL.mode), &gRFAL.fifo.bytesTotal,
            &gRFAL.nfcvData.nfcvOffset, gRFAL.nfcvData.codingBuffer,
            RFAL_MIN((uint16_t)ST25R3916_FIFO_DEPTH,
                     (uint16_t)sizeof(gRFAL.nfcvData.codingBuffer)),
        ...
        st25r3916SetNumTxBits(
            (uint16_t)rfalConvBytesToBits(gRFAL.fifo.bytesTotal));

        /* Load FIFO with coded bytes */
        st25r3916WriteFifo(gRFAL.nfcvData.codingBuffer,
                           gRFAL.fifo.bytesWritten);
...
#endif /* RFAL_FEATURE_NFCV */
      {
        ...
        gRFAL.fifo.bytesTotal =
            (uint16_t)rfalCalcNumBytes(gRFAL.TxRx.ctx.txBufLen);
        ...
        gRFAL.fifo.bytesWritten =
            RFAL_MIN(gRFAL.fifo.bytesTotal, ST25R3916_FIFO_DEPTH);
      ...
      /*******************************************************************************/
      /* If we're in Passive Listen mode ensure that the external field is still
       * On  */
      if (rfalIsModePassiveListen(gRFAL.mode)) {
        if (!rfalIsExtFieldOn()) {
    ...
    /*******************************************************************************/
    case RFAL_TXRX_STATE_TX_TRANSMIT: /*  PRQA S 2003 # MISRA 16.3 - Intentional
                                         fall through */
      ...
      if ((gRFAL.TxRx.ctx.flags & (uint32_t)RFAL_TXRX_FLAGS_CRC_TX_MANUAL) !=
          0U) {
      ...
      gRFAL.TxRx.state = ((gRFAL.fifo.bytesWritten < gRFAL.fifo.bytesTotal)
    ...
    case RFAL_TXRX_STATE_TX_WAIT_WL:
      ...
      if (((irqs & ST25R3916_IRQ_MASK_FWL) != 0U) &&
          ((irqs & ST25R3916_IRQ_MASK_TXE) == 0U)) {
    ...
    /*******************************************************************************/
    case RFAL_TXRX_STATE_TX_RELOAD_FIFO: /*  PRQA S 2003 # MISRA 16.3 -
      ...
      if ((RFAL_MODE_POLL_NFCV == gRFAL.mode) ||
        ...
        /* Load FIFO with the remaining length or maximum available (which fit
         * on the coding buffer) */
        maxLen = (uint16_t)RFAL_MIN(
            (gRFAL.fifo.bytesTotal - gRFAL.fifo.bytesWritten),
            gRFAL.fifo.expWL);
        maxLen =
            (uint16_t)RFAL_MIN(maxLen, sizeof(gRFAL.nfcvData.codingBuffer));
        tmp = 0;
        ...
        ret = rfalIso15693VCDCode(
            ...
            (((gRFAL.nfcvData.origCtx.flags &
               (uint32_t)RFAL_TXRX_FLAGS_NFCV_FLAG_MANUAL) != 0U)
                 ...
                 : true),
            (RFAL_MODE_POLL_PICOPASS == gRFAL.mode), &gRFAL.fifo.bytesTotal,
            &gRFAL.nfcvData.nfcvOffset, gRFAL.nfcvData.codingBuffer, maxLen,
            &tmp);
...
#endif /* RFAL_FEATURE_NFCV */
      {
        ...
        tmp = RFAL_MIN((gRFAL.fifo.bytesTotal - gRFAL.fifo.bytesWritten),
                       gRFAL.fifo.expWL); /* tmp holds the number of bytes
                                             written on this iteration */
    ...
    case RFAL_TXRX_STATE_TX_WAIT_TXE:
      ...
      irqs = st25r3916GetInterrupt((ST25R3916_IRQ_MASK_FWL |
      ...
      if ((irqs & ST25R3916_IRQ_MASK_TXE) != 0U) {
        ...
        break; /* Ignore ST25R3916 FIFO WL if total TxLen is already on the FIFO
                */
      } else if (!rfalIsModeActiveComm(
                     gRFAL.mode)) { /* ST25R3916_IRQ_MASK_EOF will fall in here,
                                       in rare cases this may happen when field
                                       goes during CE transmit */
        gRFAL.TxRx.status = RFAL_ERR_IO;
        gRFAL.TxRx.state = RFAL_TXRX_STATE_TX_FAIL;
        break;
      } else {
    ...
    /*******************************************************************************/
    case RFAL_TXRX_STATE_TX_DONE: /*  PRQA S 2003 # MISRA 16.3 - Intentional
                                     fall through */
...
static void rfalTransceiveRx(void) {
  ...
  switch (gRFAL.TxRx.state) {
    ...
    case RFAL_TXRX_STATE_RX_WAIT_RXS:
      ...
      /*******************************************************************************/
      irqs = st25r3916GetInterrupt((ST25R3916_IRQ_MASK_RXS |
                                    ST25R3916_IRQ_MASK_NRE |
                                    ST25R3916_IRQ_MASK_EOF));
      if (irqs == ST25R3916_IRQ_MASK_NONE) {
        break; /* No interrupt to process */
      ...
      if (((irqs & ST25R3916_IRQ_MASK_NRE) != 0U) &&
          ((irqs & ST25R3916_IRQ_MASK_RXS) == 0U)) {
      ...
      if (((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) &&
          ((irqs & ST25R3916_IRQ_MASK_RXS) == 0U)) {
        /* In AP2P a Field On has already occurred - treat this as timeout |
         * mute */
        gRFAL.TxRx.status =
            (rfalIsModeActiveComm(gRFAL.mode) ? RFAL_ERR_TIMEOUT
                                              : RFAL_ERR_LINK_LOSS);
        gRFAL.TxRx.state = RFAL_TXRX_STATE_RX_FAIL;
        break;
      }
      ...
      if ((irqs & ST25R3916_IRQ_MASK_RXS) != 0U) {
        ...
        /* Rarely on corrupted frames I_rxs gets signaled but I_rxe is not
         * signaled    */
    ...
    /*******************************************************************************/
    case RFAL_TXRX_STATE_RX_WAIT_RXE: /*  PRQA S 2003 # MISRA 16.3 - Intentional
                                         fall through */

      irqs |= st25r3916GetInterrupt(
          (ST25R3916_IRQ_MASK_RXE | ST25R3916_IRQ_MASK_FWL |
           ST25R3916_IRQ_MASK_EOF | ST25R3916_IRQ_MASK_RX_REST |
           ST25R3916_IRQ_MASK_WU_F));
      if (irqs == ST25R3916_IRQ_MASK_NONE) {
        /*******************************************************************************/
        /* REMARK: Silicon workaround ST25R3916 Errata #2.1.2 */
        /* ST25R396 may indicate RXS without RXE afterwards, this happens rarely
         * on    */
        /* corrupted frames. */
      ...
      if ((irqs & ST25R3916_IRQ_MASK_RX_REST) != 0U) {
        /* RX_REST indicates that Receiver has been reseted due to EMD,
         * therefore a RXS + RXE should * follow if a good reception is followed
         * within the valid initial timeout                   */
      ...
      irqs |= st25r3916GetInterrupt(
          (ST25R3916_IRQ_MASK_CRC | ST25R3916_IRQ_MASK_PAR |
           ST25R3916_IRQ_MASK_ERR1 | ST25R3916_IRQ_MASK_ERR2 |
    ...
    /*******************************************************************************/
    case RFAL_TXRX_STATE_RX_ERR_CHECK: /*  PRQA S 2003 # MISRA 16.3 -
                                          Intentional fall through */
      ...
      /* Discard Soft Framing errors in CTS as Correlator does not support
         no_eof */
      else if ((rfalIsModePassivePoll(gRFAL.mode)) &&
               ((irqs & ST25R3916_IRQ_MASK_ERR2) != 0U) &&
               (gRFAL.mode != RFAL_MODE_POLL_B_CTS)) {
        gRFAL.TxRx.status = RFAL_ERR_FRAMING;
        gRFAL.TxRx.state = RFAL_TXRX_STATE_RX_READ_DATA;
    ...
    /*******************************************************************************/
    case RFAL_TXRX_STATE_RX_READ_DATA: /*  PRQA S 2003 # MISRA 16.3 -
                                          Intentional fall through */
      ...
      if (((gRFAL.TxRx.ctx.flags & (uint32_t)RFAL_TXRX_FLAGS_CRC_RX_KEEP) ==
           0U)) {
        ...
        if ((uint16_t)(gRFAL.fifo.bytesTotal + tmp) > 0U) {
          ...
          /* If the CRC was already placed into rxBuffer (due to WL interrupt
           * where CRC was already in FIFO Read) cannot remove it from rxBuf.
           * Can only remove it from rxBufLen not indicate the presence of CRC
           */
      ...
      if (gRFAL.fifo.bytesTotal >
          rfalConvBitsToBytes(gRFAL.TxRx.ctx.rxBufLen)) {
        tmp = (uint16_t)(rfalConvBitsToBytes(gRFAL.TxRx.ctx.rxBufLen) -
                         gRFAL.fifo.bytesWritten);
      ...
      /*******************************************************************************/
      /* Retrieve remaining bytes from FIFO to rxBuf, and assign total length
       * rcvd   */
...
#if RFAL_FEATURE_NFCV
      /*******************************************************************************/
      /* Decode sub bit stream into payload bits for NFCV, if no error found so
       * far  */
      if (((RFAL_MODE_POLL_NFCV == gRFAL.mode) ||
           (RFAL_MODE_POLL_PICOPASS == gRFAL.mode)) &&
          (gRFAL.TxRx.status == RFAL_ERR_BUSY)) {
        ...
        ret = rfalIso15693VICCDecode(
            gRFAL.TxRx.ctx.rxBuf, gRFAL.fifo.bytesTotal,
            gRFAL.nfcvData.origCtx.rxBuf,
            rfalConvBitsToBytes(gRFAL.nfcvData.origCtx.rxBufLen), &offset,
            gRFAL.nfcvData.origCtx.rxRcvdLen, gRFAL.nfcvData.ignoreBits,
            (RFAL_MODE_POLL_PICOPASS == gRFAL.mode));

        if (((RFAL_ERR_NONE == ret) || (RFAL_ERR_CRC == ret)) &&
            (((uint32_t)RFAL_TXRX_FLAGS_CRC_RX_KEEP &
              gRFAL.nfcvData.origCtx.flags) == 0U) &&
            ((*gRFAL.nfcvData.origCtx.rxRcvdLen % RFAL_BITS_IN_BYTE) == 0U) &&
            (*gRFAL.nfcvData.origCtx.rxRcvdLen >=
             rfalConvBytesToBits(RFAL_CRC_LEN))) {
          *gRFAL.nfcvData.origCtx.rxRcvdLen -=
    ...
    /*******************************************************************************/
    case RFAL_TXRX_STATE_RX_DONE: /*  PRQA S 2003 # MISRA 16.3 - Intentional
                                     fall through */
    ...
    case RFAL_TXRX_STATE_RX_READ_FIFO:
      ...
      /*******************************************************************************/
      /* Retrieve incoming bytes from FIFO to rxBuf, and store already read
       * amount   */
    ...
    case RFAL_TXRX_STATE_RX_WAIT_EON:
      ...
      if ((irqs & ST25R3916_IRQ_MASK_EON) != 0U) {
        ...
        if (st25r3916CheckReg(ST25R3916_REG_AUX_MOD,
                              ST25R3916_REG_AUX_MOD_rgs_am,
                              ST25R3916_REG_AUX_MOD_rgs_am)) {
          /* Set Analog configurations for our own following Field On */
          rfalSetAnalogConfig((RFAL_ANALOG_CONFIG_TECH_CHIP |
                               RFAL_ANALOG_CONFIG_CHIP_FIELD_ON));
        }
#endif /* ST25R3916B */
      }
    ...
    case RFAL_TXRX_STATE_RX_WAIT_EOF:
      ...
      if ((irqs & ST25R3916_IRQ_MASK_CAT) != 0U) {
        ...
        gRFAL.TxRx.state =
            ((gRFAL.TxRx.status != RFAL_ERR_BUSY) ? RFAL_TXRX_STATE_RX_FAIL
...
static void rfalFIFOStatusUpdate(void) {
  if (gRFAL.fifo.status[RFAL_FIFO_STATUS_REG2] == RFAL_FIFO_STATUS_INVALID) {
    st25r3916ReadMultipleRegisters(ST25R3916_REG_FIFO_STATUS1,
                                   gRFAL.fifo.status,
                                   ST25R3916_FIFO_STATUS_LEN);
  }
}
...
static uint16_t rfalFIFOStatusGetNumBytes(void) {
  ...
  result = ((((uint16_t)gRFAL.fifo.status[RFAL_FIFO_STATUS_REG2] &
...
static bool rfalFIFOStatusIsIncompleteByte(void) {
  rfalFIFOStatusUpdate();
  return ((gRFAL.fifo.status[RFAL_FIFO_STATUS_REG2] &
           ST25R3916_REG_FIFO_STATUS2_fifo_lb_mask) != 0U);
}
...
static bool rfalFIFOStatusIsMissingPar(void) {
  rfalFIFOStatusUpdate();
  return ((gRFAL.fifo.status[RFAL_FIFO_STATUS_REG2] &
           ST25R3916_REG_FIFO_STATUS2_np_lb) != 0U);
}
...
static uint8_t rfalFIFOGetNumIncompleteBits(void) {
  rfalFIFOStatusUpdate();
  return ((gRFAL.fifo.status[RFAL_FIFO_STATUS_REG2] &
           ST25R3916_REG_FIFO_STATUS2_fifo_lb_mask) >>
...
ReturnCode rfalISO14443ATransceiveShortFrame(rfal14443AShortFrameCmd txCmd,
                                             uint8_t *rxBuf, uint8_t rxBufLen,
                                             uint16_t *rxRcvdLen,
                                             uint32_t fwt) {
  ReturnCode ret;
  uint8_t directCmd;
  ...
  if ((!st25r3916IsTxEnabled()) || (gRFAL.state < RFAL_STATE_MODE_SET) ||
      ((gRFAL.mode != RFAL_MODE_POLL_NFCA) &&
       (gRFAL.mode != RFAL_MODE_POLL_NFCA_T1T))) {
    return RFAL_ERR_WRONG_STATE;
  }
  ...
  st25r3916SetNoResponseTime(rfalConv1fcTo64fc(
      RFAL_MIN((fwt + RFAL_FWT_ADJUSTMENT + RFAL_FWT_A_ADJUSTMENT),
               RFAL_ST25R3916_NRT_MAX_1FC)));
        ...
        (uint8_t)rfalConv1fcTo64fc(
            ((RFAL_FDT_LISTEN_MRT_ADJUSTMENT + RFAL_FDT_LISTEN_A_ADJUSTMENT) >
             gRFAL.timings.FDTListen)
                ? RFAL_ST25R3916_MRT_MIN_1FC
                : (gRFAL.timings.FDTListen - (RFAL_FDT_LISTEN_MRT_ADJUSTMENT +
                                              RFAL_FDT_LISTEN_A_ADJUSTMENT))));
  }
  ...
  /*******************************************************************************/
  /* Clear nbtx bits before sending WUPA/REQA - otherwise ST25R3916 will report
   * parity error, Note2 of the register */
  st25r3916WriteRegister(ST25R3916_REG_NUM_TX_BYTES2, 0);

  /* Send either WUPA or REQA. All affected tags will backscatter ATQA and
  ...
  if (st25r3916WaitForInterruptsTimed(
          ST25R3916_IRQ_MASK_TXE,
          (uint16_t)RFAL_MAX(rfalConv1fcToMs(fwt),
                             RFAL_ST25R3916_SW_TMR_MIN_1MS)) == 0U) {
    ret = RFAL_ERR_IO;
  } else {
    /*Check if Observation Mode is enabled and set it on ST25R391x */
    rfalCheckEnableObsModeRx();
...
ReturnCode rfalISO14443ATransceiveAnticollisionFrame(uint8_t *buf,
  ...
  RFAL_EXIT_ON_ERR(ret, rfalISO14443AStartTransceiveAnticollisionFrame(
                            buf, bytesToSend, bitsToSend, rxLength, fwt));
...
/*******************************************************************************/
ReturnCode rfalISO14443AStartTransceiveAnticollisionFrame(uint8_t *buf,
                                                          uint8_t *bytesToSend,
                                                          uint8_t *bitsToSend,
                                                          uint16_t *rxLength,
                                                          uint32_t fwt) {
  ...
  /* Check for valid parameters */
  if ((buf == NULL) || (bytesToSend == NULL) || (bitsToSend == NULL) ||
      (rxLength == NULL)) {
    return RFAL_ERR_PARAM;
  }
  ...
  rfalSetAnalogConfig((RFAL_ANALOG_CONFIG_POLL | RFAL_ANALOG_CONFIG_TECH_NFCA |
                       RFAL_ANALOG_CONFIG_BITRATE_COMMON |
                       RFAL_ANALOG_CONFIG_ANTICOL));
  ...
  st25r3916SetRegisterBits(ST25R3916_REG_ISO14443A_NFC,
                           ST25R3916_REG_ISO14443A_NFC_antcl);
  ...
  /* Disable Automatic Gain Control (AGC) for better detection of collisions if
   * using Coherent Receiver */
  ctx.flags |= (st25r3916CheckReg(ST25R3916_REG_AUX, ST25R3916_REG_AUX_dis_corr,
...
ReturnCode rfalISO14443AGetTransceiveAnticollisionFrameStatus(void) {
  ...
  if ((*gRFAL.nfcaData.bitsToSend) > 0U) {
    gRFAL.nfcaData.buf[(*gRFAL.nfcaData.bytesToSend)] >>=
        (*gRFAL.nfcaData.bitsToSend);
    gRFAL.nfcaData.buf[(*gRFAL.nfcaData.bytesToSend)] <<=
        (*gRFAL.nfcaData.bitsToSend);
    gRFAL.nfcaData.buf[(*gRFAL.nfcaData.bytesToSend)] |=
        gRFAL.nfcaData.collByte;
  }
...
ReturnCode rfalISO15693TransceiveAnticollisionFrame(uint8_t *txBuf,
  ...
  /* Check if RFAL is properly initialized */
  if ((gRFAL.state < RFAL_STATE_MODE_SET) ||
      (gRFAL.mode != RFAL_MODE_POLL_NFCV)) {
    return RFAL_ERR_WRONG_STATE;
  }

  /*******************************************************************************/
  /* Set speficic Analog Config for Anticolission if needed */
  rfalSetAnalogConfig((RFAL_ANALOG_CONFIG_POLL | RFAL_ANALOG_CONFIG_TECH_NFCV |
                       RFAL_ANALOG_CONFIG_BITRATE_COMMON |
                       RFAL_ANALOG_CONFIG_ANTICOL));
  ...
  ctx.flags =
      ...
      ((txBufLen == 0U)
           ...
           : (uint32_t)
                 RFAL_TXRX_FLAGS_NFCV_FLAG_AUTO); /* Disable Automatic Gain
                                                     Control (AGC) for better
  ...
  if (((ret == RFAL_ERR_RF_COLLISION) || (ret == RFAL_ERR_CRC) ||
    ...
    platformDelay(
        (uint8_t)((RFAL_ISO15693_INV_RES_LEN -
                   rfalConvBitsToBytes(*ctx.rxRcvdLen)) /
                  ((RFAL_ISO15693_INV_RES_LEN / RFAL_ISO15693_INV_RES_DUR) +
                   1U)));
  }
...
/*******************************************************************************/
ReturnCode rfalISO15693TransceiveEOFAnticollision(uint8_t *rxBuf,
                                                  uint8_t rxBufLen,
                                                  uint16_t *actLen) {
  uint8_t dummy;

  return rfalISO15693TransceiveAnticollisionFrame(&dummy, 0, rxBuf, rxBufLen,
                                                  actLen);
}

/*******************************************************************************/
ReturnCode rfalISO15693TransceiveEOF(uint8_t *rxBuf, uint16_t rxBufLen,
                                     uint16_t *actLen) {
  ReturnCode ret;
  uint8_t dummy;
  ...
  /*******************************************************************************/
  /* Run Transceive blocking */
  ret = rfalTransceiveBlockingTxRx(&dummy, 0, rxBuf, rxBufLen, actLen,
...
/*******************************************************************************/
ReturnCode rfalFeliCaPoll(rfalFeliCaPollSlots slots, uint16_t sysCode,
                          uint8_t reqCode, rfalFeliCaPollRes *pollResList,
  ...
  RFAL_EXIT_ON_ERR(
      ret,
      rfalStartFeliCaPoll(slots, sysCode, reqCode, pollResList, pollResListSize,
                          devicesDetected, collisionsDetected));
...
/*******************************************************************************/
ReturnCode rfalStartFeliCaPoll(rfalFeliCaPollSlots slots, uint16_t sysCode,
                               uint8_t reqCode, rfalFeliCaPollRes *pollResList,
  ...
  ReturnCode ret;
  uint8_t
      frame[RFAL_FELICA_POLL_REQ_LEN -
            RFAL_FELICA_LEN_LEN];  // LEN is added by ST25R391x automatically
  uint8_t frameIdx;
   ...
   */
  RFAL_EXIT_ON_ERR(ret, rfalTransceiveBlockingTx(
                            frame, (uint16_t)frameIdx,
                            (uint8_t *)gRFAL.nfcfData.pollResponses,
                            RFAL_FELICA_POLL_RES_LEN, &gRFAL.nfcfData.actLen,
                            (RFAL_TXRX_FLAGS_DEFAULT),
                            rfalConv64fcTo1fc(RFAL_FELICA_POLL_DELAY_TIME +
...
ReturnCode rfalGetFeliCaPollStatus(void) {
  ...
  if (ret != RFAL_ERR_TIMEOUT) {
    ...
    /* The Rx reEnable is done before the check of NRT to be as fast as possible
     * for the upcoming slot       * Tslot = 1208us | SENSF_RES (19 payload
     * bytes at 212) = 1135us -> Potentially ~75us between responses */
    st25r3916ExecuteCommand(ST25R3916_CMD_UNMASK_RECEIVE_DATA);
    st25r3916ExecuteCommand(ST25R3916_CMD_RESET_RXGAIN);
    rfalFIFOStatusClear();
    ...
    if (ret == RFAL_ERR_NONE) {
      ...
      gRFAL.TxRx.ctx.rxBuf =
          (uint8_t *)gRFAL.nfcfData.pollResponses[gRFAL.nfcfData.devDetected];
    }
  ...
  if ((gRFAL.nfcfData.pollResList != NULL) &&
      (gRFAL.nfcfData.pollResListSize > 0U) &&
      (gRFAL.nfcfData.devDetected > 0U)) {
    RFAL_MEMCPY(gRFAL.nfcfData.pollResList, gRFAL.nfcfData.pollResponses,
                (RFAL_FELICA_POLL_RES_LEN *
                 (uint32_t)RFAL_MIN(gRFAL.nfcfData.pollResListSize,
                                    gRFAL.nfcfData.devDetected)));
  }
  ...
  return (
      ((gRFAL.nfcfData.colDetected != 0U) || (gRFAL.nfcfData.devDetected != 0U))
          ? RFAL_ERR_NONE
          : ret);
}
...
/*******************************************************************************/
bool rfalIsExtFieldOn(void) { return st25r3916IsExtFieldOn(); }

#if RFAL_FEATURE_LISTEN_MODE

/*******************************************************************************/
ReturnCode rfalListenStart(uint32_t lmMask, const rfalLmConfPA *confA,
                           const rfalLmConfPB *confB, const rfalLmConfPF *confF,
                           uint8_t *rxBuf, uint16_t rxBufLen, uint16_t *rxLen) {
  t_rfalPTMem PTMem; /*  PRQA S 0759 # MISRA 19.2 - Allocating Union where
                        members are of the same type, just different names. Thus
  ...
  gRFAL.Lm.mdReg = (ST25R3916_REG_MODE_targ_init | ST25R3916_REG_MODE_om_nfc |
                    ST25R3916_REG_MODE_nfc_ar_off);
  ...
  if ((lmMask & RFAL_LM_MASK_NFCA) != 0U) {
    ...
    /* Set SEL_RES */
    *(pPTMem++) = ((confA->nfcidLen == RFAL_LM_NFCID_LEN_04)
    ...
    *(pPTMem++) = (confA->SEL_RES & ~RFAL_LM_NFCID_INCOMPLETE);
    ...
    gRFAL.Lm.mdReg |= (ST25R3916_REG_MODE_targ_targ | ST25R3916_REG_MODE_om3 |
                       ST25R3916_REG_MODE_om0 | ST25R3916_REG_MODE_nfc_ar_off);

    gRFAL.Lm.mdIrqs |= (ST25R3916_IRQ_MASK_WU_A | ST25R3916_IRQ_MASK_WU_A_X |
  ...
  if ((lmMask & RFAL_LM_MASK_NFCF) != 0U) {
    ...
    gRFAL.Lm.mdReg |= (ST25R3916_REG_MODE_targ_targ | ST25R3916_REG_MODE_om3 |
                       ST25R3916_REG_MODE_om2 | ST25R3916_REG_MODE_nfc_ar_off);

    /* In CE NFC-F any data without error will be passed to FIFO, to support CUP
     */
    gRFAL.Lm.mdIrqs |= (ST25R3916_IRQ_MASK_WU_F | ST25R3916_IRQ_MASK_RXE_PTA |
  ...
  if ((lmMask & RFAL_LM_MASK_ACTIVE_P2P) != 0U) {
    ...
    /* Set Target mode, Bit Rate detection and Automatic Response RF Collision
     * Avoidance */
    gRFAL.Lm.mdReg |= (ST25R3916_REG_MODE_targ_targ | ST25R3916_REG_MODE_om3 |
                       ST25R3916_REG_MODE_om2 | ST25R3916_REG_MODE_om0 |
  ...
  if ((gRFAL.Lm.mdReg & ST25R3916_REG_MODE_targ) ==
    ...
    *gRFAL.Lm.rxLen = 0;
    ...
    /* On Bit Rate Detection Mode ST25R391x will filter incoming frames during
     * MRT time starting on External Field On event, use 512/fc steps */
    st25r3916SetRegisterBits(ST25R3916_REG_TIMER_EMV_CONTROL,
                             ST25R3916_REG_TIMER_EMV_CONTROL_mrt_step_512);
    st25r3916WriteRegister(ST25R3916_REG_MASK_RX_TIMER,
                           (uint8_t)rfalConv1fcTo512fc(RFAL_LM_GT));

    /* Restore default settings on NFCIP1 mode, Receiving parity + CRC bits and
     * manual Tx Parity*/
    st25r3916ClrRegisterBits(ST25R3916_REG_ISO14443A_NFC,
                             (ST25R3916_REG_ISO14443A_NFC_no_tx_par |
                              ST25R3916_REG_ISO14443A_NFC_no_rx_par |
    ...
    /* Not on SetState(POWER OFF) as otherwise would be applied on every Field
     * Event */
    rfalSetAnalogConfig(
...
static ReturnCode rfalRunListenModeWorker(void) {
  ...
  switch (gRFAL.Lm.state) {
    ...
    /*******************************************************************************/
    case RFAL_LM_STATE_IDLE: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall
                                through */

      irqs = st25r3916GetInterrupt(
          (ST25R3916_IRQ_MASK_NFCT | ST25R3916_IRQ_MASK_WU_F |
           ST25R3916_IRQ_MASK_RXE | ST25R3916_IRQ_MASK_EOF |
           ST25R3916_IRQ_MASK_RXE_PTA));
      if (irqs == ST25R3916_IRQ_MASK_NONE) {
        break; /* No interrupt to process */
      }
      ...
      if ((irqs & ST25R3916_IRQ_MASK_NFCT) != 0U) {
        ...
        gRFAL.Lm.brDetected =
            (rfalBitRate)(newBr); /* PRQA S 4342 # MISRA 10.5 - Guaranteed that
                                     no invalid enum values may be created. See
                                     also equalityGuard_RFAL_BR_106 ff.*/
      }

      /* If EOF has already been received processing of other events is
       * neglectable */
      if (((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) && (!gRFAL.Lm.dataFlag)) {
        ...
        rfalListenSetState(RFAL_LM_STATE_READY_F);
                 ...
                 (gRFAL.Lm.brDetected != RFAL_BR_KEEP)) {
        irqs = st25r3916GetInterrupt(
            (ST25R3916_IRQ_MASK_WU_F | ST25R3916_IRQ_MASK_RXE |
             ST25R3916_IRQ_MASK_EOF | ST25R3916_IRQ_MASK_CRC |
             ST25R3916_IRQ_MASK_PAR | ST25R3916_IRQ_MASK_ERR2 |
        ...
        *gRFAL.Lm.rxLen = st25r3916GetNumFIFOBytes();
        st25r3916ReadFifo(
            gRFAL.Lm.rxBuf,
            RFAL_MIN(*gRFAL.Lm.rxLen, rfalConvBitsToBytes(gRFAL.Lm.rxBufLen)));

#ifdef ST25R3916
        /*******************************************************************************/
        /* REMARK: Silicon workaround ST25R3916 Errata #TBD */
        /* In bitrate detection mode CRC is not checked for NFC-A frames */
        if ((*gRFAL.Lm.rxLen > RFAL_CRC_LEN) &&
            (gRFAL.Lm.brDetected == RFAL_BR_106)) {
          if (rfalCrcCalculateCcitt(RFAL_ISO14443A_CRC_INTVAL, gRFAL.Lm.rxBuf,
                                    *gRFAL.Lm.rxLen) != 0U) {
            st25r3916ExecuteCommand(ST25R3916_CMD_CLEAR_FIFO);
            st25r3916ExecuteCommand(ST25R3916_CMD_UNMASK_RECEIVE_DATA);
            st25r3916TxOff();
        ...
        /* Check if the data we got has at least the CRC and remove it,
         * otherwise leave at 0 */
        *gRFAL.Lm.rxLen -=
            ((*gRFAL.Lm.rxLen > RFAL_CRC_LEN) ? RFAL_CRC_LEN : *gRFAL.Lm.rxLen);
        *gRFAL.Lm.rxLen = (uint16_t)rfalConvBytesToBits(*gRFAL.Lm.rxLen);
        gRFAL.Lm.dataFlag = true;

        /*Check if Observation Mode was enabled and disable it on ST25R391x */
        rfalCheckDisableObsMode();
      } else if (((irqs & ST25R3916_IRQ_MASK_RXE_PTA) != 0U) &&
                 (gRFAL.Lm.brDetected != RFAL_BR_KEEP)) {
    ...
    case RFAL_LM_STATE_READY_F:
      ...
      irqs = st25r3916GetInterrupt((ST25R3916_IRQ_MASK_WU_F |
                                    ST25R3916_IRQ_MASK_RXE |
                                    ST25R3916_IRQ_MASK_EOF));
      if (irqs == ST25R3916_IRQ_MASK_NONE) {
        break; /* No interrupt to process */
      ...
      if ((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) {
        ...
        irqs |= st25r3916GetInterrupt((ST25R3916_IRQ_MASK_CRC |
    ...
    case RFAL_LM_STATE_ACTIVE_Ax:
      ...
      if ((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) {
        ...
        irqs |= st25r3916GetInterrupt(
            (ST25R3916_IRQ_MASK_PAR | ST25R3916_IRQ_MASK_CRC |
             ST25R3916_IRQ_MASK_ERR2 | ST25R3916_IRQ_MASK_ERR1));
        *gRFAL.Lm.rxLen = st25r3916GetNumFIFOBytes();
        ...
        if (((irqs & ST25R3916_IRQ_MASK_CRC) != 0U) ||
          ...
          *gRFAL.Lm.rxLen = 0;
          ...
          if (gRFAL.Lm.state == RFAL_LM_STATE_ACTIVE_Ax) {
            rfalListenSleepStart(RFAL_LM_STATE_SLEEP_A, gRFAL.Lm.rxBuf,
                                 gRFAL.Lm.rxBufLen, gRFAL.Lm.rxLen);
          } else {
            rfalListenSetState(RFAL_LM_STATE_IDLE);
          }
    ...
    case RFAL_LM_STATE_SLEEP_AF:
      ...
      if ((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) {
        ...
        *gRFAL.Lm.rxLen = 0;
        ...
        /* REMARK: In order to support CUP or proprietary frames, handling could
         * be added here */
      } else if (((irqs & ST25R3916_IRQ_MASK_RXE_PTA) != 0U) &&
                 (gRFAL.Lm.brDetected != RFAL_BR_KEEP)) {
        if (((gRFAL.Lm.mdMask & RFAL_LM_MASK_NFCA) != 0U) &&
...
ReturnCode rfalListenStop(void) {
  ...
  /* As there's no Off mode, set default value: ISO14443A with automatic RF
  ...
  st25r3916DisableInterrupts(
      (ST25R3916_IRQ_MASK_RXE_PTA | ST25R3916_IRQ_MASK_WU_F |
       ST25R3916_IRQ_MASK_WU_A | ST25R3916_IRQ_MASK_WU_A_X |
       ST25R3916_IRQ_MASK_RFU2 | ST25R3916_IRQ_MASK_OSC));
  st25r3916GetInterrupt((ST25R3916_IRQ_MASK_RXE_PTA | ST25R3916_IRQ_MASK_WU_F |
                         ST25R3916_IRQ_MASK_WU_A | ST25R3916_IRQ_MASK_WU_A_X |
                         ST25R3916_IRQ_MASK_RFU2 | ST25R3916_IRQ_MASK_TXE));

  /* Set Analog configurations for Listen Off event */
  rfalSetAnalogConfig(
      (RFAL_ANALOG_CONFIG_TECH_CHIP | RFAL_ANALOG_CONFIG_CHIP_LISTEN_OFF));

  return RFAL_ERR_NONE;
}

/*******************************************************************************/
ReturnCode rfalListenSleepStart(rfalLmState sleepSt, uint8_t *rxBuf,
                                uint16_t rxBufLen, uint16_t *rxLen) {
  /* Check if RFAL is not initialized */
  if (gRFAL.state < RFAL_STATE_INIT) {
    return RFAL_ERR_WRONG_STATE;
  }
  ...
  switch (sleepSt) {
    ...
    case RFAL_LM_STATE_SLEEP_A:
      ...
      st25r3916ChangeRegisterBits(
          ...
          (ST25R3916_REG_MODE_targ_targ | ST25R3916_REG_MODE_om3 |
           ST25R3916_REG_MODE_om0 | ST25R3916_REG_MODE_nfc_ar_off));
      break;
  ...
  st25r3916ClearAndEnableInterrupts(
      (ST25R3916_IRQ_MASK_NFCT | ST25R3916_IRQ_MASK_RXS |
       ST25R3916_IRQ_MASK_CRC | ST25R3916_IRQ_MASK_ERR1 |
       ST25R3916_IRQ_MASK_ERR2 | ST25R3916_IRQ_MASK_PAR |
    ...
    switch (newState) {
      ...
      case RFAL_LM_STATE_POWER_OFF:
        ...
        if ((gRFAL.Lm.mdMask & RFAL_LM_MASK_NFCA) != 0U) {
          ...
          /* Prepares the NFCIP-1 Passive target logic to wait in the Sense/Idle
           * state */
        ...
        if ((gRFAL.Lm.mdMask & RFAL_LM_MASK_ACTIVE_P2P) != 0U) {
          /* Ensure automatic response RF Collision Avoidance is back to only
           * after Rx */
          st25r3916ChangeRegisterBits(ST25R3916_REG_MODE,
                                      ST25R3916_REG_MODE_nfc_ar_mask,
                                      ST25R3916_REG_MODE_nfc_ar_auto_rx);

          /* Ensure that our field is Off, as automatic response RF Collision
           * Avoidance may have been triggered */
          st25r3916TxOff();
        }
        ...
        st25r3916ClearAndEnableInterrupts(
            (ST25R3916_IRQ_MASK_NFCT | ST25R3916_IRQ_MASK_RXS |
             ST25R3916_IRQ_MASK_CRC | ST25R3916_IRQ_MASK_ERR1 |
             ST25R3916_IRQ_MASK_OSC | ST25R3916_IRQ_MASK_ERR2 |
             ST25R3916_IRQ_MASK_PAR | ST25R3916_IRQ_MASK_EON |
             ST25R3916_IRQ_MASK_EOF | gRFAL.Lm.mdIrqs));
      ...
      case RFAL_LM_STATE_IDLE:
        ...
        if (!st25r3916CheckReg(ST25R3916_REG_OP_CONTROL,
                               ST25R3916_REG_OP_CONTROL_en,
                               ST25R3916_REG_OP_CONTROL_en)) {
        ...
        /*******************************************************************************/
        /* In Active P2P the Initiator may:  Turn its field On;  LM goes into
         * IDLE state; Initiator sends an unexpected frame raising a Protocol
         * error; Initiator turns its field Off and ST25R3916 performs the
         * automatic RF Collision Avoidance keeping our field On; upon a
         * Protocol error upper layer sets again the state to IDLE to clear
         * dataFlag and wait for next data.
         *
         * Ensure that when upper layer calls SetState(IDLE), it restores
         * initial configuration and that check whether an external Field is
         * still present     */
        if ((gRFAL.Lm.mdMask & RFAL_LM_MASK_ACTIVE_P2P) != 0U) {
        ...
        /* If we are in ACTIVE_A, reEnable Listen for A before going to IDLE,
      ...
      case RFAL_LM_STATE_READY_F:
        ...
        /*******************************************************************************/
        /* If we're coming from BitRate detection mode, the Bit Rate Definition
         * reg still has the last bit rate used. If a frame is received between
         * setting the mode to Listen NFCA and setting Bit Rate Definition reg,
         * it will raise a framing error. Set the bitrate immediately, and then
         * the normal SetMode procedure          */
        st25r3916SetBitrate((uint8_t)gRFAL.Lm.brDetected,
                            (uint8_t)gRFAL.Lm.brDetected);
        /*******************************************************************************/
        ...
        /* Set Mode NFC-F only */
        ret = rfalSetMode(RFAL_MODE_LISTEN_NFCF, gRFAL.Lm.brDetected,
                          gRFAL.Lm.brDetected);
        ...
        /* Clear any previous transmission errors (if Reader polled for
         * other/unsupported technologies) */
        st25r3916GetInterrupt((ST25R3916_IRQ_MASK_PAR | ST25R3916_IRQ_MASK_CRC |
                               ST25R3916_IRQ_MASK_ERR2 |
                               ST25R3916_IRQ_MASK_ERR1));
      ...
      case RFAL_LM_STATE_READY_A:
        ...
        /* Set Mode NFC-A only */
        ret = rfalSetMode(RFAL_MODE_LISTEN_NFCA, gRFAL.Lm.brDetected,
                          gRFAL.Lm.brDetected);
      ...
      case RFAL_LM_STATE_SLEEP_B:
      case RFAL_LM_STATE_SLEEP_AF:
        /* These sleep states have to be set by the rfalListenSleepStart()
         * method */
...
ReturnCode rfalWakeUpModeStart(const rfalWakeUpConfig *config) {
  ...
  if (config == NULL) {
    ...
    gRFAL.wum.cfg.refWU.enabled =
        true; /* Obtain WU reference from WU mode, not in Ready Mode */
    gRFAL.wum.cfg.refWU.refDelay =
        RFAL_WUM_PERIOD_10MS; /* WU reference after this time in WU mode      */
  ...
  st25r3916WriteRegister(
      ...
      (st25r3916IsAATOn() ? RFAL_ST25R3916B_AAT_SETTLE : 0x00));
#endif /* ST25R3916B */
  ...
  rfalSetAnalogConfig(
      (RFAL_ANALOG_CONFIG_TECH_CHIP | RFAL_ANALOG_CONFIG_CHIP_WAKEUP_ON));
  ...
  reg |=
      (uint8_t)(((uint8_t)gRFAL.wum.cfg.period < (uint8_t)RFAL_WUM_PERIOD_100MS)
  ...
  if (gRFAL.wum.cfg.swTagDetect) {
    ...
    if (gRFAL.wum.cfg.indAmp.enabled) {
      ...
      if ((!gRFAL.wum.cfg.indAmp.autoAvg) && (!gRFAL.wum.cfg.refWU.enabled)) {
        ...
        st25r3916WriteRegister(ST25R3916_REG_AMPLITUDE_MEASURE_REF,
                               (uint8_t)gRFAL.wum.cfg.indAmp.reference);
      }
    ...
    if (gRFAL.wum.cfg.indPha.enabled) {
      ...
      if ((!gRFAL.wum.cfg.indPha.autoAvg) && (!gRFAL.wum.cfg.refWU.enabled)) {
        ...
        st25r3916WriteRegister(ST25R3916_REG_PHASE_MEASURE_REF,
                               (uint8_t)gRFAL.wum.cfg.indPha.reference);
      }
    ...
    if (gRFAL.wum.cfg.cap.enabled) {
       ...
       * RFAL_ANALOG_CONFIG_CHIP_WAKEUP_ON */
      ...
      aux |= (uint8_t)(gRFAL.wum.cfg.cap.aaInclMeas
                           ...
                           : 0x00U);
      aux |=
          (uint8_t)(((uint8_t)gRFAL.wum.cfg.cap.aaWeight
      ...
      if ((!gRFAL.wum.cfg.cap.autoAvg) || (gRFAL.wum.cfg.swTagDetect)) {
        ...
        st25r3916WriteRegister(ST25R3916_REG_CAPACITANCE_MEASURE_REF,
                               (uint8_t)gRFAL.wum.cfg.cap.reference);
      }
  ...
  RFAL_MEMSET(
      &gRFAL.wum.info, 0x00,
      sizeof(
          gRFAL.wum.info)); /* clear info struct to avoid old data being used */
  gRFAL.wum.state = RFAL_WUM_STATE_ENABLED;
  gRFAL.state = RFAL_STATE_WUM;

  /*******************************************************************************/
  /* If reference is to be obtained by WU mode, set the WU config so that
   * triggers WU IRQ upon first measurement */
  if (gRFAL.wum.cfg.refWU.enabled) {
    if (gRFAL.wum.cfg.indAmp.enabled) {
      st25r3916WriteRegister(ST25R3916_REG_AMPLITUDE_MEASURE_REF, 0);
      st25r3916ChangeRegisterBits(
          ST25R3916_REG_AMPLITUDE_MEASURE_CONF,
          ST25R3916_REG_AMPLITUDE_MEASURE_CONF_am_d_mask, 0);
      gRFAL.wum.refWUTrg = ST25R3916_IRQ_MASK_WAM;
    }

    if (gRFAL.wum.cfg.indPha.enabled) {
      st25r3916WriteRegister(ST25R3916_REG_PHASE_MEASURE_REF, 0);
      st25r3916ChangeRegisterBits(ST25R3916_REG_PHASE_MEASURE_CONF,
                                  ST25R3916_REG_PHASE_MEASURE_CONF_pm_d_mask,
                                  0);
      gRFAL.wum.refWUTrg = ST25R3916_IRQ_MASK_WPH;
    }

    /* Set config timing for the ref measurement */
    reg &= ~(ST25R3916_REG_WUP_TIMER_CONTROL_wur |
             ST25R3916_REG_WUP_TIMER_CONTROL_wut_mask);
    reg |= (uint8_t)(((uint8_t)gRFAL.wum.cfg.refWU.refDelay & 0x0FU)
                     << ST25R3916_REG_WUP_TIMER_CONTROL_wut_shift);
    reg |= (uint8_t)(((uint8_t)gRFAL.wum.cfg.refWU.refDelay <
                      (uint8_t)RFAL_WUM_PERIOD_100MS)
                         ? ST25R3916_REG_WUP_TIMER_CONTROL_wur
                         : 0x00U);
  ...
  /* Enable Low Power Wake-Up Mode (Disable: Oscilattor, Tx, Rx and External
   * Field Detector) */
...
bool rfalWakeUpModeIsEnabled(void) {
  return ((gRFAL.state == RFAL_STATE_WUM) &&
          (gRFAL.wum.state >= RFAL_WUM_STATE_ENABLED));
}
...
ReturnCode rfalWakeUpModeGetInfo(bool force, rfalWakeUpInfo *info) {
  ...
  /* WUT IRQ is signaled when WUT expires. Delay slightly for the actual
  ...
  if (gRFAL.wum.cfg.indAmp.enabled) {
    ...
    info->indAmp.lastMeas =
        gRFAL.wum.info.indAmp.lastMeas; /* For the case of swTagDetect==1 */

    /* Only retrieve the reference from the device if needed */
    if ((force || (info->irqWut) || (gRFAL.wum.info.indAmp.irqWu)) &&
        (!gRFAL.wum.cfg.swTagDetect)) {
      if (gRFAL.wum.cfg.indAmp.autoAvg) {
        ...
        gRFAL.wum.cfg.indAmp.reference =
            aux; /* Store last value for subsequenct calls */
      }
      st25r3916ReadRegister(ST25R3916_REG_AMPLITUDE_MEASURE_RESULT,
                            &info->indAmp.lastMeas);
      gRFAL.wum.info.indAmp.lastMeas =
          info->indAmp.lastMeas; /* Store last value for subsequenct calls */
    }
  ...
  if (gRFAL.wum.cfg.indPha.enabled) {
    ...
    info->indPha.lastMeas =
        gRFAL.wum.info.indPha.lastMeas; /* For the case of swTagDetect==1 */

    /* Only retrieve the reference from the device if needed */
    if ((force || (info->irqWut) || (gRFAL.wum.info.indPha.irqWu)) &&
        (!gRFAL.wum.cfg.swTagDetect)) {
      ...
      st25r3916ReadRegister(ST25R3916_REG_PHASE_MEASURE_RESULT,
                            &info->indPha.lastMeas);
      gRFAL.wum.info.indPha.lastMeas =
          info->indPha.lastMeas; /* Store last value for subsequenct calls */
    }
  ...
  if (gRFAL.wum.cfg.cap.enabled) {
    ...
    if (force || (info->irqWut) || (gRFAL.wum.info.cap.irqWu)) {
      ...
      if (gRFAL.wum.cfg.cap.autoAvg) {
        st25r3916ReadRegister(ST25R3916_REG_CAPACITANCE_MEASURE_AA_RESULT,
                              &aux);
        info->cap.reference = aux;
        gRFAL.wum.cfg.cap.reference =
            aux; /* Store last value for subsequenct calls */
      }
      st25r3916ReadRegister(ST25R3916_REG_CAPACITANCE_MEASURE_RESULT,
                            &info->cap.lastMeas);
      gRFAL.wum.info.cap.lastMeas =
          info->cap.lastMeas; /* Store last value for subsequenct calls */
    }
...
static void rfalRunWakeUpModeWorker(void) {
  ...
  switch (gRFAL.wum.state) {
    ...
    case RFAL_WUM_STATE_ENABLED_WOKE:
      ...
      irqs = st25r3916GetInterrupt(
          (ST25R3916_IRQ_MASK_WT | ST25R3916_IRQ_MASK_WAM |
           ST25R3916_IRQ_MASK_WPH | ST25R3916_IRQ_MASK_WCAP));
      if (irqs == ST25R3916_IRQ_MASK_NONE) {
        break; /* No interrupt to process */
      }
      ...
      if ((irqs & ST25R3916_IRQ_MASK_WT) != 0U) {
        ...
        if (gRFAL.wum.cfg.swTagDetect) {
          ...
          if (gRFAL.wum.cfg.indAmp.enabled) {
            ...
            if ((gRFAL.wum.cfg.indAmp.autoAvg) &&
                ((gRFAL.wum.cfg.indAmp.aaInclMeas) || (!woke))) {
              gRFAL.wum.cfg.indAmp.reference = rfalWakeUpModeFilter(
                  gRFAL.wum.cfg.indAmp.reference, value,
                  (RFAL_WU_MIN_WEIGHT_VAL
          ...
          if (gRFAL.wum.cfg.indPha.enabled) {
            ...
            /* Update moving reference if enabled */
            if ((gRFAL.wum.cfg.indPha.autoAvg) &&
                ((gRFAL.wum.cfg.indPha.aaInclMeas) || (!woke))) {
          ...
          /* Re-Enable low power Wake-Up mode for wto to trigger another
           * measurement(s) */
          st25r3916ChangeRegisterBits(
              ST25R3916_REG_OP_CONTROL,
              (ST25R3916_REG_OP_CONTROL_en | ST25R3916_REG_OP_CONTROL_wu),
    ...
    case RFAL_WUM_STATE_INITIALIZING:
      ...
      if ((gRFAL.wum.cfg.refWU.enabled == true) &&
          ((irqs & gRFAL.wum.refWUTrg) != 0U)) {
        st25r3916ClrRegisterBits(ST25R3916_REG_OP_CONTROL,
                                 ST25R3916_REG_OP_CONTROL_wu);
        st25r3916GetInterrupt(
            (ST25R3916_IRQ_MASK_WAM | ST25R3916_IRQ_MASK_WPH));
        ...
        if (gRFAL.wum.cfg.indAmp.enabled) {
          ...
          st25r3916ChangeRegisterBits(
              ST25R3916_REG_AMPLITUDE_MEASURE_CONF,
              ST25R3916_REG_AMPLITUDE_MEASURE_CONF_am_d_mask,
              ((gRFAL.wum.cfg.indAmp.delta)
               << ST25R3916_REG_AMPLITUDE_MEASURE_CONF_am_d_shift));
        }
        ...
        if (gRFAL.wum.cfg.indPha.enabled) {
          ...
          st25r3916ChangeRegisterBits(
              ...
              ST25R3916_REG_PHASE_MEASURE_CONF_pm_d_mask,
              ((gRFAL.wum.cfg.indPha.delta)
               << ST25R3916_REG_PHASE_MEASURE_CONF_pm_d_shift));
        }

        /* Set WU period and enter WU mode */
        aux = (uint8_t)(((uint8_t)gRFAL.wum.cfg.period & 0x0FU)
                        << ST25R3916_REG_WUP_TIMER_CONTROL_wut_shift);
        aux |= (uint8_t)(((uint8_t)gRFAL.wum.cfg.period <
                          (uint8_t)RFAL_WUM_PERIOD_100MS)
                             ? ST25R3916_REG_WUP_TIMER_CONTROL_wur
                             : 0x00U);
        st25r3916ChangeRegisterBits(ST25R3916_REG_WUP_TIMER_CONTROL,
                                    (ST25R3916_REG_WUP_TIMER_CONTROL_wur |
                                     ST25R3916_REG_WUP_TIMER_CONTROL_wut_mask),
                                    aux);
        st25r3916SetRegisterBits(ST25R3916_REG_OP_CONTROL,
...
ReturnCode rfalWakeUpModeStop(void) {
  ...
  st25r3916ClrRegisterBits(ST25R3916_REG_OP_CONTROL,
                           ST25R3916_REG_OP_CONTROL_wu);
  st25r3916DisableInterrupts((ST25R3916_IRQ_MASK_WT | ST25R3916_IRQ_MASK_WAM |
                              ST25R3916_IRQ_MASK_WPH |
  ...
  rfalSetAnalogConfig(
      (RFAL_ANALOG_CONFIG_TECH_CHIP | RFAL_ANALOG_CONFIG_CHIP_WAKEUP_OFF));
...
/*******************************************************************************/
ReturnCode rfalWlcPWptMonitorStop(void) { return RFAL_ERR_NOTSUPP; }
...
ReturnCode rfalLowPowerModeStart(rfalLpMode mode) {
  ...
  /* Stop any ongoing activity and set the device in low power by disabling
   * oscillator, transmitter, receiver and external field detector */
  st25r3916ExecuteCommand(ST25R3916_CMD_STOP);
  st25r3916ClrRegisterBits(
      ST25R3916_REG_OP_CONTROL,
      (ST25R3916_REG_OP_CONTROL_en | ST25R3916_REG_OP_CONTROL_rx_en |
...
ReturnCode rfalLowPowerModeStop(void) {
  ...
  rfalSetAnalogConfig(
      (RFAL_ANALOG_CONFIG_TECH_CHIP | RFAL_ANALOG_CONFIG_CHIP_LOWPOWER_OFF));
...
ReturnCode rfalChipSetRFO(uint8_t rfo) {
  return st25r3916ChangeRegisterBits(ST25R3916_REG_TX_DRIVER,
                                     ST25R3916_REG_TX_DRIVER_d_res_mask, rfo);
}
...
ReturnCode rfalChipSetLMMod(uint8_t mod, uint8_t unmod) {
  return st25r3916WriteRegister(ST25R3916_REG_PT_MOD,
                                (((mod << ST25R3916_REG_PT_MOD_ptm_res_shift) &
                                  ST25R3916_REG_PT_MOD_ptm_res_mask) |
                                 ((unmod & ST25R3916_REG_PT_MOD_pt_res_mask))));
}
...
ReturnCode rfalChipGetLMMod(uint8_t *mod, uint8_t *unmod) {
  ...
  if (unmod != NULL) {
    (*unmod) = ((reg >> ST25R3916_REG_PT_MOD_pt_res_shift) &
                ST25R3916_REG_PT_MOD_pt_res_mask);
  }
...
ReturnCode rfalChipMeasureAmplitude(uint8_t *result) {
  ...
  /* Set values as per defaults of DS. These regs/bits influence receiver chain
  ...
  st25r3916WriteRegister(ST25R3916_REG_OP_CONTROL,
                         (reg_opc & ~ST25R3916_REG_OP_CONTROL_rx_chn));
  st25r3916WriteRegister(
      ST25R3916_REG_MODE,
  ...
  st25r3916WriteRegister(ST25R3916_REG_RX_CONF1,
                         (reg_conf1 & ~ST25R3916_REG_RX_CONF1_ch_sel_AM));
  st25r3916WriteRegister(ST25R3916_REG_RX_CONF2,
                         ((reg_conf2 & ~(ST25R3916_REG_RX_CONF2_demod_mode |
...
/*******************************************************************************/
extern uint8_t
    invalid_size_of_stream_configs[(sizeof(struct st25r3916StreamConfig) ==
                                    sizeof(struct iso15693StreamConfig))
                                       ? 1
                                       : (-1)];
