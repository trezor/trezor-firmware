...
#define RFAL_NFCA_SLP_CMD \
  0x50U /*!< SLP cmd (byte1)    Digital 1.1  6.9.1 & Table 20 */
#define RFAL_NFCA_SLP_BYTE2 \
  0x00U /*!< SLP byte2          Digital 1.1  6.9.1 & Table 20 */
#define RFAL_NFCA_SLP_CMD_POS \
  0U /*!< SLP cmd position   Digital 1.1  6.9.1 & Table 20 */
#define RFAL_NFCA_SLP_BYTE2_POS \
  1U /*!< SLP byte2 position Digital 1.1  6.9.1 & Table 20 */

#define RFAL_NFCA_SDD_CT \
  0x88U /*!< Cascade Tag value Digital 1.1 6.7.2              */
#define RFAL_NFCA_SDD_CT_LEN \
  1U /*!< Cascade Tag length                               */

#define RFAL_NFCA_SLP_REQ_LEN \
  2U /*!< SLP_REQ length                                   */

#define RFAL_NFCA_SEL_CMD_LEN \
  1U /*!< SEL_CMD length                                   */
...
#define RFAL_NFCA_T_RETRANS \
  5U /*!< t RETRANSMISSION [3, 33]ms   EMVCo 2.6  A.5      */
#define RFAL_NFCA_N_RETRANS \
  2U /*!< Number of retries            EMVCo 2.6  9.6.1.3  */
...
#define rfalNfcaSelPar(nBy, nbi)      \
  (uint8_t)((((nBy) << 4U) & 0xF0U) | \
            ((nbi) &                  \
             0x0FU)) /*!< Calculates SEL_PAR with the bytes/bits to be sent */
...
typedef struct {
  ...
  rfalNfcaSelRes
      *selRes; /*!< Location to place of the SEL_RES(SAK) (Single CR)       */
...
static ReturnCode rfalNfcaPollerStartSingleCollisionResolution(
    uint8_t devLimit, bool *collPending, rfalNfcaSelRes *selRes,
    uint8_t *nfcId1, uint8_t *nfcId1Len);
...
static ReturnCode rfalNfcaPollerStartSingleCollisionResolution(
    ...
    uint8_t *nfcId1, uint8_t *nfcId1Len) {
  /* Check parameters */
  if ((collPending == NULL) || (selRes == NULL) || (nfcId1 == NULL) ||
      (nfcId1Len == NULL)) {
...
static ReturnCode rfalNfcaPollerGetSingleCollisionResolutionStatus(void) {
  ReturnCode ret;
  uint8_t collBit =
      1U; /* standards mandate or recommend collision bit to be set to One. */
  ...
  switch (gNfca.CR.state) {
    ...
    /*******************************************************************************/
    case RFAL_NFCA_CR_SDD_TX: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall
                                 through */

      /* Calculate SEL_CMD and SEL_PAR with the bytes/bits to be sent */
      gNfca.CR.selReq.selCmd = rfalNfcaCLn2SELCMD(gNfca.CR.cascadeLv);
      gNfca.CR.selReq.selPar =
          rfalNfcaSelPar(gNfca.CR.bytesTxRx, gNfca.CR.bitsTxRx);

      /* Send SDD_REQ (Anticollision frame) */
      rfalISO14443AStartTransceiveAnticollisionFrame(
          (uint8_t *)&gNfca.CR.selReq, &gNfca.CR.bytesTxRx, &gNfca.CR.bitsTxRx,
          &gNfca.CR.rxLen, RFAL_NFCA_FDTMIN);
    ...
    case RFAL_NFCA_CR_SDD:
      ...
      RFAL_EXIT_ON_BUSY(ret,
                        rfalISO14443AGetTransceiveAnticollisionFrameStatus());

      /* Retry upon timeout  EMVCo 2.6  9.6.1.3 */
      if ((ret == RFAL_ERR_TIMEOUT) && (gNfca.CR.devLimit == 0U) &&
          (gNfca.CR.retries != 0U)) {
        gNfca.CR.retries--;
        platformTimerDestroy(gNfca.CR.tmrFDT);
      ...
      if ((ret == RFAL_ERR_TIMEOUT) && (gNfca.CR.backtrackCnt != 0U) &&
          (!gNfca.CR.doBacktrack) &&
          (!((RFAL_NFCA_SDD_REQ_LEN == gNfca.CR.bytesTxRx) &&
             (0U == gNfca.CR.bitsTxRx)))) {
        ...
        /* Algorithm below does a post-increment, decrement to go back to
         * current position */
        ...
        collBit =
            (uint8_t)((0U == collBit) ? 1U : 0U); /* Invert the collision bit */
      ...
      if (ret == RFAL_ERR_RF_COLLISION) {
        ...
        if (((gNfca.CR.bytesTxRx + ((gNfca.CR.bitsTxRx != 0U) ? 1U : 0U)) >
            ...
            (gNfca.CR.backtrackCnt != 0U)) {
          gNfca.CR.backtrackCnt--;
          gNfca.CR.bytesTxRx =
              (RFAL_NFCA_CASCADE_1_UID_LEN + RFAL_NFCA_SDD_REQ_LEN) - 1U;
          gNfca.CR.bitsTxRx = 7;
          collBit =
              (uint8_t)(((uint8_t *)&gNfca.CR.selReq)[gNfca.CR.bytesTxRx] &
                        (1U
                         << gNfca.CR.bitsTxRx)); /* Not a real collision,
                                                    extract the actual bit for
                                                    the subsequent code */
        }

        /* Activity 1.0 & 1.1  9.3.4.12: If CON_DEVICES_LIMIT has a value of 0,
         * then NFC Forum Device is configured to perform collision detection
         * only       */
        ...
        *gNfca.CR.collPend = true;
        ...
        /* Set and select the collision bit, with the number of bytes/bits
         * successfully TxRx */
        if (collBit != 0U) {
       ...
       */
      if ((gNfca.CR.devLimit == 0U) &&
          (gNfca.CR.rxLen != sizeof(rfalNfcaSddRes))) {
        return RFAL_ERR_PROTO;
      }

      /* Check if the received BCC match */
      if (gNfca.CR.selReq.bcc !=
          rfalNfcaCalculateBcc(gNfca.CR.selReq.nfcid1,
    ...
    case RFAL_NFCA_CR_SEL_TX:
      ...
      rfalTransceiveBlockingTx(
          (uint8_t *)&gNfca.CR.selReq, sizeof(rfalNfcaSelReq),
          (uint8_t *)gNfca.CR.selRes, sizeof(rfalNfcaSelRes), &gNfca.CR.rxLen,
          RFAL_TXRX_FLAGS_DEFAULT, RFAL_NFCA_FDTMIN);
    ...
    case RFAL_NFCA_CR_SEL:
      ...
      if (*gNfca.CR.selReq.nfcid1 == RFAL_NFCA_SDD_CT) {
        /* Cascade Tag present, store nfcid1 bytes (excluding cascade tag) and
         * continue for next CL */
        RFAL_MEMCPY(&gNfca.CR.nfcId1[*gNfca.CR.nfcId1Len],
...
ReturnCode rfalNfcaPollerCheckPresence(rfal14443AShortFrameCmd cmd,
  ...
  /* Digital 1.1 6.10.1.3  For Commands ALL_REQ, SENS_REQ, SDD_REQ, and SEL_REQ,
   * the NFC Forum Device      * MUST treat receipt of a Listen Frame at a time
   * after FDT(Listen, min) as a Timeour Error */

  ret = rfalISO14443ATransceiveShortFrame(
      cmd, (uint8_t *)sensRes,
      (uint8_t)rfalConvBytesToBits(sizeof(rfalNfcaSensRes)), &rcvLen,
      RFAL_NFCA_FDTMIN);
  if ((ret == RFAL_ERR_RF_COLLISION) || (ret == RFAL_ERR_CRC) ||
      (ret == RFAL_ERR_NOMEM) || (ret == RFAL_ERR_FRAMING) ||
      (ret == RFAL_ERR_PAR) || (ret == RFAL_ERR_INCOMPLETE_BYTE)) {
    ret = RFAL_ERR_NONE;
  }
...
ReturnCode rfalNfcaPollerTechnologyDetection(rfalComplianceMode compMode,
  ...
  RFAL_EXIT_ON_ERR(ret,
                   rfalNfcaPollerStartTechnologyDetection(compMode, sensRes));
...
ReturnCode rfalNfcaPollerStartTechnologyDetection(rfalComplianceMode compMode,
  ...
  gNfca.DT.ret =
      rfalNfcaPollerCheckPresence(((compMode == RFAL_COMPLIANCE_MODE_EMV)
                                       ? RFAL_14443A_SHORTFRAME_CMD_WUPA
                                       : RFAL_14443A_SHORTFRAME_CMD_REQA),
                                  sensRes);

  /* Send SLP_REQ as  Activity 1.1  9.2.3.6 and EMVCo 2.6  9.2.1.3 */
  if ((gNfca.DT.compMode != RFAL_COMPLIANCE_MODE_ISO) &&
      (gNfca.DT.ret == RFAL_ERR_NONE)) {
...
ReturnCode rfalNfcaPollerSingleCollisionResolution(uint8_t devLimit,
  ...
  RFAL_EXIT_ON_ERR(ret, rfalNfcaPollerStartSingleCollisionResolution(
...
ReturnCode rfalNfcaPollerStartFullCollisionResolution(
  ...
  *devCnt = 0;
  ...
  /*******************************************************************************/
  /* Send ALL_REQ before Anticollision if a Sleep was sent before
   * Activity 1.1  9.3.4.1 and EMVco 2.6  9.3.2.1 */
  if (compMode != RFAL_COMPLIANCE_MODE_ISO) {
    ret = rfalISO14443ATransceiveShortFrame(
        RFAL_14443A_SHORTFRAME_CMD_WUPA, (uint8_t *)&nfcaDevList->sensRes,
        (uint8_t)rfalConvBytesToBits(sizeof(rfalNfcaSensRes)), &rcvLen,
        RFAL_NFCA_FDTMIN);
    if (ret != RFAL_ERR_NONE) {
      if ((compMode == RFAL_COMPLIANCE_MODE_EMV) ||
          ((ret != RFAL_ERR_RF_COLLISION) && (ret != RFAL_ERR_CRC) &&
           (ret != RFAL_ERR_FRAMING) && (ret != RFAL_ERR_PAR) &&
           (ret != RFAL_ERR_INCOMPLETE_BYTE))) {
        return ret;
      }
    }
    ...
    if ((ret == RFAL_ERR_NONE) &&
        (rfalConvBytesToBits(sizeof(rfalNfcaSensRes)) != rcvLen)) {
      return RFAL_ERR_PROTO;
    }
  }
  ...
  /* Restore the prev SENS_RES, assuming that the SENS_RES received is from
   * first device When only one device is detected it's not woken up then we'll
   * have no SENS_RES (ATQA) */
  ...
  if (rfalNfcaIsSensResT1T(&nfcaDevList->sensRes) && (devLimit != 0U) &&
      (ret == RFAL_ERR_NONE) && (compMode != RFAL_COMPLIANCE_MODE_EMV)) {
  ...
  RFAL_EXIT_ON_ERR(
      ret, rfalNfcaPollerStartSingleCollisionResolution(
               devLimit, &gNfca.CR.collPending, &nfcaDevList->selRes,
               (uint8_t *)&nfcaDevList->nfcId1, &nfcaDevList->nfcId1Len));
...
ReturnCode rfalNfcaPollerGetFullCollisionResolutionStatus(void) {
  ...
  switch (gNfca.CR.fState) {
    ...
    /*******************************************************************************/
    case RFAL_NFCA_CR_FULL_RESTART: /*  PRQA S 2003 # MISRA 16.3 - Intentional
                                       fall through */
      ...
      newDevType =
          ((uint8_t)gNfca.CR.nfcaDevList[*gNfca.CR.devCnt].selRes.sak) &
          RFAL_NFCA_SEL_RES_CONF_MASK; /* MISRA 10.8 */
      /* PRQA S 4342 1 # MISRA 10.5 - Guaranteed that no invalid enum values are
       * created: see guard_eq_RFAL_NFCA_T2T, .... */
      gNfca.CR.nfcaDevList[*gNfca.CR.devCnt].type =
          (rfalNfcaListenDeviceType)newDevType;
      gNfca.CR.nfcaDevList[*gNfca.CR.devCnt].isSleep = false;
      (*gNfca.CR.devCnt)++;

      /* If a collision was detected and device counter is lower than limit
       * Activity 1.1  9.3.4.21 */
    ...
    case RFAL_NFCA_CR_FULL_SLPCHECK:
      ...
      ret = rfalNfcaPollerCheckPresence(
          RFAL_14443A_SHORTFRAME_CMD_REQA,
          &gNfca.CR.nfcaDevList[*gNfca.CR.devCnt].sensRes);
        ...
        if ((*gNfca.CR.devCnt < gNfca.CR.devLimit) && (gNfca.CR.collPending)) {
          RFAL_EXIT_ON_ERR(
              ...
              rfalNfcaPollerStartSingleCollisionResolution(
                  gNfca.CR.devLimit, &gNfca.CR.collPending,
...
ReturnCode rfalNfcaPollerFullCollisionResolution(
  ...
  RFAL_EXIT_ON_ERR(ret, rfalNfcaPollerStartFullCollisionResolution(
                            compMode, devLimit, nfcaDevList, devCnt));
  ...
  *devCnt = 0;
  ...
  do {
    ...
    ret = rfalNfcaPollerFullCollisionResolution(
        ...
        (devLimit - *devCnt), &nfcaDevList[*devCnt], &tmpDevCnt);
    ...
    if ((ret == RFAL_ERR_NONE) && (tmpDevCnt > 0U)) {
      *devCnt += tmpDevCnt;
      ...
      if (*devCnt < devLimit) {
        ...
        ret = rfalNfcaPollerCheckPresence(RFAL_14443A_SHORTFRAME_CMD_REQA,
                                          &nfcaDevList[*devCnt].sensRes);
        if (ret == RFAL_ERR_NONE) {
          firstRound = false;
          continue;
        }
...
ReturnCode rfalNfcaPollerStartSelect(const uint8_t *nfcid1, uint8_t nfcidLen,
                                     rfalNfcaSelRes *selRes) {
  if ((nfcid1 == NULL) || (nfcidLen > RFAL_NFCA_CASCADE_3_UID_LEN) ||
      (selRes == NULL)) {
    return RFAL_ERR_PARAM;
  }
    ...
    if (gNfca.SEL.cascadeLv <= gNfca.SEL.fCascadeLv) {
      ...
      if (gNfca.SEL.fCascadeLv != gNfca.SEL.cascadeLv) {
        *selReq.nfcid1 = RFAL_NFCA_SDD_CT;
        RFAL_MEMCPY(&selReq.nfcid1[RFAL_NFCA_SDD_CT_LEN],
                    &gNfca.SEL.nfcid1[gNfca.SEL.nfcidOffset],
                    (RFAL_NFCA_CASCADE_1_UID_LEN - RFAL_NFCA_SDD_CT_LEN));
        gNfca.SEL.nfcidOffset +=
            (RFAL_NFCA_CASCADE_1_UID_LEN - RFAL_NFCA_SDD_CT_LEN);
      } else {
        RFAL_MEMCPY(selReq.nfcid1, &gNfca.SEL.nfcid1[gNfca.SEL.nfcidOffset],
                    RFAL_NFCA_CASCADE_1_UID_LEN);
      }

      /* Calculate nfcid's BCC */
      selReq.bcc = rfalNfcaCalculateBcc((uint8_t *)&selReq.nfcid1,
                                        sizeof(selReq.nfcid1));

      /*******************************************************************************/
      /* Send SEL_REQ  */
      RFAL_EXIT_ON_ERR(ret, rfalTransceiveBlockingTx(
                                (uint8_t *)&selReq, sizeof(rfalNfcaSelReq),
                                (uint8_t *)gNfca.SEL.selRes,
                                sizeof(rfalNfcaSelRes), &gNfca.SEL.rxLen,
                                RFAL_TXRX_FLAGS_DEFAULT, RFAL_NFCA_FDTMIN));
...
ReturnCode rfalNfcaPollerStartSleep(void) {
  ...
  rfalCreateByteFlagsTxRxContext(
      ctx, (uint8_t *)&gNfca.slpReq, sizeof(rfalNfcaSlpReq),
      (uint8_t *)&gNfca.slpReq, sizeof(gNfca.slpReq), NULL,
      RFAL_TXRX_FLAGS_DEFAULT, RFAL_NFCA_SLP_FWT);
...
ReturnCode rfalNfcaPollerGetSleepStatus(void) {
  ...
  /* ISO14443-3 6.4.3  HLTA - If PICC responds with any modulation during 1 ms
     this response shall be interpreted as not acknowledge Digital 2.0  6.9.2.1
     & EMVCo 3.0  5.6.2.1 - consider the HLTA command always acknowledged No
     check to be compliant with NFC and EMVCo, and to improve interoprability
     (Kovio RFID Tag)
  */
  RFAL_EXIT_ON_BUSY(ret, rfalGetTransceiveStatus());
...
bool rfalNfcaListenerIsSleepReq(const uint8_t *buf, uint16_t bufLen) {
  ...
  if ((bufLen != sizeof(rfalNfcaSlpReq)) ||
      (buf[RFAL_NFCA_SLP_CMD_POS] != RFAL_NFCA_SLP_CMD) ||
...
extern uint8_t
    guard_eq_RFAL_NFCA_T2T[((RFAL_NFCA_SEL_RES_CONF_MASK &
                             (uint8_t)RFAL_NFCA_T2T) == (uint8_t)RFAL_NFCA_T2T)
                               ? 1
                               : (-1)];
extern uint8_t
    guard_eq_RFAL_NFCA_T4T[((RFAL_NFCA_SEL_RES_CONF_MASK &
                             (uint8_t)RFAL_NFCA_T4T) == (uint8_t)RFAL_NFCA_T4T)
                               ? 1
                               : (-1)];
extern uint8_t guard_eq_RFAL_NFCA_NFCDEP[((RFAL_NFCA_SEL_RES_CONF_MASK &
                                           (uint8_t)RFAL_NFCA_NFCDEP) ==
                                          (uint8_t)RFAL_NFCA_NFCDEP)
                                             ? 1
                                             : (-1)];
extern uint8_t guard_eq_RFAL_NFCA_T4T_NFCDEP[((RFAL_NFCA_SEL_RES_CONF_MASK &
