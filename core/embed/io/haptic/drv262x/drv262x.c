...
static ts_t register_waveform(haptic_waveform_list_t *list,
  ...
  if (waveform->length == 0 || waveform->length > HAPTIC_LIB_MAX_WAVEFORMS) {
    return TS_EINVAL;
  }

  if (waveform->repeat > 7) {
    return TS_EINVAL;
  }
...
ts_t haptic_init(void) {
  ...
  drv->model = ((reg_value >> 4) > 1) ? DRV2625_CHIP : DRV2624_CHIP;
  ...
  switch (effect) {
    case HAPTIC_BUTTON_PRESS:
      if (drv->model == DRV2625_CHIP) {
        ...
        return haptic_play_waveform(1);  // Sharp button click effect
      }
      break;
