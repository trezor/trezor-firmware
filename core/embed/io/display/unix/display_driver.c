...
static float emulator_scale(void) {
  ...
  // rejected as well.
  if (*end != '\0' || !(scale >= EMULATOR_MIN_SCALE) ||
...
#include FOREGROUND_FILE
  ...
  if (drv->background || drv->foreground) {
    ...
    SDL_SetWindowSize(drv->window, logical_width, logical_height);
  ...
  if (window_width != logical_width || window_height != logical_height) {
    ...
    if (!SDL_SetRenderLogicalPresentation(drv->renderer, logical_width,
                                          logical_height,
                                          SDL_LOGICAL_PRESENTATION_LETTERBOX)) {
      LOG_WARN("SDL_SetRenderLogicalPresentation failed: %s", SDL_GetError());
    }
