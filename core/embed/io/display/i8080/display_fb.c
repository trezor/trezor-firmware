...
static void start_fb_copy(void) {
  ...
  if (!fb_queue_peeked(&drv->ready_frames)) {
    ...
    if (fb_idx >= 0) {
      ...
      // Native 16-bit-wide FMC bank: DMA the frame buffer straight across,
      // 16 bits per beat, no byte reordering (see bg_copy_start_const_out_16()
      // - the frame buffer already holds each pixel in the layout the panel
      // expects, matching what the CPU-loop path below writes verbatim via
      // ISSUE_PIXEL_DATA()). One atomic 16-bit bus cycle per pixel - no CPU
      // involvement, unlike the 8-bit MSB-first case below.
...
#elif defined(DISPLAY_I8080_8BIT_MSB_FIRST)
