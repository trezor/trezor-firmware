...
static inline uint8_t display_pixel_2_byte_encode(display_color_t px[8]) {
  // Combine eight pixels into a single byte
  uint8_t byte = 0;
  for (int i = 0; i < 8; i++) {
    byte |= ((uint8_t)px[i] & 0x1U) << i;
  }
...
static bool display_io_init(void) {
  ...
  GPIO_InitStruct.Pull = GPIO_PULLDOWN; // Busy active high => use pull-down
...
static bool display_spi_init(void) {
  ...
  drv->hspi.Init.ReadyMasterManagement = SPI_RDY_MASTER_MANAGEMENT_EXTERNALLY;
  drv->hspi.Init.ReadyPolarity = SPI_RDY_POLARITY_LOW;
...
static void display_panel_window_set(uint16_t x, uint16_t y, uint16_t w,
  ...
  cmd_data[0] = x / PIXELS_PER_BYTE;
  cmd_data[1] = (x + w - 1) / PIXELS_PER_BYTE;

  // Set Ram-X address start/end position 
  display_spi_transmit(DISPLAY_EP_SPI_TX_CMD, (uint8_t[]){0x44}, 1);
  display_spi_transmit(DISPLAY_EP_SPI_TX_RX_DATA, cmd_data, 2);

  // Set RAM x address
  display_spi_transmit(DISPLAY_EP_SPI_TX_CMD, (uint8_t[]){0x4E}, 1);
  display_spi_transmit(DISPLAY_EP_SPI_TX_RX_DATA, cmd_data, 1);

  // Set partial window Y start and end
  cmd_data[0] = (y + h - 1) % 256;
  cmd_data[1] = (y + h - 1) / 256;
  cmd_data[2] = y % 256;
  cmd_data[3] = y / 256;

  // Set Ram-Y address start/end position  
  display_spi_transmit(DISPLAY_EP_SPI_TX_CMD, (uint8_t[]){0x45}, 1);
  display_spi_transmit(DISPLAY_EP_SPI_TX_RX_DATA, cmd_data, 4);

  // Set RAM y address
  display_spi_transmit(DISPLAY_EP_SPI_TX_CMD, (uint8_t[]){0x4F}, 1);
  display_spi_transmit(DISPLAY_EP_SPI_TX_RX_DATA, cmd_data, 2);
}
...
static void display_panel_refresh(void) {
  ...
  if (drv->panel_mode == DISPLAY_SLOW_MODE) {
    param = 0xF7;
  } else if (drv->panel_mode == DISPLAY_FAST_MODE) {
    param = 0xC7;
  } else if (drv->panel_mode == DISPLAY_PARTIAL_MODE) {
    param = 0xFF;
  } else {
    // Invalid refresh mode, do nothing
    return;
  }
...
static void display_color_fill(display_color_t color) {
  ...
  display_panel_window_set(0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT);

  uint8_t byte = display_pixel_2_byte_encode((display_color_t[]){
      color, color, color, color, color, color, color, color});

  display_spi_transmit(DISPLAY_EP_SPI_TX_CMD, (uint8_t[]){0x24}, 1);

  for(int i = 0; i < DISPLAY_BYTES; i++) {
    display_spi_transmit(DISPLAY_EP_SPI_TX_RX_DATA, &byte, 1);
  }

  // Initialize the RED RAM too.
  display_spi_transmit(DISPLAY_EP_SPI_TX_CMD, (uint8_t[]){0x26}, 1);
...
static void display_img_show(const uint8_t* img_data, uint16_t x, uint16_t y,
  ...
  display_panel_init(mode);

  display_panel_window_set(x, y, w, h);  

  if (DISPLAY_PARTIAL_MODE != mode) {
    display_spi_transmit(DISPLAY_EP_SPI_TX_CMD, (uint8_t[]){0x26}, 1);

#if 1    
    for (int i = y; i < (y + h); i++) {
      display_spi_transmit(
          DISPLAY_EP_SPI_TX_RX_DATA,
          &img_data[i * DISPLAY_WIDTH / PIXELS_PER_BYTE + x / PIXELS_PER_BYTE],
          (x + w - 1) / PIXELS_PER_BYTE - x / PIXELS_PER_BYTE + 1);
    }
#else
    for (int i = y; i < (y + h); i++) {
      for (int j = x / PIXELS_PER_BYTE; j <= (x + w - 1) / PIXELS_PER_BYTE; j++) {
        uint8_t img_data_tmp = img_data[i * DISPLAY_WIDTH / PIXELS_PER_BYTE + j];
...
bool display_ep_init(display_content_mode_t mode) {
  ...
  if (!display_panel_init(DISPLAY_SLOW_MODE)) {
    goto cleanup;
  }
...
void display_ep_demo(void) {
  ...
  display_img_show(gImage_basemapT, 0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT,
                   DISPLAY_SLOW_MODE);
  systick_delay_ms(2000);
  display_img_show(gImage_main1, 0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT,
                   DISPLAY_FAST_MODE);
  systick_delay_ms(2000);
  display_img_show(gImage_logo, 0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT,
                   DISPLAY_PARTIAL_MODE);
  systick_delay_ms(2000);
  display_img_show(IMAGE_DATA, 0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT,
                   DISPLAY_PARTIAL_MODE);
  systick_delay_ms(2000);
  display_img_show(gImage_basemapT, 0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT,
                   DISPLAY_PARTIAL_MODE);
