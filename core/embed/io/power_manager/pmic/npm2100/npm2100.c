...
#ifdef NPM2100_USE_INTERRUPT_PIN
      {NPM2100_INTEN_SYSTEM_SET, NPM2100_EVENT_SYSTEM_DIETWARN},
      {NPM2100_REQUESTSET, NPM2100_REQUEST_DIETEMPENA},
#endif
  };
...
// Initializes the PMIC driver (see `pmic.h`)
//
// Opens the I2C bus, creates the timer used for ADC conversion delays,
// optionally configures the INT line, latches the restart cause and programs
// the nPM2100 registers to their default values. All acquired resources are
// released if any of the steps fails.
bool pmic_init(void) {
...
// Deinitializes the PMIC driver (see `pmic.h`)
//
// Disables the INT line, releases the I2C bus and the timer and clears the
// driver state. Can be safely called on a partially initialized driver.
...
// Suspends the driver background operations (see `pmic.h`)
//
// Only marks the driver as suspending; the FSM finishes the pending operation
// and enters the suspended state once it becomes idle.
...
// Resumes the driver background operations (see `pmic.h`)
//
// Clears the suspended state and kicks the FSM so that requests deferred
// during the suspension are processed.
...
// Checks whether the driver background operations are suspended
// (see `pmic.h`)
...
// Requests entering the ship mode (see `pmic.h`)
//
// The request is only scheduled here; the FSM writes the SHIP task register
// once the driver becomes idle and the device powers off shortly after.
...
// Returns the cause of the last restart (see `pmic.h`)
//
// Reports the content of the RESET register latched during the driver
// initialization, before the register was cleared.
...
// Starts the asynchronous measurement (see `pmic.h`)
//
// Schedules sequential conversion of the VBAT, VSYS and die temperature
// channels; the report is passed to the callback once all channels are read
// out. Only one measurement can be in progress at a time. Passing a NULL
// callback detaches the pending report instead of starting a new conversion.
...
// Synchronous variant of the `pmic_measure` function (see `pmic.h`)
//
// Starts the asynchronous measurement and blocks in WFI until it completes.
...
static void npm2100_i2c_callback(void* context, i2c_packet_t* packet) {
  ...
  switch (drv->state) {
    ...
    case NPM2100_STATE_ADC_READOUT:
      ...
      if (drv->adc_channel_id == NPM2100_ADC_CHANNEL_DIETEMP) {
...
#ifdef NPM2100_USE_INTERRUPT_PIN
// nPM2100 INT line interrupt handler
//
// The INT pin is active low and stays asserted until the nPM2100 event
// registers are cleared, so the handler only schedules the clearing through
// the FSM and returns.
