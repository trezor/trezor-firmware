...
pub fn def_module(lib: &mut CLibrary) -> Result<()> {
    ...
    if cfg!(feature = "pmic") {
        ...
        // (feature name, hardware source) for each concrete PMIC driver.
        ...
        if cfg!(feature = "pmic_power_latch") {
            drivers.push((
                "pmic_power_latch",
                "power_manager/pmic/power_latch/power_latch.c",
            ));
        }
        ...
        // has an integrated charger; the power latch does not.
        if cfg!(feature = "pmic_npm1300") {
