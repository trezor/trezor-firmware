...
uint16_t usb_emu_port(uint16_t port_offset) {
#ifdef TREZOR_EMULATOR
  const char *base_port = getenv("TREZOR_UDP_PORT");
  if (base_port != NULL) {
    return atoi(base_port) + port_offset;
  } else
#endif
  {
    return USB_IFACE_BASE_PORT + port_offset;
  }
}
...
static secbool usb_wire_iface_init(uint8_t *iface_num) {
  static uint8_t wire_iface_buffer[64];
...
static secbool usb_debug_iface_init(uint8_t *iface_num) {
  static uint8_t debug_iface_buffer[64];
...
static secbool usb_webauthn_iface_init(uint8_t *iface_num) {
  ...
  static uint8_t webauthn_iface_buffer[64];
