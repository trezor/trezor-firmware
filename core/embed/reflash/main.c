...
static void flash_from_sdcard(const flash_area_t* area, uint32_t source, uint32_t length)
{
    static uint32_t buf[SDCARD_BLOCK_SIZE / sizeof(uint32_t)];

    _Static_assert(SDCARD_BLOCK_SIZE % FLASH_BLOCK_SIZE == 0);
    ensure(sectrue * (source % SDCARD_BLOCK_SIZE == 0), "source not a multiple of block size");
    ensure(sectrue * (length % SDCARD_BLOCK_SIZE == 0), "length not a multiple of block size");

    for (uint32_t i = 0; i < length / SDCARD_BLOCK_SIZE; i++) {
        term_printf("read %d\n", (unsigned int)(i + source / SDCARD_BLOCK_SIZE));

        ensure(sdcard_read_blocks(buf, i + source / SDCARD_BLOCK_SIZE, 1), "sdcard_read_blocks");
...
int main(void)
{
    ...
    ensure(flash_area_erase(&BOARDLOADER_AREA, progress_callback), "flash_erase_sectors");
    ensure(flash_area_erase(&BOOTLOADER_AREA, progress_callback), "flash_erase_sectors");
    term_printf("\n");
    term_printf("erased\n");
...
#define BOOTLOADER_TOTAL_SIZE (128 * 1024)
    ...
    flash_from_sdcard(&BOOTLOADER_AREA, BOARDLOADER_TOTAL_SIZE, BOOTLOADER_TOTAL_SIZE);
