...
fn main() {
    ...
    board_capabilities::def_module(&mut lib);

    if cfg!(feature = "consumption_mask") {
        consumption_mask::def_module(&mut lib);
    }

    fwutils::def_module(&mut lib);

    if cfg!(feature = "hash_processor") {
        hash_processor::def_module(&mut lib);
    }

    if cfg!(feature = "hw_revision") {
        hw_revision::def_module(&mut lib);
    }

    if cfg!(feature = "backup_ram") {
        backup_ram::def_module(&mut lib);
    }

    monoctr::def_module(&mut lib);

    option_bytes::def_module(&mut lib);

    image::def_module(&mut lib);

    if cfg!(feature = "iwdg") {
        iwdg::def_module(&mut lib);
    }

    if cfg!(feature = "optiga") {
        optiga::def_module(&mut lib);
    }

    //if cfg!(feature = "random_delays") {
    random_delays::def_module(&mut lib);
    //}

    rng::def_module(&mut lib);

    rsod::def_module(&mut lib);

    if cfg!(feature = "secret") {
        secret::def_module(&mut lib);
    }

    secret_keys::def_module(&mut lib);

    if cfg!(feature = "secure_aes") {
        secure_aes::def_module(&mut lib);
    }

    if cfg!(feature = "storage") {
        storage::def_module(&mut lib);
    }
    ...
    if cfg!(feature = "tamper") {
        tamper::def_module(&mut lib);
    }

    if cfg!(feature = "telemetry") {
        telemetry::def_module(&mut lib);
    }

    if cfg!(feature = "time_estimate") {
        time_estimate::def_module(&mut lib);
    }

    unit_properties::def_module(&mut lib);

    if !cfg!(feature = "mcu_emulator") && !cfg!(feature = "secure_mode") {
