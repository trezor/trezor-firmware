...
static const uint8_t WRONG_BATCH[5] = {0x19, 0x07, 0x1F, 0x0A, 0x04};
...
#endif
#if !PRODUCTION
  // this will get merge conflict with tropic_change_pin_check
  // and when it does, use the change from there
  if (tropic_custom_session_start(NULL, TROPIC_PRIVILEGED_PAIRING_KEY_SLOT) ==
      LT_OK) {
    return true;
  }
...
#include <sec/tropic_config.h>

static secbool tropic_ensure_i_config(const struct lt_config_t *expected) {
  tropic_driver_t *drv = &g_tropic_driver;
  ...
  for (uint8_t i = 0; i < LT_CONFIG_OBJ_CNT; i++) {
    ...
    uint32_t irrecoverable = expected->obj[i] & ~current;
    if (irrecoverable != 0) {
      return secfalse;
    }

    // Bits that are currently 1 but are expected to be 0: flip them.
    uint32_t to_flip = ~expected->obj[i] & current;
...
static secbool tropic_ensure_r_config(const struct lt_config_t *expected) {
  tropic_driver_t *drv = &g_tropic_driver;

  struct lt_config_t current = {0};
  for (uint8_t i = 0; i < LT_CONFIG_OBJ_CNT; i++) {
    if (TROPIC_RETRY_COMMAND(lt_r_config_read(
            &drv->handle, TROPIC_CONFIG_ADDRS[i], &current.obj[i])) != LT_OK) {
      return secfalse;
    }
  }

  if (memcmp(&current, expected, sizeof(struct lt_config_t)) == 0) {
    return sectrue;
  }

  return lt_erase_and_write_R_config_retry(&drv->handle, expected) == LT_OK
             ? sectrue
             : secfalse;
}

static bool tropic_get_batch_id(uint8_t batch_id[5]) {
  tropic_driver_t *drv = &g_tropic_driver;

  lt_chip_id_t chip_id = {0};
  if (lt_get_info_chip_id(&drv->handle, &chip_id) != LT_OK) {
    return false;
  }

  memcpy(batch_id, chip_id.batch_id, sizeof(chip_id.batch_id));
  return true;
}
...
secbool tropic_ensure_configuration(void) {
  ...
  if (memcmp(batch_id, WRONG_BATCH, sizeof(WRONG_BATCH)) != 0) {
    // we do not set the configuration on new batches
    return sectrue;
  }

  if (tropic_ensure_i_config(&TROPIC_I_CONFIG) != sectrue) {
    return secfalse;
  }

  if (tropic_ensure_r_config(&TROPIC_R_CONFIG) != sectrue) {
    return secfalse;
  }
