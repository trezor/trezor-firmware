...
void hash_processor_init(void) {
  ...
  DMA_Handle.Instance = GPDMA1_Channel11;
  ...
  NVIC_SetPriority(GPDMA1_Channel11_IRQn, IRQ_PRI_NORMAL);
...
static void hash_processor_sha256_calc_dma(const uint8_t *data, uint32_t len,
  ...
  while (len > 0) {
    ...
    HAL_HASH_Start_DMA(&hhash, (uint8_t *)data, chunk, hash);
...
void hash_processor_sha256_calc(const uint8_t *data, uint32_t len,
  ...
  if (((uint32_t)data & 0x3) == 0) {
    ...
    HAL_HASH_Start(&hhash, (uint8_t *)data, len, hash, 1000);
  }
}
...
void hash_processor_sha256_update(hash_sha256_context_t *ctx,
  ...
  if (ctx->length > 0) {
    ...
    if (ctx->length == HASH_SHA256_BUFFER_SIZE) {
      HAL_HASH_Accumulate(&hhash, (uint8_t *)ctx->buffer,
                          HASH_SHA256_BUFFER_SIZE, 1000);
      ctx->length = 0;
  ...
  while (len_aligned > 0) {
    ...
    HAL_HASH_Accumulate(&hhash, (uint8_t *)data, chunk, 1000);
    data += chunk;
    len_aligned -= chunk;
  }
