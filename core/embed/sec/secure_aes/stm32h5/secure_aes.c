...
secbool secure_aes_init(void) {
  // Unlike the STM32U5, the STM32H5 SAES has no dedicated secure HSI (SHSI).
  // The SAES is clocked by the AHB bus clock together with a dedicated kernel
  // clock controlled through the RCC. After the clock is enabled and the reset
  // released, the SAES automatically draws random numbers from the RNG and
  // asserts BUSY in SAES_SR; it must not be used until BUSY clears (RM0517,
  // "SAES reset and clocks").
  //
  // NOTE: the SAES kernel clock and the RNG must be clocked for this to
  // complete at runtime. That depends on the full clock configuration in
  // SystemInit (see the H5 startup TODO); with the preliminary HSI-only clock
  // the RNG source must still be available.
  __HAL_RCC_SAES_CLK_ENABLE();
  __HAL_RCC_SAES_FORCE_RESET();
  __HAL_RCC_SAES_RELEASE_RESET();

  // Wait until the SAES has finished fetching randomness from the RNG.
