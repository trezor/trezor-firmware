...
static void secret_disable_access(void) {
  // Unlike the STM32U5 (which disables access to the HDP area for the rest of
  // the boot via FLASH->SECHDPCR ACCDIS bits), the STM32H5 hides the secret
  // (HDP) area by advancing the temporal isolation level (HDPL) through SBS.
  // Writing 0x6A to SBS_HDPLCR.INCR_HDPL increments HDPL by one, after which the
  // current level's HDP area is inaccessible to all subsequent (higher) levels
  // until the next reset (RM0517, SBS_HDPLCR).
  //
  // NOTE: this relies on the secret sectors being configured as the HDP area
  // for this boot stage in the flash option bytes (HDP1R/HDP2R), which is
  // provisioned once via STM32CubeProgrammer (see sec/option_bytes/stm32h5).
