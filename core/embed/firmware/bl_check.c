...
#if PRODUCTION || BOOTLOADER_QA
static void uzlib_prepare(
    struct uzlib_uncomp *decomp, uint8_t *window, const void *src, uint32_t srcsize, void *dest,
    uint32_t destsize)
{
...
void check_and_replace_bootloader(void)
{
    ...
    const uint32_t *data =
        (const uint32_t *)&_binary_embed_firmware_bootloaders_bootloader_bin_deflated_start;
    const uint32_t len =
    ...
    uzlib_prepare(&decomp, decomp_window, data, len, decomp_out, sizeof(decomp_out));

    ensure(
        (uzlib_uncompress(&decomp) == TINF_OK) ? sectrue : secfalse,
        "Bootloader header decompression failed");

    const image_header *new_bld_hdr =
        read_image_header((uint8_t *)decomp_out, BOOTLOADER_IMAGE_MAGIC, BOOTLOADER_IMAGE_MAXSIZE);

    ensure(
        new_bld_hdr == (const image_header *)decomp_out ? sectrue : secfalse,
    ...
    const image_header *current_bld_hdr =
        read_image_header(bl_data, BOOTLOADER_IMAGE_MAGIC, BOOTLOADER_IMAGE_MAXSIZE);

    // cannot find valid header for current bootloader, something is wrong
    ensure(
        current_bld_hdr == (const image_header *)bl_data ? sectrue : secfalse,
    ...
    do {
        ...
        uint32_t size_padded = FLASH_ALIGN(size);
            ...
            flash_area_write_data_padded(&BOOTLOADER_AREA, offset, p, size, 0, size_padded), NULL);
        offset += size_padded;
    ...
    if (offset < bl_len) {
        // fill the rest of the bootloader area with 0x00
            ...
            flash_area_write_data_padded(&BOOTLOADER_AREA, offset, NULL, 0, 0, bl_len - offset),
            NULL);
    }
