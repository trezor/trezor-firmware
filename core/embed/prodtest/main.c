...
static void test_wipe(void)
{
    ...
    display_clear();
    display_text_center(
        DISPLAY_RESX / 2, DISPLAY_RESY / 2 + 10, "WIPED", -1, FONT_BOLD, COLOR_WHITE, COLOR_BLACK);
    display_refresh();
    vcp_println("OK");
}
...
static void test_otp_read(void)
{
    ...
    ensure(flash_otp_read(FLASH_OTP_BLOCK_BATCH, 0, data, FLASH_OTP_BLOCK_SIZE), NULL);
...
static void test_otp_read_device_variant()
{
    ...
    if (sectrue != flash_otp_read(FLASH_OTP_BLOCK_DEVICE_VARIANT, 0, data, sizeof(data))) {
        vcp_println("ERROR");
        return;
    }
...
static void test_otp_write_device_variant(const char *args)
{
    ...
    if (arg_len != 0 && arg_num < sizeof(data)) {
        ...
        flash_otp_write(FLASH_OTP_BLOCK_DEVICE_VARIANT, 0, (const uint8_t *)data, sizeof(data)),
        NULL);
...
int main(void)
{
    ...
    if (sectrue == flash_otp_read(FLASH_OTP_BLOCK_BATCH, 0, (uint8_t *)dom, 32) &&
        ...
        display_text_center(
            DISPLAY_RESX / 2, DISPLAY_RESY - 30, dom + 8, -1, FONT_BOLD, COLOR_WHITE, COLOR_BLACK);
    }
