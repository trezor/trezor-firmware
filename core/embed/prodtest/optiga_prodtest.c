...
static const optiga_metadata_item KEY_USE_SIGN = OPTIGA_META_VALUE(OPTIGA_KEY_USAGE_SIGN);
static const optiga_metadata_item TYPE_PTFBIND = OPTIGA_META_VALUE(OPTIGA_DATA_TYPE_PTFBIND);
...
static bool set_metadata(uint16_t oid, const optiga_metadata *metadata)
{
    ...
    size_t size = 0;
    optiga_result ret = optiga_serialize_metadata(metadata, serialized, sizeof(serialized), &size);
    if (OPTIGA_SUCCESS != ret) {
        vcp_println("ERROR optiga_serialize_metadata error %d for OID 0x%04x.", ret, oid);
        return false;
    }

    optiga_set_data_object(oid, true, serialized, size);

    ret = optiga_get_data_object(oid, true, serialized, sizeof(serialized), &size);
    if (OPTIGA_SUCCESS != ret) {
...
void optiga_lock(void)
{
    ...
    // Delete trust anchor.
    optiga_result ret = optiga_set_data_object(OID_TRUST_ANCHOR, false, (const uint8_t *)"\0", 1);
    if (OPTIGA_SUCCESS != ret) {
        vcp_println("ERROR optiga_set_data error %d for 0x%04x.", ret, OID_TRUST_ANCHOR);
        return;
    }
...
optiga_locked_status get_optiga_locked_status(void)
{
    ...
    const uint16_t oids[] = {
        OID_CERT_DEV, OID_CERT_FIDO, OID_KEY_DEV, OID_KEY_FIDO, OID_KEY_PAIRING};
    ...
    for (size_t i = 0; i < sizeof(oids) / sizeof(oids[0]); ++i) {
        ...
        optiga_result ret = optiga_get_data_object(
            oids[i], true, metadata_buffer, sizeof(metadata_buffer), &metadata_size);
        if (OPTIGA_SUCCESS != ret) {
            vcp_println("ERROR optiga_get_metadata error %d for OID 0x%04x.", ret, oids[i]);
            return OPTIGA_LOCKED_ERROR;
        }

        optiga_metadata stored_metadata = {0};
        ret = optiga_parse_metadata(metadata_buffer, metadata_size, &stored_metadata);
        if (OPTIGA_SUCCESS != ret) {
            vcp_println("ERROR optiga_parse_metadata error %d.", ret);
            return OPTIGA_LOCKED_ERROR;
        }
...
void optigaid_read(void)
{
    ...
    optiga_result ret = optiga_get_data_object(
        OPTIGA_OID_COPROC_UID, false, optiga_id, sizeof(optiga_id), &optiga_id_size);
    if (OPTIGA_SUCCESS != ret) {
        vcp_println(
            "ERROR optiga_get_data_object error %d for 0x%04x.", ret, OPTIGA_OID_COPROC_UID);
        return;
    }
...
void cert_read(uint16_t oid)
{
    ...
    size_t cert_size = 0;
    optiga_result ret = optiga_get_data_object(oid, false, cert, sizeof(cert), &cert_size);
    if (OPTIGA_SUCCESS != ret) {
    ...
    if (cert[0] == 0xC0) {
        ...
        if (tls_identity_size + 3 > cert_size || cert_chain_size + 3 > tls_identity_size ||
...
void cert_write(uint16_t oid, char *data)
{
    ...
    if (OPTIGA_SUCCESS != ret || cert_size != len || memcmp(data_bytes, cert, len) != 0) {
        vcp_println("ERROR optiga_get_data_object error %d for 0x%04x.", ret, oid);
        return;
    }
...
void pubkey_read(uint16_t oid)
{
    ...
    static const uint8_t BASE_POINT[] = {
        0x03, 0x42, 0x00, 0x04, 0x6b, 0x17, 0xd1, 0xf2, 0xe1, 0x2c, 0x42, 0x47, 0xf8, 0xbc,
        0xe6, 0xe5, 0x63, 0xa4, 0x40, 0xf2, 0x77, 0x03, 0x7d, 0x81, 0x2d, 0xeb, 0x33, 0xa0,
        0xf4, 0xa1, 0x39, 0x45, 0xd8, 0x98, 0xc2, 0x96, 0x4f, 0xe3, 0x42, 0xe2, 0xfe, 0x1a,
        0x7f, 0x9b, 0x8e, 0xe7, 0xeb, 0x4a, 0x7c, 0x0f, 0x9e, 0x16, 0x2b, 0xce, 0x33, 0x57,
        0x6b, 0x31, 0x5e, 0xce, 0xcb, 0xb6, 0x40, 0x68, 0x37, 0xbf, 0x51, 0xf5};
    uint8_t public_key[32] = {0};
    size_t public_key_size = 0;
    optiga_result ret = optiga_calc_ssec(
        OPTIGA_CURVE_P256, oid, BASE_POINT, sizeof(BASE_POINT), public_key, sizeof(public_key),
        &public_key_size);
    if (OPTIGA_SUCCESS != ret) {
...
void keyfido_write(char *data)
{
    ...
    optiga_result ret = optiga_calc_ssec(
        OPTIGA_CURVE_P256, OID_KEY_DEV, public_key, sizeof(public_key), secret, sizeof(secret),
        &secret_size);
    if (OPTIGA_SUCCESS != ret) {
        memzero(secret, sizeof(secret));
        vcp_println("ERROR optiga_calc_ssec error %d.", ret);
    ...
    aes_ret = aes_cbc_decrypt(&data_bytes[CIPHERTEXT_OFFSET], fido_key, sizeof(fido_key), iv, &ctx);
    memzero(&ctx, sizeof(ctx));
    memzero(secret, sizeof(secret));
    if (EXIT_SUCCESS != aes_ret) {
    ...
    memzero(&metadata, sizeof(metadata));
    metadata.change = OPTIGA_ACCESS_CONDITION(OPTIGA_ACCESS_COND_INT, OID_TRUST_ANCHOR);
...
void sec_read(void)
{
    ...
    optiga_result ret = optiga_get_data_object(OPTIGA_OID_SEC, false, &sec, sizeof(sec), &size);
    if (OPTIGA_SUCCESS != ret || sizeof(sec) != size) {
        vcp_println("ERROR optiga_get_data_object error %d for 0x%04x.", ret, OPTIGA_OID_SEC);
        return;
    }
...
static bool get_cert_extensions(DER_ITEM *tbs_cert, DER_ITEM *extensions)
{
    ...
    while (der_read_item(&tbs_cert->buf, &cert_item)) {
        if (cert_item.id == DER_X509_EXTENSIONS) {
            ...
            return der_read_item(&cert_item.buf, extensions) && extensions->id == DER_SEQUENCE;
        }
    }
    return false;
}

static bool get_extension_value(
    const uint8_t *extension_oid, size_t extension_oid_size, DER_ITEM *extensions,
...
static bool get_authority_key_digest(DER_ITEM *tbs_cert, const uint8_t **authority_key_digest)
{
    ...
    if (!get_extension_value(
            OID_AUTHORITY_KEY_IDENTIFIER, sizeof(OID_AUTHORITY_KEY_IDENTIFIER), &extensions,
            &extension_value)) {
        vcp_println(
    ...
    if (!der_read_item(&extension_value.buf, &auth_key_id) || auth_key_id.id != DER_SEQUENCE) {
        vcp_println(
    ...
    DER_ITEM key_id = {0};
    if (!der_read_item(&auth_key_id.buf, &key_id) || key_id.id != DER_X509_KEY_IDENTIFIER) {
        vcp_println("ERROR get_authority_key_digest, failed to find keyIdentifier field.");
        return false;
    }
    ...
    if (buffer_remaining(&key_id.buf) != SHA1_DIGEST_LENGTH ||
        ...
        vcp_println("ERROR get_authority_key_digest, invalid length of keyIdentifier.");
        return false;
    }
...
bool check_device_cert_chain(const uint8_t *chain, size_t chain_size)
{
    ...
    if (optiga_calc_sign(
            OID_KEY_DEV, digest, sizeof(digest), &der_sig[2], sizeof(der_sig) - 2, &der_sig_size) !=
        OPTIGA_SUCCESS) {
    ...
    while (buffer_remaining(&chain_reader) > 0) {
        ...
        if (!der_read_item(&chain_reader, &cert) || cert.id != DER_SEQUENCE) {
            vcp_println("ERROR check_device_cert_chain, der_read_item 1, cert %d.", cert_count);
            return false;
        }
        ...
        if (!der_read_item(&cert.buf, &tbs_cert)) {
            vcp_println("ERROR check_device_cert_chain, der_read_item 2, cert %d.", cert_count);
            return false;
        }
        ...
        for (int i = 0; i < 7; ++i) {
            if (!der_read_item(&tbs_cert.buf, &pub_key_info)) {
                vcp_println("ERROR check_device_cert_chain, der_read_item 3, cert %d.", cert_count);
                return false;
            }
        }
        ...
        for (int i = 0; i < 2; ++i) {
            if (!der_read_item(&pub_key_info.buf, &pub_key)) {
                vcp_println("ERROR check_device_cert_chain, der_read_item 4, cert %d.", cert_count);
                return false;
            }
        }

        if (!buffer_get(&pub_key.buf, &unused_bits) || buffer_remaining(&pub_key.buf) != 65 ||
            !buffer_ptr(&pub_key.buf, &pub_key_bytes)) {
            vcp_println("ERROR check_device_cert_chain, reading public key, cert %d.", cert_count);
            return false;
        }

        // Verify the previous signature.
        if (ecdsa_verify_digest(&nist256p1, pub_key_bytes, sig, digest) != 0) {
            vcp_println("ERROR check_device_cert_chain, ecdsa_verify_digest, cert %d.", cert_count);
            return false;
        }
        ...
        DER_ITEM sig_alg = {0};
        if (!der_read_item(&cert.buf, &sig_alg) || sig_alg.buf.size != sizeof(ECDSA_WITH_SHA256) ||
            memcmp(ECDSA_WITH_SHA256, sig_alg.buf.data, sizeof(ECDSA_WITH_SHA256)) != 0) {
            vcp_println(
        ...
        if (!der_read_item(&cert.buf, &sig_val) || sig_val.id != DER_BIT_STRING ||
            ...
            vcp_println(
                "ERROR check_device_cert_chain, reading signatureValue, cert %d.", cert_count);
            return false;
        }
        ...
        if (!buffer_ptr(&sig_val.buf, &sig_bytes) ||
            ecdsa_sig_from_der(sig_bytes, buffer_remaining(&sig_val.buf), sig) != 0) {
            vcp_println("ERROR check_device_cert_chain, ecdsa_sig_from_der, cert %d.", cert_count);
            return false;
        }
    }
    ...
    for (int recid = 0; recid < 4; ++recid) {
        if (ecdsa_recover_pub_from_sig(&nist256p1, pub_key, sig, digest, recid) == 0) {
            sha1_Raw(pub_key, sizeof(pub_key), pub_key_digest);
            if (memcmp(authority_key_digest, pub_key_digest, sizeof(pub_key_digest)) == 0) {
                return true;
            }
        }
    }
