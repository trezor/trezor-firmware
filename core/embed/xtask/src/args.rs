...
impl BuildArgs {
    ...
    pub fn resolve_features(&self) -> Result<Vec<&'static str>> {
        ...
        if self.force_bootloader_upgrade {
            features.push("force_bootloader_upgrade");
        }
    ...
    pub fn configure_cargo(&self, cmd: &mut process::Command) -> Result<()> {
        ...
        if self.emulator && self.asan {
            ...
            //
            // -Zbuild-std is required so that the standard library is also compiled with
            // sanitizer instrumentation.
            let host = helpers::host_triple()?;
            cmd.args(["--target", &host]);
            cmd.arg("-Zbuild-std");
            cmd.args([
                "--config",
...
pub struct CombineArgs {
    ...
    /// Generate Intel HEX output instead of binary
    #[arg(long)]
    pub ihex: bool,
}
