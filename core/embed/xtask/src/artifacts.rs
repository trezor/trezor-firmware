...
use crate::helpers::{elf_path, profile_dir, target_dir};
...
pub fn collect_artifacts(args: &BuildArgs, is_dependency: bool) -> Result<()> {
    let artifact_dir = target_dir()?.join("artifacts").join(args.model.model_id());
    ...
    if !args.emulator {
        ...
        if args.component.flashable() {
            // Copy {binary}.mem.ld file for flashing
            artifacts.push((
                profile.join(format!("{binary_name}.mem.ld")),
                format!("{name}.mem.ld"),
            ));
        }
    }
