...
#define MSIRC0_VALUE \
  96000000U /*!< Value of the Internal MSI RC0 oscillator in Hz*/
#endif      /* MSI_VALUE */

#if !defined(MSIRC1_VALUE)
#define MSIRC1_VALUE \
  24000000U /*!< Value of the Internal MSI RC1 oscillator in Hz*/
#endif      /* MSI_VALUE */

#define DEFAULT_FREQ 96000000U
...
#error Unsupported LSI frequency
  ...
  if (lsi_div != (csr_temp & RCC_CSR_LSIPREDIV)) {
    ...
    MODIFY_REG(RCC->CSR, RCC_CSR_LSIPREDIV, lsi_div);
  }
...
void SystemInit(void) {
  ...
  // todo not using LPM, check if needed
  ...
  RCC->CR &= ~(RCC_CR_HSEON | RCC_CR_HSECSSON | RCC_CR_HSI48ON);

  // configure EPOD booster to 16 MHz
  // todo do this with respect to selected HSE range, now asuming 32MHz
  RCC->CFGR4 = RCC_EPODBOOSTER_SOURCE_HSE | RCC_EPODBOOSTER_DIV2;
  ...
  MODIFY_REG(PWR->VOSR, PWR_VOSR_R2EN, (PWR_VOSR_R1EN | PWR_VOSR_BOOSTEN));

  while (HAL_IS_BIT_CLR(PWR->VOSR, PWR_VOSR_R1RDY | PWR_VOSR_BOOSTRDY))
    ;
  ...
  while ((FLASH->ACR & FLASH_ACR_LATENCY) != FLASH_ACR_LATENCY_2)
    ;
  MODIFY_REG(RCC->CFGR3, RCC_CFGR3_PPRE3, RCC_HCLK_DIV1 << RCC_CFGR3_PPRE3_Pos);
  MODIFY_REG(RCC->CFGR2, RCC_CFGR2_PPRE2, RCC_HCLK_DIV1 << RCC_CFGR2_PPRE2_Pos);
  MODIFY_REG(RCC->CFGR2, RCC_CFGR2_PPRE1, RCC_HCLK_DIV1 << RCC_CFGR2_PPRE1_Pos);
  MODIFY_REG(RCC->CFGR2, RCC_CFGR2_HPRE, RCC_SYSCLK_DIV1 << RCC_CFGR2_HPRE_Pos);

  // set MSIRC 0 (96 MHz) as MSIS source,
  MODIFY_REG(RCC->ICSCR1, RCC_ICSCR1_MSISDIV | RCC_MSI_RC1 | RCC_ICSCR1_MSIRGSEL,
             RCC_ICSCR1_MSISDIV_0 | RCC_MSI_RC0 | RCC_ICSCR1_MSIRGSEL);

  MODIFY_REG(RCC->CFGR1, RCC_CFGR1_SW, RCC_SYSCLKSOURCE_MSIS);
