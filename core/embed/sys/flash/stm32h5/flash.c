...
// STM32H5F5xx: 4 MB dual-bank flash, 8 KB sectors, 256 sectors per bank.
...
#define FLASH_STATUS_ALL_FLAGS                                            \
  (FLASH_SR_PGSERR | FLASH_SR_WRPERR | FLASH_SR_STRBERR | FLASH_SR_INCERR | \
   FLASH_SR_EOP)
...
secbool flash_unlock_write(void) {
  HAL_FLASH_Unlock();
  FLASH->NSCCR = FLASH_STATUS_ALL_FLAGS;  // clear all status flags
#if defined(__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U)
  FLASH->SECCCR = FLASH_STATUS_ALL_FLAGS;  // clear all status flags
#endif
  return sectrue;
}
...
secbool flash_sector_erase(uint16_t sector) {
  ...
  if (sector >= FLASH_BANK_SECTORS) {
    EraseInitStruct.Banks = FLASH_BANK_2;
    EraseInitStruct.Sector = sector - FLASH_BANK_SECTORS;
  }
