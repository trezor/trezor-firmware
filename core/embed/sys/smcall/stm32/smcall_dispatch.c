...
PIN_UI_WAIT_CALLBACK ns_storage_init_callback_t;

static ns_storage_init_callback_t storage_init_callback = NULL;

static secbool storage_init_callback_wrapper(
    uint32_t wait, uint32_t progress, enum storage_ui_message_t message) {
  if (storage_init_callback != NULL) {
    return storage_init_callback(wait, progress, message);
  } else {
    return secfalse;
  }
}
...
__attribute((no_stack_protector)) void smcall_handler(uint32_t *args,
  ...
  switch (smcall) {
    ...
    case SMCALL_STORAGE_INIT: {
      // !@# check args[0] if it is in a non-secure memory region
      storage_init_callback =
          (ns_storage_init_callback_t)cmse_nsfptr_create(args[0]);
