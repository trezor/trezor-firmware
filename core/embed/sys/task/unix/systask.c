...
static void systask_yield(void) {
  ...
  if (current_task->killed) {
    if (current_task->exit_handler != NULL) {
      current_task->exit_handler();
...
static void systask_kill(systask_t* task) {
  ...
  if (task == &scheduler->kernel_task) {
    ...
    if (task->exit_handler != NULL) {
      if (task == scheduler->active_task) {
        task->exit_handler();
      } else {
        // Switch to the task => the task will
        // invoke exit_handler since task->killed == true and return
