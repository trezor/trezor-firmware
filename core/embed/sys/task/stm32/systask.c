...
void systask_enable_tls(systask_t* task, mpu_area_t tls) {
  // !@# implement tls size check
  task->tls_addr = (void*)tls.start;
  task->tls_size = MIN(tls.size, sizeof(task->tls_copy));  // !@# refactor this
}
...
bool systask_push_call(systask_t* task, void* entrypoint, uint32_t arg1,
  ...
  // Registers r4-r11
  uint32_t regs[8] = {0};
  regs[9 - 4] = task->got_addr;  // r9 = GOT address
