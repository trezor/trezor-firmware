...
typedef struct {
  ...
  // (the kernel task is not included)
...
static inline void remove_poller(sysevent_dispatcher_t *dispatcher,
                                 size_t idx) {
  if (idx > 0) {
    if (idx < ARRAY_LENGTH(dispatcher->pollers) &&
        ARRAY_LENGTH(dispatcher->pollers) > 2) {
      for (size_t j = idx; j < dispatcher->pollers_count; j++) {
...
void sysevents_poll(const sysevents_t *awaited, sysevents_t *signalled,
  ...
  uint32_t prio = 0;  // == 0 for kernel task, highest priority
  ...
  if (active_task != kernel_task) {
    // Add the task to the end of the polling list.
    // Round-robin scheduling for unprivileged tasks is used.
    prio = dispatcher->pollers_count + 1;
    ++dispatcher->pollers_count;
  }
  ...
  for (;;) {
    ...
    for (size_t i = 0; i <= dispatcher->pollers_count; i++) {
      ...
      handles_to_read |= awaited->read_ready;
      handles_to_write |= awaited->write_ready;
    }
    ...
    for (size_t prio = 0; prio <= dispatcher->pollers_count; prio++) {
      ...
      sysevents_t *signalled = poller->signalled;
      bool ready = (signalled->read_ready | signalled->write_ready) != 0;
      if (ready || timed_out) {
        if (poller->task == kernel_task) {
          return;
        } else {
          remove_poller(dispatcher, prio);
          systask_yield_to(poller->task);
          break;
        }
      }
    }
...
// !@# consider calling it from kernel task only
// (from polling loop when task is killed????)
void sysevents_notify_task_killed(systask_t *task) {
  sysevent_dispatcher_t *dispatcher = &g_sysevent_dispatcher;
  // Remove task from poller list
  // (kernel task is not included)
  for (size_t i = 1; i <= dispatcher->pollers_count; i++) {
