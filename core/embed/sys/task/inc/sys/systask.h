...
typedef struct {
  ...
  /** Exit handler called just before the task is terminated to perform any
   *  cleanup operations */
  systask_exit_handler_t exit_handler;
#endif
...
#ifdef TREZOR_EMULATOR
/**
 * @brief Sets the exit handler called just before the task is terminated
 *
 * If the task is NULL, the handler is set for the currently active task.
 *
 * @param task Pointer to the task, or NULL for the currently active task.
 * @param handler Exit handler to set.
 */
