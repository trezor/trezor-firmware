...
// SRAM regions must be in order of ascending start address and must not
// overlap. On the STM32H5F5xx, SRAM1..SRAM5 are contiguous and already in
// ascending order (SRAM4 precedes SRAM5, unlike some STM32U5 parts). There is
// no SRAM6.
...
// Note: the STM32H5F5xx has no GFXMMU, so tz_set_gfxmmu_unpriv (present on the
// STM32U5A9/U5G9) is intentionally not defined here.
