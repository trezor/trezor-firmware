...
static bool _usb_write(pb_ostream_t *stream, const pb_byte_t *buf, size_t count)
{
    ...
    while (written < count) {
        ...
        if (state->packet_pos + remaining <= USB_PACKET_SIZE) {
            ...
            memcpy(
                state->buf + state->packet_pos, buf + written, USB_PACKET_SIZE - state->packet_pos);
            written += USB_PACKET_SIZE - state->packet_pos;
            // send packet
            int r = usb_webusb_write_blocking(
                state->iface_num, state->buf, USB_PACKET_SIZE, USB_TIMEOUT);
...
static secbool _send_msg(
    uint8_t iface_num, uint16_t msg_id, const pb_msgdesc_t *fields, const void *msg)
{
    // determine message size by serializing it into a dummy stream
    pb_ostream_t sizestream = {
        .callback = NULL, .state = NULL, .max_size = SIZE_MAX, .bytes_written = 0, .errmsg = NULL};
...
#define MSG_SEND_ASSIGN_BYTES(FIELD, VALUE, LEN)                                     \
    {                                                                                \
        ...
        memcpy(msg_send.FIELD.bytes, VALUE, MIN(LEN, sizeof(msg_send.FIELD.bytes))); \
    ...
    for (int retry = 0;; retry++) {
        ...
        if (r != USB_PACKET_SIZE) {  // reading failed
            if (r == 0 && retry < 10) {
                ...
                // error
                error_shutdown_ex(
                    "USB ERROR", "Error reading from USB. Try different USB cable.", NULL);
            }
        }
        return;  // success
    }
...
static secbool _recv_msg(
    uint8_t iface_num, uint32_t msg_size, uint8_t *buf, const pb_msgdesc_t *fields, void *msg)
{
    usb_read_state state = {
        .iface_num = iface_num, .packet_index = 0, .packet_pos = MSG_HEADER1_LEN, .buf = buf};

    pb_istream_t stream = {
        .callback = &_usb_read, .state = &state, .bytes_left = msg_size, .errmsg = NULL};
...
static void send_msg_features(
    uint8_t iface_num, const vendor_header *const vhdr, const image_header *const hdr)
{
...
#if USE_OPTIGA
    MSG_SEND_ASSIGN_VALUE(bootloader_locked, (secret_bootloader_locked() == sectrue));
#endif
    MSG_SEND(Features);
}
...
void process_msg_FirmwareErase(uint8_t iface_num, uint32_t msg_size, uint8_t *buf)
{
    ...
    if ((firmware_remaining > 0) && ((firmware_remaining % sizeof(uint32_t)) == 0) &&
        (firmware_remaining <= FIRMWARE_IMAGE_MAXSIZE)) {
        // request new firmware
        chunk_requested = (firmware_remaining > IMAGE_INIT_CHUNK_SIZE) ? IMAGE_INIT_CHUNK_SIZE
                                                                       : firmware_remaining;
...
static void detect_installation(
    const vendor_header *current_vhdr, const image_header *current_hdr,
    const vendor_header *const new_vhdr, const image_header *const new_hdr, secbool *is_new,
    secbool *keep_seed, secbool *is_newvendor, secbool *is_upgrade)
{
    *is_new = secfalse;
    *keep_seed = secfalse;
...
int process_msg_FirmwareUpload(uint8_t iface_num, uint32_t msg_size, uint8_t *buf)
{
    ...
    if (firmware_block == 0) {
        if (headers_offset == 0) {
            ...
            const image_header *received_hdr = read_image_header(
                CHUNK_BUFFER_PTR + vhdr.hdrlen, FIRMWARE_IMAGE_MAGIC, FIRMWARE_IMAGE_MAXSIZE);

            if (received_hdr != (const image_header *)(CHUNK_BUFFER_PTR + vhdr.hdrlen)) {
                MSG_SEND_INIT(Failure);
                MSG_SEND_ASSIGN_VALUE(code, FailureType_Failure_ProcessError);
            ...
            if (sectrue !=
                check_image_header_sig(received_hdr, vhdr.vsig_m, vhdr.vsig_n, vhdr.vpub)) {
                MSG_SEND_INIT(Failure);
                MSG_SEND_ASSIGN_VALUE(code, FailureType_Failure_ProcessError);
            ...
            if (sectrue != read_vendor_header((const uint8_t *)FIRMWARE_START, &current_vhdr)) {
                is_new = sectrue;
            }
            ...
            if (is_new == secfalse) {
                ...
                if (current_hdr != (const image_header *)(FIRMWARE_START + current_vhdr.hdrlen)) {
                    is_new = sectrue;
                }
            }
            ...
            if (is_new == secfalse) {
                detect_installation(
                    &current_vhdr, current_hdr, &vhdr, &hdr, &is_new, &should_keep_seed,
                    &is_newvendor, &is_upgrade);
            }
            ...
            if (bootargs_get_command() == BOOT_COMMAND_INSTALL_UPGRADE) {
                ...
                IMAGE_HASH_INIT(&ctx);
                IMAGE_HASH_UPDATE(&ctx, CHUNK_BUFFER_PTR, vhdr.hdrlen + received_hdr->hdrlen);
            ...
            if (sectrue == is_new || sectrue == is_ilu) {
                ...
                response = ui_screen_install_confirm(
                    &vhdr, &hdr, should_keep_seed, is_newvendor, version_cmp);
            }
...
#endif
                ensure(flash_area_erase_bulk(STORAGE_AREAS, STORAGE_AREAS_COUNT, NULL), NULL);
            }
            ...
            uint32_t chunk_limit =
                (firmware_remaining > IMAGE_CHUNK_SIZE) ? IMAGE_CHUNK_SIZE : firmware_remaining;
    ...
    if (flash_area_get_address(&FIRMWARE_AREA, firmware_block * IMAGE_CHUNK_SIZE, 0) == NULL) {
        MSG_SEND_INIT(Failure);
        MSG_SEND_ASSIGN_VALUE(code, FailureType_Failure_ProcessError);
    ...
    if (sectrue != check_single_hash(
                       hdr.hashes + firmware_block * 32, CHUNK_BUFFER_PTR + headers_offset,
    ...
    while (bytes_remaining > 0) {
        ...
        if (write_offset >= erase_offset) {
            ...
            ensure(flash_area_erase_partial(&FIRMWARE_AREA, erase_offset, &bytes_erased), NULL);
        ...
        ensure(flash_area_write_data(&FIRMWARE_AREA, write_offset, src, bytes_to_write), NULL);
