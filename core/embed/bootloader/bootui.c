...
static void format_ver(const char *format, uint32_t version, char *buffer, size_t buffer_len)
{
    mini_snprintf(
        buffer, buffer_len, format, (int)(version & 0xFF), (int)((version >> 8) & 0xFF),
        (int)((version >> 16) & 0xFF)
...
#ifndef NEW_RENDERING
static void ui_screen_boot_old(const vendor_header *const vhdr, const image_header *const hdr)
{
    ...
    if (show_string) {
        char ver_str[64];
        display_text_center(
            DISPLAY_RESX / 2, DISPLAY_RESY - 5 - 50, vhdr->vstr, vhdr->vstr_len, FONT_NORMAL,
            COLOR_BL_BG, boot_background);
        format_ver("%d.%d.%d", fw_version, ver_str, sizeof(ver_str));
        display_text_center(
            DISPLAY_RESX / 2, DISPLAY_RESY - 5 - 25, ver_str, -1, FONT_NORMAL, COLOR_BL_BG,
            boot_background);
    }
#else
    ...
    if (memcmp(vimg, "TOIG\x18\x00\x18\x00", 8) == 0) {
        uint32_t datalen = *(uint32_t *)(vimg + 8);
        display_icon((DISPLAY_RESX - 22) / 2, 0, vimg, datalen + 12, COLOR_BL_BG, boot_background);
    }

    if (show_string) {
        char ver_str[64];
        display_text_center(
            DISPLAY_RESX / 2, 36, vhdr->vstr, vhdr->vstr_len, FONT_NORMAL, COLOR_BL_BG,
            boot_background);
        format_ver("%d.%d.%d", fw_version, ver_str, sizeof(ver_str));
        display_text_center(
            DISPLAY_RESX / 2, 46, ver_str, -1, FONT_NORMAL, COLOR_BL_BG, boot_background);
    }
...
static void ui_screen_boot_wait(int wait_seconds)
{
    ...
    display_bar(0, BOOT_WAIT_Y_TOP, DISPLAY_RESX, BOOT_WAIT_HEIGHT, boot_background);
    display_text_center(
        DISPLAY_RESX / 2, DISPLAY_RESY - 5, wait_str, -1, FONT_NORMAL, COLOR_BL_BG,
        boot_background);
    display_pixeldata_dirty();
    display_refresh();
}
...
void ui_screen_boot(const vendor_header *const vhdr, const image_header *const hdr, int wait)
{
    ...
    screen_boot(red_screen, vendor_str, vendor_str_len, hdr->version, vhdr->vimg, vimg_len, wait);
}
#else  // NEW_RENDERING
...
uint32_t ui_screen_intro(const vendor_header *const vhdr, const image_header *const hdr, bool fw_ok)
{
    char bld_ver[32];
    char ver_str[64];
