...
static usb_result_t bootloader_usb_loop(
...
static void check_bootloader_version(void)
{
    ...
    for (int i = 0; i < FLASH_OTP_BLOCK_SIZE * 8; i++) {
        ...
        flash_otp_write(FLASH_OTP_BLOCK_BOOTLOADER_VERSION, 0, bits, FLASH_OTP_BLOCK_SIZE), NULL);

    uint8_t bits2[FLASH_OTP_BLOCK_SIZE];
    ensure(
        flash_otp_read(FLASH_OTP_BLOCK_BOOTLOADER_VERSION, 0, bits2, FLASH_OTP_BLOCK_SIZE), NULL);

    ensure(
        sectrue * (0 == memcmp(bits, bits2, FLASH_OTP_BLOCK_SIZE)),
...
void real_jump_to_firmware(void)
{
    ...
    ensure(read_vendor_header((const uint8_t *)FIRMWARE_START, &vhdr), "Firmware is corrupted");
    ...
    hdr = read_image_header(
        (const uint8_t *)(size_t)(FIRMWARE_START + vhdr.hdrlen), FIRMWARE_IMAGE_MAGIC,
        FIRMWARE_IMAGE_MAXSIZE);

    ensure(
        hdr == (const image_header *)(size_t)(FIRMWARE_START + vhdr.hdrlen) ? sectrue : secfalse,
        "Firmware is corrupted");

    ensure(check_image_model(hdr), "Wrong firmware model");

    ensure(
        check_image_header_sig(hdr, vhdr.vsig_m, vhdr.vsig_n, vhdr.vpub), "Firmware is corrupted");

    ensure(
        check_image_contents(hdr, IMAGE_HEADER_SIZE + vhdr.hdrlen, &FIRMWARE_AREA),
        "Firmware is corrupted");
...
int bootloader_main(void)
{
    ...
    if (sectrue == model_ok) {
        header_present = check_image_header_sig(hdr, vhdr.vsig_m, vhdr.vsig_n, vhdr.vpub);
    }
    ...
    ensure(dont_optimize_out_true * (firmware_present == firmware_present_backup), NULL);
    ...
    if (touched || stay_in_bootloader == sectrue || firmware_present != sectrue ||
        ...
        while (true) {
            ...
            if (continue_to_firmware != continue_to_firmware_backup) {
                ...
                dont_optimize_out_true * (continue_to_firmware == continue_to_firmware_backup),
                NULL);
...
#ifdef STM32U5
    if (sectrue == firmware_present && firmware_jump_fn != jump_to_fw_through_reset) {
        firmware_jump_fn = real_jump_to_firmware;
    }
#else
