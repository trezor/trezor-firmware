...
void usage(void)
{
    ...
    printf("  -f FIRMWARE  run interaction-less update for the specified image\n");
#ifdef USE_OPTIGA
...
bool load_firmware(const char *filename, uint8_t *hash)
{
    ...
    const image_header *hdr =
        read_image_header(buffer + vhdr.hdrlen, FIRMWARE_IMAGE_MAGIC, FIRMWARE_IMAGE_MAXSIZE);
...
__attribute__((noreturn)) int main(int argc, char **argv)
{
    ...
    (void)!flash_otp_write(FLASH_OTP_BLOCK_DEVICE_VARIANT, 0, otp_data, sizeof(otp_data));
...
{
    bool storage_is_erased = storage_empty(&STORAGE_AREAS[0]) && storage_empty(&STORAGE_AREAS[1]);

    if (storage_is_erased) {
        printf("STORAGE WAS ERASED\n");
        error_shutdown_ex("BOOTLOADER EXIT", "Jumped to firmware", "STORAGE WAS ERASED");
    } else {
        printf("storage was retained\n");
        error_shutdown_ex("BOOTLOADER EXIT", "Jumped to firmware", "STORAGE WAS RETAINED");
    }
}
