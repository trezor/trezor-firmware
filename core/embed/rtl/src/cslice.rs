...
impl<T> CSlicePtr<T> {
    ...
    const fn from_ptr_and_len(ptr: *const T, len: usize) -> Self {
        // sanity check: if ptr is null, len must be 0.
        // only a debug_assert because we don't care in practice, either option
        // will just coerce to null.
...
mod tests {
    ...
    fn test_from_ptr_and_len_coercion() {
        ...
        // null + zero length → null
        let fp = unsafe { CSlice::<u32>::from_ptr_and_len(core::ptr::null(), 0) };
        assert!(fp.is_empty());
        assert_eq!(fp.as_slice(), &[]);
