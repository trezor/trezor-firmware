...
#[cfg(feature = "app_loading")]
use trezor_app_sdk::structs::{ArchivedSlice, ArchivedTrezorCryptoEnum, TrezorCryptoResult_};
...
extern "C" fn new_deserialize_crypto_message(
    ...
    let block = |_args: &[Obj], kwargs: &Map| {
        ...
        let result: Obj = match archived {
            ArchivedTrezorCryptoEnum::GetXpub {
                address_n,
                xpub_magic,
            } => {
            ...
            ArchivedTrezorCryptoEnum::GetPublicKey {
                address_n,
                compressed,
            } => {
                let dp_obj = obj_from_dp_slice(address_n);
            ...
            ArchivedTrezorCryptoEnum::SignDigest {
                address_n,
                digest,
                compressed,
            } => {
            ...
            ArchivedTrezorCryptoEnum::GetAddressMac { address_n, address } => {
                (obj_from_dp_slice(address_n), address.as_str().try_into()?).try_into()?
            }
...
extern "C" fn new_send_crypto_result(n_args: usize, args: *const Obj, kwargs: *mut Map) -> Obj {
    let block = |_args: &[Obj], kwargs: &Map| {
        ...
        let msg = if obj.is_str() {
            let data = unwrap!(unsafe { crate::micropython::buffer::get_buffer(obj) });
            match data.len() {
                111 => TrezorCryptoResult_::Xpub(unwrap!(data.try_into())),
                _ => {
            ...
            let data = unwrap!(unsafe { crate::micropython::buffer::get_buffer(obj) });
            match data.len() {
                32 => TrezorCryptoResult_::AddressMac(unwrap!(data.try_into())),
                65 => TrezorCryptoResult_::Signature(unwrap!(data.try_into())),
                _ => {
                    dbg_println!("Unexpected data length for crypto result: {}", data.len());
                    return Err(Error::TypeError);
                }
            ...
            match tag {
                0 => {
                    ...
                    TrezorCryptoResult_::PublicKey(unwrap!(data.try_into()))
                }
                _ => {
                    dbg_println!("Unexpected type tag for crypto result: {}", tag);
                    return Err(Error::TypeError);
                }
            }
        };
