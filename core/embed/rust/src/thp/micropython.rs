...
extern "C" fn thp_packet_in_channel(n_args: usize, args: *const Obj) -> Obj {
    let block = |args: &[Obj], _kwargs: &Map| {
        ...
        let mut thp = THP_CONTEXT.try_lock().ok_or(CANNOT_UNLOCK)?;
        let res = thp.packet_in_channel(iface_num, channel_id, packet_buffer, receive_buffer)?;
        res.try_into()
    };
...
extern "C" fn thp_packet_out(n_args: usize, args: *const Obj) -> Obj {
    let block = |args: &[Obj], _kwargs: &Map| {
        ...
        let written = match channel_id {
            Some(cid) => thp.packet_out_channel(iface_num, cid, send_buffer, packet_buffer)?,
            None => thp.packet_out(iface_num, packet_buffer)?,
        };
        Ok(written.into())
    };
...
extern "C" fn thp_message_out(iface_num: Obj, channel_id: Obj, receive_buffer_obj: Obj) -> Obj {
    let block = || {
        let iface_num: u8 = iface_num.try_into()?;
        ...
        let (sid, message_type, message_len) = {
            ...
            let mut thp = THP_CONTEXT.try_lock().ok_or(CANNOT_UNLOCK)?;
            thp.message_out(iface_num, channel_id, receive_buffer)?
        };
...
extern "C" fn thp_message_in(n_args: usize, args: *const Obj) -> Obj {
    let block = |args: &[Obj], _kwargs: &Map| {
        ...
        let mut thp = THP_CONTEXT.try_lock().ok_or(CANNOT_UNLOCK)?;
        thp.message_in(iface_num, channel_id, plaintext_len, send_buffer)?;
        Ok(Obj::const_none())
    };
...
extern "C" fn thp_message_retransmit(iface_num: Obj, channel_id: Obj) -> Obj {
    let block = || {
        ...
        let mut thp = THP_CONTEXT.try_lock().ok_or(CANNOT_UNLOCK)?;
        let channel_ok = thp.message_retransmit(iface_num, channel_id)?;
        Ok(channel_ok.into())
    };
...
extern "C" fn thp_send_transport_busy(iface_num: Obj, channel_id: Obj) -> Obj {
    let block = || {
        ...
        let mut thp = THP_CONTEXT.try_lock().ok_or(CANNOT_UNLOCK)?;
        thp.send_transport_busy(iface_num, channel_id)?;
        Ok(Obj::const_none())
    };
...
extern "C" fn thp_channel_info(iface_num: Obj, channel_id: Obj) -> Obj {
    let block = || {
        ...
        let credential = {
            ...
            Obj::from_option(aux.get_credential(iface_num, channel_id))?
        };
...
extern "C" fn thp_channel_paired(iface_num: Obj, channel_id: Obj) -> Obj {
    let block = || {
        ...
        let mut thp = THP_CONTEXT.try_lock().ok_or(CANNOT_UNLOCK)?;
        let replaced_channel_id = thp.channel_paired(iface_num, channel_id)?;
...
extern "C" fn thp_channel_close(iface_num: Obj, channel_id: Obj) -> Obj {
    let block = || {
        ...
        let mut thp = THP_CONTEXT.try_lock().ok_or(CANNOT_UNLOCK)?;
        thp.channel_close(iface_num, channel_id);
        Ok(Obj::const_none())
    };
...
extern "C" fn thp_channel_get_closed(iface_num: Obj) -> Obj {
    let block = || {
        ...
        let mut thp = THP_CONTEXT.try_lock().ok_or(CANNOT_UNLOCK)?;
        let closed = thp.channel_get_closed(iface_num);
        if closed.is_empty() {
...
pub static mp_module_trezorthp: Module = obj_module! {
    ...
    /// def message_out(iface_num: int, channel_id: int, receive_buffer: memoryview) -> tuple[int, int, memoryview]:
    ///     """
