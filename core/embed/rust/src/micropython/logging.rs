...
fn _log(level: &str, args: &[Obj]) -> Result<Obj, Error> {
    if args.len() < 2 {
...
extern "C" fn py_debug(n_args: usize, args: *const Obj) -> Obj {
    let block = |args: &[Obj], _kwargs: &Map| _log("32mDEBUG", args);
    unsafe { util::try_with_args_and_kwargs(n_args, args, &Map::EMPTY, block) }
}

extern "C" fn py_info(n_args: usize, args: *const Obj) -> Obj {
    let block = |args: &[Obj], _kwargs: &Map| _log("36mINFO", args);
    unsafe { util::try_with_args_and_kwargs(n_args, args, &Map::EMPTY, block) }
}

extern "C" fn py_warning(n_args: usize, args: *const Obj) -> Obj {
    let block = |args: &[Obj], _kwargs: &Map| _log("33mWARNING", args);
    unsafe { util::try_with_args_and_kwargs(n_args, args, &Map::EMPTY, block) }
}

extern "C" fn py_error(n_args: usize, args: *const Obj) -> Obj {
    let block = |args: &[Obj], _kwargs: &Map| _log("31mERROR", args);
    unsafe { util::try_with_args_and_kwargs(n_args, args, &Map::EMPTY, block) }
}
...
pub static mp_module_trezorlog: Module = obj_module! {
    ...
    ///     ...
    Qstr::MP_QSTR_debug => obj_fn_var_min!(2, py_debug).as_obj(),

    /// def info(name: str, msg: str, *args: Any) -> None:
    ///     ...
    Qstr::MP_QSTR_info => obj_fn_var_min!(2, py_info).as_obj(),

    /// def warning(name: str, msg: str, *args: Any) -> None:
    ///     ...
    Qstr::MP_QSTR_warning => obj_fn_var_min!(2, py_warning).as_obj(),

    /// def error(name: str, msg: str, *args: Any) -> None:
    ///     ...
    Qstr::MP_QSTR_error => obj_fn_var_min!(2, py_error).as_obj(),
};
