...
impl<T: ?Sized> Gc<T> {
    ...
    /// is pointing to a memory previously allocated on the GC heap, i.e.
    /// through `Gc::new()`, or through the MicroPython interpreter.
    pub unsafe fn from_raw(ptr: *mut T) -> Self {
        // SAFETY: The caller must guarantee that `ptr` points to a previously
        // GC-allocated memory.
