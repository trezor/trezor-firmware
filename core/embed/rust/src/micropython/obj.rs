...
impl TryFrom<(Obj, Obj, Obj, Obj, Obj)> for Obj {
    type Error = Error;

    fn try_from(val: (Obj, Obj, Obj, Obj, Obj)) -> Result<Self, Self::Error> {
        // SAFETY:
        //  - Should work with any micropython objects.
        // EXCEPTION: Will raise if allocation fails.
        let values = [val.0, val.1, val.2, val.3, val.4];
        let obj = catch_exception(|| unsafe { ffi::mp_obj_new_tuple(5, values.as_ptr()) })?;
        if obj.is_null() {
            Err(Error::AllocationFailed)
        } else {
            Ok(obj)
...
impl TryFrom<(Obj, Obj, Obj, Obj, Obj, Obj)> for Obj {
    type Error = Error;

    fn try_from(val: (Obj, Obj, Obj, Obj, Obj, Obj)) -> Result<Self, Self::Error> {
        // SAFETY:
        //  - Should work with any micropython objects.
        // EXCEPTION: Will raise if allocation fails.
        let values = [val.0, val.1, val.2, val.3, val.4, val.5];
        let obj = catch_exception(|| unsafe { ffi::mp_obj_new_tuple(6, values.as_ptr()) })?;
        if obj.is_null() {
            Err(Error::AllocationFailed)
        } else {
            Ok(obj)
