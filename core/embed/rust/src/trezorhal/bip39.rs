...
extern "C" {
    ...
    // pub static wordlist: [*const cty::c_char; 2049usize];
}
...
// TODO: how to import it from C code?
// trezor-crypto/bip39_english.h
// ... undefined reference to `wordlist'
// pub fn get_all_words() -> Vec<String<20>, 2048> {
//     let mut words: Vec<String<20>, 2048> = Vec::new();
//     unsafe {
//         for word in wordlist.iter() {
//             let word_slice =
// CStr::from_ptr(*word).to_str().unwrap_unchecked();             let word_str:
// String<20> = word_slice.into();             if words.capacity() ==
// words.len() {                 break;
//             }
//             words.push(word_str);
//         }
...
pub fn get_all_words() -> Vec<String<20>, 2048> {
    ...
    for word in wordlist {
        words.push(String::from(word)).unwrap();
    }
    words
}

/// Finds all valid letters that can be used to form a word.
pub fn get_all_possible_letters(
    prefix: String<20>,
    all_words: Vec<String<20>, 2048>,
) -> Vec<String<1>, 50> {
    let mut all_letters: Vec<String<1>, 50> = Vec::new();
    for word in all_words {
        if word.starts_with(prefix.as_str()) {
            // The word can be already complete, so there is no char after it
            if word == prefix {
                continue;
            }
            ...
            let mut b = [0; 2];
            let char_slice = char.encode_utf8(&mut b);

            let mut char_str: String<1> = String::new();
            char_str.push_str(char_slice).unwrap();

            if all_letters.contains(&char_str) {
                continue;
            }
            all_letters.push(char_str).unwrap();
        }
    }
    // It does not need to be sorted, as all_words already is alphabetical
    all_letters
}

/// Finds all valid words suitable with some prefix
pub fn get_all_possible_words(
    prefix: String<20>,
    all_words: Vec<String<20>, 2048>,
) -> Vec<String<20>, 2048> {
    if prefix.is_empty() {
    ...
    let mut all_possible_words: Vec<String<20>, 2048> = Vec::new();
    for word in all_words {
        if word.starts_with(prefix.as_str()) {
            all_possible_words.push(word).unwrap();
        }
    }
    all_possible_words
}
...
mod tests {
    ...
    /// Helper abstracting all the String creations and assertions
    fn _test_possible_letters<const M: usize, const N: usize>(
        prefix_slice: &str,
        all_words_slices: [&str; M],
        expected_letters_slices: [&str; N],
    ) {
        let prefix = String::from(prefix_slice);

        let all_words: Vec<String<20>, 2048> =
            all_words_slices.iter().map(|s| String::from(*s)).collect();

        let all_letters = get_all_possible_letters(prefix, all_words);

        let expected_letters: Vec<String<1>, 50> = expected_letters_slices
            .iter()
            .map(|s| String::from(*s))
            .collect();

        assert_eq!(all_letters, expected_letters);
    }

    /// Helper abstracting all the String creations and assertions
    fn _test_possible_words<const M: usize, const N: usize>(
        prefix_slice: &str,
        all_words_slices: [&str; M],
        expected_words_slices: [&str; N],
    ) {
        let prefix = String::from(prefix_slice);
        ...
        let all_possible_words = get_all_possible_words(prefix, all_words);

        let expected_words: Vec<String<20>, 50> = expected_words_slices
            .iter()
            .map(|s| String::from(*s))
            .collect();

        assert_eq!(all_possible_words, expected_words);
    }

    #[test]
    fn test_get_all_possible_letters_empty() {
        _test_possible_letters(
            "",
            ["bat", "cat", "cry", "dog", "man", "mat", "rat"],
            ["b", "c", "d", "m", "r"],
        );
    }

    #[test]
    fn test_get_all_possible_letters() {
        _test_possible_letters(
            "cr",
            [
                "bat", "cat", "crawl", "crowd", "cry", "crystal", "man", "mat", "rat",
            ],
            ["a", "o", "y"],
        );
    }
    ...
    #[test]
    fn test_get_all_possible_letters_nothing_there() {
        _test_possible_letters(
            "day",
            [
                "bat", "cat", "day", "crawl", "crowd", "cry", "crystal", "man", "mat", "rat",
            ],
            [],
        );
    }
    ...
    #[test]
    fn test_get_all_possible_words_empty() {
        _test_possible_words(
            "",
            ["bat", "cat", "cry", "dog", "man", "mat", "rat"],
            ["bat", "cat", "cry", "dog", "man", "mat", "rat"],
        );
    ...
    fn test_get_all_possible_words() {
        _test_possible_words(
            ...
            ["crawl", "crowd", "cry", "crystal"],
        );
    }

    #[test]
    fn test_get_all_possible_words_just_one() {
        _test_possible_words(
            "day",
            [
                "bat", "cat", "day", "crawl", "crowd", "cry", "crystal", "man", "mat", "rat",
