...
extern "C" fn py_connection_strflags() -> Obj {
    let block = || {
        let s = state();
        if !s.state_known {
            return "unknown".try_into();
        }
        let mut buf: ShortString = ShortString::new();
        if s.connectable {
            let _ = buf.push_str("connectable");
            let _ = buf.push(',');
        }
        if s.connected {
            let _ = buf.push_str("connected");
            let _ = buf.push(',');
        }
        if s.pairing {
            let _ = buf.push_str("pairing");
            let _ = buf.push(',');
        }
        if s.pairing_requested {
            let _ = buf.push_str("pairing_requested");
            let _ = buf.push(',');
        }
        buf.pop();
        buf.as_str().try_into()
...
pub static mp_module_trezorble: Module = obj_module! {
    ...
    /// def connection_strflags() -> int:
    ///     """
    ///     Returns current connection state as a string.
    ///     """
    Qstr::MP_QSTR_connection_strflags => obj_fn_0!(py_connection_strflags).as_obj(),
