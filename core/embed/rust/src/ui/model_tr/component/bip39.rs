...
pub struct Bip39Page<T> {
    prompt: T,
    letter_choices: Vec<String<1>, 50>, // TODO: could use char
    word_choices: Vec<String<20>, 2048>,
    ...
    T: Deref<Target = str>,
{
    pub fn new(prompt: T) -> Self {
        ...
        let all_words = bip39::get_all_words();
        let letter_choices =
            bip39::get_all_possible_letters(word_buffer.clone(), all_words.clone());
        let word_choices = bip39::get_all_possible_words(word_buffer.clone(), all_words.clone());
    ...
    fn reflect_letter_change(&mut self) {
        self.word_choices =
            bip39::get_all_possible_words(self.word_buffer.clone(), self.all_words.clone());

        if self.word_choices.len() < WORD_THRESHOLD {
            self.offer_words = true;
        } else {
            self.offer_words = false;
            self.letter_choices =
                bip39::get_all_possible_letters(self.word_buffer.clone(), self.all_words.clone());
        }
    ...
    fn append_current_letter(&mut self) {
        ...
        self.word_buffer.push_str(new_letter.as_str()).unwrap();
    }
    ...
    fn get_previous_choice(&self) -> &str {
        self.get_choice(self.page_counter - 1)
    }

    fn get_next_choice(&self) -> &str {
        self.get_choice(self.page_counter + 1)
    }

    fn get_choice(&self, index: u8) -> &str {
        if self.offer_words {
            &self.word_choices[index as usize]
        } else {
            &self.letter_choices[index as usize]
        }
    }
