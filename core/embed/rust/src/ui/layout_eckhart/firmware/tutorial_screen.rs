...
impl Component for TutorialScreen {
    ...
    fn event(&mut self, ctx: &mut EventCtx, event: Event) -> Option<Self::Msg> {
        if let Some(ActionBarMsg::Confirmed) = self.action_bar.event(ctx, event) {
            return Some(TutorialScreenMsg::Confirmed);
        }

        // TutorialScreen reacts to ANIM_FRAME_TIMER
        match event {
...
impl Swipable for TutorialScreen {
...
impl crate::trace::Trace for TutorialScreen {
    fn trace(&self, t: &mut dyn crate::trace::Tracer) {
        t.component("TutorialScreen");
    }
}
