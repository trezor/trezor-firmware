...
use super::{
    super::component::{Button, ButtonContent, ButtonStyleSheet, ConnectionIndicator, FuelGauge},
    constant::{HEIGHT, SCREEN, WIDTH},
    theme::{self, firmware::button_homebar_style, Gradient, ScreenBackground},
...
impl Homescreen {
    pub fn new(
        ...
        // Homebar
        let (style_sheet, gradient) = notification_center.homebar_style();
        let btn = Button::new(notification_center.homebar_content(bootscreen, locked))
            .styled(style_sheet)
            .with_gradient(gradient);

        let is_alert = notification_center
            .notification_level()
            .map(|level| matches!(level, NotificationLevel::Alert))
            .unwrap_or(false);

        Ok(Self {
            header: HomescreenHeader::new(label.clone(), image_used, !is_alert),
            notification_center,
...
impl HomescreenNotificationCenter {
    pub fn new(
        ...
        let actionable_notification = notification.as_ref().is_some_and(|n| {
            // TODO: consider passing the "actionable" flag from the application layer
            matches!(
                n.text,
                text if text == TR::homescreen__title_pin_not_set.into()
                    || text == TR::homescreen__title_backup_needed.into()
                    || text == TR::homescreen__title_backup_failed.into()
            )
        });
        ...
        let hint = if actionable_notification {
            None
        } else {
            match notification {
                Some(ref notification) => Some(Self::get_notification_hint(notification)),
    ...
    pub fn homebar_content(&self, bootscreen: bool, locked: bool) -> ButtonContent {
    ...
    pub fn homebar_style(&self) -> (ButtonStyleSheet, Gradient) {
        ...
        if self.led_color.is_some() {
            match event {
                ...
                Event::Timer(_) if self.led_timer.expire(event) => {
                    self.led_active = !self.led_active;
                    self.led_timer.start(ctx, LED_TOGGLE_DURATION);
                    ctx.request_paint();
                }
                _ => {}
