...
impl HoldToConfirmAnim {
    ...
    pub fn start(&mut self) {
        self.state = AnimState::Growing {
            timer: Stopwatch::new_started(),
        };
    }

    pub fn stop(&mut self) {
        if let AnimState::Growing { timer } = &self.state {
            let started_at = timer.elapsed();
    ...
    fn is_animating(&self) -> bool {
        match &self.state {
            ...
            AnimState::Growing { timer } => timer.is_running_within(self.total_duration),
            AnimState::RollingBack { timer, .. } => {
                timer.is_running_within(self.rollback_duration())
            }
            AnimState::Finalizing { timer } => timer.is_running_within(Self::FINALIZING_DURATION),
        }
    }
...
impl Component for HoldToConfirmAnim {
    ...
    fn event(&mut self, ctx: &mut EventCtx, event: Event) -> Option<Self::Msg> {
        if let Event::Timer(EventCtx::ANIM_FRAME_TIMER) = event {
            ...
            if let AnimState::Finalizing { timer } = &self.state {
                if timer.is_running() && !timer.is_running_within(Self::FINALIZING_DURATION) {
                    #[cfg(feature = "rgb_led")]
                    rgb_led::set_color(0);
    ...
    fn render<'s>(&'s self, target: &mut impl Renderer<'s>) {
        match &self.state {
            AnimState::Idle => {}
            AnimState::RollingBack { timer, started_at } => {
                let rollback_elapsed = timer.elapsed();
