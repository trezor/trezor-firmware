    ...
    component::{
        ...
        keyboard::{
            common::{
                ...
                KEYBOARD_INPUT_INSETS, KEYBOARD_INPUT_RADIUS,
...
impl<'a> PinKeyboard<'a> {
    ...
    fn update_keypad(&mut self, ctx: &mut EventCtx) {
        let (keys_state, erase_state, confirm_state, cancel_state) = match self.input.display_style
        {
            DisplayStyle::Shown => {
                // Disable the entire active keypad
                (
                    ButtonState::Disabled, // Disable all key buttons
                    ButtonState::Disabled, // Disable erase button
                ...
                if self.input.is_full() {
                    ...
                    (
                        ...
                        if self.allow_cancel {
                            ButtonState::Enabled // Enable cancel if
                                                 // possible
                        } else {
                            ButtonState::Hidden
                        },
