use crate::{
    ...
    ui::{
        component::{
            ...
            Component, Event, EventCtx, Label, Never, SwipeDetect,
        },
        event::SwipeEvent,
        flow::Swipable,
...
impl Component for NumberInputScreen {
    ...
    fn event(&mut self, ctx: &mut EventCtx, event: Event) -> Option<Self::Msg> {
        ...
        match self.swipe.event(ctx, event, self.swipe_config) {
            Some(SwipeEvent::End(Direction::Up)) => {
                self.number_input.increase(ctx);
                return None;
            }
            Some(SwipeEvent::End(Direction::Down)) => {
                self.number_input.decrease(ctx);
                return None;
            }
            _ => {}
        }
...
impl NumberInput {
    const SIDE_PADDING: i16 = 24;
    pub fn new(min: u32, max: u32, value: u32) -> Self {
        let dec = Button::with_icon(theme::ICON_MINUS).styled(theme::button_default());
        let inc = Button::with_icon(theme::ICON_PLUS).styled(theme::button_default());
        let confirm = Button::empty().styled(theme::button_keyboard());
        let value = value.clamp(min, max);
        Self {
            total_area: Rect::zero(),
            number_area: Rect::zero(),
            dec,
            inc,
            min,
            max,
    ...
    fn render_borders<'s>(&'s self, target: &mut impl Renderer<'s>) {
        ...
        let top_line = Rect::from_bottom_right_and_size(
            self.total_area.top_right(),
            Offset::new(self.total_area.width(), 1),
        );
        let bottom_line = Rect::from_top_right_and_size(
            self.total_area.bottom_right(),
            Offset::new(self.total_area.width(), 1),
        );
    ...
    fn render_number<'s>(&'s self, target: &mut impl Renderer<'s>) {
        ...
        if let Some(text) = strutil::format_i64(self.value as i64, &mut buf) {
            let font = fonts::FONT_SATOSHI_EXTRALIGHT_46;
            let y_offset = Offset::y(font.text_height() / 2);

            shape::Text::new(self.number_area.center().ofs(y_offset), text, font)
...
impl Component for NumberInput {
    ...
    fn place(&mut self, bounds: Rect) -> Rect {
        self.total_area = bounds.inset(Insets::sides(Self::SIDE_PADDING));
        let grid = Grid::new(self.total_area, 1, 3);
        self.dec.place(grid.row_col(0, 0));
        self.number_area = grid.row_col(0, 1);
