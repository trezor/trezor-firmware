...
use super::super::{
    cshape::ScreenBorder,
    theme::{
        ACTION_BAR_HEIGHT, GREY, HEADER_HEIGHT, RED, SIDE_INSETS, TEXT_NORMAL, TEXT_SMALL,
        TEXT_VERTICAL_SPACING,
    },
};

const TEXT_SMALL_RED: TextStyle = TextStyle {
    text_color: RED,
    ..TEXT_SMALL
};
const TEXT_SMALL_GREY: TextStyle = TextStyle {
    text_color: GREY,
    ..TEXT_SMALL
};

/// Full-screen component showing Eckhart RSOD. To keep it minimal, this screen does not use any
/// other components.
...
impl<'a> ErrorScreen<'a> {
    pub fn new(title: TString<'a>, message: TString<'a>, footer: TString<'a>) -> Self {
        ...
        let footer = Label::centered(footer, TEXT_SMALL_GREY).top_aligned();
