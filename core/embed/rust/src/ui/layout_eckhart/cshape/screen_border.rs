...
impl ScreenBorder {
    ...
    pub const fn new(color: Color) -> Self {
        ...
        let bottom_bar_rect = Rect {
            x0: Self::SCREEN_INSET.x0 + ICON_BORDER_BL.toif.width(),
            y0: Self::SCREEN_INSET.y1 - Self::LINE_WIDTH,
            x1: Self::SCREEN_INSET.x1 - ICON_BORDER_BR.toif.width(),
            y1: Self::SCREEN_INSET.y1,
        };

        // Left bar: from the bottom edge of top-left icon to the top edge of
        // bottom-left icon.
        let left_bar_rect = Rect {
            x0: Self::SCREEN_INSET.x0,
            y0: Self::SCREEN_INSET.y0 + Self::TOP_ARC_HEIGHT,
            x1: Self::SCREEN_INSET.x0 + Self::LINE_WIDTH,
            y1: Self::SCREEN_INSET.y1 - ICON_BORDER_BL.toif.height(),
        };

        // Right bar: from the bottom edge of top-right icon to the top edge of
        // bottom-right icon.
        let right_bar_rect = Rect {
            x0: Self::SCREEN_INSET.x1 - Self::LINE_WIDTH,
            y0: Self::SCREEN_INSET.y0 + Self::TOP_ARC_HEIGHT,
            x1: Self::SCREEN_INSET.x1,
            y1: Self::SCREEN_INSET.y1 - ICON_BORDER_BR.toif.height(),
        };
        Self {
            color,
    ...
    pub fn render<'s>(&'s self, alpha: u8, target: &mut impl Renderer<'s>) {
        ...
        [
            (
                Self::SCREEN_INSET.top_left(),
                ICON_BORDER_TOP.toif,
                Alignment2D::TOP_LEFT,
            ),
            (
                Self::SCREEN_INSET.bottom_left(),
                ICON_BORDER_BL.toif,
                Alignment2D::BOTTOM_LEFT,
            ),
            (
                Self::SCREEN_INSET.bottom_right(),
                ICON_BORDER_BR.toif,
