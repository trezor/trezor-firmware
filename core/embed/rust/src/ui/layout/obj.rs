...
impl LayoutObjInner {
    ...
    pub fn root(&self) -> &impl Deref<Target = dyn ObjComponent> {
        unwrap!(self.root.as_ref())
    }

    pub fn root_mut(&mut self) -> &mut impl DerefMut<Target = dyn ObjComponent> {
    ...
    fn obj_event(&mut self, event: Event) -> Result<Obj, Error> {
        ...
        if self.event_ctx.needs_repaint_root() {
            // if yes, send a RequestPaint event, check that nothing happens
            self.marked_for_clear = true;
            self.marked_for_paint = true;
            let mut dummy_ctx = EventCtx::new();
            // paint_msg must not be an error
            let paint_msg = root.obj_event(&mut dummy_ctx, Event::RequestPaint);
            // it must not return a result
    ...
    fn obj_paint_if_requested(&mut self) -> bool {
        if self.marked_for_clear && self.marked_for_paint {
...
impl LayoutObj {
    ...
    pub fn skip_first_paint(&self) {
        self.inner_mut().marked_for_paint = false;
    }
}
...
extern "C" fn ui_layout_request_complete_repaint(this: Obj) -> Obj {
    let block = || {
        ...
        this.inner_mut().obj_request_clear();
        Ok(Obj::const_none())
    };
    unsafe { util::try_or_raise(block) }
}
