...
impl BootloaderUI for UIDelizia {
    ...
    fn screen_install_confirm(
        ...
        // Both variants differ only in CONTENT -- same styles, same alignment. Pick
        // the content, then build each button once, so the styling chain is
        // instantiated once instead of twice.
