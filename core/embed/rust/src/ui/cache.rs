//! LRU-style paging cache used by long-content screens (e.g. Eckhart's
//! `LongContentScreen`) to hold a small window of pages fetched on demand
//! from the extapp over IPC.
...
impl<const T: usize, const N: usize> Cache<T, N> {
    ...
    pub fn current_data(&self) -> Option<TString<'static>> {
        ...
        // SAFETY: `content` outlives the returned `TString` for as long as
        // `self` is alive, and `content_len` is always `<= content.len()`
        // (enforced by `set_content`'s `debug_assert`).
        unsafe {
            Some(
                StrBuffer::from_ptr_and_len(
                    self.nodes[current].content.as_ptr(),
                    self.nodes[current].content_len as u16,
                )
                .into(),
            )
        }
