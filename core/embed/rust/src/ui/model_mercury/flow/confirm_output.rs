...
fn new_confirm_output_obj(_args: &[Obj], kwargs: &Map) -> Result<Obj, error::Error> {
    ...
    if account.is_some() && account_path.is_some() {
        menu = menu.item(theme::ICON_CHEVRON_RIGHT, TR::address_details__account_info.into());
        menu_items.push(MENU_ITEM_ACCOUNT_INFO);
    }
    menu = menu.danger(theme::ICON_CANCEL, cancel_text.unwrap_or(TR::send__cancel_sign.into()));
    menu_items.push(MENU_ITEM_CANCEL);
    ...
    let res = if amount.is_some() {
        let content_amount = ConfirmBlobParams::new(TR::words__amount.into(), amount.unwrap_or(Obj::const_none()), None)
        .with_subtitle(title)
        .with_menu_button()
        .with_footer(TR::instructions__swipe_up.into(), None)
        .with_text_mono(text_mono)
