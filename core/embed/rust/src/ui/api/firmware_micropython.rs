...
use trezor_app_sdk::structs::{
    ArchivedSlice, ArchivedStrSlice, ArchivedTrezorProgressEnum, ArchivedTrezorUiEnum, Property,
    StrExt, StrSlice, TrezorUiResult,
};
...
extern "C" fn new_process_ipc_message(n_args: usize, args: *const Obj, kwargs: *mut Map) -> Obj {
    let block = |_args: &[Obj], kwargs: &Map| {
        ...
        let archived =
            unsafe { access_unchecked::<ArchivedTrezorUiEnum>(unwrap!(get_buffer(obj))) };
        ...
        fn wrap(
            ...
            br_code: i32,
            br_name: Option<&ArchivedStrSlice>,
        ) -> Result<(Gc<LayoutObj>, i32, Obj), Error> {
            Ok((
                LayoutObj::new_root(layout)?,
                br_code,
                match br_name {
                    Some(s) => Obj::try_from(s.as_str())?,
                    None => Obj::const_none(),
                },
            ))
        }

        fn tstr(s: &ArchivedStrSlice) -> TString<'static> {
            unwrap!(StrBuffer::alloc(s.as_str())).into()
        }

        fn tstr_opt(s: &ArchivedOption<ArchivedStrSlice>) -> Option<TString<'static>> {
            s.as_ref()
        ...
        match archived {
        ArchivedTrezorUiEnum::SelectMenu(m) => {
            let mut vec = heapless::Vec::<TString<'static>, 5>::new();
            for item in m.items.as_slice() {
        ...
        ArchivedTrezorUiEnum::ConfirmTrade(m) => wrap(
        ...
        ArchivedTrezorUiEnum::ConfirmAction(m) => wrap(
        ...
        ArchivedTrezorUiEnum::ShowInfoWithCancel(m) => wrap(
        ...
        ArchivedTrezorUiEnum::ConfirmValueIntro(m) => (
            ModelUI::confirm_value_intro(
                tstr(&m.title),
                m.value.as_str().try_into()?,
                tstr_opt(&m.subtitle),
                tstr_opt(&m.verb),
        ...
        ArchivedTrezorUiEnum::ConfirmSummary(m) => wrap(
        ...
        ArchivedTrezorUiEnum::ConfirmValue(m) => wrap(
            ModelUI::confirm_value(
                tstr(&m.title),
                Obj::try_from(m.value.as_str())?,
                tstr_opt(&m.description),
                m.is_data,
                None,
                tstr_opt(&m.subtitle),
        ...
        ArchivedTrezorUiEnum::ShowWarning(m) => (
        ...
        ArchivedTrezorUiEnum::ShowMismatch(m) => wrap(
            ModelUI::show_mismatch(tstr(&m.title))?,
            m.br_code.to_native(),
            None,
        )?,
        ArchivedTrezorUiEnum::ShowDanger(m) => wrap(
            ModelUI::show_danger(
                tstr(&m.title),
                tstr(&m.content),
                TString::empty(),
        ...
        ArchivedTrezorUiEnum::ShowSuccess(m) => (
        ...
        ArchivedTrezorUiEnum::RequestNumber(m) => wrap(
        ...
        ArchivedTrezorUiEnum::ConfirmProperties(m) => wrap(
        ...
        ArchivedTrezorUiEnum::ShowProperties(m) => wrap(
        ...
        ArchivedTrezorUiEnum::ShowPublicKey(m) => {
        ...
        ArchivedTrezorUiEnum::ConfirmWithInfo(m) => (
        ...
        ArchivedTrezorUiEnum::ShowAddress(m) => wrap(
