use crate::{
    ...
    strutil::TString,
    ui::{
        layout::result::{CANCELLED, CONFIRMED, INFO},
        ui_features::ModelUI,
        ui_features_fw::UIFeaturesFirmware,
    },
};

// free-standing functions exported to MicroPython mirror `trait UIFeaturesFirmware`
...
extern "C" fn show_info(n_args: usize, args: *const Obj, kwargs: *mut Map) -> Obj {
    let block = move |_args: &[Obj], kwargs: &Map| {
        ...
        let allow_cancel = kwargs.get(Qstr::MP_QSTR_allow_cancel)?.try_into()?;
        let time_ms = kwargs.get(Qstr::MP_QSTR_time_ms)?.try_into()?;

        let obj = ModelUI::show_info(title, button, description, allow_cancel, time_ms);
