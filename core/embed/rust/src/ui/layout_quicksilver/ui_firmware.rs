...
impl FirmwareUI for UIQuicksilver {
    fn confirm_action(
    ...
    fn confirm_value(
        ...
        ConfirmValue::new(title, data, description)
    ...
    fn confirm_value_intro(
        ...
        ConfirmValue::new(title, data, Some(TR::instructions__view_all_data.into()))
            .with_verb(verb)
    ...
    fn show_share_words(
        ...
        Err::<RootComponent<Empty, ModelUI>, Error>(Error::ValueError(
            c"use show_share_words_quicksilver instead",
        ))
    }
    ...
    fn show_remaining_shares(_pages_iterable: Obj) -> Result<impl LayoutMaybeTrace, Error> {
        // Quicksilver: remaining shares is a part of `continue_recovery` flow
        Err::<RootComponent<Empty, ModelUI>, Error>(Error::ValueError(
