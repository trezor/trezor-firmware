...
use hashbuffers::{
    ...
    BlockData, CodecError, DataBlock, SlotsBlock,
};
use micropython::{buffer, gc::Gc, list::List, py_object::GcObject, util, Obj};

use crate::protobuf::{
    decode::Decoder as PbDecoder,
    defs::{EnumDef, FieldDef, FieldType, MsgDef},
};
...
impl From<CodecError> for Error {
    fn from(error: CodecError) -> Self {
        dbg_println!("Hashbuffers codec error: {:?}", error);
        Error::value_error(c"Hashbuffers codec error")
    }
}

pub extern "C" fn hashbuffers_decode(buf: Obj, msg_def: Obj, enable_experimental: Obj) -> Obj {
    let block = || -> Result<Obj, micropython::Error> {
        ...
        if !enable_experimental && def.is_experimental {
            return Err(error::experimental_not_enabled().into());
        }

        let buf = unsafe { buffer::get_buffer(buf) }?;

        Ok(HbDecoder::decode_data(buf, &def, enable_experimental)?)
    };
...
impl<'a> HbDecoder<'a> {
    ...
    pub fn decode_by(&self, msg: &MsgDef) -> Result<Obj, Error> {
        let obj = self.pb_decoder.empty_message(msg)?;
        {
            let mut borrow = obj.borrow_mut();
            let map = borrow.map_mut();
    ...
    fn decode_singular(&self, field: &FieldDef) -> Result<Obj, Error> {
        ...
        dbg_println!(
            "Decoding singular field: ({}) {}",
            vidx,
            field.name().as_str()
        );
        match field.get_type() {
            FieldType::UVarInt => Ok(Obj::try_from_option(self.table.get_u32(vidx))?),
            FieldType::UVarInt64 => Ok(Obj::try_from_option(self.table.get_u64(vidx))?),
            FieldType::SVarInt => Ok(Obj::try_from_option(self.table.get_i32(vidx))?),
            FieldType::SVarInt64 => Ok(Obj::try_from_option(self.table.get_i64(vidx))?),
    ...
    where
        T: AnyBitPattern,
        T: TryInto<Obj, Error = micropython::Error>,
    {
    ...
    fn decode_repeated(&self, field: &FieldDef) -> Result<Gc<List>, Error> {
        ...
        match field.get_type() {
            ...
            FieldType::Bytes => self.decode_slots(field, vidx, |data| Ok(data.try_into()?)),
