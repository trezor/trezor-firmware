...
secbool firmware_verify_tree(firmware_tree_info_t* info) {
  ...
  // (firmware_type == 0) or mismatched device stays unbootable until a real
  // install stamps the matching variant (so a bare/direct-flashed image also
  // does not run until installed).
