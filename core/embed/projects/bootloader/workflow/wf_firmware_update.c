...
static upload_status_t process_msg_FirmwareUpload(protob_iface_t *iface,
                                                  uint32_t msg_size,
                                                  uint8_t *buf,
                                                  firmware_update_ctx_t *ctx) {
  FirmwareUpload msg;

  const secbool r = recv_msg_firmware_upload(
      iface, &msg, buf, msg_size, ctx, fw_data_received,
