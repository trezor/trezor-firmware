...
secbool recv_msg_firmware_upload(protob_iface_t *iface, FirmwareUpload *msg,
                                 uint8_t *buf, uint32_t msg_size, void *ctx,
...
void recv_msg_unknown(protob_iface_t *iface, uint32_t msg_size, uint8_t *buf) {
  codec_flush(iface->wire, msg_size, buf);
  send_msg_failure(iface, FailureType_Failure_UnexpectedMessage,
                   "Unexpected message");
}

void protob_init(protob_iface_t *iface, wire_iface_t *wire) {
  iface->wire = wire;
}
