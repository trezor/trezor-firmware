...
static secbool boot_sequence(void) {
  ...
  if (bld_locked) {
    ...
    screen_bootloader_entry_progress(0, true);
    progress_start = systick_ms();

    bool btn_down = button_is_down(BTN_POWER);
    while (btn_down) {
      uint32_t progress = systick_ms() - progress_start;
      if (progress > 1000) {
        progress = 1000;
      }
      screen_bootloader_entry_progress(progress, false);

      btn_down = button_is_down(BTN_POWER);
    }
