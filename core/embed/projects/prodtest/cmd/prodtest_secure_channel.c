...
static void prodtest_secure_channel_handshake_1(cli_t* cli) {
  ...
  uint8_t output[SECURE_CHANNEL_RESPONSE_SIZE] = {0};

  if (!secure_channel_handshake_1((uint8_t*)&output)) {
    cli_error(cli, CLI_ERROR, "`secure_channel_handshake_1()` failed.");
    return;
  }

  cli_ok_hexdata(cli, (uint8_t*)&output, sizeof(output));
}
...
static void prodtest_secure_channel_handshake_2(cli_t* cli) {
  ...
  uint8_t input[SECURE_CHANNEL_REQUEST_SIZE] = {0};
  ...
  if (input_length != SECURE_CHANNEL_REQUEST_SIZE) {
    cli_error(cli, CLI_ERROR, "Unexpected input length. Expecting %d bytes.",
              (int)SECURE_CHANNEL_REQUEST_SIZE);
  }
