    ...
    case '\r':  // ENTER key
      if (cli_cmd_byte_idx > 0) {
        ...
        do {
          ...
          if (byte_idx >= sizeof(tx_buf.data)) {
            cli->write(cli, "\r\n", 2);
            cli_error(cli, CLI_ERROR_INVALID_ARG, "Too many bytes in command.");
            goto clean_up;
          }

          tx_buf.data[byte_idx++] = (uint8_t)decoded_byte;
          buf_ptr = endptr;
        } while (1);

        cli->write(cli, "\r\n", 2);
        cli_trace(cli, "Sending to NFC:");
        cli_ok_hexdata(cli, tx_buf.data, byte_idx);
...
static ts_t nfc_select_app(cli_t* cli, const uint8_t* aid_select_app,
  ...
  if (ts_ok(nfc_status)) {
    nfc_apdu_message_t tx_buf = {0};
    nfc_apdu_message_t rx_buf = {0};
    memcpy(tx_buf.data, apdu_buffer_ptr, apdu_buffer_len);
...
static ts_t nfc_read_file(cli_t* cli, uint16_t offset, uint16_t* exp_len,
  ...
  if (ts_ok(nfc_status)) {
    ...
    if (ts_ok(nfc_status)) {
      if (read_data != NULL) {
        memcpy(read_data, rx_buf.data, rx_buf.data_len);
      }
      if (exp_len != NULL) {
        *exp_len = rx_buf.data_len;
      }
    }
  ...
  while (true) {
    ...
    if (event_flag == NFC_EVENT_CONNECTED) {
      ...
      nfc_status = noise_initiator_handle_response1(
          &initiator, rx_buf.data, rx_buf.data_len, request_buf,
      ...
      nfc_status = noise_initiator_handle_response2(&initiator, rx_buf.data,
                                                    rx_buf.data_len);
      if (ts_ok(nfc_status)) {
