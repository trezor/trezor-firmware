...
static void prodtest_rtc_set_time(cli_t* cli) {
...
static void prodtest_rtc_get_time(cli_t* cli) {
  if (cli_arg_count(cli) > 0) {
    cli_error_arg_count(cli);
    return;
  }
