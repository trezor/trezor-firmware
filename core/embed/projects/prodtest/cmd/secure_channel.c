...
bool secure_channel_handshake_2(
  ...
  memcpy(ephemeral_key_backup, noise_context.initiator_ephemeral_private_key,
         sizeof(ephemeral_key_backup));
  for (int i = 0; i < sizeof(hsm_public_keys) / sizeof(hsm_public_keys[0]);
       i++) {
    memcpy(noise_context.initiator_ephemeral_private_key, ephemeral_key_backup,
           sizeof(ephemeral_key_backup));
