...
static void prodtest_fuel_gauge(cli_t *cli){
    ...
    while(true){
        ...
        fuel_gauge_update(&fg, systick_ms()-tick, report.vbat, report.ibat, report.ntc_temp);
        tick = systick_ms();

        cli_progress(cli, "V: %d.%02d I: %d.%02d SOC: %d.%02d", (int)report.vbat, (int)(report.vbat * 1000) % 1000,
        (int)report.ibat, (int)(report.ibat * 1000) % 1000,
        (int)fg.state_of_charge, (int)(fg.state_of_charge * 1000) % 1000);

        mini_snprintf(display_text, 100, "V: %d.%02d I: %d.%02d SOC: %d.%02d", (int)report.vbat, (int)(report.vbat * 1000) % 1000,
                                                                               (int)report.ibat, (int)(report.ibat * 1000) % 1000,
                                                                               (int)fg.state_of_charge, (int)(fg.state_of_charge * 1000) % 1000);
