...
static void prodtest_display_image(cli_t* cli) {
  ...
  if (0 == strcmp(phase, "begin")) {
    ...
    if (!display_get_frame_buffer(&image_fb)) {
      cli_error(cli, PRODTEST_ERR_DISPLAY_IMAGE_FB_GET,
    ...
    if (image_fb.ptr == NULL) {
      cli_error(cli, PRODTEST_ERR_DISPLAY_IMAGE_NOT_STARTED,
                "Transfer not started. Use 'begin' first.");
      return;
    }
    ...
    cli_error(cli, PRODTEST_ERR_DISPLAY_IMAGE_UNKNOWN_PHASE,
  ...
  do {
    for (int i = 0; i < PRODTEST_IMAGES_COUNT; i++) {
      ...
      if (!display_get_frame_buffer(&fb)) {
        cli_error(cli, PRODTEST_ERR_DISPLAY_SLIDESHOW_FB_GET,
                  "Cannot get frame buffer.");
        return;
      }
