...
void binary_update(cli_t* cli, bool (*finalize)(uint8_t* data, size_t len)) {
  ...
  if (0 == strcmp(phase, "begin")) {
    ...
    if (binary_len + chunk_len > sizeof(binary_buffer)) {
      cli_error(cli, CLI_ERROR, "Buffer overflow (have %u, %u more)",
                (unsigned)binary_len, (unsigned)chunk_len);
      goto cleanup;
    }

    memcpy(&binary_buffer[binary_len], chunk_buf, chunk_len);
    ...
    if (!finalize(binary_buffer, binary_len)) {
      binary_len = 0;
      cli_error(cli, CLI_ERROR, "Error while finalizing the update", phase);
      goto cleanup;
    }
