...
bool dma2d_rgb565_fill(const gfx_bitblt_t* bb)
{
    ...
    if (bb->src_alpha == 255) {
        ...
        dma2d_handle.Init.OutputOffset = bb->dst_stride / sizeof(uint16_t) - bb->width;
        HAL_DMA2D_Init(&dma2d_handle);

        HAL_DMA2D_Start(
            &dma2d_handle, gfx_color_to_color32(bb->src_fg),
            (uint32_t)bb->dst_row + bb->dst_x * sizeof(uint16_t), bb->width, bb->height);
    } else {
#ifdef STM32U5
        dma2d_handle.Init.ColorMode = DMA2D_OUTPUT_RGB565;
        ...
        dma2d_handle.LayerCfg[0].InputOffset = bb->dst_stride / sizeof(uint16_t) - bb->width;
...
static void dma2d_config_clut(uint32_t layer, gfx_color32_t fg, gfx_color32_t bg)
{
...
#define GRADIENT_STEPS 16
    ...
    volatile uint32_t* clut = layer ? dma2d_handle.Instance->FGCLUT : dma2d_handle.Instance->BGCLUT;
...
static void dma2d_rgb565_copy_mono4_first_col(gfx_bitblt_t* bb, const gfx_color16_t* gradient)
{
...
static void dma2d_rgb565_copy_mono4_last_col(gfx_bitblt_t* bb, const gfx_color16_t* gradient)
{
...
bool dma2d_rgb565_copy_mono4(const gfx_bitblt_t* params)
{
    ...
    dma2d_config_clut(1, gfx_color_to_color32(bb->src_fg), gfx_color_to_color32(bb->src_bg));
...
bool dma2d_rgb565_copy_rgb565(const gfx_bitblt_t* bb)
{
    ...
    dma2d_handle.LayerCfg[1].InputColorMode = DMA2D_INPUT_RGB565;
    dma2d_handle.LayerCfg[1].InputOffset = bb->src_stride / sizeof(uint16_t) - bb->width;
    dma2d_handle.LayerCfg[1].AlphaMode = 0;
    dma2d_handle.LayerCfg[1].InputAlpha = 0;
    HAL_DMA2D_ConfigLayer(&dma2d_handle, 1);

    HAL_DMA2D_Start(
        &dma2d_handle, (uint32_t)bb->src_row + bb->src_x * sizeof(uint16_t),
        (uint32_t)bb->dst_row + bb->dst_x * sizeof(uint16_t), bb->width, bb->height);
    return true;
}
...
static void dma2d_rgb565_blend_mono4_first_col(const gfx_bitblt_t* bb)
{
    ...
    while (height-- > 0) {
        ...
        dst_ptr[0] = gfx_color16_blend_a8(bb->src_fg, gfx_color16_to_color(dst_ptr[0]), fg_alpha);
        dst_ptr += bb->dst_stride / sizeof(*dst_ptr);
        src_ptr += bb->src_stride / sizeof(*src_ptr);
    }
}
...
bool dma2d_rgb565_blend_mono8(const gfx_bitblt_t* bb)
{
    ...
    HAL_DMA2D_BlendingStart(
        &dma2d_handle, (uint32_t)bb->src_row + bb->src_x,
        (uint32_t)bb->dst_row + bb->dst_x * sizeof(uint16_t),
        (uint32_t)bb->dst_row + bb->dst_x * sizeof(uint16_t), bb->width, bb->height);
...
bool dma2d_rgba8888_fill(const gfx_bitblt_t* bb)
{
    ...
    if (bb->src_alpha == 255) {
        ...
        dma2d_handle.Init.Mode = DMA2D_R2M;
        dma2d_handle.Init.OutputOffset = bb->dst_stride / sizeof(uint32_t) - bb->width;
        HAL_DMA2D_Init(&dma2d_handle);

        HAL_DMA2D_Start(
            &dma2d_handle, gfx_color_to_color32(bb->src_fg),
            (uint32_t)bb->dst_row + bb->dst_x * sizeof(uint32_t), bb->width, bb->height);
    } else {
#ifdef STM32U5
        dma2d_handle.Init.ColorMode = DMA2D_OUTPUT_ARGB8888;
        dma2d_handle.Init.Mode = DMA2D_M2M_BLEND_FG;
        ...
        dma2d_handle.LayerCfg[0].InputOffset = bb->dst_stride / sizeof(uint32_t) - bb->width;
        dma2d_handle.LayerCfg[0].AlphaMode = 0;
        dma2d_handle.LayerCfg[0].InputAlpha = 0;
        HAL_DMA2D_ConfigLayer(&dma2d_handle, 0);
...
static void dma2d_rgba8888_copy_mono4_first_col(gfx_bitblt_t* bb, const gfx_color32_t* gradient)
{
...
static void dma2d_rgba8888_copy_mono4_last_col(gfx_bitblt_t* bb, const gfx_color32_t* gradient)
{
...
static void dma2d_rgba8888_blend_mono4_first_col(const gfx_bitblt_t* bb)
{
    ...
    while (height-- > 0) {
        uint8_t fg_alpha = src_ptr[0] >> 4;
        fg_alpha = (fg_alpha * bb->src_alpha) / 15;
        dst_ptr[0] = gfx_color32_blend_a8(bb->src_fg, gfx_color32_to_color(dst_ptr[0]), fg_alpha);
        dst_ptr += bb->dst_stride / sizeof(*dst_ptr);
        src_ptr += bb->src_stride / sizeof(*src_ptr);
    }
}
...
bool dma2d_rgba8888_copy_rgba8888(const gfx_bitblt_t* bb)
{
    ...
    dma2d_handle.LayerCfg[1].InputColorMode = DMA2D_INPUT_ARGB8888;
    dma2d_handle.LayerCfg[1].InputOffset = bb->src_stride / sizeof(uint32_t) - bb->width;
    dma2d_handle.LayerCfg[1].AlphaMode = 0;
    dma2d_handle.LayerCfg[1].InputAlpha = 0;
    HAL_DMA2D_ConfigLayer(&dma2d_handle, 1);

    HAL_DMA2D_Start(
        &dma2d_handle, (uint32_t)bb->src_row + bb->src_x * sizeof(uint32_t),
        (uint32_t)bb->dst_row + bb->dst_x * sizeof(uint32_t), bb->width, bb->height);
    return true;
}
