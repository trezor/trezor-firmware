...
void trezor_shutdown(void) {
  syscall_invoke0(SYSCALL_TREZOR_SHUTDOWN);
  while (1)
    ;
}
...
void wait_random(void) { syscall_invoke0(SYSCALL_WAIT_RANDOM); }

void rdi_refresh_session_delay(void) {
  syscall_invoke0(SYSCALL_RDI_REFRESH_SESSION_DELAY);
}
...
void storage_init(PIN_UI_WAIT_CALLBACK callback, const uint8_t *salt,
  ...
  syscall_invoke3((uint32_t)callback, (uint32_t)salt, salt_len,
...
uint32_t flash_area_get_size(const flash_area_t *area) {
  return syscall_invoke1((uint32_t)area, SYSCALL_FLASH_AREA_GET_SIZE);
}

const void *flash_area_get_address(const flash_area_t *area, uint32_t offset,
                                   uint32_t size) {
  return (const void *)syscall_invoke3((uint32_t)area, offset, size,
                                       SYSCALL_FLASH_AREA_GET_ADDRESS);
}
