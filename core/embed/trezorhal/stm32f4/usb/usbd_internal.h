...
// If the class is NULL the slot is cleared.
void usb_set_iface_class(uint8_t iface_num, const USBD_ClassTypeDef *class);

// Returns the pointer to the next free bytes in the buffer
// reserved for the class driver descriptors (interface, endpoint, ...)
//
// The function checks if the remaining space is enough and
// returns NULL if not.
void *usb_desc_alloc_iface(size_t desc_len);

// Adds the class driver descriptors to the the configuration descriptor and
// updates configuration descriptor length and number of interfaces.
//
// The class driver must call this function after all the its
// descriptors have been written to the memory preallocated
