...
secbool usb_vcp_add(const usb_vcp_info_t *info)
{
    ...
    usb_vcp_descriptor_block_t *d = usb_alloc_class_descriptors(sizeof(usb_vcp_descriptor_block_t));
...
static uint8_t usb_vcp_class_setup(USBD_HandleTypeDef *dev, USBD_SetupReqTypedef *req)
{
    usb_vcp_state_t *state = (usb_vcp_state_t *)dev->pUserData;
    ...
    if ((req->bmRequest & USB_REQ_DIR_MASK) == USB_REQ_DIR_D2H) {
        if (req->bRequest == USB_CDC_GET_LINE_CODING) {
            USBD_CtlSendData(
                dev, UNCONST(&line_coding), MIN_8bits(req->wLength, sizeof(line_coding)));
        } else {
            USBD_CtlSendData(
                dev, state->cmd_buffer, MIN_8bits(req->wLength, USB_CDC_MAX_CMD_PACKET_LEN));
        }
