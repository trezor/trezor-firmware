...
{
    usb_webusb_state_t *state = (usb_webusb_state_t *)usb_get_iface_state(info->iface_num, NULL);
...
int usb_webusb_read(uint8_t iface_num, uint8_t *buf, uint32_t len)
{
    ...
    USBD_LL_PrepareReceive(
        state->dev_handle, state->ep_out, state->rx_buffer, state->max_packet_len);
...
int usb_webusb_write(uint8_t iface_num, const uint8_t *buf, uint32_t len)
{
    ...
    USBD_LL_Transmit(state->dev_handle, state->ep_in, UNCONST(buf), (uint16_t)len);
...
static uint8_t usb_webusb_class_deinit(USBD_HandleTypeDef *dev, uint8_t cfg_idx)
{
    usb_webusb_state_t *state = (usb_webusb_state_t *)dev->pUserData;
...
static uint8_t usb_webusb_class_setup(USBD_HandleTypeDef *dev, USBD_SetupReqTypedef *req)
{
    usb_webusb_state_t *state = (usb_webusb_state_t *)dev->pUserData;
...
static uint8_t usb_webusb_class_data_in(USBD_HandleTypeDef *dev, uint8_t ep_num)
{
    usb_webusb_state_t *state = (usb_webusb_state_t *)dev->pUserData;
...
static uint8_t usb_webusb_class_data_out(USBD_HandleTypeDef *dev, uint8_t ep_num)
{
    usb_webusb_state_t *state = (usb_webusb_state_t *)dev->pUserData;
