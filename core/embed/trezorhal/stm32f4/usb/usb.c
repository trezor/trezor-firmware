...
secbool usb_init(const usb_dev_info_t *dev_info)
{
    ...
    drv->dev_desc.bcdUSB = (sectrue == drv->usb21_enabled) ? 0x0210 : 0x0200;  // USB 2.1 or USB 2.0
    ...
    drv->dev_desc.iManufacturer = USBD_IDX_MFC_STR;     // Index of manufacturer string
    drv->dev_desc.iProduct = USBD_IDX_PRODUCT_STR;      // Index of product string
    drv->dev_desc.iSerialNumber = USBD_IDX_SERIAL_STR;  // Index of serial number string
    ...
    drv->config_desc->bNumInterfaces = 0;  // will be updated later via usb_set_iface_class()
    drv->config_desc->bConfigurationValue = 0x01;
    drv->config_desc->iConfiguration = 0;
    drv->config_desc->bmAttributes = 0x80;  // 0x80 = bus powered; 0xC0 = self powered
...
secbool usb_start(void)
{
    ...
    if (USBD_OK !=
        USBD_Init(&drv->dev_handle, (USBD_DescriptorsTypeDef *)&usb_descriptors, USB_PHY_ID)) {
        usb_stop();
        return secfalse;
    }

    if (USBD_OK != USBD_RegisterClass(&drv->dev_handle, (USBD_ClassTypeDef *)&usb_class)) {
        usb_stop();
        return secfalse;
    }
...
static uint8_t *usb_get_bos_descriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
    ...
    if (sectrue == drv->usb21_enabled) {
        static uint8_t bos[] = {
            ...
            0x38, 0xb6, 0x08, 0x34, 0xa9, 0x09, 0xa0, 0x47, 0x8b, 0xfd, 0xa0, 0x76, 0x88, 0x15,
            0xb6, 0x65,               // uint128_t platformCompatibilityUUID
...
static uint8_t usb_class_setup(USBD_HandleTypeDef *dev, USBD_SetupReqTypedef *req)
{
    ...
    if ((req->bmRequest & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_VENDOR) {
        if ((req->bmRequest & USB_REQ_RECIPIENT_MASK) == USB_REQ_RECIPIENT_DEVICE) {
            if (sectrue == drv->usb21_enabled && req->bRequest == USB_WEBUSB_VENDOR_CODE) {
                if (req->wIndex == USB_WEBUSB_REQ_GET_URL &&
                    ...
                    wait_random();
                    USBD_CtlSendData(
                        dev, UNCONST(webusb_url), MIN_8bits(req->wLength, sizeof(webusb_url)));
                    return USBD_OK;
                } else {
                    wait_random();
            ...
            } else if (sectrue == drv->usb21_enabled && req->bRequest == USB_WINUSB_VENDOR_CODE) {
                if (req->wIndex == USB_WINUSB_REQ_GET_COMPATIBLE_ID_FEATURE_DESCRIPTOR) {
                    ...
                    wait_random();
                    USBD_CtlSendData(
                        dev, UNCONST(winusb_wcid), MIN_8bits(req->wLength, sizeof(winusb_wcid)));
                    return USBD_OK;
                } else {
                    wait_random();
                    USBD_CtlError(dev, req);
        ...
        if ((req->bmRequest & USB_REQ_RECIPIENT_MASK) == USB_REQ_RECIPIENT_INTERFACE) {
            if (sectrue == drv->usb21_enabled && req->bRequest == USB_WINUSB_VENDOR_CODE) {
                if (req->wIndex == USB_WINUSB_REQ_GET_EXTENDED_PROPERTIES_OS_FEATURE_DESCRIPTOR &&
                    ...
                    static const uint8_t winusb_guid[] = {
                        ...
                        '{', 0x00, 'c', 0x00, '6', 0x00, 'c', 0x00, '3', 0x00, '7', 0x00, '4', 0x00,
                        'a', 0x00, '6', 0x00, '-', 0x00, '2', 0x00, '2', 0x00, '8', 0x00, '5', 0x00,
                        '-', 0x00, '4', 0x00, 'c', 0x00, 'b', 0x00, '8', 0x00, '-', 0x00, 'a', 0x00,
                        'b', 0x00, '4', 0x00, '3', 0x00, '-', 0x00, '1', 0x00, '7', 0x00, '6', 0x00,
                        '4', 0x00, '7', 0x00, 'c', 0x00, 'e', 0x00, 'a', 0x00, '5', 0x00, '0', 0x00,
                        '3', 0x00, 'd', 0x00, '}', 0x00, 0x00, 0x00, 0x00, 0x00,  // propertyData
                    };
                    wait_random();
                    USBD_CtlSendData(
                        dev, UNCONST(winusb_guid), MIN_8bits(req->wLength, sizeof(winusb_guid)));
                    return USBD_OK;
                } else {
                    wait_random();
                    USBD_CtlError(dev, req);
