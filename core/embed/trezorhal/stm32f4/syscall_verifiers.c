...
void systask_exit_error__verified(systask_t *task, const char *title,
                                  size_t title_len, const char *message,
                                  size_t message_len, const char *footer,
                                  size_t footer_len) {
  char title_copy[64] = {0};
  char message_copy[64] = {0};
  char footer_copy[64] = {0};

  title_len = title ? MIN(title_len, sizeof(title_copy) - 1) : 0;
  message_len = message ? MIN(message_len, sizeof(message_copy) - 1) : 0;
  footer_len = footer ? MIN(footer_len, sizeof(footer_copy) - 1) : 0;
  ...
  if (message != NULL) {
    ...
    message = strcpy(message_copy, message);
  }
  ...
  if (footer != NULL) {
    ...
    footer = strcpy(footer_copy, footer);
  }
...
void systask_exit_fatal__verified(systask_t *task, const char *message,
                                  size_t message_len, const char *file,
                                  size_t file_len, int line) {
  char message_copy[64] = {0};
  char file_copy[64] = {0};

  message_len = message ? MIN(message_len, sizeof(message_copy) - 1) : 0;
  file_len = file ? MIN(file_len, sizeof(file_copy) - 1) : 0;
...
void display_copy_rgb565__verified(const gfx_bitblt_t *bb) {
  ...
  uint8_t *src_ptr = (uint8_t *)bb->src_row;
  size_t src_len = bb->src_stride * bb->height;
