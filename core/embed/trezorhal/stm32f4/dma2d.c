...
void dma2d_setup_4bpp_over_16bpp(uint16_t overlay_color)
{
    ...
    dma2d_handle.LayerCfg[1].InputOffset = 0;
    dma2d_handle.LayerCfg[1].AlphaMode = 0;
    dma2d_handle.LayerCfg[1].InputAlpha = 0xFF000000 | rgb565_to_rgb888(overlay_color);
...
void dma2d_setup_4bpp_over_4bpp(uint16_t fg_color, uint16_t bg_color, uint16_t overlay_color)
{
    dma2d_handle.Init.Mode = DMA2D_M2M_BLEND;
    dma2d_handle.Init.OutputOffset = display_get_window_offset();
    dma2d_handle.LayerCfg[1].InputColorMode = DMA2D_INPUT_A4;
...
void dma2d_start(uint8_t* in_addr, uint8_t* out_addr, int32_t pixels)
{
    ...
    current_height = 1;
    HAL_DMA2D_Start(&dma2d_handle, (uint32_t)in_addr, (uint32_t)out_addr, pixels, 1);
}
...
void dma2d_start_const(uint16_t color, uint8_t* out_addr, int32_t pixels)
{
    current_width = pixels;
    current_height = 1;
    HAL_DMA2D_Start(&dma2d_handle, rgb565_to_rgb888(color), (uint32_t)out_addr, pixels, 1);
}

void dma2d_start_const_multiline(uint16_t color, uint8_t* out_addr, int32_t width, int32_t height)
{
    current_width = width;
    current_height = height;
    HAL_DMA2D_Start(&dma2d_handle, rgb565_to_rgb888(color), (uint32_t)out_addr, width, height);
}

void dma2d_start_blend(uint8_t* overlay_addr, uint8_t* bg_addr, uint8_t* out_addr, int32_t pixels)
{
    current_width = pixels;
    current_height = 1;
    HAL_DMA2D_BlendingStart(
        &dma2d_handle, (uint32_t)overlay_addr, (uint32_t)bg_addr, (uint32_t)out_addr, pixels, 1);
}
