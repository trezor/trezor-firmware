...
__attribute__((section(".framebuffer_select"))) static uint32_t act_frame_buffer = 0;
...
void display_setup_fmc(void)
{
...
#ifdef USE_DISP_I8080_16BIT_DW
    external_display_data_sram.Init.MemoryDataWidth = FMC_NORSRAM_MEM_BUS_WIDTH_16;
#elif USE_DISP_I8080_8BIT_DW
    ...
    HAL_SRAM_Init(&external_display_data_sram, &normal_mode_timing, &ext_mode_timing);
...
void DISPLAY_TE_INTERRUPT_HANDLER(void)
{
    ...
    if (act_frame_buffer == 1) {
        bg_copy_start_const_out_8(
            (uint8_t *)PhysFrameBuffer1, (uint8_t *)DISPLAY_DATA_ADDRESS,
            DISPLAY_RESX * DISPLAY_RESY * 2, NULL);

    } else {
        bg_copy_start_const_out_8(
            (uint8_t *)PhysFrameBuffer0, (uint8_t *)DISPLAY_DATA_ADDRESS,
            DISPLAY_RESX * DISPLAY_RESY * 2, NULL);
    }
...
uint8_t *display_get_wr_addr(void)
{
    return (uint8_t *)DISPLAY_DATA_ADDRESS;
}
