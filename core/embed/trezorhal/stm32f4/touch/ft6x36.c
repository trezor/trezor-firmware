...
static void ft6x36_panel_correction(uint16_t x, uint16_t y, uint16_t* x_new, uint16_t* y_new)
{
#ifdef TOUCH_PANEL_LX154A2422CPT23
...
void trace_regs(uint8_t* regs)
{
    ...
    printf(
        "%04ld [gesture=%02X, nb_touches=%d, flags=%c, x=%3d, y=%3d]\r\n", time, gesture,
        nb_touches, event, x, y);
}
#endif
...
uint32_t touch_get_event(void)
{
    ...
    // Extract touch coordinates
    uint16_t x_raw = ((regs[FT6X63_REG_P1_XH] & 0x0F) << 8) | regs[FT6X63_REG_P1_XL];
    uint16_t y_raw = ((regs[FT6X63_REG_P1_YH] & 0x0F) << 8) | regs[FT6X63_REG_P1_YL];
