...
__attribute__((noreturn)) static void _reboot_to_bootloader(boot_command_t boot_command)
{
...
__attribute__((noreturn)) static void _reboot_to_bootloader(boot_command_t boot_command)
{
    ...
    jump_to_with_flag(IMAGE_CODE_ALIGN(BOOTLOADER_START + IMAGE_HEADER_SIZE), boot_command);
    for (;;)
        ;
}
#endif
...
void svc_reboot_to_bootloader(void)
{
    ...
    if (is_mode_unprivileged() && !is_mode_handler()) {
        ...
        __asm__ __volatile__("svc %0" ::"i"(SVC_REBOOT_TO_BOOTLOADER), "r"(r0) : "memory");
    } else {
...
void SVC_C_Handler(uint32_t *stack)
{
    ...
    switch (svc_number) {
        ...
        case SVC_REBOOT_TO_BOOTLOADER:
            ...
            __asm__ volatile("mov r0, %[boot_command]" ::[boot_command] "r"(&stack[0]));
