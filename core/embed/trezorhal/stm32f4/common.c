...
void hal_delay_us(uint16_t delay_us)
{
    ...
    uint32_t t =
        hal_ticks_ms() * 1000 + (((SystemCoreClock / 1000) - val) / (SystemCoreClock / 1000000));
    uint32_t t2 = t;
    do {
...
void clear_otg_hs_memory(void)
{
    ...
    memset_reg(
        ...
        (volatile void *)(USB_OTG_HS_DATA_FIFO_RAM + USB_OTG_HS_DATA_FIFO_SIZE), 0);
...
void invalidate_firmware(void)
{
    ...
    for (int i = 0; i < (1024 / FLASH_BLOCK_SIZE); i += FLASH_BLOCK_SIZE) {
        ...
        ensure(flash_area_write_block(&FIRMWARE_AREA, i * FLASH_BLOCK_SIZE, data), NULL);
    }
