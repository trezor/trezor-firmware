...
void display_init(void)
{
    ...
    drv->buffer =
        SDL_CreateRGBSurface(0, DISPLAY_RESX, DISPLAY_RESY, 16, 0xF800, 0x07E0, 0x001F, 0x0000);
    drv->texture = SDL_CreateTexture(
        drv->renderer, SDL_PIXELFORMAT_RGB565, SDL_TEXTUREACCESS_STREAMING, DISPLAY_RESX,
        DISPLAY_RESY);
...
#define CONCAT_LEN(name) CONCAT_LEN_HELPER(name)
    drv->background = IMG_LoadTexture_RW(
        drv->renderer, SDL_RWFromMem(BACKGROUND_NAME, CONCAT_LEN(BACKGROUND_NAME)), 0);
    if (drv->background) {
        ...
        SDL_SetWindowSize(
            drv->window, DISPLAY_RESX + 2 * EMULATOR_BORDER, DISPLAY_RESY + 2 * EMULATOR_BORDER);
...
static void copy_mono_framebuf(display_driver_t *drv)
{
    for (int y = 0; y < DISPLAY_RESY; y++) {
        uint16_t *dst = (uint16_t *)((uint8_t *)drv->buffer->pixels + drv->buffer->pitch * y);
...
void display_refresh(void)
{
    ...
    SDL_UpdateTexture(drv->texture, NULL, drv->buffer->pixels, drv->buffer->pitch);
#define BACKLIGHT_NORMAL 150
    SDL_SetTextureAlphaMod(drv->texture, MIN(255, 255 * drv->backlight_level / BACKLIGHT_NORMAL));
    if (drv->background) {
        const SDL_Rect r = {TOUCH_OFFSET_X, TOUCH_OFFSET_Y, DISPLAY_RESX, DISPLAY_RESY};
        SDL_RenderCopyEx(drv->renderer, drv->texture, NULL, &r, drv->orientation_angle, NULL, 0);
    } else {
...
void display_fill(const gfx_bitblt_t *bb)
{
    ...
    bb_new.dst_row = (uint8_t *)drv->buffer->pixels + (drv->buffer->pitch * bb_new.dst_y);
...
const char *display_save(const char *prefix)
{
    ...
    SDL_Surface *crop = SDL_CreateRGBSurface(
        ...
        drv->buffer->format->Rmask, drv->buffer->format->Gmask, drv->buffer->format->Bmask,
        drv->buffer->format->Amask);
    SDL_BlitSurface(drv->buffer, &rect, crop, NULL);
    // compare with previous screen, skip if equal
    if (drv->prev_saved != NULL) {
        if (memcmp(drv->prev_saved->pixels, crop->pixels, crop->pitch * crop->h) == 0) {
