...
void display_pixeldata(pixel_color c)
{
    ...
    if (PIXELWINDOW.pos.x <= PIXELWINDOW.end.x && PIXELWINDOW.pos.y <= PIXELWINDOW.end.y) {
        ((pixel_color *)BUFFER->pixels)
            [PIXELWINDOW.pos.x + PIXELWINDOW.pos.y * BUFFER->pitch / sizeof(pixel_color)] = c;
    }
    PIXELWINDOW.pos.x++;
...
void display_init(void)
{
    ...
    BUFFER = SDL_CreateRGBSurface(
        0, MAX_DISPLAY_RESX, MAX_DISPLAY_RESY, 16, 0xF800, 0x07E0, 0x001F, 0x0000);
...
#define CONCAT_LEN(name) CONCAT_LEN_HELPER(name)
    ...
    if (BACKGROUND) {
        ...
        SDL_SetWindowSize(
            WINDOW, DISPLAY_RESX + 2 * EMULATOR_BORDER, DISPLAY_RESY + 2 * EMULATOR_BORDER);
