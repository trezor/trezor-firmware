...
static bool is_inside_display(int x, int y)
{
    return x >= sdl_touch_offset_x && y >= sdl_touch_offset_y &&
           x - sdl_touch_offset_x < sdl_display_res_x && y - sdl_touch_offset_y < sdl_display_res_y;
}
...
static void handle_mouse_events(
    touch_driver_t* driver, SDL_Event event, int* ev_type, int* ev_x, int* ev_y)
{
    ...
    switch (event.type) {
        ...
        case SDL_MOUSEBUTTONUP:
            if (driver->state != IDLE) {
                *ev_x = inside_display ? event.button.x - sdl_touch_offset_x : driver->last_x;
                *ev_y = inside_display ? event.button.y - sdl_touch_offset_y : driver->last_y;
                *ev_type = TOUCH_END;
                driver->state = IDLE;
            }
            break;
        ...
        case SDL_MOUSEMOTION:
            if (driver->state != IDLE) {
                if (inside_display) {
                    ...
                    *ev_type = (driver->state == MOUSE_DOWN_OUTSIDE) ? TOUCH_START : TOUCH_MOVE;
                    driver->state = MOUSE_DOWN_INSIDE;
                } else {
    ...
    touch_driver_t* driver, SDL_Event event, int* ev_type, int* ev_x, int* ev_y)
{
    ...
    if (event.type == SDL_KEYDOWN && !event.key.repeat && !is_button_swipe_initiated(driver)) {
