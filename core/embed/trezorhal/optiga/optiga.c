...
static const uint8_t COUNTER_RESET_EXTRA[] = {0, 0, 0, 0, 0, 0, 0, PIN_MAX_TRIES + 1};
...
static const optiga_metadata_item TYPE_AUTOREF = OPTIGA_META_VALUE(OPTIGA_DATA_TYPE_AUTOREF);
static const optiga_metadata_item TYPE_PRESSEC = OPTIGA_META_VALUE(OPTIGA_DATA_TYPE_PRESSEC);
...
optiga_sign_result optiga_sign(
    uint8_t index, const uint8_t *digest, size_t digest_size, uint8_t *signature,
    size_t max_sig_size, size_t *sig_size)
{
    ...
    optiga_result res = optiga_calc_sign(
        OPTIGA_OID_ECC_KEY + index, digest, digest_size, &signature[2], max_sig_size - 2, sig_size);
    if (res != OPTIGA_SUCCESS) {
        uint8_t error_code = 0;
        if (res == OPTIGA_ERR_CMD && optiga_get_error_code(&error_code) == OPTIGA_SUCCESS &&
...
bool optiga_cert_size(uint8_t index, size_t *cert_size)
{
    *cert_size = 0;
    ...
    optiga_result ret = optiga_get_data_object(
        OPTIGA_OID_CERT + index, true, metadata_bytes, sizeof(metadata_bytes), &metadata_size);
...
bool optiga_read_cert(uint8_t index, uint8_t *cert, size_t max_cert_size, size_t *cert_size)
{
    if (index >= OPTIGA_CERT_COUNT) {
        return false;
    }
    ...
    optiga_result ret =
        optiga_get_data_object(OPTIGA_OID_CERT + index, false, cert, max_cert_size, cert_size);
...
{
    size_t size = 0;
    optiga_result ret = optiga_get_data_object(OPTIGA_OID_SEC, false, sec, sizeof(uint8_t), &size);
...
void optiga_set_sec_max(void)
{
    uint8_t invalid_point[] = {
        0x03, 0x42, 0x00, 0x04, 0xe2, 0x67, 0x5b, 0xe0, 0xbb, 0xf4, 0xfb, 0x9d, 0xec, 0xaa,
        0x1e, 0x96, 0xac, 0xc8, 0xa7, 0xca, 0xd0, 0x05, 0x84, 0xfe, 0xfd, 0x7f, 0x24, 0xc6,
        0xe7, 0x72, 0x5b, 0x56, 0xb3, 0x45, 0x06, 0x67, 0xbc, 0x73, 0xe3, 0xb8, 0xf5, 0x5d,
        0x1c, 0xad, 0xa0, 0x3e, 0x59, 0x1b, 0x3b, 0x9c, 0x6e, 0xc4, 0xb6, 0xd1, 0x05, 0xf7,
        0xd8, 0xc0, 0x67, 0x0d, 0xfb, 0xcc, 0xea, 0xb1, 0x65, 0xdb, 0xa6, 0x5f};
    uint8_t buffer[32] = {0};
    size_t size = 0;
    optiga_calc_ssec(
        OPTIGA_CURVE_P256, OID_PIN_ECDH, invalid_point, sizeof(invalid_point), buffer,
        sizeof(buffer), &size);
}
...
uint32_t optiga_estimate_time_ms(storage_pin_op_t op)
{
    ...
    uint32_t throttling_delay = sec > 127 ? (sec - 127) * OPTIGA_T_MAX_MS / 128 : 0;
...
static bool read_metadata(uint16_t oid, optiga_metadata *metadata)
{
    ...
    size_t size = 0;
    if (optiga_get_data_object(oid, true, serialized, sizeof(serialized), &size) !=
        OPTIGA_SUCCESS) {
        return false;
    }
...
static bool write_metadata(uint16_t oid, const optiga_metadata *metadata)
{
    ...
    if (optiga_serialize_metadata(metadata, serialized, sizeof(serialized), &size) !=
        OPTIGA_SUCCESS) {
        return false;
    }
...
static bool optiga_pin_init_metadata(void)
{
    ...
    if (write_metadata(OID_PIN_TOTAL_CTR, &metadata)) {
        optiga_result res = optiga_set_data_object(
            OID_PIN_TOTAL_CTR, false, PIN_TOTAL_CTR_INIT, sizeof(PIN_TOTAL_CTR_INIT));
        if (res != OPTIGA_SUCCESS) {
            return false;
        }
    }
...
static bool optiga_pin_init_stretch(void)
{
    ...
    size_t size = 0;
    return optiga_gen_key_pair(
               OPTIGA_CURVE_P256, OPTIGA_KEY_USAGE_KEYAGREE, OID_PIN_ECDH, public_key,
               sizeof(public_key), &size) == OPTIGA_SUCCESS;
}
    ...
    const uint8_t input[OPTIGA_PIN_SECRET_SIZE], bool version4)
{
    ...
    if (optiga_encrypt_sym(
            OPTIGA_SYM_MODE_CMAC, OID_PIN_CMAC, input, OPTIGA_PIN_SECRET_SIZE, buffer,
            sizeof(buffer), &size) != OPTIGA_SUCCESS) {
        ret = false;
        goto end;
    }
    ...
    if (version4) {
        ...
        if (optiga_encrypt_sym(
                OPTIGA_SYM_MODE_HMAC_SHA256, OID_PIN_HMAC_V4, input, OPTIGA_PIN_SECRET_SIZE, buffer,
                sizeof(buffer), &size) != OPTIGA_SUCCESS) {
            ret = false;
            goto end;
        }
    ...
    if (optiga_calc_ssec(
            OPTIGA_CURVE_P256, OID_PIN_ECDH, encoded_point, sizeof(encoded_point), buffer,
            sizeof(buffer), &size) != OPTIGA_SUCCESS) {
        ret = false;
        goto end;
    }
...
static bool optiga_pin_stretch_cmac_ecdh(
    optiga_ui_progress_t ui_progress, uint8_t stretched_pin[OPTIGA_PIN_SECRET_SIZE])
{
    // This step hardens the PIN verification process in case an attacker is able
    // to extract the secret value of a data object in Optiga that has a
    // particular configuration, but does not allow secret extraction for other
...
bool optiga_pin_set(optiga_ui_progress_t ui_progress, uint8_t stretched_pin[OPTIGA_PIN_SECRET_SIZE])
{
    ...
    if (optiga_set_data_object(OID_PIN_SECRET, false, pin_secret, sizeof(pin_secret)) !=
        OPTIGA_SUCCESS) {
        ret = false;
        goto end;
    }
    ...
    if (optiga_set_auto_state(
            OPTIGA_OID_SESSION_CTX, OID_PIN_SECRET, pin_secret, sizeof(pin_secret)) !=
        OPTIGA_SUCCESS) {
        ret = false;
        goto end;
    }
    ...
    uint8_t hmac_buffer[ENCRYPT_SYM_PREFIX_SIZE + OPTIGA_PIN_SECRET_SIZE] = {0x61, 0x00, 0x20};
    hmac_sha256(
        pin_hmac, sizeof(pin_hmac), digest, sizeof(digest), &hmac_buffer[ENCRYPT_SYM_PREFIX_SIZE]);

    // Stretch the PIN with the result.
    hmac_sha256(
        stretched_pin, OPTIGA_PIN_SECRET_SIZE, hmac_buffer, sizeof(hmac_buffer), stretched_pin);
    ...
    if (optiga_set_data_object(OID_STRETCHED_PIN, false, digest, sizeof(digest)) !=
        OPTIGA_SUCCESS) {
        ret = false;
        goto end;
    }
    ...
    if (optiga_set_data_object(
            OID_STRETCHED_PIN_CTR, false, COUNTER_RESET_EXTRA, sizeof(COUNTER_RESET_EXTRA)) !=
        OPTIGA_SUCCESS) {
        ret = false;
        goto end;
    }
    ...
    // OID_PIN_HMAC_CTR.
    if (optiga_set_auto_state(OPTIGA_OID_SESSION_CTX, OID_STRETCHED_PIN, digest, sizeof(digest)) !=
        OPTIGA_SUCCESS) {
        ret = false;
        goto end;
    }
    ...
    if (optiga_set_data_object(OID_PIN_HMAC, false, pin_hmac, sizeof(pin_hmac)) != OPTIGA_SUCCESS) {
        ret = false;
        goto end;
    }

    // Initialize the PIN counter which limits the use of OID_PIN_HMAC.
    if (optiga_set_data_object(OID_PIN_HMAC_CTR, false, COUNTER_RESET, sizeof(COUNTER_RESET)) !=
        OPTIGA_SUCCESS) {
        ret = false;
        goto end;
    }
    ...
    hmac_sha256(
        stretched_pin, OPTIGA_PIN_SECRET_SIZE, pin_secret, sizeof(pin_secret), stretched_pin);
...
optiga_pin_result optiga_pin_verify_v4(
    optiga_ui_progress_t ui_progress, const uint8_t pin_secret[OPTIGA_PIN_SECRET_SIZE],
    uint8_t out_secret[OPTIGA_PIN_SECRET_SIZE])
{
    ...
    optiga_result res = optiga_set_auto_state(
        OPTIGA_OID_SESSION_CTX, OID_STRETCHED_PIN, stretched_pin, sizeof(stretched_pin));
    if (res != OPTIGA_SUCCESS) {
        uint8_t error_code = 0;
        if (res != OPTIGA_ERR_CMD || optiga_get_error_code(&error_code) != OPTIGA_SUCCESS) {
            ret = OPTIGA_PIN_ERROR;
    ...
    size_t size = 0;
    if (optiga_get_data_object(OID_PIN_SECRET, false, out_secret, OPTIGA_PIN_SECRET_SIZE, &size) !=
            OPTIGA_SUCCESS ||
    ...
    if (optiga_set_auto_state(
            OPTIGA_OID_SESSION_CTX, OID_PIN_SECRET, out_secret, OPTIGA_PIN_SECRET_SIZE) !=
        OPTIGA_SUCCESS) {
        ret = OPTIGA_PIN_ERROR;
        goto end;
    }
    ...
    hmac_sha256(pin_secret, OPTIGA_PIN_SECRET_SIZE, out_secret, OPTIGA_PIN_SECRET_SIZE, out_secret);
...
static optiga_pin_result optiga_pin_stretch_hmac(uint8_t stretched_pin[OPTIGA_PIN_SECRET_SIZE])
{
    optiga_pin_result ret = OPTIGA_PIN_SUCCESS;
    ...
    optiga_result res = optiga_encrypt_sym(
        OPTIGA_SYM_MODE_HMAC_SHA256, OID_PIN_HMAC, digest, sizeof(digest), hmac_buffer,
        sizeof(hmac_buffer), &size);
    if (res != OPTIGA_SUCCESS) {
        uint8_t error_code = 0;
        if (res == OPTIGA_ERR_CMD && optiga_get_error_code(&error_code) == OPTIGA_SUCCESS &&
            error_code == OPTIGA_ERR_CODE_ACCESS_COND) {
    ...
    // Stretch the PIN with the result.
    hmac_sha256(stretched_pin, OPTIGA_PIN_SECRET_SIZE, hmac_buffer, size, stretched_pin);
    ...
    optiga_ui_progress_t ui_progress, uint8_t stretched_pin[OPTIGA_PIN_SECRET_SIZE])
{
    ...
    optiga_result res =
        optiga_set_auto_state(OPTIGA_OID_SESSION_CTX, OID_STRETCHED_PIN, digest, sizeof(digest));
    if (res != OPTIGA_SUCCESS) {
        uint8_t error_code = 0;
        if (res != OPTIGA_ERR_CMD || optiga_get_error_code(&error_code) != OPTIGA_SUCCESS) {
            ret = OPTIGA_PIN_ERROR;
    ...
    uint8_t pin_secret[OPTIGA_PIN_SECRET_SIZE] = {0};
    size_t size = 0;
    if (optiga_get_data_object(OID_PIN_SECRET, false, pin_secret, OPTIGA_PIN_SECRET_SIZE, &size) !=
        OPTIGA_SUCCESS) {
        ret = OPTIGA_PIN_ERROR;
        goto end;
    }
    ...
    hmac_sha256(stretched_pin, OPTIGA_PIN_SECRET_SIZE, pin_secret, size, stretched_pin);
    ...
    if (optiga_set_data_object(
            OID_STRETCHED_PIN_CTR, false, COUNTER_RESET, sizeof(COUNTER_RESET)) != OPTIGA_SUCCESS) {
        ret = OPTIGA_PIN_ERROR;
        goto end;
    }
...
static bool optiga_get_counter_rem(uint16_t oid, uint32_t *ctr)
{
    ...
    if (optiga_get_data_object(oid, false, counter, sizeof(counter), &counter_size) !=
            OPTIGA_SUCCESS ||
...
bool optiga_pin_get_rem(uint32_t *ctr)
{
    ...
    if (ctr1 > ctr2) {
        if (optiga_count_data_object(OID_PIN_HMAC_CTR, ctr1 - ctr2) != OPTIGA_SUCCESS) {
            return false;
        }
        *ctr = ctr2;
    } else if (ctr2 > ctr1) {
        if (optiga_count_data_object(OID_STRETCHED_PIN_CTR, ctr2 - ctr1) != OPTIGA_SUCCESS) {
            return false;
        }
        *ctr = ctr1;
    } else {
...
bool optiga_pin_decrease_rem_v4(uint32_t count)
{
    ...
    return optiga_count_data_object(OID_STRETCHED_PIN_CTR, count) == OPTIGA_SUCCESS;
}
...
bool optiga_pin_decrease_rem(uint32_t count)
{
    ...
    return optiga_count_data_object(OID_PIN_HMAC_CTR, count) == OPTIGA_SUCCESS &&
           optiga_count_data_object(OID_STRETCHED_PIN_CTR, count) == OPTIGA_SUCCESS;
}
