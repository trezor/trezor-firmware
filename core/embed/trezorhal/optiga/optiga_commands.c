...
const optiga_metadata_item OPTIGA_META_LCS_OPERATIONAL = OPTIGA_META_VALUE(OPTIGA_LCS_OP);
const optiga_metadata_item OPTIGA_META_ACCESS_ALWAYS = OPTIGA_META_VALUE(OPTIGA_ACCESS_COND_ALW);
const optiga_metadata_item OPTIGA_META_ACCESS_NEVER = OPTIGA_META_VALUE(OPTIGA_ACCESS_COND_NEV);
const optiga_metadata_item OPTIGA_META_KEY_USE_ENC = OPTIGA_META_VALUE(OPTIGA_KEY_USAGE_ENC);
const optiga_metadata_item OPTIGA_META_KEY_USE_KEYAGREE =
    OPTIGA_META_VALUE(OPTIGA_KEY_USAGE_KEYAGREE);
const optiga_metadata_item OPTIGA_META_VERSION_DEFAULT = {(const uint8_t[]){0x00, 0x00}, 2};
...
static optiga_result process_output_varlen(uint8_t *data, size_t max_data_size, size_t *data_size)
{
    uint8_t *out_data = NULL;
    size_t out_size = 0;
    optiga_result ret = process_output(&out_data, &out_size);
...
static void write_prefixed_data(uint8_t **ptr, const uint8_t *data, size_t data_size)
{
...
optiga_result optiga_parse_metadata(
    const uint8_t *serialized, size_t serialized_size, optiga_metadata *metadata)
{
    memzero(metadata, sizeof(*metadata));

    if (serialized_size < 2 || serialized[0] != 0x20 || serialized[1] + 2 != serialized_size) {
...
optiga_result optiga_serialize_metadata(
    const optiga_metadata *metadata, uint8_t *serialized, size_t max_serialized,
    size_t *serialized_size)
{
    *serialized_size = 0;
    ...
    for (int i = 0; i < METADATA_TAG_COUNT; ++i) {
        optiga_metadata_item *item = (void *)((char *)metadata + METADATA_OFFSET_TAG_MAP[i].offset);
        if (item->ptr == NULL) {
            continue;
        }
...
bool optiga_compare_metadata(const optiga_metadata *expected, const optiga_metadata *stored)
{
    for (int i = 0; i < METADATA_TAG_COUNT; ++i) {
...
optiga_result optiga_open_application(void)
{
    ...
    optiga_result ret =
        optiga_execute_command(OPEN_APP, sizeof(OPEN_APP), tx_buffer, sizeof(tx_buffer), &tx_size);
    if (ret != OPTIGA_SUCCESS) {
        return ret;
    }
...
optiga_result optiga_get_error_code(uint8_t *error_code)
{
    ...
    *(ptr++) = 0x00;  // get data
    ...
    optiga_result ret =
        optiga_execute_command(tx_buffer, tx_size, tx_buffer, sizeof(tx_buffer), &tx_size);
    if (ret != OPTIGA_SUCCESS) {
        return ret;
    }
 ...
 */
optiga_result optiga_get_data_object(
    uint16_t oid, bool get_metadata, uint8_t *data, size_t max_data_size, size_t *data_size)
{
    tx_size = 6;
    uint8_t *ptr = tx_buffer;
 ...
 */
optiga_result optiga_set_data_object(
    uint16_t oid, bool set_metadata, const uint8_t *data, size_t data_size)
{
    tx_size = data_size + 8;
...
{
    if (random_size < OPTIGA_RANDOM_MIN_SIZE || random_size > OPTIGA_RANDOM_MAX_SIZE) {
        return OPTIGA_ERR_SIZE;
    }
 ...
 */
optiga_result optiga_encrypt_sym(
    optiga_sym_mode mode, uint16_t oid, const uint8_t *input, size_t input_size, uint8_t *output,
    size_t max_output_size, size_t *output_size)
{
 ...
 */
optiga_result optiga_set_auto_state(
    uint16_t nonce_oid, uint16_t key_oid, const uint8_t *key, size_t key_size)
{
    uint8_t nonce[16] = {0};
    ...
    *(ptr++) = 0x43;  // verification value tag
    ...
    OPTIGA_LOG(__func__, tx_buffer, tx_size)
    ret = optiga_execute_command(tx_buffer, tx_size, tx_buffer, sizeof(tx_buffer), &tx_size);
    if (ret != OPTIGA_SUCCESS) {
        return ret;
    }
...
optiga_result optiga_clear_auto_state(uint16_t key_oid)
{
    ...
    *(ptr++) = 0x43;        // verification value tag
    ...
    if (tx_size != 4 || tx_buffer[0] != 0xff || tx_buffer[2] != 0 || tx_buffer[3] != 0) {
        return OPTIGA_ERR_UNEXPECTED;
    }
 ...
 */
optiga_result optiga_calc_sign(
 ...
 */
optiga_result optiga_verify_sign(
    optiga_curve curve, const uint8_t *public_key, size_t public_key_size, const uint8_t *digest,
    size_t digest_size, const uint8_t *signature, size_t sig_size)
{
 ...
 * The public_key is encoded as a DER BIT STRING.
 */
optiga_result optiga_gen_key_pair(
    optiga_curve curve, optiga_key_usage usage, uint16_t oid, uint8_t *public_key,
    size_t max_public_key_size, size_t *public_key_size)
{
    tx_size = 13;
    uint8_t *ptr = tx_buffer;
    ...
    *(ptr++) = usage;
    ...
    return process_output_varlen(public_key, max_public_key_size, public_key_size);
}
 ...
 */
optiga_result optiga_gen_sym_key(optiga_aes algorithm, optiga_key_usage usage, uint16_t oid)
{
    tx_size = 13;
    uint8_t *ptr = tx_buffer;
 ...
 * The public_key is encoded as a DER BIT STRING.
 */
optiga_result optiga_calc_ssec(
    optiga_curve curve, uint16_t oid, const uint8_t *public_key, size_t public_key_size,
    uint8_t *secret, size_t max_secret_size, size_t *secret_size)
{
 ...
 */
optiga_result optiga_derive_key(
    optiga_key_derivation deriv, uint16_t oid, const uint8_t *salt, size_t salt_size, uint8_t *info,
...
optiga_result optiga_set_trust_anchor(void)
{
    ...
    const uint8_t TA_CERT[] = {
        ,
    };

    return optiga_set_data_object(OPTIGA_OID_CA_CERT, false, TA_CERT, sizeof(TA_CERT));
}
...
optiga_result optiga_set_priv_key(uint16_t oid, const uint8_t priv_key[32])
{
    ...
    optiga_result ret =
        optiga_get_data_object(oid, true, metadata_buffer, sizeof(metadata_buffer), &metadata_size);
    if (ret != OPTIGA_SUCCESS) {
        return ret;
    }
    ...
    if (metadata.version.ptr != NULL) {
        ...
        payload_version = ((metadata.version.ptr[0] & 0x7f) << 8) + metadata.version.ptr[1];
    }
    payload_version += 1;
    ...
    uint8_t sop_cmd1[145] = {
        0x83, 0x01, 0x00, 0x8d, 0x30, 0x00, 0x8a, 0x84, 0x43, 0xA1, 0x01, 0x26, 0xA1, 0x04,
        0x42, 0xE0, 0xE8, 0x58, 0x3C, 0x86, 0x01, 0xF6, 0xF6, 0x84, 0x22, 0x18, 0x23, 0x03,
        0x82, 0x03, 0x10, 0x82, 0x82, 0x20, 0x58, 0x25, 0x82, 0x18, 0x29, 0x58, 0x20, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0xF6, 0x82, 0x40, 0x42, 0xE0, 0xF2, 0x58, 0x40,
    };
    ...
    if (0 != ecdsa_sign_digest(&nist256p1, TA_PRIV_KEY, digest, &sop_cmd1[81], NULL, NULL)) {
    ...
    ret =
        optiga_execute_command(sop_cmd1, sizeof(sop_cmd1), tx_buffer, sizeof(tx_buffer), &tx_size);
    if (ret != OPTIGA_SUCCESS) {
        memzero(sop_cmd2, sizeof(sop_cmd2));
        return ret;
    }
    ...
    ret =
        optiga_execute_command(sop_cmd2, sizeof(sop_cmd2), tx_buffer, sizeof(tx_buffer), &tx_size);
    memzero(sop_cmd2, sizeof(sop_cmd2));
    if (ret != OPTIGA_SUCCESS) {
        return ret;
    }
