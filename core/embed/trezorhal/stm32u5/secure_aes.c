    ...
    const uint8_t* input, size_t size, uint8_t* output, secure_aes_keysel_t key)
{
    ...
    if ((size_t)input % sizeof(uint32_t) != 0 || (size_t)output % sizeof(uint32_t) != 0) {
        ...
        while (tmp_size >= AES_BLOCK_SIZE) {
            ...
            if (HAL_CRYP_Encrypt(
                    &hcryp, input_buffer, AES_BLOCK_SIZE, output_buffer, HAL_MAX_DELAY) != HAL_OK) {
        ...
        if (HAL_CRYP_Encrypt(&hcryp, (uint32_t*)input, size, (uint32_t*)output, HAL_MAX_DELAY) !=
            HAL_OK) {
            return secfalse;
        }
    }
    ...
    const uint8_t* input, size_t size, uint8_t* output, secure_aes_keysel_t key)
{
    ...
    if ((size_t)input % sizeof(uint32_t) != 0 || (size_t)output % sizeof(uint32_t) != 0) {
        ...
        if (HAL_CRYP_Decrypt(&hcryp, (uint32_t*)input, size, (uint32_t*)output, HAL_MAX_DELAY) !=
            HAL_OK) {
            return secfalse;
        }
    }
