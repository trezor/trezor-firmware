...
void HAL_DMA_XferCpltCallback(DMA_HandleTypeDef *hdma)
{
    ...
    if (dma_transfer_remaining > 0) {
        uint32_t data_to_send =
            dma_transfer_remaining > MAX_DATA_SIZE ? MAX_DATA_SIZE : dma_transfer_remaining;

        HAL_DMA_Start_IT(
            hdma, (uint32_t) & ((uint8_t *)data_src)[dma_data_transferred], (uint32_t)data_dst,
            data_to_send);
    }
}
...
bool bg_copy_pending(void)
{
    return dma_transfer_remaining > 0;
}
...
void bg_copy_start_const_out_8(
    const uint8_t *src, uint8_t *dst, size_t size, bg_copy_callback_t callback)
{
