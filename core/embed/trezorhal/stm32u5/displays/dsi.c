...
__attribute__((section(".fb1"))) ALIGN_32BYTES(uint32_t PhysFrameBuffer0[184320]);
__attribute__((section(".fb2"))) ALIGN_32BYTES(uint32_t PhysFrameBuffer1[184320]);
...
int32_t BSP_LCD_DisplayOn(uint32_t Instance)
{
    ...
    if (Instance >= LCD_INSTANCES_NBR) {
        ...
        if (HAL_DSI_ShortWrite(
                &hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_DISPLAY_ON, 0x00) != HAL_OK) {
            status = BSP_ERROR_WRONG_PARAM;
        }
    }
...
int32_t BSP_LCD_DisplayOff(uint32_t Instance)
{
    ...
    if (Instance >= LCD_INSTANCES_NBR) {
        ...
        /* Set the display off */
        if (HAL_DSI_ShortWrite(
                &hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_DISPLAY_OFF, 0x00) != HAL_OK) {
            status = BSP_ERROR_WRONG_PARAM;
        }
    }
...
MX_LTDC_ConfigLayer(LTDC_HandleTypeDef *hltdc, uint32_t LayerIndex, uint32_t fb_addr)
{
    LTDC_LayerCfgTypeDef LayerCfg = {0};
...
int32_t BSP_LCD_GFXMMU_RegisterDefaultMspCallbacks(uint32_t Instance)
{
    ...
    if (Instance >= LCD_INSTANCES_NBR) {
        ...
        if (HAL_GFXMMU_RegisterCallback(&hlcd_gfxmmu, HAL_GFXMMU_MSPINIT_CB_ID, GFXMMU_MspInit) !=
            HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else if (
...
int32_t BSP_LCD_GFXMMU_RegisterMspCallbacks(uint32_t Instance, BSP_LCD_GFXMMU_Cb_t *Callback)
{
    ...
    if (Instance >= LCD_INSTANCES_NBR) {
        ...
        if (HAL_GFXMMU_RegisterCallback(
                &hlcd_gfxmmu, HAL_GFXMMU_MSPINIT_CB_ID, Callback->pMspGfxmmuInitCb) != HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else if (
            HAL_GFXMMU_RegisterCallback(
                &hlcd_gfxmmu, HAL_GFXMMU_MSPDEINIT_CB_ID, Callback->pMspGfxmmuDeInitCb) != HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else {
            LcdGfxmmu_IsMspCbValid[Instance] = 1U;
        }
...
int32_t BSP_LCD_LTDC_RegisterDefaultMspCallbacks(uint32_t Instance)
{
    ...
    if (Instance >= LCD_INSTANCES_NBR) {
        ...
        if (HAL_LTDC_RegisterCallback(&hlcd_ltdc, HAL_LTDC_MSPINIT_CB_ID, LTDC_MspInit) != HAL_OK) {
            ...
            HAL_LTDC_RegisterCallback(&hlcd_ltdc, HAL_LTDC_MSPDEINIT_CB_ID, LTDC_MspDeInit) !=
            HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else {
...
int32_t BSP_LCD_LTDC_RegisterMspCallbacks(uint32_t Instance, BSP_LCD_LTDC_Cb_t *Callback)
{
    ...
    if (Instance >= LCD_INSTANCES_NBR) {
        ...
        /* Register MspInit/MspDeInit Callbacks */
        if (HAL_LTDC_RegisterCallback(
                &hlcd_ltdc, HAL_LTDC_MSPINIT_CB_ID, Callback->pMspLtdcInitCb) != HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else if (
            HAL_LTDC_RegisterCallback(
                &hlcd_ltdc, HAL_LTDC_MSPDEINIT_CB_ID, Callback->pMspLtdcDeInitCb) != HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else {
            LcdLtdc_IsMspCbValid[Instance] = 1U;
        }
...
int32_t BSP_LCD_DSI_RegisterDefaultMspCallbacks(uint32_t Instance)
{
    ...
    if (Instance >= LCD_INSTANCES_NBR) {
        ...
        if (HAL_DSI_RegisterCallback(&hlcd_dsi, HAL_DSI_MSPINIT_CB_ID, DSI_MspInit) != HAL_OK) {
            ...
            HAL_DSI_RegisterCallback(&hlcd_dsi, HAL_DSI_MSPDEINIT_CB_ID, DSI_MspDeInit) != HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else {
...
int32_t BSP_LCD_DSI_RegisterMspCallbacks(uint32_t Instance, BSP_LCD_DSI_Cb_t *Callback)
{
    ...
    if (Instance >= LCD_INSTANCES_NBR) {
        ...
        /* Register MspInit/MspDeInit Callbacks */
        if (HAL_DSI_RegisterCallback(&hlcd_dsi, HAL_DSI_MSPINIT_CB_ID, Callback->pMspDsiInitCb) !=
            HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else if (
            HAL_DSI_RegisterCallback(
                &hlcd_dsi, HAL_DSI_MSPDEINIT_CB_ID, Callback->pMspDsiDeInitCb) != HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else {
            LcdDsi_IsMspCbValid[Instance] = 1U;
        }
...
int32_t BSP_LCD_DMA2D_RegisterDefaultMspCallbacks(uint32_t Instance)
{
    ...
    if (Instance >= LCD_INSTANCES_NBR) {
        ...
        /* Register default MspInit/MspDeInit Callback */
        if (HAL_DMA2D_RegisterCallback(&hlcd_dma2d, HAL_DMA2D_MSPINIT_CB_ID, DMA2D_MspInit) !=
            HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else if (
            HAL_DMA2D_RegisterCallback(&hlcd_dma2d, HAL_DMA2D_MSPDEINIT_CB_ID, DMA2D_MspDeInit) !=
            HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else {
...
int32_t BSP_LCD_DMA2D_RegisterMspCallbacks(uint32_t Instance, BSP_LCD_DMA2D_Cb_t *Callback)
{
    ...
    if (Instance >= LCD_INSTANCES_NBR) {
        ...
        /* Register MspInit/MspDeInit Callbacks */
        if (HAL_DMA2D_RegisterCallback(
                &hlcd_dma2d, HAL_DMA2D_MSPINIT_CB_ID, Callback->pMspDma2dInitCb) != HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else if (
            HAL_DMA2D_RegisterCallback(
                &hlcd_dma2d, HAL_DMA2D_MSPDEINIT_CB_ID, Callback->pMspDma2dDeInitCb) != HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else {
            LcdDma2d_IsMspCbValid[Instance] = 1U;
        }
...
static int32_t LCD_Init(void)
{
    ...
    if (status == BSP_ERROR_NONE) {
        ...
        /* Initialize LUT */
            ...
            HAL_GFXMMU_ConfigLut(&hlcd_gfxmmu, 0, LCD_WIDTH, (uint32_t)&gfxmmu_lut) != HAL_OK) {
            status = BSP_ERROR_PERIPH_FAILURE;
        } else {
    ...
    if (status == BSP_ERROR_NONE) {
            ...
            if (MX_LTDC_ClockConfig(&hlcd_ltdc) != HAL_OK) {
                ...
                if (status == BSP_ERROR_NONE) {
                    ...
                    if (MX_LTDC_Init(&hlcd_ltdc) != HAL_OK) {
                        ...
                        if (MX_LTDC_ConfigLayer(
                                &hlcd_ltdc, LTDC_LAYER_1, GFXMMU_VIRTUAL_BUFFER0_BASE_S) !=
                            HAL_OK) {
                            status = BSP_ERROR_PERIPH_FAILURE;
                        }
                    }
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 3, 0xB9, InitParam1) !=
            HAL_OK) {
            ErrorNumber++;
        }
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 16, 0xB1, InitParam2) !=
            HAL_OK) {
            ErrorNumber++;
        }
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 9, 0xB2, InitParam3) !=
            HAL_OK) {
            ErrorNumber++;
        }
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 10, 0xB4, InitParam4) !=
            HAL_OK) {
            ErrorNumber++;
        }
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 4, 0xC7, InitParam5) !=
            HAL_OK) {
            ErrorNumber++;
        }

        /* Set Panel Related Registers */
        if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, 0xCC, 0x02) != HAL_OK) {
            ErrorNumber++;
        }

        if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, 0xD2, 0x77) != HAL_OK) {
            ErrorNumber++;
        }

        uint8_t InitParam6[37] = {0x00, 0x07, 0x00, 0x00, 0x00, 0x08, 0x08, 0x32, 0x10, 0x01,
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 37, 0xD3, InitParam6) !=
            HAL_OK) {
            ErrorNumber++;
        }
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 34, 0xD5, InitParam7) !=
            HAL_OK) {
            ErrorNumber++;
        }

        uint8_t InitParam8[32] = {0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x19, 0x19, 0x18,
                                  0x18, 0x19, 0x19, 0x18, 0x18, 0x06, 0x07, 0x04, 0x05, 0x02, 0x03,
                                  0x00, 0x01, 0x20, 0x21, 0x22, 0x23, 0x18, 0x18, 0x18, 0x18};
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 35, 0xD6, InitParam8) !=
            HAL_OK) {
            ErrorNumber++;
        }

        /* SET GAMMA */
        uint8_t InitParam9[42] = {0x00, 0x16, 0x1B, 0x30, 0x36, 0x3F, 0x24, 0x40, 0x09, 0x0D, 0x0F,
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 42, 0xE0, InitParam9) !=
            HAL_OK) {
            ErrorNumber++;
        }

        uint8_t InitParam10[3] = {0x2C, 0x2C, 00};
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 3, 0xB6, InitParam10) !=
            HAL_OK) {
            ErrorNumber++;
        }

        if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, 0xBD, 0x00) != HAL_OK) {
            ErrorNumber++;
        }

        uint8_t InitParam11[] = {0x01, 0x00, 0x07, 0x0F, 0x16, 0x1F, 0x27, 0x30, 0x38, 0x40, 0x47,
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 42, 0xC1, InitParam11) !=
            HAL_OK) {
            ErrorNumber++;
        }

        if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, 0xBD, 0x01) != HAL_OK) {
            ErrorNumber++;
        }

        uint8_t InitParam12[] = {0x00, 0x08, 0x0F, 0x16, 0x1F, 0x28, 0x31, 0x39, 0x41, 0x48, 0x51,
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 42, 0xC1, InitParam12) !=
            HAL_OK) {
            ErrorNumber++;
        }

        if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, 0xBD, 0x02) != HAL_OK) {
            ErrorNumber++;
        }

        uint8_t InitParam13[42] = {0x00, 0x09, 0x0F, 0x18, 0x21, 0x2A, 0x34, 0x3C, 0x45, 0x4C, 0x56,
        ...
        if (HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, 42, 0xC1, InitParam13) !=
            HAL_OK) {
            ErrorNumber++;
        }
        ...
        /* Exit Sleep Mode*/
        if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P0, 0x11, 0x00) != HAL_OK) {
            ErrorNumber++;
        }
        ...
        /* Display On */
        if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P0, 0x29, 0x00) != HAL_OK) {
            ErrorNumber++;
        }
...
static int32_t LCD_DeInit(void)
{
    ...
    /* Set display off */
    if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_DISPLAY_OFF, 0x00) !=
        HAL_OK) {
        ErrorNumber++;
    }
    ...
    if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P0, DSI_ENTER_SLEEP_MODE, 0x0) !=
        HAL_OK) {
        ErrorNumber++;
    }
...
void display_pixeldata(uint16_t c)
{
    ...
    address = (address + (4 * ((cursor_y + 120) * PIXEL_PER_LINE + (cursor_x + 120))));
...
{
    return window_x1 - window_x0 + 1;
}

uint16_t display_get_window_height(void)
{
    return window_y1 - window_y0 + 1;
}
...
void display_init(void)
{
    ...
    memset(PhysFrameBuffer0, 0x00, sizeof(PhysFrameBuffer0));
    memset(PhysFrameBuffer1, 0x00, sizeof(PhysFrameBuffer1));
    memcpy(gfxmmu_lut, gfxmmu_lut_config_argb8888, sizeof(gfxmmu_lut_config_argb8888));
