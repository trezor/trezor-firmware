...
static bool drv2625_set_reg(uint8_t addr, uint8_t value)
{
    ...
    return i2c_transmit(DRV2625_I2C_INSTANCE, DRV2625_I2C_ADDRESS, data, sizeof(data), 1) == HAL_OK;
}
...
bool haptic_init(void)
{
    ...
    // select library
    if (!drv2625_set_reg(DRV2625_REG_LIBRARY, LIB_SEL | DRV2625_REG_LIBRARY_GAIN_25)) {
        return false;
    }

    if (!drv2625_set_reg(
            DRV2625_REG_LRAERM, LRA_ERM_SEL | LOOP_SEL | DRV2625_REG_LRAERM_AUTO_BRK_OL)) {
    }
    ...
    if (!drv2625_set_reg(DRV2625_REG_LRA_WAVE_SHAPE, DRV2625_REG_LRA_WAVE_SHAPE_SINE)) {
        return false;
    }

    if (!drv2625_set_reg(DRV2625_REG_OL_LRA_PERIOD_LO, ACTUATOR_LRA_PERIOD & 0xFF)) {
        return false;
    }

    if (!drv2625_set_reg(DRV2625_REG_OL_LRA_PERIOD_HI, ACTUATOR_LRA_PERIOD >> 8)) {
        return false;
    }
...
static bool haptic_play_rtp(int8_t amplitude, uint16_t duration_ms)
{
    ...
    if (!driver->playing_rtp) {
        if (!drv2625_set_reg(
                DRV2625_REG_MODE, DRV2625_REG_MODE_RTP | DRV2625_REG_MODE_TRGFUNC_ENABLE)) {
            return false;
        }
...
bool haptic_play_custom(int8_t amplitude_pct, uint16_t duration_ms)
{
    ...
    return haptic_play_rtp((int8_t)((amplitude_pct * MAX_AMPLITUDE) / 100), duration_ms);
}
