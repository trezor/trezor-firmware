...
void mpu_config_bootloader(void) {
  ...
  MPU->RLAR = REGION_END(storage_start + storage_size - 1) | ATTR_IDX_FLASH;
  ...
  MPU->RBAR = FLASH_OTP_BASE | LL_MPU_REGION_ALL_RW |
              LL_MPU_INSTRUCTION_ACCESS_DISABLE | SHAREABILITY_FLASH;
  MPU->RLAR = REGION_END(FLASH_OTP_BASE + FLASH_OTP_SIZE - 1) | ATTR_IDX_FLASH;
...
void mpu_config_firmware(void) {
  ...
  MPU->RLAR = REGION_END(FLASH_BASE + 0x40000 - 1) | ATTR_IDX_FLASH;
