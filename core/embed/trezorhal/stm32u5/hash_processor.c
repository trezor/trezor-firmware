...
void hash_processor_init(void)
{
    ...
    DMA_Handle.Init.TransferAllocatedPort = DMA_SRC_ALLOCATED_PORT1 | DMA_DEST_ALLOCATED_PORT0;
    DMA_Handle.Init.TransferEventMode = DMA_TCEM_BLOCK_TRANSFER;
    DMA_Handle.Init.Mode = DMA_NORMAL;
    HAL_DMA_Init(&DMA_Handle);
    HAL_DMA_ConfigChannelAttributes(
        &DMA_Handle, DMA_CHANNEL_SEC | DMA_CHANNEL_SRC_SEC | DMA_CHANNEL_DEST_SEC);
...
static void hash_processor_sha256_calc_dma(const uint8_t *data, uint32_t len, uint8_t *hash)
{
    while (len > 0) {
...
void hash_processor_sha256_calc(const uint8_t *data, uint32_t len, uint8_t *hash)
{
...
void hash_processor_sha256_update(hash_sha265_context_t *ctx, const uint8_t *data, uint32_t len)
{
    if (ctx->length > 0) {
        ...
        if (ctx->length == HASH_SHA256_BUFFER_SIZE) {
            HAL_HASHEx_SHA256_Accmlt(&hhash, (uint8_t *)ctx->buffer, HASH_SHA256_BUFFER_SIZE);
            ctx->length = 0;
...
void hash_processor_sha256_final(hash_sha265_context_t *ctx, uint8_t *output)
{
    ...
    HAL_HASHEx_SHA256_Accmlt_End(
        &hhash, (uint8_t *)ctx->buffer, ctx->length, (uint8_t *)tmp_out, 1000);
    ctx->length = 0;
    memzero(ctx->buffer, HASH_SHA256_BUFFER_SIZE);
