...
{
    uint8_t *addr = (uint8_t *)flash_area_get_address(&SECRET_AREA, 0, sizeof(SECRET_HEADER_MAGIC));
    ...
    bootloader_locked =
        memcmp(addr, SECRET_HEADER_MAGIC, sizeof(SECRET_HEADER_MAGIC)) == 0 ? sectrue : secfalse;
...
void secret_write(const uint8_t *data, uint32_t offset, uint32_t len)
{
    ...
    for (int i = 0; i < len / 16; i++) {
            ...
            flash_area_write_quadword(&SECRET_AREA, offset + (i * 16), (uint32_t *)&data[(i * 16)]),
            "secret write");
    }
...
{
    return ((TAMP_S->SECCFGR & TAMP_SECCFGR_BHKLOCK) == TAMP_SECCFGR_BHKLOCK) * sectrue;
}
...
void secret_bhk_regenerate(void)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        secbool res = flash_area_write_quadword(&BHK_AREA, i * 4 * sizeof(uint32_t), val);
...
static void secret_optiga_cache(void)
{
    ...
    secbool ok = secret_read((uint8_t *)secret, SECRET_OPTIGA_KEY_OFFSET, SECRET_OPTIGA_KEY_LEN);
...
secbool secret_optiga_set(const uint8_t secret[SECRET_OPTIGA_KEY_LEN])
{
    ...
    if (sectrue !=
        secure_aes_ecb_encrypt_hw(secret, sizeof(secret_enc), secret_enc, SECURE_AES_KEY_DHUK)) {
        return secfalse;
    }
