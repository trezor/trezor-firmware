...
const void *flash_get_address(uint16_t sector, uint32_t offset, uint32_t size)
{
    ...
    uint32_t base_addr = flash_sector_is_secure(sector) ? FLASH_BASE_S : FLASH_BASE_NS;
...
secbool flash_sector_erase(uint16_t sector)
{
    ...
    const uint32_t *sector_start = (const uint32_t *)flash_get_address(sector, 0, 0);

    const uint32_t *sector_end = sector_start + flash_sector_size(sector, 1) / sizeof(uint32_t);
...
secbool flash_write_quadword(uint16_t sector, uint32_t offset, const uint32_t *data)
{
    uint32_t address = (uint32_t)flash_get_address(sector, offset, FLASH_QUADWORD_SIZE);
    if (address == 0) {
        return secfalse;
    }
    ...
    if (HAL_OK != HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, address, (uint32_t)data)) {
        return secfalse;
    }
...
secbool flash_write_burst(uint16_t sector, uint32_t offset, const uint32_t *data)
{
    uint32_t address = (uint32_t)flash_get_address(sector, offset, FLASH_BURST_SIZE);
    if (address == 0) {
        return secfalse;
    }
    ...
    if (HAL_OK != HAL_FLASH_Program(FLASH_TYPEPROGRAM_BURST, address, (uint32_t)data)) {
        return secfalse;
    }
