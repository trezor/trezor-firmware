...
fn main() -> Result<()> {
    xbuild::build(|lib| {
        ...
        if cfg!(feature = "ward_service_thp") {
            if !cfg!(feature = "thp") {
                panic!("ward_service_thp needs thp: there is no THP stack to serve the interface");
            }
            lib.add_define("USE_WARD_SERVICE_THP", None);
        }
...
impl<'a> MpyBuilder<'a> {
    ...
    fn create_sed_scripts(&self) -> Result<Vec<String>> {
        ...
        let ward_service_thp = py_bool(cfg!(feature = "ward_service_thp"));
        ...
        let mut exprs: Vec<String> = vec![
            ...
            format!(r"s/utils\.USE_WARD_SERVICE_THP/{ward_service_thp}/g"),
