...
use xbuild::{CLibrary, InputFiles, Result, WrapErr, bail_unsupported};
...
fn main() -> Result<()> {
    xbuild::build(|lib| {
        ...
        let attrs = cfg!(not(feature = "emulator")).then(|| {
            let mut attrs = xbuild::CompileAttrs::default();
            attrs.add_flag("-O3");
            attrs
        });
        ...
        let mpy_builder = MpyBuilder::new(lib);
...
impl<'a> MpyBuilder<'a> {
    ...
    fn build_protobuf_headers(&self) -> Result<PathBuf> {
        ...
        xbuild::run_command(&mut cmd, inputs.as_paths(), [&output])
    ...
    fn build_protobuf_blobs(&self, qstrdefs_generated: &Path) -> Result<()> {
        ...
        xbuild::run_command(&mut cmd, inputs.as_paths(), &outputs)
    ...
    pub fn build_frozen_modules(&self, qstr_preprocessed: &Path) -> Result<PathBuf> {
        ...
        // Compile all modules to .mpy files using mpy-cross
        let compile_func = |file: &&PathBuf| self.build_frozen_module(file, &sed_scripts);
        let mpy_files = xbuild::run_parallel(py_files.as_paths(), compile_func)
            .context("Failed to build frozen modules")?;

        /*let mut mpy_files = Vec::new();
        for file in py_files.as_paths() {
            let mpy_file = self.build_frozen_module(file, &sed_scripts)?;
    ...
    fn build_frozen_module(&self, source: &Path, sed_scripts: &[String]) -> Result<PathBuf> {
        ...
        let mpy_cross = self.mpy_dir.join("mpy-cross/mpy-cross");
    ...
    fn get_py_files(&self) -> Result<InputFiles> {
        ...
        if cfg!(feature = "power_manager") {
            files.add(src.join("trezor/power_manager"), "*.py")?;
        }
        ...
        if cfg!(feature = "thp") && cfg!(feature = "pyopt") {
