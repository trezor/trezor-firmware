...
/// def slip44_id(self) -> int:
///     """
///     Return the SLIP-0044 coin type identifier for the application.
///     """
static mp_obj_t mod_trezorapp_slip44_id(mp_obj_t self) {
  mp_obj_AppImage_t *image = MP_OBJ_TO_PTR(self);

  app_image_info_t info;
  app_image_get_info_or_raise(image->handle, &info);

  return mp_obj_new_int(info.slip44_id);
}
static MP_DEFINE_CONST_FUN_OBJ_1(mod_trezorapp_slip44_id_obj,
                                 mod_trezorapp_slip44_id);
...
static const mp_rom_map_elem_t mod_trezorapp_AppImage_locals_dict_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_slip44_id), MP_ROM_PTR(&mod_trezorapp_slip44_id_obj)},
};
