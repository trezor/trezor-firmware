...
STATIC mp_obj_t mod_trezorcrypto_secp256k1_anti_exfil_commit_nonce(
  ...
  if (entropy_buffer.len != 32) {
    mp_raise_ValueError(MP_ERROR_TEXT("Invalid length of entropy commitment"));
  }
  vstr_t nonce_vstr = {0};
  ...
  if (0 != zkp_ecdsa_anti_exfil_commit_nonce(
    ...
    mp_raise_ValueError(MP_ERROR_TEXT("Anti-exfil commit failed"));
  }
...
STATIC mp_obj_t mod_trezorcrypto_secp256k1_anti_exfil_sign(mp_obj_t secret_key,
  ...
  if (entropy_buffer.len != 32) {
    mp_raise_ValueError(MP_ERROR_TEXT("Invalid length of entropy"));
  }
  ...
  if (0 != zkp_ecdsa_anti_exfil_sign_digest(
    ...
    mp_raise_ValueError(MP_ERROR_TEXT("Anti-exfil sign failed"));
  }
