...
font_glyph_iter_t font_glyph_iter_init(const int font, const uint8_t *text, const int len)
{
...
static uint16_t next_utf8_codepoint(font_glyph_iter_t *iter)
{
    ...
    if (iter->remaining >= 2 && ((iter->text[0] & 0b11100000) == 0b11000000) &&
        ...
        out = (((uint16_t)iter->text[0] & 0b00011111) << 6) | (iter->text[1] & 0b00111111);
        iter->text += 2;
        iter->remaining -= 2;
        return out;
    }
    ...
    if (iter->remaining >= 3 && ((iter->text[0] & 0b11110000) == 0b11100000) &&
        ...
        out = (((uint16_t)iter->text[0] & 0b00001111) << 12) |
              (((uint16_t)iter->text[1] & 0b00111111) << 6) | (iter->text[2] & 0b00111111);
        iter->text += 3;
        iter->remaining -= 3;
        return out;
    }
