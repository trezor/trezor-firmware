...
static int16_t unit_variant_get_build_year(void)
{
    ...
    secbool result = flash_otp_read(FLASH_OTP_BLOCK_BATCH, 0, data, FLASH_OTP_BLOCK_SIZE);
...
void unit_variant_init(void)
{
    ...
    secbool result = flash_otp_read(FLASH_OTP_BLOCK_DEVICE_VARIANT, 0, data, FLASH_OTP_BLOCK_SIZE);
...
{
    return unit_variant_color;
}

uint8_t unit_variant_get_packaging(void)
{
    return unit_variant_packaging;
}

bool unit_variant_get_btconly(void)
{
    return unit_variant_btconly;
}

bool unit_variant_present(void)
{
    return unit_variant_ok;
}
