...
void gfx_draw_text(gfx_offset_t pos, const char* text, size_t maxlen, const gfx_text_attr_t* attr)
{
    if (text == NULL) {
        return;
    }
    ...
    for (int i = 0; i < maxlen; i++) {
        ...
        gfx_draw_bitmap(gfx_rect(pos.x, pos.y, DISPLAY_RESX, DISPLAY_RESY), &bitmap);
...
void display_text(
    int x, int y, const char* text, int textlen, int font, uint16_t fg_color, uint16_t bg_color)
{
