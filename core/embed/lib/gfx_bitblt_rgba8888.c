...
#endif
    {
        ...
        if (bb->src_alpha == 255) {
            ...
            while (height-- > 0) {
                for (int x = 0; x < bb->width; x++) {
                    dst_ptr[x] =
                        gfx_color32_blend_a8(bb->src_fg, gfx_color32_to_color(dst_ptr[x]), alpha);
                }
                dst_ptr += bb->dst_stride / sizeof(*dst_ptr);
            }
        }
...
#endif
    {
        const gfx_color32_t* gradient = gfx_color32_gradient_a4(bb->src_fg, bb->src_bg);
...
#endif
    {
        ...
        while (height-- > 0) {
            for (int x = 0; x < bb->width; x++) {
                ...
                dst_ptr[x] =
                    gfx_color32_blend_a8(bb->src_fg, gfx_color32_to_color(dst_ptr[x]), fg_alpha);
            }
            dst_ptr += bb->dst_stride / sizeof(*dst_ptr);
            src_row += bb->src_stride / sizeof(*src_row);
        }
