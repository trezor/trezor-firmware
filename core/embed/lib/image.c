...
static secbool compute_pubkey(
    uint8_t sig_m, uint8_t sig_n, const uint8_t *const *pub, uint8_t sigmask,
...
secbool check_image_header_sig(
    const image_header *const hdr, uint8_t key_m, uint8_t key_n, const uint8_t *const *keys)
{
    ...
    return sectrue * (0 == ed25519_sign_open(
                               fingerprint, IMAGE_HASH_DIGEST_LENGTH, pub,
...
secbool check_vendor_header_sig(
    const vendor_header *const vhdr, uint8_t key_m, uint8_t key_n, const uint8_t *const *keys)
{
    ...
    return sectrue *
           (0 == ed25519_sign_open(
                     hash, IMAGE_HASH_DIGEST_LENGTH, pub, *(const ed25519_signature *)vhdr->sig));
}

secbool check_vendor_header_keys(const vendor_header *const vhdr)
{
    return check_vendor_header_sig(vhdr, BOOTLOADER_KEY_M, BOOTLOADER_KEY_N, BOOTLOADER_KEYS);
}
...
secbool check_image_contents(
    const image_header *const hdr, uint32_t firstskip, const flash_area_t *area)
{
    if (0 == area) {
        return secfalse;
    }
    ...
    const uint8_t *addr = (uint8_t *)flash_area_get_address(area, firstskip, padding_size);
    ...
    while (offset < end_offset) {
        size_t bytes_to_check =
            MIN(IMAGE_CHUNK_SIZE - (offset % IMAGE_CHUNK_SIZE), end_offset - offset);
        ...
        if (sectrue != check_single_hash(hdr->hashes + hash_offset, data, bytes_to_check)) {
            return secfalse;
        }
    ...
    if (offset < end_offset) {
        ...
        uint32_t expected_word =
            expected_byte << 24 | expected_byte << 16 | expected_byte << 8 | expected_byte;
        ...
        while (offset < end_offset) {
            ...
            const uint8_t *bytes =
                (const uint8_t *)flash_area_get_address(area, offset, single_bytes_to_check);
            if (!bytes) {
                return secfalse;
            }
    ...
    const uint8_t *header, size_t header_size, firmware_header_info_t *info)
{
    ...
    if ((ihdr = read_image_header(
             header + vhdr.hdrlen, FIRMWARE_IMAGE_MAGIC, FIRMWARE_IMAGE_MAXSIZE)) == NULL) {
        return secfalse;
    }
    if (sectrue != check_image_header_sig(ihdr, vhdr.vsig_m, vhdr.vsig_n, vhdr.vpub)) {
        return secfalse;
    }
