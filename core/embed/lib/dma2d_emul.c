...
void dma2d_start_blend(uint8_t* overlay_addr, uint8_t* bg_addr, uint8_t* out_addr, int32_t pixels)
{
    ...
    for (int i = 0; i < pixels; i++) {
        if (mode == DMA2D_MODE_4BPP_OVER_4BPP) {
            ...
            uint16_t final_odd_color = interpolate_color(dma2d_color, c_odd_bg, odd_overlay_pix);
            PIXELDATA(final_odd_color);

            uint8_t even_overlay_pix = c >> 4;
            uint8_t even_bg_pix = b >> 4;
            uint16_t c_even_bg = clut_bg[even_bg_pix];
            uint16_t final_even_color = interpolate_color(dma2d_color, c_even_bg, even_overlay_pix);
