...
int main(void)
{
    ...
    if (sectrue == firmware_present) {
        firmware_present = check_image_header_sig(hdr, vhdr.vsig_m, vhdr.vsig_n, vhdr.vpub);
    }
    if (sectrue == firmware_present) {
        firmware_present =
    ...
    ensure(read_vendor_header((const uint8_t *)FIRMWARE_START, &vhdr), "invalid vendor header");
    ...
    hdr = read_image_header(
        ...
        hdr == (const image_header *)(FIRMWARE_START + vhdr.hdrlen) ? sectrue : secfalse,
