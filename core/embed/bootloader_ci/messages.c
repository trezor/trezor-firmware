...
static void detect_installation(
    ...
    secbool *is_upgrade, secbool *is_downgrade_wipe)
{
    *is_new = secfalse;
    *is_upgrade = secfalse;
...
int process_msg_FirmwareUpload(uint8_t iface_num, uint32_t msg_size, uint8_t *buf)
{
    ...
    if (firmware_block == 0) {
        if (headers_offset == 0) {
            ...
            const image_header *received_hdr = read_image_header(
                chunk_buffer + vhdr.hdrlen, FIRMWARE_IMAGE_MAGIC, FIRMWARE_IMAGE_MAXSIZE);
            ...
            if (is_new == secfalse) {
                detect_installation(
                    &current_vhdr, current_hdr, &vhdr, &hdr, &is_new, &is_upgrade,
                    &is_downgrade_wipe);
            }
            ...
            ensure(flash_area_erase(&FIRMWARE_AREA, ui_screen_install_progress_erase), NULL);
        }
    }
    ...
    if (sectrue != check_single_hash(
                       hdr.hashes + firmware_block * 32, chunk_buffer + headers_offset,
    ...
    for (int i = 0; i < chunk_size / FLASH_BLOCK_SIZE; i++) {
            ...
            flash_area_write_block(
                &FIRMWARE_AREA, firmware_block * IMAGE_CHUNK_SIZE + i * FLASH_BLOCK_SIZE,
...
int process_msg_WipeDevice(uint8_t iface_num, uint32_t msg_size, uint8_t *buf)
{
    ...
    if (sectrue !=
        flash_area_erase_bulk(wipe_area, STORAGE_AREAS_COUNT + 1, ui_screen_wipe_progress)) {
        MSG_SEND_INIT(Failure);
        MSG_SEND_ASSIGN_VALUE(code, FailureType_Failure_ProcessError);
