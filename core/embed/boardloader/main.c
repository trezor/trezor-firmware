...
uint8_t get_bootloader_min_version(void)
{
    const uint8_t *counter_addr = flash_area_get_address(
        &SECRET_AREA, SECRET_MONOTONIC_COUNTER_OFFSET, SECRET_MONOTONIC_COUNTER_LEN);
...
void write_bootloader_min_version(uint8_t version)
{
    if (version > get_bootloader_min_version()) {
        for (int i = 0; i < version; i++) {
            uint32_t data[4] = {0};
            secret_write((uint8_t *)data, SECRET_MONOTONIC_COUNTER_OFFSET + i * 16, 16);
        }
    }
}
#endif

struct BoardCapabilities capabilities __attribute__((section(".capabilities_section"))) = {
...
static uint32_t check_sdcard(void)
{
    ...
    const secbool read_status =
        sdcard_read_blocks(sdcard_buf, 0, BOOTLOADER_IMAGE_MAXSIZE / SDCARD_BLOCK_SIZE);

    sdcard_power_off();

    if (sectrue == read_status) {
        const image_header *hdr = read_image_header(
            (const uint8_t *)sdcard_buf, BOOTLOADER_IMAGE_MAGIC, BOOTLOADER_IMAGE_MAXSIZE);
        ...
        if (sectrue !=
            check_image_header_sig(hdr, BOARDLOADER_KEY_M, BOARDLOADER_KEY_N, BOARDLOADER_KEYS)) {
            return 0;
        }

        _Static_assert(
            IMAGE_CHUNK_SIZE >= BOOTLOADER_IMAGE_MAXSIZE,
            "BOOTLOADER IMAGE MAXSIZE too large for IMAGE_CHUNK_SIZE");

        if (sectrue !=
            (check_single_hash(
                hdr->hashes, ((const uint8_t *)sdcard_buf) + hdr->hdrlen, hdr->codelen))) {
            return 0;
        }
...
static secbool copy_sdcard(void)
{
    ...
    term_printf("copying new bootloader from SD card\n\n");
        ...
        flash_area_write_data(&BOOTLOADER_AREA, 0, sdcard_buf, IMAGE_HEADER_SIZE + codelen), NULL);
...
int main(void)
{
    ...
    if (sectrue != flash_configure_option_bytes()) {
        ...
        const secbool r = flash_area_erase_bulk(STORAGE_AREAS, STORAGE_AREAS_COUNT, NULL);
        (void)r;
        return 2;
    }
    ...
    if ((old_hdr != NULL) &&
        (sectrue ==
         check_image_header_sig(old_hdr, BOARDLOADER_KEY_M, BOARDLOADER_KEY_N, BOARDLOADER_KEYS)) &&
        (sectrue == check_image_contents(old_hdr, IMAGE_HEADER_SIZE, &BOOTLOADER_AREA))) {
    ...
    const image_header *hdr = read_image_header(
        ...
        hdr == (const image_header *)BOOTLOADER_START ? sectrue : secfalse,
        ...
        check_image_contents(hdr, IMAGE_HEADER_SIZE, &BOOTLOADER_AREA), "invalid bootloader hash");

#ifdef STM32U5
    uint8_t bld_min_version = get_bootloader_min_version();
    ensure((hdr->monotonic >= bld_min_version) * sectrue, "BOOTLOADER DOWNGRADED");
