...
bool elf_load(const void* elf_ptr, size_t elf_size, void* ram_ptr,
  ...
  for (int i = 0; i < elf.ehdr->e_shnum; i++) {
    ...
    if (section_name != NULL) {
      if (strcmp(section_name, ".got") == 0) {
        got_addr = map_va(&elf, shdr->sh_addr);
  ...
  if (relsz > 0) {
    ...
    Elf32_Rel* rel = (Elf32_Rel*)(map_va(&elf, relva));
    Elf32_Rel* rel_end = rel + relsz / sizeof(Elf32_Rel);
  ...
  if (!systask_init(&applet->task, stack_base, stack_size, got_addr, applet)) {
    return false;
  }
