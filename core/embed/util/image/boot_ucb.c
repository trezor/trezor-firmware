...
void adjust_to_secure_flash(uint32_t* address) {
  if (*address < 0x0c000000) {  // !@# TODO: improve this
    *address += 0x0c000000 - 0x08000000;
  }
}
#endif
...
secbool boot_ucb_read(boot_ucb_t* ucb) {
  ...
  *ucb = *((const boot_ucb_t*)BOOTUCB_START);
  ...
  // Check if the signed part of the header is within the valid address range
  if (ucb->header_address < min_address ||
      ucb->header_address > max_address - sizeof(boot_header_signed_t)) {
    return secfalse;
  }

  const boot_header_signed_t* hdr = (boot_header_signed_t*)ucb->header_address;

  // Check if both signed and unsigned part of the header are within
