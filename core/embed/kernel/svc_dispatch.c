...
void svc_init(void) { NVIC_SetPriority(SVCall_IRQn, IRQ_PRI_LOWEST); }
...
void syscall_handler(uint32_t *args, uint32_t syscall) {
  switch (syscall) {
    ...
    case SYSCALL_STORAGE_INIT: {
      PIN_UI_WAIT_CALLBACK callback =
          NULL; /*(PIN_UI_WAIT_CALLBACK)args[0]; !@#*/
...
__attribute__((naked, no_stack_protector)) static void jump_to_unprivileged(
    uint32_t app_start) {
  __asm__ volatile(
      "ldr r12, [r0, #0]     \n"  // stack pointer
      "sub r12, r12, #32     \n"
