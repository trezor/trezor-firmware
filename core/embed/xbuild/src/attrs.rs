...
impl CompileAttrs {
    ...
    pub fn import_library_metadata(&mut self, lib_name: &str) -> Result<()> {
        ...
        if self.flags.is_empty() {
            // Add flags only if we don't already have any, to avoid duplication
            // in the case of diamond dependencies. This means that flags are
            // imported only on the first call.
