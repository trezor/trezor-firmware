...
use crate::helpers::{delete_file_if_exists, ensure_parent_directory};
    ...
    O: AsRef<Path>,
{
    ...
    run_if_changed(inputs, outputs, Some(args.as_str()), cc_dep, || {
        ...
        if !cmd_output.status.success() {
            bail!(format_command_error(cmd, &cmd_output));
        }
    ...
    F: FnOnce() -> Result<()>,
{
    ...
    if outputs.is_empty() {
        bail!("run_if_changed requires at least one output file");
    }
    ...
    if should_run {
        ...
        if let Some(cc_dep) = cc_dep {
            // Ensure the .d file was created by the command
            if !cc_dep.exists() {
                bail!(
                    "Expected dependency file {} was not created",
                    cc_dep.display()
                );
            }
            ...
            if let Ok(content) = fs::read_to_string(cc_dep) {
                for line in cc_dep_paths(&content) {
...
fn format_command_error(cmd: &std::process::Command, cmd_output: &std::process::Output) -> String {
