...
fn get_firmware_vendor() -> &'static str {
    ...
    } else if has_feature("bootloader_qa") || has_feature("bootloader_devel") {
...
fn link_as(lib: &CLibrary, binary_type: &str) -> Result<()> {
    let this_lib = links_name()?;

    if has_feature("emulator") {
        println!("cargo:rustc-link-lib={this_lib}");

        for dep in lib.get_libs() {
