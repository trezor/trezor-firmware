...
use crate::CLibrary;
use crate::helpers::{links_name, path_from_env, rust_analyser_is_running};
...
impl CLibrary {
    fn link_as(&self, binary_type: &str) -> Result<()> {
        ...
        if has_feature("emulator") {
            ...
            // Copy the memory layout linker script to the output directory
            // so we can read it when flashing the binary or generating the
            // combined binary.

            let workspace_dir = path_from_env("CARGO_MANIFEST_DIR")?;
            let memory_ld = workspace_dir.join("../..").join(&memory_ld);

            let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());
            let memory_ld_dst = out_dir.join(format!("../../../{binary_type}.mem.ld"));

            fs::copy(&memory_ld, &memory_ld_dst).with_context(|| {
                format!(
                    "Failed to copy `{}` to `{}`",
                    &memory_ld.display(),
                    memory_ld_dst.display()
                )
            })?;
