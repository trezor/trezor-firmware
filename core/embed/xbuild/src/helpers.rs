...
///
/// This derives the target directory from `OUT_DIR`, `PROFILE`, and `TARGET`
/// so it works for both native builds (`target/<profile>/...`) and
/// cross-compilation (`target/<triple>/<profile>/...`).
pub fn cargo_target_dir() -> Result<PathBuf> {
    let out_dir = path_from_env("OUT_DIR")?;
    let profile = env::var("PROFILE").wrap_err("Failed to get PROFILE")?;
...
pub fn is_rust_analyzer() -> bool {
    env::var("IS_RUST_ANALYZER").is_ok()
}

/// Emits `cargo:rerun-if-changed` directives for the given files.
pub fn emit_rerun_if_changed<I, P>(files: I)
where
