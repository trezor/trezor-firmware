...
def encode_der_signature(recoverable_signature: bytes) -> AnyBytes:
    assert len(recoverable_signature) == 65
    return encode_seq((recoverable_signature[1:33], recoverable_signature[33:65]))


def encode_raw_signature(recoverable_signature: bytes) -> AnyBytes:
    assert len(recoverable_signature) == 65
    return recoverable_signature[1:65]


def encode_bip137_signature(
    recoverable_signature: AnyBytes, script_type: InputScriptType
) -> bytes:
...
def decode_bip137_signature(signature: AnyBytes) -> tuple[InputScriptType, bytes]:
