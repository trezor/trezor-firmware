...
class InterfaceContext:
    ...
    def should_read(self) -> bool:
        ...
        expecting_message = any(
            ifctx.active_channel and ifctx.active_channel.expecting_message
            for ifctx in self.thp_ctx._iface_ctxs
        )
        expecting_ack = any(
            ifctx.active_channel and ifctx.active_channel.expecting_ack
            for ifctx in self.thp_ctx._iface_ctxs
        )
        if __debug__ and _TRACE:
            log.debug(
    ...
    async def write_loop(self) -> None:
        ...
        while True:
            ...
            result = await race(self.write_all_channels(packet), _WRITE_TIMEOUT)
            if isinstance(result, int):
                if self.active_channel:
                    self.active_channel.clear(exc=Timeout("THP write is blocked"))
                # TODO close all channels on _write_loop_exit?
            self.clear_closed_sessions()
            self.recompute_timeouts()
            if __debug__ and _TRACE:
    ...
    async def write_all_channels(self, packet: AnyBuffer) -> None:
        await self.write_all_packets(self.write_packet_broadcast, packet)
        await self.write_all_packets(self.write_packet_inactive, packet)
        if self.active_channel:
            await self.write_all_packets(self.active_channel.write_packet, packet)

    async def write_all_packets(
        self, source: Callable[[AnyBuffer], bool], packet: AnyBuffer
    ) -> None:
        while source(packet):
            if __debug__ and _TRACE:
                log.debug(
    ...
    # Actually broadcast or handshake.
    def write_packet_broadcast(self, packet: AnyBuffer) -> bool:
        return trezorthp.packet_out(self._iface.iface_num(), None, EMPTY_BUFFER, packet)

    def write_packet_inactive(self, packet: AnyBuffer) -> bool:
        while self.inactive_channels:
            if trezorthp.packet_out(
                self._iface.iface_num(), self.inactive_channels[0], EMPTY_BUFFER, packet
            ):
                return True
            self.inactive_channels.pop(0)
        return False
                ...
                if isinstance(res, int):
                    ...
                    if ok:
                        if __debug__:
                            log.warning(
                                __name__,
                                "(cid: %s) retransmitting message after %s ms",
                                hex(channel_id)[2:],
                                timeout_ms,
                                iface=self._iface,
                            )
                        ...
                        if __debug__:
                            log.error(
                                __name__,
                                "(cid: %s) retransmission timeout",
                                hex(channel_id)[2:],
                                iface=self._iface,
                            )
                        self.active_channel.clear(
                            exc=Timeout("THP retransmission timeout")
                        )
                    else:
                        if __debug__:
                            log.error(
                                __name__,
                                "(cid: %s) retransmission timeout, channel closed",
                                hex(channel_id)[2:],
                                iface=self._iface,
                            )
                        self.clear_closed_sessions()
