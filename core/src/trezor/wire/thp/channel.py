...
class Channel:
    ...
    def __init__(
        ...
        self.incoming_box: loop.mailbox[None | ChannelPreemptedException] = (
            loop.mailbox()
        )
        ...
        try:
            if info.credential and (inner := unwrap_credential(info.credential)):
    ...
    def read_packet(self, packet_buffer: AnyBytes) -> None:
        iface_num = self.iface.iface_num()
        buffer_size = 8192  # FIXME get it from trezorthp
