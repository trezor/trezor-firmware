...
if __debug__:
    ...
    from trezor.enums import DebugWaitType, MessageType, TouchEventType
    ...
    async def dispatch_DebugLinkDecision(
        ...
        try:
            ...
            if x is not None and y is not None:
                if msg.touch_event_type == TouchEventType.TOUCH_START:
                    ui.CURRENT_LAYOUT._event(
                        ui.CURRENT_LAYOUT.layout.touch_event, io.TOUCH_START, x, y
                    )
                elif msg.touch_event_type == TouchEventType.TOUCH_END:
                    ui.CURRENT_LAYOUT._event(
                        ui.CURRENT_LAYOUT.layout.touch_event, io.TOUCH_END, x, y
                    )
                else:
