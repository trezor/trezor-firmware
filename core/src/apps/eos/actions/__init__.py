...
async def process_action(sha: HashWriter, action: EosTxActionAck) -> None:
    ...
    if account == "eosio":
        ...
        await process_unknown_action(w, action)

    writers.write_action_common(sha, action.common)
    writers.write_bytes_prefixed(sha, w)


async def process_unknown_action(w: Writer, action: EosTxActionAck) -> None:
