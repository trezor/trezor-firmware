...
def generate_clsag_simple(message, pubs, in_sk, a, cout, index, mg_buff):
    """
...
def _generate_clsag(message, P, p, C_nonzero, z, Cout, index, mg_buff):
    ...
    crypto.sc_mul_into(tmp_sc, z, crypto.sc_inv_eight())  # -8*z
    crypto.scalarmult_into(sD, H, tmp_sc)  # sig.D = -8*z*H
