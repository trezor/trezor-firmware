...
def token_amount(
    amount: int | bytes | None, _network: EthereumNetworkInfo
) -> str | None:
    if amount is None:
        return None
    else:
...
def address_name(
    address: int | bytes | None, network: EthereumNetworkInfo
) -> str | None:
    if address is None:
        return None
    else:
...
class BindingContext:
    def __init__(
        self, deployments: Iterable[tuple[int, bytes]], metadata: dict[str, str]
    ) -> None:
...
class FieldFormat:
    def __init__(
        ...
        format: Callable[
            [int | bytes | None, EthereumNetworkInfo], str | None
        ],  # custom formatter
        threshold: (
            int | None
        ) = None,  # TODO: this should be part of the tokenAmount formatter
    ) -> None:
        self.path = path
        self.label = label
        self.format = format
...
class FieldType:
    def parse(self, arg: memoryview, field_format: FieldFormat) -> int | bytes | None:
        raise NotImplementedError


class AddressFieldType(FieldType):
    def parse(self, arg: memoryview, field_format: FieldFormat) -> int | bytes | None:
        assert all(
            byte == 0 for byte in arg[: SC_ARGUMENT_BYTES - SC_ARGUMENT_ADDRESS_BYTES]
        )
        return bytes(arg[SC_ARGUMENT_BYTES - SC_ARGUMENT_ADDRESS_BYTES :])


class Uint256FieldType(FieldType):
    def parse(self, arg: memoryview, field_format: FieldFormat) -> int | bytes | None:
...
class DisplayFormat:
    ...
    def parse_fields(
        self, data_reader: BufferReader, network: EthereumNetworkInfo
    ) -> Iterable[tuple[int | bytes | None, StrPropertyType]]:
        for field_type, field_format in zip(self.field_types, self.field_formats):
            ...
            value = field_type.parse(arg, field_format)
            yield (
                value,
                (
                    field_format.label,
...
ONEINCH_METADATA = {"owner": "1inch Aggregation Router V6"}
...
LIFI_METADATA = {"owner": "LiFI Diamond"}
...
UNISWAP_METADATA = {"owner": "Uniswap V3 Router"}
