...
async def _handle_approve(
    args: list[tuple[int | bytes | None, StrPropertyType]],
    msg: MsgInSignTx,
    network: EthereumNetworkInfo,
    ...
    maximum_fee: str,
    fee_items: Iterable[StrPropertyType],
    _handle_unknown_token: Callable[[str, bool], Coroutine[Any, Any, None]],
) -> None:
    from trezor import TR
    ...
    await _handle_unknown_token(title, False)

    recipient_str = None
    for context in constants.APPROVE_DISPLAY_FORMAT.binding_contexts or []:
        if context.matches(msg.chain_id, recipient):
...
async def _handle_transfer(
    ...
    await _handle_unknown_token(ethereum_address_title(), True)

    is_contract_interaction = data_length > 0 and not (
        len(msg.to) in (40, 42) and len(msg.value) == 0
    )  # TODO

    if is_contract_interaction:
        await require_confirm_other_data(msg.data_initial_chunk, data_length)

    await require_confirm_tx(
        address_from_bytes(recipient, network) if recipient else None,
        value,
        msg.address_n,
        maximum_fee,
        fee_items,
...
async def _handle_known_contract_calls(
    ...
    for f in constants.ALL_DISPLAY_FORMATS:
        if f.func_sig == func_sig:
            ...
            try:
                args = list(f.parse_fields(data_reader, network))
                break
    ...
    async def _handle_unknown_token(title: str, confirm_address: bool) -> None:
    ...
    if func_sig == APPROVE.func_sig:
        ...
        return True
    elif func_sig == TRANSFER.func_sig:
        if payment_req_verifier is not None:
            raise DataError("Payment Requests not supported for the TRANSFER call")
        await _handle_transfer(
            args, msg, network, token, maximum_fee, fee_items, _handle_unknown_token
        )
        return True
    ...
    for f in constants.ALL_DISPLAY_FORMATS:
        if func_sig == f.func_sig:
            if payment_req_verifier is not None:
                raise DataError(
                    "Payment Requests not supported for contract interactions"
                )

            for context in f.binding_contexts or []:
                if context.matches(msg.chain_id, address_bytes):
                    break
            else:
                return False
