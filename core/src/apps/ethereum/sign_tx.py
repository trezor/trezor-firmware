...
async def sign_tx(
    ...
    TX_TYPE_EIP_7702 = constants.TX_TYPE_EIP_7702  # local_cache_attribute

    address_bytes = bytes_from_address(msg.to)

    # check
    strict_tx_types = [1, 6, None]
    if safety_checks.is_strict():
        if msg.tx_type not in strict_tx_types:
            raise DataError("tx_type out of bounds")
    else:
        if msg.tx_type == TX_TYPE_EIP_7702:
            if address_bytes not in constants.EIP_7702_KNOWN_ADDRESSES:
                raise DataError("Unknown EIP-7702 address")
        elif msg.tx_type not in strict_tx_types:
            raise DataError("tx_type out of bounds")
    ...
    if func_sig == SC_FUNC_SIG_APPROVE:
        ...
        if payment_req_verifier is not None:
            ...
            await require_confirm_tx(
                ...
                defs.network,
                token,
                is_send=not is_contract_interaction and tx_type != TX_TYPE_EIP_7702,
                chunkify=bool(msg.chunkify),
            )
