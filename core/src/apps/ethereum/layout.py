...
async def require_confirm_approve(
    ...
    from . import addresses, networks, tokens
    from trezor.ui.layouts import confirm_ethereum_approve

    if to_bytes in addresses.KNOWN_ADDRESSES:
        to_str = addresses.KNOWN_ADDRESSES[to_bytes]
        chunkify = False
    else:
    ...
    total_amount = format_ethereum_amount(value, token, network) if value is not None else None
...
def require_confirm_address(address_bytes: bytes, title: str | None = None, subtitle: str | None = None, verb: str | None = None, br_name: str | None = None) -> Awaitable[None]:
