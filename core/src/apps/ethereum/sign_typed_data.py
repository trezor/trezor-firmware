...
async def create_struct_hash(ctx, struct_path, struct, confirm_member):
    ...
    for member_index in range(struct.num_members):
        value_name, value_type, value_dep, encoded_value = await encode_value(
            ctx, struct_path + [member_index], confirm_member
        )
        if value_dep:
...
async def encode_value(ctx, member_path, confirm_member):
    ...
    if member.member_value is None:
        dependencies, encoded_value = await create_struct_hash(
            ctx, member_path, member, confirm_member
        )
    elif member.member_type == "string":
        encoded_value = encode_solidity_static(
            "bytes32", message_digest(member.member_value)
        )
    elif member.member_type == "bytes":
        encoded_value = encode_solidity_static(
            "bytes32", message_digest(bytes_from_hex(member.member_value))
        )
