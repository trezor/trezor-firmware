...
def validate_xpub_path_against_script_type(
    ...
    """
    Checks whether the path is a plausible xpub export point for the given
    script type. A GetPublicKey message does not say whether the xpub is meant
    for multisig use, so patterns for both cases are accepted.

    A path is an export point of a pattern if it is a prefix of the pattern
    ending either at the deepest hardened level or at the account level,
    whichever is deeper. Patterns with an unhardened account level thus have
    two export points, e.g. m/45' and m/45'/coin_type/account for Casa.

    A pattern with no hardened component has no export point, so the root xpub
    is never exportable this way.
    """
