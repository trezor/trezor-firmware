...
from apps.ripple.serialize import serialize, amount_to_bytes
...
class TestRippleSerializer(unittest.TestCase):
    def test_amount(self):
        ...
        self.assertEqual(amount_to_bytes(0), unhexlify("4000000000000000"))
        self.assertEqual(amount_to_bytes(1), unhexlify("4000000000000001"))
        self.assertEqual(amount_to_bytes(93493429243), unhexlify("40000015c4a483fb"))
        self.assertEqual(amount_to_bytes(-1), unhexlify("0000000000000001"))
        with self.assertRaises(ValueError):
            amount_to_bytes(1000000000000000000)  # too large
        with self.assertRaises(Exception):
            amount_to_bytes(1.1)  # float numbers not supported

        self.assertEqual(amount_to_bytes(RippleIssuedAmount("015841551A748AD2C1F76FF6ECB0CCCD00000000","1","r3kmLJN5D28dHuH8vZNUZpMC43pEHpaocV")), unhexlify("d4838d7ea4c68000015841551a748ad2c1f76ff6ecb0cccd00000000550fc62003e785dc231a1058a05e56e3f09cf4e6"))
    ...
    def test_transactions(self):
        ...
        common = RippleSignTx(None, 10, None, 1, None, transaction_type='Payment', payment = payment, account=source_address)
        self.assertEqual(serialize(common, "payment",for_signing=True), unhexlify(
        ...
        common = RippleSignTx(None, 99, None, 99, None, transaction_type='Payment', payment = payment, account=source_address)
        self.assertEqual(serialize(common, "payment",for_signing=True), unhexlify(
        ...
        common = RippleSignTx(None, 10, 0, 2, None, transaction_type='Payment', payment = payment, account=source_address)
        self.assertEqual(serialize(common, "payment",for_signing=True), unhexlify(
        ...
        common = RippleSignTx(None, 15, 0, 144, None, transaction_type='Payment', payment = payment, account=source_address)
        ...
        common = RippleSignTx(None, 12, 0, 1, None, transaction_type='Payment', payment = payment, account=source_address)
        self.assertEqual(serialize(common, "payment", for_signing=True), unhexlify(
    ...
    def test_transactions_for_signing(self):
        ...
        common = RippleSignTx(None, 10, 2147483648, 1, None, transaction_type='Payment', payment = payment, account=source_address, signing_pub_key="ed5f5ac8b98974a3ca843326d9b88cebd0560177b973ee0b149f782cfaa06dc66a")

        tx = serialize(
            common, "payment",
        )
        tx = get_network_prefix(False) + tx
