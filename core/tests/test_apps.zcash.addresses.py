...
from trezor.enums.ZcashReceiverTypecode import P2PKH, P2SH, SAPLING, ORCHARD

# source: https://github.com/jarys/zcash-test-vectors/blob/trezor/unified_address.py
# (receivers: Dict[ZcashReceiverTypecode, bytes], unified_address: str)
TESTVECTORS_UNIFIED = [
	({
...
class TestZcashAddress(unittest.TestCase):
	def test_encode_unified(self):
		for tv in TESTVECTORS_UNIFIED:
			ua = addresses.encode_unified(tv[0], COIN)
			self.assertEqual(ua, tv[1])

	def test_decode_unified(self):
		for tv in TESTVECTORS_UNIFIED:
			receivers = addresses.decode_unified(tv[1], COIN)
