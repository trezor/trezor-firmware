...
def _get_sell_payment_request(include_nonce_str="DEBUG NONCE") -> PaymentRequest:
    text_memo = TextMemo(text="text memo text")
    ...
    return PaymentRequest(
        ...
        nonce=(bytes(include_nonce_str, "utf-8") if include_nonce_str else None),
        memos=[
...
def _get_coin_swap_request(include_nonce_str="DEBUG NONCE") -> PaymentRequest:
...
class TestPaymentRequestVerfier(unittest.TestCase):
    ...
    def test_payment_requests_without_nonce_in_prod(self):
        with self.assertRaises(wire.DataError) as e:
            PaymentRequestVerifier(
                payment_request=_get_coin_swap_request(include_nonce_str=False),
                slip44_id=1,
                keychain=_get_test_keychain(),
                amount_size_bytes=12345,
            )
    ...
    def test_payment_requests_unknown_memo(self):
        ...
        self.assertEqual(
            e.value.message, "Exactly one memo type must be specified in each PaymentRequestMemo."
        )
    ...
    def test_payment_requests_with_wrong_nonce_in_debug(self):
        with self.assertRaises(wire.DataError) as e:
            PaymentRequestVerifier(
                payment_request=_get_coin_swap_request(include_nonce_str="ANOTHER NONCE"),
                slip44_id=1,
                keychain=_get_test_keychain(),
                amount_size_bytes=12345,
            )
