...
from apps.common.payment_request import PaymentRequestVerifier, _is_coin_swap
...
# Copy of PROD (non DEBUG) version of `verify_payment_request_is_supported` from `apps.common.payment_request`
def verify_payment_request_is_supported(self, payment_request: PaymentRequest) -> None:
...
def patch_prod(func: Callable[P, None]) -> Callable[P, None]:
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> bytes:
        with patch(
            ...
            "verify_payment_request_is_supported",
            verify_payment_request_is_supported,
        ):
            with patch(
                PaymentRequestVerifier,
