...
class TestValidateXpubPath(unittest.TestCase):
    def test_export_points(self):
        ...
        # BIP-84: the ordinary account level.
        self.assertTrue(
            is_export_point([H_(84), H_(0), H_(0)], InputScriptType.SPENDWITNESS)
        )
        # One level too deep is not an export point.
        self.assertFalse(
...
class TestAccountNames(unittest.TestCase):
    ...
    def test_leaves(self):
        ...
        # Casa, BIP-45, Unchained.
        self.assertEqual(
            self._name([H_(45), 0, 0, 0, 0], InputScriptType.SPENDP2SHWITNESS),
            "Multisig",
        )
        self.assertEqual(
            self._name([H_(45), 0, 0, 0], InputScriptType.SPENDADDRESS), "Multisig"
        )
        self.assertEqual(
            self._name(
...
class TestSignMessageBip48(TestCaseWithContext):
    """BIP-48 message signing: the account node and the leaf, see #7717."""
