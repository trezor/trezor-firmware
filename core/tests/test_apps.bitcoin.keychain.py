...
class TestAccountNames(unittest.TestCase):
    ...
    def _name(self, address_n, script_type, export_point=False):
        coin = _get_coin_by_name("Bitcoin")
        return address_n_to_name(
            coin, address_n, script_type, export_point=export_point
        )

    def test_export_points(self):
        from trezor.enums import InputScriptType

        # The cosigner node, shared by BIP-45 and Casa. Which scheme names it
        # follows from the script type: BIP-45 is raw multisig, Casa is P2SH.
        self.assertEqual(
            self._name([H_(45)], InputScriptType.SPENDADDRESS, export_point=True),
            "BIP 45 multisig",
        )
        self.assertEqual(
            self._name([H_(45)], InputScriptType.SPENDP2SHWITNESS, export_point=True),
            "Casa",
        )

        # Casa's deeper export point, and Unchained's. Both carry an account
        # level, so both are numbered whether or not it is hardened.
        self.assertEqual(
            self._name(
                [H_(45), 0, 0], InputScriptType.SPENDP2SHWITNESS, export_point=True
            ),
            "Casa #1",
        )
        self.assertEqual(
    ...
    def test_leaves_are_named(self):
        """A leaf is named like its account node, as BIP-48 leaves already are."""
        from trezor.enums import InputScriptType
        ...
        # BIP-45 numbers nothing: its second level is the cosigner index.
        self.assertEqual(
            self._name([H_(45), 0, 0, 0], InputScriptType.SPENDADDRESS),
            "BIP 45 multisig",
        )
        self.assertEqual(
            self._name(
    ...
    def test_multisig_script_type(self):
        ...
        self.assertEqual(
            self._name([H_(45)], InputScriptType.SPENDMULTISIG, export_point=True),
            "BIP 45 multisig",
        )
        self.assertEqual(
            self._name(
    ...
    def test_greenaddress(self):
        """The subaccount node is named; the unhardened scheme has no export point."""
        from trezor.enums import InputScriptType
        ...
        self.assertEqual(
            self._name([H_(3), H_(1), 1, 0], InputScriptType.SPENDADDRESS),
            "GreenAddress",
        )
        # PATTERN_GREENADDRESS_A has no account type at all, so it stays unnamed.
        self.assertIsNone(
    ...
    def test_other_names_do_not_move(self):
        ...
        self.assertEqual(
            self._name(
                [H_(44), H_(0), H_(0)], InputScriptType.SPENDADDRESS, export_point=True
            ),
            "Legacy #1",
        )
        self.assertEqual(
    ...
    def test_or_unknown_export_point(self):
        from trezor.enums import InputScriptType

        coin = _get_coin_by_name("Bitcoin")

        # Export points are not named unless the caller asks for it.
        self.assertEqual(
            address_n_to_name_or_unknown(
                coin, [H_(48), H_(0), H_(0), H_(2)], InputScriptType.SPENDWITNESS
            ),
            "Unknown path",
        )
        self.assertEqual(
        ...
        # SignMessage opts in, and then they are.
        self.assertEqual(
            address_n_to_name_or_unknown(
                coin,
                [H_(48), H_(0), H_(0), H_(2)],
                InputScriptType.SPENDWITNESS,
                allow_export_point=True,
            ),
            "BTC BIP 48 multisig #1",
        )
        self.assertEqual(
            address_n_to_name_or_unknown(
                coin,
        ...
        # A leaf keeps its name either way.
        self.assertEqual(
            address_n_to_name_or_unknown(
                coin, [H_(44), H_(0), H_(0), 0, 0], InputScriptType.SPENDADDRESS
            ),
            "BTC Legacy #1",
        )
