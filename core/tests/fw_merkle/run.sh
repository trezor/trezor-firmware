...
#
# Compiles the *real* on-device tree math (embed/sec/image/boot_header_merkle.c)
...
#
#   FWM3: multi-variant founder firmware_root + per-variant proofs over full
...
# The device source is COMPILED, not textually included: shims.h is forced in so it
# keeps its normal include block for the real build.
gcc -O2 -Wall -Wextra \
