...
class TestStorageCacheEncryption(unittest.TestCase):
    ...
    def assert_encrypted(self, c) -> None:
        self.assertTrue(c.is_encrypted)
        self.assertIsNotNone(c.nonce)
        self.assertIsNotNone(c.authentication_tag)

    def assert_plaintext(self, c) -> None:
        self.assertFalse(c.is_encrypted)
        self.assertIsNone(c.nonce)
        self.assertIsNone(c.authentication_tag)

    def _roundtrip(self, cache_instance, keys) -> None:
    ...
    def test_cache_encryption_empty_session_skipped(self):
        # an empty session (no sensitive fields) must be left untouched
        session = self._session()
        encrypt_cache()
        self.assert_plaintext(session)

    def test_cache_encryption_preauthorized_skipped(self):
    ...
    def _corrupt_and_expect_failure(self, session) -> None:
    ...
    def test_cache_encryption_corrupted_authentication_tag(self):
        ...
        self._fill(session, self.KEYS)
        encrypt_cache()
        assert session.authentication_tag is not None
    ...
    def test_cache_encryption_double_encrypt(self):
        ...
        nonce, tag = session.nonce, session.authentication_tag

        encrypt_cache()  # already encrypted -> no-op

        self.assert_encrypted(session)
        self.assertEqual(nonce, session.nonce)
        self.assertEqual(tag, session.authentication_tag)
        for key in self.KEYS:
    ...
    def test_cache_encryption_decrypt_isolates_failure(self):
        ...
        encrypt_cache()
        assert s1.authentication_tag is not None
    ...
    def test_cache_encryption_clear_while_encrypted(self):
