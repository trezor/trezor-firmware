...
class TestCardanoDerivation(unittest.TestCase):
    def test_icarus(self):
        ...
        secret = cardano.derive_icarus(mnemonic_bits, "", False)
        self.assertEqual(
        ...
        secret_trezor = cardano.derive_icarus(mnemonic_bits, "", True)
        self.assertEqual(secret, secret_trezor)

        secret = cardano.derive_icarus(mnemonic_bits, "foo", False)
        self.assertEqual(
            hexlify(secret).decode(),
        ...
        secret_trezor = cardano.derive_icarus(mnemonic_bits, "foo", True)
        self.assertEqual(secret, secret_trezor)
    ...
    def test_icarus_trezor(self):
        ...
        mnemonic_bits = bip39.mnemonic_to_bits(mnemonic)
        secret = cardano.derive_icarus(mnemonic_bits, "", True)
        self.assertEqual(
            hexlify(secret).decode(),
        ...
        secret_icarus = cardano.derive_icarus(mnemonic_bits, "", False)
        ...
        secret_icarus = cardano.derive_icarus(mnemonic_bits, PASSPHRASE, False)
        self.assertNotEqual(secret, secret_icarus)
