...
from trezorlib.debuglink import TrezorTestContext as Client, MessageFilter
...
from .input_flows import (
    ...
    InputFlowSignTxDataGoBack,
    InputFlowSignTxDataScrollDown,
    InputFlowSignTxDataSkip,
...
class MessageFilterGenerator:
    def __getattr__(self, key: str) -> t.Callable[..., "MessageFilter"]:
        message_type = getattr(ethereum_messages, key)
...
def make_defs(parameters: dict) -> ethereum_messages.Definitions:
    ...
    if parameters.get("fake_defs", True):
        ...
        return ethereum_messages.Definitions(encoded_network=network)
    else:
...
def test_signtx(
    session: Session, instance_id: int, chunkify: bool, parameters: dict, result: dict
):
    input_flow = (
...
def _do_test_signtx(
    ...
    with session.test_ctx as client:
        ...
        sig_v, sig_r, sig_s = ethereum_ext.sign_tx(
            session,
            instance_id,
...
def test_signtx_fee_info(session: Session, instance_id: int):
    input_flow = InputFlowSignTxShowFeeInfo(session).get()
    _do_test_signtx(
        session,
        instance_id,
...
def test_signtx_go_back_from_summary(session: Session, instance_id: int):
    input_flow = InputFlowSignTxGoBackFromSummary(session).get()
    _do_test_signtx(
        session,
        instance_id,
        example_input_data["parameters"],
...
@parametrize_using_common_fixtures("sign_tx_eip1559.json")
@pytest.mark.parametrize("chunkify", (True, False))
def test_signtx_eip1559(
    ...
    with session.test_ctx as client:
        ...
        sig_v, sig_r, sig_s = ethereum_ext.sign_tx_eip1559(
            session,
            instance_id,
            n=parse_path(parameters["path"]),
            nonce=int(parameters["nonce"], 16),
...
def test_sanity_checks(session: Session, instance_id: int):
...
def test_data_streaming(session: Session, instance_id: int):
    ...
    with session.test_ctx as client:
        ...
        client.set_expected_responses(
            [
                trezor_messages.ButtonRequest(
                    code=trezor_messages.ButtonRequestType.SignTx
                ),
                trezor_messages.ButtonRequest(
                    code=trezor_messages.ButtonRequestType.SignTx
                ),
                ...
                message_filters.TxRequest(data_length=None),
            ]
        )
...
def test_sanity_checks_eip1559(session: Session, instance_id: int):
    """Is not vectorized because these are internal-only tests that do not
    need to be exposed to the public.
    """
    # contract creation without data should fail.
    with pytest.raises(TrezorFailure, match=r"DataError"):
        ethereum_ext.sign_tx_eip1559(
            session,
            instance_id=instance_id,
            n=parse_path("m/44h/60h/0h/0/100"),
            nonce=0,
...
def input_flow_data_skip(client: Client | Session, cancel: bool = False):
    return InputFlowSignTxDataSkip(client, cancel).get()


def input_flow_data_scroll_down(client: Client | Session, cancel: bool = False):
    return InputFlowSignTxDataScrollDown(client, cancel).get()


def input_flow_data_go_back(client: Client | Session, cancel: bool = False):
    return InputFlowSignTxDataGoBack(client, cancel).get()
...
def test_signtx_data_pagination(session: Session, instance_id: int, flow):
    def _sign_tx_call():
        ethereum_ext.sign_tx(
            session,
            instance_id=instance_id,
...
@parametrize_using_common_fixtures("sign_tx_staking.json")
@pytest.mark.parametrize("chunkify", (True, False))
def test_signtx_staking(
    session: Session, instance_id: int, chunkify: bool, parameters: dict, result: dict
):
    input_flow = InputFlowSignTxStaking(session).get()
    _do_test_signtx(
        session,
        instance_id,
        parameters,
...
@parametrize_using_common_fixtures(
    "sign_tx_staking_data_error.json",
    "sign_tx_error.json",
)
...
@parametrize_using_common_fixtures("sign_tx_staking_eip1559.json")
def test_signtx_staking_eip1559(
    session: Session, instance_id: int, parameters: dict, result: dict
):
    with session.test_ctx:
        sig_v, sig_r, sig_s = ethereum_ext.sign_tx_eip1559(
...
def test_signtx_payment_req(
    ...
    if has_refund:
        ...
        refund_memo.address_resp = ethereum_ext.get_authenticated_address(
            session, instance_id, refund_memo.address_n
        )
