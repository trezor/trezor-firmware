...
def test_data_streaming(session: Session, instance_id: int):
    ...
    with session.test_ctx as client:
        ...
        def br_sign_tx(n):
            return trezor_messages.ButtonRequest(
...
def test_signtx_data_pagination(
    session: Session, instance_id: int, scroll: bool, size: int
):
    ...
    # test pagination
    flow = InputFlowSignTxData(session, scroll=scroll, cancel=False)
    with session.test_ctx as client:
        client.set_input_flow(flow.get())
        _sign_tx_call()

    # test cancellation
    flow = InputFlowSignTxData(session, scroll=scroll, cancel=True)
