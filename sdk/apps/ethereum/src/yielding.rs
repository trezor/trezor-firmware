...
pub(crate) fn get_approver<'a>(
    ...
    if data_length > initial_data.len() as u32 {
        return Ok((None, None));
    }
        ...
        .any(|sig| func_sig == *sig)
    {
        ...
        handler = prepare_claim_rewards(data, msg, network, maximum_fee, fee_items, sender_bytes)?;
    }
...
fn prepare_vault_tx<'a>(
    ...
    if data.len() < min_data_len || !unwrap!(msg.value.as_deref()).is_empty() {
