...
fn prepare_vault_tx<'a>(
    ...
    let receiver = clear_signing::parse_address(receiver_bytes).c()?;

    let (owner, rest) = if is_deposit {
        (None, rest)
    } else {
        let (owner_bytes, rest) = rest.split_at(32);
        (Some(clear_signing::parse_address(owner_bytes).c()?), rest)
    };

    if !is_vault_tx_safe(&vault, sender_bytes, &receiver, owner.as_deref()).c()? {
        return Ok(None);
    }
