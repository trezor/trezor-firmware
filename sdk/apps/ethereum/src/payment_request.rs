...
pub fn verify_payment_request_is_supported(_payment_request: &PaymentRequest) -> Result<()> {
    #[cfg(not(feature = "dev_keys"))]
...
impl PaymentRequestVerifier {
    ...
    pub fn verify(&mut self) -> Result<()> {
        ...
        #[allow(unused_mut)]
        let mut result =
            crypto::nist256p1_verify(&PUBLIC_KEY, &self.signature, &self.h_pr.digest());

        #[cfg(feature = "dev_keys")]
        if !result {
            result =
                crypto::nist256p1_verify(&DEBUG_PUBLIC_KEY, &self.signature, &self.h_pr.digest());
        }
