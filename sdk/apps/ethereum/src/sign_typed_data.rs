...
pub fn sign_typed_data(msg: SignTypedData) -> Result<TypedDataSignature> {
    ...
    let address_bytes = get_eth_pubkey_hash(&dp)?;
    ...
    let sig = TypedDataSignature {
        ...
        address: address_from_bytes(&address_bytes, Some(definitions.network())).c()?,
    };
...
impl TypedDataEnvelope {
        ...
        F: Fn(u32) -> u32 + Copy,
    {
        ...
        let current_type: TypedDataStructAck = unwrap!(wire_request(&req));
...
fn get_value(field: &FieldType, member_value_path: &[u32]) -> Result<Vec<u8>> {
    ...
    let res: TypedDataValueAck = wire_request(&req).c()?;
    let value = res.value;
