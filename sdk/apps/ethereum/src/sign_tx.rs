...
pub fn sign_tx(mut msg: SignTx) -> Result<TxRequest> {
    ...
    let definitions =
        Definitions::from_encoded(encoded_network, encoded_token, Some(msg.chain_id), None).c()?;

    check_common_fields(data_length, data_initial_chunk, to, msg.chain_id).c()?;

    let address_bytes = bytes_from_address(to).c()?;
    ...
    let sender_bytes = get_eth_pubkey_hash(&dp).c()?;

    let gas_price = U256::from_big_endian(&msg.gas_price);
    let gas_limit = U256::from_big_endian(&msg.gas_limit);
    let maximum_fee =
        format_ethereum_amount(gas_price * gas_limit, None, definitions.network(), false).c()?;
    let fee_items = get_fee_items_regular(gas_price, gas_limit, definitions.network()).c()?;
    ...
    let initial_data =
        request_initial_data(&mut hasher, data_length as usize, data_initial_chunk).c()?;
    ...
    let res = sign_digest(&msg, &digest).c()?;
...
pub fn get_total_length(
    ...
    let to_bytes = bytes_from_address(to).c()?;
