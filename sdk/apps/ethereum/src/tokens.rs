...
pub(crate) fn token_by_chain_address(chain_id: u64, address: &[u8]) -> Option<TokenInfo> {
    tokens_for_chain(chain_id)
        .iter()
        .find(|(addr, _, _, _)| address == *addr)
        .map(|(addr, symbol, decimals, name)| TokenInfo {
            symbol: (*symbol).into(),
...
fn tokens_for_chain(chain_id: u64) -> &'static [TokenDef] {
    match chain_id {
        1 => ETH_TOKENS,
        _ => &[],
    }
...
mod tests {
    ...
    #[test]
    fn test_token_by_chain_address() {
        let token = token_by_chain_address(
            1,
            b"\x7f\xc6\x65\x00\xc8\x4a\x76\xad\x7e\x9c\x93\x43\x7b\xfc\x5a\xc3\x3e\x2d\xda\xe9",
        )
        .unwrap();
        assert_eq!(token.symbol, "AAVE");

        // invalid adress, invalid chain
        let token = token_by_chain_address(999, b"\x00\xff");
        assert!(token.is_none());

        let unknown = unknown_token();

        assert_eq!(unknown.symbol, "Wei UNKN");
        assert_eq!(unknown.decimals, 0);
    }
}
