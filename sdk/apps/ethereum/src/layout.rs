...
pub(crate) fn require_confirm_approve<'a>(
    ...
    let token_address_str = address_from_bytes(token_address, Some(network)).c()?;
    ...
    confirm_ethereum_approve(
        &addr_pad(recipient_addr, chunkify).c()?,
        recipient_str,
        is_unknown_token,
...
fn confirm_ethereum_approve<'a>(
    ...
    ui::error_if_not_confirmed(ui::confirm_action(ui::ConfirmAction::new(
        title,
        action,
        None,
        None,
    ...
    if let Some(recipient_str) = recipient_str {
        ui::interact_with_info_flow(
            |name| {
                ui::confirm_with_info(ui::ConfirmWithInfo::new(
                    title,
                    Some(subtitle),
                    &[StrExt::mono(recipient_str)],
            ...
            |name| {
                ui::show_info_with_cancel(ui::ShowInfoWithCancel::new(
                    title,
...
pub(crate) fn require_confirm_stake<'a>(
    ...
    let address_str = addr_pad(
        &address_from_bytes(address_bytes, Some(network)).c()?,
        chunkify,
    )
    .c()?;
    let total_amount = format_ethereum_amount(value, None, network, false).c()?;
    let (account, account_path) = dp.account_and_path();
...
pub(crate) fn require_confirm_vault_tx<'a>(
    ...
    let amount = format_ethereum_amount(value, Some(&token), network, false).c()?;
    let (account, account_path) = dp.account_and_path();
...
pub(crate) fn should_show_domain(name: &[u8], version: &[u8]) -> Result<bool> {
    ...
    let para = [
        StrExt::plain(tr!("ethereum__name_and_version")),
        StrExt::plain(&domain_name),
        StrExt::plain(&domain_version),
    ];
...
pub(crate) fn should_show_struct(
    ...
    let para = [
        StrExt::plain(description),
        StrExt::plain(&contains),
        StrExt::plain(&field_names),
    ];
...
pub(crate) fn should_show_array(
    ...
    let para = [StrExt::plain(&array_of_plural)];
    ui::should_show_more(
    ...
    loop {
        ...
        match res {
            ...
            Err(_) => {
                ...
                if matches!(
                    ui::show_mismatch(ui::ShowMismatch::new(
                        "Address mismatch?",
                        ButtonRequestType::Other.into()
                    )),
