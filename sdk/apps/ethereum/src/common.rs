...
pub(crate) fn request_initial_data(
    ...
    while data_left > 0 {
        let resp = send_request_chunk(data_left as u32).c()?;
...
pub(crate) fn send_request_chunk(data_left: u32) -> Result<TxAck> {
    ...
    let resp: TxAck = wire_request(&req, MessageType::TxRequest).c()?;
...
pub(crate) fn confirm_tx_data<'a>(
    ...
    let recipient_str = if address_bytes.is_empty() {
        ...
        Some(address_from_bytes(address_bytes, Some(defs.network())).c()?)
    };
    ...
    if let Some(mut payment_request_verifier) = payment_request_verifier {
        ...
        Ok((
            Some(get_progress_indicator(data_length, 0).c()?),
        ...
        let confirm_data_chunk = if data_length > 0 {
            ...
            get_progress_indicator(data_length, 0).c()?
        };
        ...
        Ok((
            ...
            Some(Box::new(move || {
                require_confirm_tx(
                    ...
                    &format_ethereum_amount(value, token, defs.network(), false).c()?,
                    address_bytes,
                    dp,
                    maximum_fee,
                    fee_items,
