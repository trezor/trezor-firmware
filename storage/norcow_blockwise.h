...
#define NORCOW_SMALL_ITEM_SIZE (FLASH_BLOCK_SIZE - NORCOW_LEN_LEN - NORCOW_KEY_LEN)
...
static secbool write_item(
    uint8_t sector, uint32_t offset, uint16_t key, const uint8_t *data, uint16_t len, uint32_t *pos)
{
    ...
    if (len <= NORCOW_SMALL_ITEM_SIZE) {
        ...
        *pos = offset + FLASH_BLOCK_SIZE;
        ...
        if (offset + FLASH_ALIGN(NORCOW_MAX_PREFIX_LEN + len) > NORCOW_SECTOR_SIZE) {
            return secfalse;
        }
...
void norcow_delete_item(const flash_area_t *area, uint32_t len, uint32_t val_offset)
{
    uint32_t end;
...
static secbool flash_area_write_bytes(
    const flash_area_t *area, uint32_t offset, uint16_t dest_len, const void *val, uint16_t len)
{
 ...
 */
secbool norcow_update_bytes(const uint16_t key, const uint8_t *data, const uint16_t len)
{
    const void *ptr = NULL;
    ...
    uint32_t sector_offset = (const uint8_t *)ptr - (const uint8_t *)norcow_ptr(
    ...
    if (norcow_write_buffer_flashed + norcow_write_buffer_filled + len > allocated_len) {
        return secfalse;
    }
    ...
    while (tmp_len > 0) {
        ...
        memcpy(&((uint8_t *)norcow_write_buffer)[norcow_write_buffer_filled], data, data_to_copy);
        data += data_to_copy;
        norcow_write_buffer_filled += data_to_copy;
        tmp_len -= data_to_copy;

        bool all_data_received =
            (norcow_write_buffer_filled + norcow_write_buffer_flashed) == allocated_len;
        ...
        if (block_full || all_data_received) {
            if (!block_full) {
                ...
                ((uint8_t *)norcow_write_buffer)[norcow_write_buffer_filled] = NORCOW_VALID_FLAG;
            }

            ensure(flash_area_write_block(area, flash_offset, norcow_write_buffer), NULL);
