...
#define NORCOW_STORAGE_START FLASH_ALIGN(NORCOW_HEADER_LEN + NORCOW_MAGIC_LEN + NORCOW_VERSION_LEN)
...
static void erase_sector(uint8_t sector, secbool set_magic)
{
    ...
    if (sectrue == set_magic) {
        ...
        flash_block_t block_magic = {NORCOW_MAGIC};
            ...
            flash_area_write_block(&STORAGE_AREAS[sector], NORCOW_HEADER_LEN, block_magic), NULL);
        flash_block_t block_version = {~NORCOW_VERSION};
        ensure(
            flash_area_write_block(
                &STORAGE_AREAS[sector], NORCOW_HEADER_LEN + NORCOW_MAGIC_LEN, block_version),
            "set version failed");
#else
        flash_block_t block = {NORCOW_MAGIC, ~NORCOW_VERSION};
        ensure(
            flash_area_write_block(&STORAGE_AREAS[sector], NORCOW_HEADER_LEN, block),
...
secbool norcow_set_ex(uint16_t key, const void *val, uint16_t len, secbool *found)
{
    ...
    *found = find_item(norcow_write_sector, key, &ptr, &len_old);
    ...
    if (norcow_free_offset + FLASH_ALIGN(NORCOW_MAX_PREFIX_LEN + len) > NORCOW_SECTOR_SIZE) {
        compact();
    }

    // Write new item.
    uint32_t pos = 0;
    if (sectrue != write_item(norcow_write_sector, norcow_free_offset, key, val, len, &pos)) {
        return secfalse;
    }
...
secbool norcow_delete(uint16_t key)
{
    ...
    uint32_t val_offset = (const uint8_t *)ptr -
                          (const uint8_t *)norcow_ptr(norcow_write_sector, 0, NORCOW_SECTOR_SIZE);
