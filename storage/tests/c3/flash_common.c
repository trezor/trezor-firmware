...
static secbool subarea_get_sector_and_offset(
    const flash_subarea_t *subarea, uint32_t offset, uint16_t *sector_out, uint32_t *offset_out)
{
...
int32_t flash_get_sector_num(const flash_area_t *area, uint32_t sector_inner_num)
{
    uint16_t sector = 0;
    ...
    const flash_area_t *area, uint32_t offset, uint16_t *sector_out, uint32_t *offset_out)
{
    ...
    for (int i = 0; i < area->num_subareas; i++) {
        ...
        return subarea_get_sector_and_offset(&area->subarea[i], tmp_offset, sector_out, offset_out);
    }
    return secfalse;
}
...
{
    for (int i = 0; i < 4; i++) {
        if (sectrue != flash_area_write_word(area, offset + i * sizeof(uint32_t), data[i])) {
            return secfalse;
        }
    }
    return sectrue;
