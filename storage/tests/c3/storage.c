...
static secbool set_wipe_code(const uint8_t *wipe_code, size_t wipe_code_len)
{
    ...
    if (sectrue !=
        norcow_set(
            WIPE_CODE_DATA_KEY, NULL, wipe_code_len + WIPE_CODE_SALT_SIZE + WIPE_CODE_TAG_SIZE)) {
        return secfalse;
    }

    // Write wipe code into the preallocated entry.
    if (sectrue != norcow_update_bytes(WIPE_CODE_DATA_KEY, 0, wipe_code, wipe_code_len)) {
        return secfalse;
    }

    // Write salt and tag into the preallocated entry.
    if (sectrue != norcow_update_bytes(
                       WIPE_CODE_DATA_KEY, wipe_code_len, salt_and_tag,
...
#if USE_OPTIGA
static void stretch_pin_optiga(
    const uint8_t *pin, size_t pin_len, const uint8_t storage_salt[STORAGE_SALT_SIZE],
    const uint8_t *ext_salt, uint8_t stretched_pin[OPTIGA_PIN_SECRET_SIZE])
{
    ...
    uint8_t kek[SHA256_DIGEST_LENGTH], uint8_t keiv[SHA256_DIGEST_LENGTH])
{
    ...
    if (ret != OPTIGA_SUCCESS) {
        ...
        ensure(ret == OPTIGA_ERR_AUTH_FAIL ? sectrue : secfalse, "optiga_pin_verify failed");
        return secfalse;
    }
    derive_kek_optiga(optiga_secret, kek, keiv);
    memzero(optiga_secret, sizeof(optiga_secret));
...
static secbool set_pin(const uint8_t *pin, size_t pin_len, const uint8_t *ext_salt)
{
    ...
    ui_progress(0);
    ensure(derive_kek_set(pin, pin_len, rand_salt, ext_salt, kek, keiv), "derive_kek_set failed");
...
static uint32_t generate_guard_key(void)
{
    ...
    do {
        guard_key = random_uniform((UINT32_MAX / GUARD_KEY_MODULUS) + 1) * GUARD_KEY_MODULUS +
                    GUARD_KEY_REMAINDER;
...
static secbool pin_fails_reset(void)
{
    ...
    for (size_t i = 0; i < PIN_LOG_WORDS; ++i) {
        ...
        if (success_log[i] != guard) {
            if (sectrue !=
                norcow_update_word(
                    PIN_LOGS_KEY, sizeof(uint32_t) * (i + GUARD_KEY_WORDS), entry_log[i])) {
                return secfalse;
            }
        }
    }
...
static secbool pin_get_fails(uint32_t *ctr)
{
    *ctr = PIN_MAX_TRIES;
    ...
    for (i = 0; i < PIN_LOG_WORDS; ++i) {
        if ((entry_log[i] & guard_mask) != guard || (success_log[i] & guard_mask) != guard ||
...
static secbool unlock(const uint8_t *pin, size_t pin_len, const uint8_t *ext_salt)
{
    ...
    uint16_t len = 0;
    if (sectrue != initialized || sectrue != norcow_get(EDEK_PVC_KEY, &rand_salt, &len) ||
        len != STORAGE_SALT_SIZE + KEYS_SIZE + PVC_SIZE) {
        memzero(&legacy_pin, sizeof(legacy_pin));
        handle_fault("no EDEK");
        return secfalse;
    ...
    if (sectrue !=
            derive_kek_unlock(
                unlock_pin, unlock_pin_len, (const uint8_t *)rand_salt, ext_salt, kek, keiv) ||
...
uint32_t storage_get_pin_rem(void)
{
    ...
    if (ctr_optiga < ctr_mcu) {
            ...
            optiga_pin_fails_increase(ctr_mcu - ctr_optiga) == OPTIGA_SUCCESS ? sectrue : secfalse,
