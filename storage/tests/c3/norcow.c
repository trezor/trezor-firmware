 ...
 */
static secbool norcow_write(
    uint8_t sector, uint32_t offset, uint32_t prefix, const uint8_t *data, uint16_t len)
{
    ...
    if (data != NULL) {
        ...
        for (uint16_t i = 0; i < len; i++, offset++) {
            ensure(flash_area_write_byte(&STORAGE_AREAS[sector], offset, data[i]), NULL);
        }
    } else {
        offset += len;
    }
...
static void erase_sector(uint8_t sector, secbool set_magic)
{
    ...
    for (uint32_t i = 0; i < NORCOW_HEADER_LEN / sizeof(uint32_t); ++i) {
            ...
            flash_write_word(norcow_sectors[sector], i * sizeof(uint32_t), header_backup[i]), NULL);
    }
    ensure(flash_lock_write(), NULL);
#endif
    ...
    if (sectrue == set_magic) {
            ...
            norcow_write(sector, NORCOW_HEADER_LEN + NORCOW_MAGIC_LEN, ~NORCOW_VERSION, NULL, 0),
            "set version failed");
    }
}
 ...
 */
static secbool read_item(
    uint8_t sector, uint32_t offset, uint16_t *key, const void **val, uint16_t *len, uint32_t *pos)
{
    *pos = offset;
...
static secbool find_start_offset(uint8_t sector, uint32_t *offset, uint32_t *version)
{
    const uint32_t *magic =
        norcow_ptr(sector, NORCOW_HEADER_LEN, NORCOW_MAGIC_LEN + NORCOW_VERSION_LEN);
    if (magic == NULL) {
        return secfalse;
    }
...
secbool norcow_set_ex(uint16_t key, const void *val, uint16_t len, secbool *found)
{
    ...
    *found = find_item(norcow_write_sector, key, &ptr, &len_old);
    ...
    uint32_t offset = 0;
    if (sectrue == *found) {
        offset = (const uint8_t *)ptr -
                 (const uint8_t *)norcow_ptr(norcow_write_sector, 0, NORCOW_SECTOR_SIZE);
        if (val != NULL && len_old == len) {
            ret = sectrue;
            ensure(flash_unlock_write(), NULL);
            for (uint16_t i = 0; i < len; i++) {
                if (sectrue != flash_area_write_byte(
                                   &STORAGE_AREAS[norcow_write_sector], offset + i,
                                   ((const uint8_t *)val)[i])) {
                    ret = secfalse;
                    break;
                }
        ...
        if (sectrue == *found) {
            ...
            uint32_t prefix = (uint32_t)len_old << 16;
                ...
                flash_area_write_word(
                    &STORAGE_AREAS[norcow_write_sector], offset - NORCOW_PREFIX_LEN, prefix),
                NULL);
            ...
            while (offset < end) {
                    ...
                    flash_area_write_word(&STORAGE_AREAS[norcow_write_sector], offset, 0x00000000),
                    NULL);
...
secbool norcow_delete(uint16_t key)
{
    ...
    uint32_t offset = (const uint8_t *)ptr -
                      (const uint8_t *)norcow_ptr(norcow_write_sector, 0, NORCOW_SECTOR_SIZE);
    ...
    while (offset < end) {
            ...
            flash_area_write_word(&STORAGE_AREAS[norcow_write_sector], offset, 0x00000000), NULL);
        offset += NORCOW_WORD_SIZE;
    }
...
secbool norcow_update_word(uint16_t key, uint16_t offset, uint32_t value)
{
    ...
    uint32_t sector_offset =
        (const uint8_t *)ptr -
        (const uint8_t *)norcow_ptr(norcow_write_sector, 0, NORCOW_SECTOR_SIZE) + offset;
    ensure(flash_unlock_write(), NULL);
    ensure(flash_area_write_word(&STORAGE_AREAS[norcow_write_sector], sector_offset, value), NULL);
    ensure(flash_lock_write(), NULL);
    return sectrue;
}
 ...
 */
secbool norcow_update_bytes(
    const uint16_t key, const uint16_t offset, const uint8_t *data, const uint16_t len)
{
    const void *ptr = NULL;
    ...
    for (uint16_t i = 0; i < len; i++, sector_offset++) {
            ...
            flash_area_write_byte(&STORAGE_AREAS[norcow_write_sector], sector_offset, data[i]),
            NULL);
    }
    ensure(flash_lock_write(), NULL);
    return sectrue;
