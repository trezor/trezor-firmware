...
uint32_t flash_area_get_size(const flash_area_t *area)
{
    ...
    for (int i = 0; i < area->num_subareas; i++) {
        size += flash_sector_size(area->subarea[i].first_sector, area->subarea[i].num_sectors);
    }
    return size;
}
...
static secbool get_sector_and_offset(
    const flash_area_t *area, uint32_t offset, uint16_t *sector_out, uint32_t *offset_out)
{
    for (int i = 0; i < area->num_subareas; i++) {
        ...
        if (offset < subarea_size) {
            ...
            *offset_out = offset - flash_sector_size(first_sector, found_sector - first_sector);
            return sectrue;
        }
        offset -= subarea_size;
    }
    ...
    for (int i = 0; i < area->num_subareas; i++) {
        ...
        if (offset < subarea_size) {
            ...
            if (offset + size <= subarea_size) {
                const uint8_t *ptr = (const uint8_t *)flash_get_address(first_sector, 0, 0);
...
secbool flash_area_write_byte(const flash_area_t *area, uint32_t offset, uint8_t data)
{
    uint16_t sector;
...
secbool flash_area_write_word(const flash_area_t *area, uint32_t offset, uint32_t data)
{
    uint16_t sector;
    uint32_t sector_offset;
    if (get_sector_and_offset(area, offset, &sector, &sector_offset) != sectrue) {
...
secbool flash_area_write_quadword(const flash_area_t *area, uint32_t offset, const uint32_t *data)
{
    uint16_t sector;
    uint32_t sector_offset;
    if (get_sector_and_offset(area, offset, &sector, &sector_offset) != sectrue) {
...
secbool flash_area_write_burst(const flash_area_t *area, uint32_t offset, const uint32_t *data)
{
    uint16_t sector;
    uint32_t sector_offset;
    if (get_sector_and_offset(area, offset, &sector, &sector_offset) != sectrue) {
...
secbool flash_area_write_block(const flash_area_t *area, uint32_t offset, const flash_block_t block)
{
    ...
    uint8_t padding, uint32_t total_size)
{
    ...
    while (total_size > 0) {
#ifdef FLASH_BURST_SIZE
        if ((offset % FLASH_BURST_SIZE) == 0 && (offset + FLASH_BURST_SIZE) <= total_size) {
...
secbool flash_area_erase(const flash_area_t *area, void (*progress)(int pos, int len))
{
...
secbool flash_area_erase_partial(const flash_area_t *area, uint32_t offset, uint32_t *bytes_erased)
{
