...
static secbool storage_upgrade_unlocked(
    const uint8_t *pin, size_t pin_len, const uint8_t *ext_salt);
static secbool storage_set_encrypted(const uint16_t key, const void *val, const uint16_t len);
static secbool storage_get_encrypted(
    const uint16_t key, void *val_dest, const uint16_t max_len, uint16_t *len);
...
static secbool set_wipe_code(const uint8_t *wipe_code, size_t wipe_code_len)
{
    ...
    uint8_t data[(MAX_WIPE_CODE_LEN + WIPE_CODE_SALT_SIZE + SHA256_DIGEST_LENGTH)] = {0};
    ...
    secbool ret = norcow_set(
        WIPE_CODE_DATA_KEY, data, wipe_code_len + WIPE_CODE_SALT_SIZE + WIPE_CODE_TAG_SIZE);
...
static secbool is_not_wipe_code(const uint8_t *pin, size_t pin_len)
{
    ...
    memcpy(
        stored_tag, (uint8_t *)wipe_code_data + wipe_code_len + WIPE_CODE_SALT_SIZE,
        sizeof(stored_tag));

    // Check integrity in case of flash read manipulation attack.
    hmac_sha256(salt, WIPE_CODE_SALT_SIZE, wipe_code, wipe_code_len, computed_tag1);
    ...
    if (sectrue != secequal(computed_tag1, computed_tag2, sizeof(computed_tag1))) {
...
static void ui_progress_add(uint32_t added_ms)
{
    ui_total += added_ms;
}
...
#if !USE_OPTIGA
static void derive_kek_v4(
    const uint8_t *pin, size_t pin_len, const uint8_t *storage_salt, const uint8_t *ext_salt,
    uint8_t kek[SHA256_DIGEST_LENGTH], uint8_t keiv[SHA256_DIGEST_LENGTH])
{
    ...
    uint8_t salt[HARDWARE_SALT_SIZE + STORAGE_SALT_SIZE + EXTERNAL_SALT_SIZE] = {0};
    size_t salt_len = 0;
    ...
    pbkdf2_hmac_sha256_Final(&ctx, pre_kek);
        ...
        secure_aes_ecb_encrypt_hw(pre_kek, SHA256_DIGEST_LENGTH, kek, SECURE_AES_KEY_XORK),
...
static void stretch_pin(
    const uint8_t *pin, size_t pin_len, const uint8_t storage_salt[STORAGE_SALT_SIZE],
    const uint8_t *ext_salt, uint8_t stretched_pin[SHA256_DIGEST_LENGTH])
{
    ...
    pbkdf2_hmac_sha256_Final(&ctx, stretched_pin_tmp);
        ...
        secure_aes_ecb_encrypt_hw(
            stretched_pin_tmp, SHA256_DIGEST_LENGTH, stretched_pin, SECURE_AES_KEY_XORK),
...
static void derive_kek_optiga_v4(
    ...
    const uint8_t optiga_secret[OPTIGA_PIN_SECRET_SIZE], uint8_t kek[SHA256_DIGEST_LENGTH],
    uint8_t keiv[SHA256_DIGEST_LENGTH])
{
    PBKDF2_HMAC_SHA256_CTX ctx = {0};
    pbkdf2_hmac_sha256_Init(&ctx, optiga_secret, OPTIGA_PIN_SECRET_SIZE, NULL, 0, 1);
    pbkdf2_hmac_sha256_Update(&ctx, 1);
    pbkdf2_hmac_sha256_Final(&ctx, kek);

    pbkdf2_hmac_sha256_Init(&ctx, optiga_secret, OPTIGA_PIN_SECRET_SIZE, NULL, 0, 2);
    pbkdf2_hmac_sha256_Update(&ctx, 1);
    pbkdf2_hmac_sha256_Final(&ctx, keiv);
...
static secbool __wur derive_kek_unlock_v4(
    const uint8_t *pin, size_t pin_len, const uint8_t *storage_salt, const uint8_t *ext_salt,
    uint8_t kek[SHA256_DIGEST_LENGTH], uint8_t keiv[SHA256_DIGEST_LENGTH])
{
    // Legacy PIN verification method used in storage versions 1, 2, 3 and 4.
    ...
    stretch_pin(pin, pin_len, storage_salt, ext_salt, stretched_pin);
    optiga_pin_result ret = optiga_pin_verify_v4(ui_progress, stretched_pin, optiga_secret);
    ...
    if (ret != OPTIGA_PIN_SUCCESS) {
        ...
        ensure(ret == OPTIGA_PIN_INVALID ? sectrue : secfalse, "optiga_pin_verify failed");
        return secfalse;
    }
...
static secbool set_pin(const uint8_t *pin, size_t pin_len, const uint8_t *ext_salt)
{
    ...
    memzero(&ctx, sizeof(ctx));
    secbool ret = norcow_set(EDEK_PVC_KEY, buffer, STORAGE_SALT_SIZE + KEYS_SIZE + PVC_SIZE);
...
static void init_wiped_storage(void)
{
    if (sectrue != initialized) {
        ...
        optiga_random_buffer(cached_keys, sizeof(cached_keys)) ? sectrue : secfalse,
    ...
    uint32_t version = NORCOW_VERSION;
        ...
        norcow_set(UNAUTH_VERSION_KEY, &version, sizeof(version)),
    ...
    ensure(set_wipe_code(WIPE_CODE_EMPTY, WIPE_CODE_EMPTY_LEN), "set_wipe_code failed");
...
static uint32_t get_lock_version(void)
{
    const void *val = NULL;
    uint16_t len = 0;
    if (sectrue != norcow_get(UNAUTH_VERSION_KEY, &val, &len) || len != sizeof(uint32_t)) {
...
secbool check_storage_version(void)
{
    uint32_t version = 0;
    uint16_t len = 0;
    if (sectrue != storage_get_encrypted(VERSION_KEY, &version, sizeof(version), &len) ||
        len != sizeof(version)) {
    ...
    if (version > norcow_active_version) {
        ...
        norcow_set(STORAGE_UPGRADED_KEY, &FALSE_WORD, sizeof(FALSE_WORD));
        storage_set_encrypted(VERSION_KEY, &norcow_active_version, sizeof(norcow_active_version));
        norcow_set(UNAUTH_VERSION_KEY, &norcow_active_version, sizeof(norcow_active_version));
    } else {
...
static secbool __wur decrypt_dek(const uint8_t *pin, size_t pin_len, const uint8_t *ext_salt)
{
    // Read the storage salt, EDEK, ESAK and PIN verification code entry.
    const void *buffer = NULL;
    uint16_t len = 0;
    if (sectrue != initialized || sectrue != norcow_get(EDEK_PVC_KEY, &buffer, &len) ||
    ...
    const uint32_t *pvc =
        (const uint32_t *)buffer + (STORAGE_SALT_SIZE + KEYS_SIZE) / sizeof(uint32_t);
...
static secbool unlock(const uint8_t *pin, size_t pin_len, const uint8_t *ext_salt)
{
...
uint32_t storage_get_pin_rem(void)
{
    ...
    if (get_lock_version() >= 5) {
        ensure(optiga_pin_get_rem(&rem_optiga) * sectrue, "optiga_pin_get_rem failed");
    } else {
        ensure(optiga_pin_get_rem_v4(&rem_optiga) * sectrue, "optiga_pin_get_rem failed");
    }
    ...
    if (rem_optiga > rem_mcu) {
        if (get_lock_version() >= 5) {
                ...
                optiga_pin_decrease_rem(rem_optiga - rem_mcu) * sectrue,
                "optiga_pin_decrease_rem failed");
        } else {
            ensure(
                optiga_pin_decrease_rem_v4(rem_optiga - rem_mcu) * sectrue,
                "optiga_pin_decrease_rem failed");
        }
    }
#endif
    ...
    const uint8_t *old_ext_salt, const uint8_t *new_ext_salt)
{
    ...
    ui_message = (oldpin_len != 0 && newpin_len == 0) ? VERIFYING_PIN_MSG : PROCESSING_MSG;
    ...
    size_t wipe_code_len)
{
    if (sectrue != initialized || pin == NULL || wipe_code == NULL ||
        (pin_len != 0 && pin_len == wipe_code_len && memcmp(pin, wipe_code, pin_len) == 0)) {
        return secfalse;
    }

    ui_progress_init(STORAGE_PIN_OP_VERIFY);
    ui_message = (pin_len != 0 && wipe_code_len == 0) ? VERIFYING_PIN_MSG : PROCESSING_MSG;
...
static secbool storage_upgrade(void)
{
    ...
    if (norcow_active_version == 0) {
        ...
        while (sectrue == norcow_get_next(&offset, &key, &val, &len)) {
            const uint8_t app = key >> 8;
            if (((app & FLAG_PUBLIC) == 0) && (app != APP_STORAGE || key == VERSION_KEY)) {
                ...
                const size_t ciphertext_len = len - CHACHA20_IV_SIZE - POLY1305_TAG_SIZE;
    ...
    if (norcow_active_version <= 2) {
        ...
        if (sectrue == norcow_get(STORAGE_UPGRADED_KEY, &val, &len) && len == sizeof(FALSE_WORD) &&
            *((uint32_t *)val) == FALSE_WORD) {
            version = 2;
        }
...
static secbool storage_upgrade_unlocked(const uint8_t *pin, size_t pin_len, const uint8_t *ext_salt)
{
    uint32_t version = 0;
    uint16_t len = 0;
    if (sectrue != storage_get_encrypted(VERSION_KEY, &version, sizeof(version), &len) ||
