...
/* The upstream endianness detection was broken: __ORDER_BIG_ENDIAN__ is
 * defined by GCC and Clang on every platform, so the byte-wise big-endian
 * path was always selected on ARM and RISC-V, and other platforms failed
 * to build. The byte-wise path is endian-agnostic and correct everywhere,
