...
static void exp_mont_384(vec384 out, const vec384 inp, const limb_t *pow,
  ...
  while (pow_bits--) {
    ...
    if (is_bit_set(pow, pow_bits)) mul_mont_384(ret, ret, inp, p, n0);
  }
  ...
  for (i = 0; !is_bit_set(pow, i++);) sqr_mont_384(sqr, sqr, sqr, p, n0);
  vec_copy(out, sqr, sizeof(sqr));
  for (; i < pow_bits; i++) {
    sqr_mont_384(sqr, sqr, sqr, p, n0);
    if (is_bit_set(pow, i)) mul_mont_384(out, out, sqr, p, n0);
  }
#endif
}
