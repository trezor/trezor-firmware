...
static void HKDF_Extract(unsigned char PRK[32], const void *salt,
...
static void HKDF_Expand(unsigned char *OKM, size_t L,
                        const unsigned char PRK[32], const void *info,
                        size_t info_len, HMAC_SHA256_CTX *ctx) {
  ...
  if (info_len != 0) sha256_bcopy(info_prime, info, info_len);
#ifndef HKDF_TESTMODE
  /* Section 2.3 KeyGen in BLS-signature draft */
...
#ifndef HKDF_TESTMODE
void blst_keygen(vec256 SK, const void *IKM, size_t IKM_len, const void *info,
                 size_t info_len) {
  struct {
    HMAC_SHA256_CTX ctx;
  ...
  HKDF_Extract(scratch.PRK, "BLS-SIG-KEYGEN-SALT-", 20, IKM, IKM_len,
               &scratch.ctx);

  /* OKM = HKDF-Expand(PRK, key_info || I2OSP(L, 2), L) */
  HKDF_Expand(scratch.OKM, sizeof(scratch.OKM), scratch.PRK, info, info_len,
              &scratch.ctx);
