...
const POINTonE1 BLS12_381_NEG_G1 =
...
void blst_p1_affine_compress(unsigned char out[48],
                             const POINTonE1_affine *in) {
  if (vec_is_zero(in->X, 2 * sizeof(in->X))) {
    vec_zero(out, 48);
    out[0] = 0xc0; /* compressed and infinitiy bits */
  } else {
    unsigned char sign = (unsigned char)POINTonE1_affine_Compress_BE(out, in);
    out[0] |= 0x80 | ((sign & 2) << 4);
  }
}
...
static limb_t POINTonE1_Uncompress_BE(POINTonE1_affine *out,
  ...
  if (!sqrt_fp(ret.Y, ret.Y)) return (limb_t)0 - BLST_POINT_NOT_ON_CURVE;
...
BLST_ERROR blst_p1_uncompress(POINTonE1_affine *out,
                              const unsigned char in[48]) {
...
static BLST_ERROR POINTonE1_Deserialize_BE(POINTonE1_affine *out,
  ...
  ret.X[sizeof(ret.X) / sizeof(limb_t) - 1] &= ((limb_t)0 - 1) >> 3;
  add_fp(temp, ret.X, ZERO_384); /* less than modulus? */
  if (!vec_is_equal(temp, ret.X, sizeof(temp))) return BLST_BAD_ENCODING;

  add_fp(temp, ret.Y, ZERO_384); /* less than modulus? */
  if (!vec_is_equal(temp, ret.Y, sizeof(temp))) return BLST_BAD_ENCODING;

  mul_fp(ret.X, ret.X, BLS12_381_RR);
  mul_fp(ret.Y, ret.Y, BLS12_381_RR);

  if (!POINTonE1_affine_on_curve(&ret)) return BLST_POINT_NOT_ON_CURVE;
...
void blst_p1_cneg(POINTonE1 *a, size_t cbit) { POINTonE1_cneg(a, cbit); }
...
limb_t blst_p1_on_curve(const POINTonE1 *p) { return POINTonE1_on_curve(p); }
