...
static const vec384x Aprime_E2 = {/* 240*i */
                                  {0},
                                  {TO_LIMB_T(0xe53a000003135242),
                                   TO_LIMB_T(0x01080c0fdef80285),
                                   TO_LIMB_T(0xe7889edbe340f6bd),
                                   TO_LIMB_T(0x0b51375126310601),
                                   TO_LIMB_T(0x02d6985717c744ab),
                                   TO_LIMB_T(0x1220b4e979ea5467)}};
static const vec384x Bprime_E2 =
    {/* 1012 + 1012*i */
...
static void map_fp2_times_Zz(vec384x map[], const vec384x isogeny_map[],
  ...
  while (n--) mul_fp2(map[n], isogeny_map[n], Zz_powers[n]);
}

static void map_fp2(vec384x acc, const vec384x x, const vec384x map[],
                    size_t n) {
  while (n--) {
    mul_fp2(acc, acc, x);
...
static void isogeny_map_to_E2(POINTonE2 *out, const POINTonE2 *p) {
   ...
   * ...
   */
  static const vec384x isogeny_map_x_num[] = {
      /* (k_(1,*)<<384) % P   */
   ...
   */
  static const vec384x isogeny_map_x_den[] = {
      /* (k_(2,*)<<384) % P   */
      {{0},
   ...
   * ...
   */
  static const vec384x isogeny_map_y_num[] = {
      /* (k_(3,*)<<384) % P   */
   ...
   */
  static const vec384x isogeny_map_y_den[] = {
      /* (k_(4,*)<<384) % P   */
...
static void map_to_isogenous_E2(POINTonE2 *p, const vec384x u) {
  ...
  static const vec384x recip_ZZZ = {
      /* 1/(Z^3) */
...
#define sgn0_fp2(a) (sgn0_pty_mont_384x((a), BLS12_381_P, p0) & 1)
  ...
  vec_select(xd, ZxA, xd, sizeof(xd),
             e1); /*              # If xd == 0, set xd = Z*A */
  ...
  e2 = recip_sqrt_fp2(
      y1, tv4); /* y1 = tv4^c1          # (gx1*gxd^3)^((p^2-9)/16) */
...
static void POINTonE2_add_n_dbl(POINTonE2 *out, const POINTonE2 *p, size_t n) {
  ...
  while (n--) POINTonE2_double(out, out);
}
