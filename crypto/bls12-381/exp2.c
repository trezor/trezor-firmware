...
static void exp_mont_384x(vec384x out, const vec384x inp, const limb_t *pow,
  ...
  while (pow_bits--) {
    ...
    if (is_bit_set(pow, pow_bits)) mul_mont_384x(ret, ret, inp, p, n0);
  }
...
static void recip_sqrt_fp2_9mod16(vec384x out, const vec384x inp) {
  ...
  exp_mont_384x(out, inp, BLS_12_381_P_2_minus_9_div_16, 758, BLS12_381_P, p0);
}
#else
...
static limb_t sqrt_align_fp2(vec384x out, const vec384x ret, const vec384x sqrt,
                             const vec384x inp) {
