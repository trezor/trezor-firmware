...
size_t blst_pairing_sizeof(void) { return (sizeof(PAIRING) + 7) & ~(size_t)7; }

void blst_pairing_init(PAIRING *ctx, int hash_or_encode, const void *DST,
                       size_t DST_len) {
...
const void *blst_pairing_get_dst(const PAIRING *ctx) { return ctx->DST; }

#define FROM_AFFINE(out, in)                                       \
  do {                                                             \
    vec_copy((out)->X, in->X, 2 * sizeof(in->X)),                  \
        vec_select((out)->Z, in->X, BLS12_381_Rx.p, sizeof(in->X), \
                   vec_is_zero(in->X, 2 * sizeof(in->X)));         \
  } while (0)
...
static BLST_ERROR PAIRING_Aggregate_PK_in_G2(PAIRING *ctx,
  ...
  if (ctx->ctrl & AGGR_MIN_PK) return BLST_AGGR_TYPE_MISMATCH;
  ...
  if (sig != NULL) {
    ...
    if (ctx->ctrl & AGGR_SIGN_SET) {
      ...
      FROM_AFFINE(S, sig);
      if (nbits != 0 && scalar != NULL) POINTonE1_mult_w5(S, S, scalar, nbits);
    }
  }
  ...
  if (PK != NULL) {
    ...
    if (nbits != 0 && scalar != NULL) POINTonE1_mult_w5(H, H, scalar, nbits);
...
BLST_ERROR blst_pairing_aggregate_pk_in_g2(PAIRING *ctx,
                                           ...
                                           const void *msg, size_t msg_len,
                                           const void *aug, size_t aug_len) {
  return PAIRING_Aggregate_PK_in_G2(ctx, PK, signature, NULL, 0, msg, msg_len,
                                    aug, aug_len);
}

BLST_ERROR blst_pairing_mul_n_aggregate_pk_in_g2(
    PAIRING *ctx, const POINTonE2_affine *PK, const POINTonE1_affine *sig,
    const limb_t *scalar, size_t nbits, const void *msg, size_t msg_len,
    const void *aug, size_t aug_len) {
  return PAIRING_Aggregate_PK_in_G2(ctx, PK, sig, scalar, nbits, msg, msg_len,
                                    aug, aug_len);
}
...
static BLST_ERROR PAIRING_Aggregate_PK_in_G1(PAIRING *ctx,
                                             ...
                                             const void *msg, size_t msg_len,
                                             const void *aug, size_t aug_len) {
  if (ctx->ctrl & AGGR_MIN_SIG) return BLST_AGGR_TYPE_MISMATCH;
  ...
  if (sig != NULL) {
    ...
    if (ctx->ctrl & AGGR_SIGN_SET) {
      ...
      ctx->ctrl |= AGGR_SIGN_SET;
      FROM_AFFINE(S, sig);
      if (nbits != 0 && scalar != NULL) POINTonE2_mult_w5(S, S, scalar, nbits);
    }
  }
...
BLST_ERROR blst_pairing_aggregate_pk_in_g1(PAIRING *ctx,
                                           ...
                                           const void *msg, size_t msg_len,
                                           const void *aug, size_t aug_len) {
  return PAIRING_Aggregate_PK_in_G1(ctx, PK, signature, NULL, 0, msg, msg_len,
                                    aug, aug_len);
}

BLST_ERROR blst_pairing_mul_n_aggregate_pk_in_g1(
    PAIRING *ctx, const POINTonE1_affine *PK, const POINTonE2_affine *sig,
    const limb_t *scalar, size_t nbits, const void *msg, size_t msg_len,
    const void *aug, size_t aug_len) {
  return PAIRING_Aggregate_PK_in_G1(ctx, PK, sig, scalar, nbits, msg, msg_len,
                                    aug, aug_len);
}
...
void blst_pairing_commit(PAIRING *ctx) { PAIRING_Commit(ctx); }
...
BLST_ERROR blst_pairing_merge(PAIRING *ctx, const PAIRING *ctx1) {
  ...
  if (ctx->nelems || ctx1->nelems) return BLST_AGGR_TYPE_MISMATCH;

  switch (ctx->ctrl & MIN_SIG_OR_PK) {
    case AGGR_MIN_SIG:
      if (ctx->ctrl & ctx1->ctrl & AGGR_SIGN_SET) {
        POINTonE1_dadd(&ctx->AggrSign.e1, &ctx->AggrSign.e1, &ctx1->AggrSign.e1,
                       NULL);
    ...
    case AGGR_MIN_PK:
      if (ctx->ctrl & ctx1->ctrl & AGGR_SIGN_SET) {
        POINTonE2_dadd(&ctx->AggrSign.e2, &ctx->AggrSign.e2, &ctx1->AggrSign.e2,
                       NULL);
      } else if (ctx1->ctrl & AGGR_SIGN_SET) {
        ctx->ctrl |= AGGR_SIGN_SET;
...
static limb_t PAIRING_FinalVerify(const PAIRING *ctx, const vec384fp12 GTsig) {
  ...
  if (!(ctx->ctrl & AGGR_GT_SET)) return 0;
...
BLST_ERROR blst_aggregate_in_g1(POINTonE1 *out, const POINTonE1 *in,
  ...
  if (zwire[0] & 0x40) { /* infinity? */
    if (in == NULL) vec_zero(out, sizeof(*out));
    return BLST_SUCCESS;
  }

  if (zwire[0] & 0x80) { /* compressed? */
    BLST_ERROR ret = POINTonE1_Uncompress(P, zwire);
    if (ret != BLST_SUCCESS) return ret;
  } else {
    POINTonE1_Deserialize_BE(P, zwire);
    if (!POINTonE1_affine_on_curve(P)) return BLST_POINT_NOT_ON_CURVE;
  }

  if (!POINTonE1_in_G1((POINTonE1 *)P)) return BLST_POINT_NOT_IN_GROUP;
...
BLST_ERROR blst_aggregate_in_g2(POINTonE2 *out, const POINTonE2 *in,
  ...
  if (zwire[0] & 0x80) { /* compressed? */
    ...
    if (!POINTonE2_affine_on_curve(P)) return BLST_POINT_NOT_ON_CURVE;
  }

  if (!POINTonE2_in_G2((POINTonE2 *)P)) return BLST_POINT_NOT_IN_GROUP;
...
BLST_ERROR blst_core_verify_pk_in_g1(const POINTonE1_affine *pk,
                                     const POINTonE2_affine *signature,
                                     int hash_or_encode, const void *msg,
                                     size_t msg_len, const void *DST,
                                     size_t DST_len, const void *aug,
                                     size_t aug_len) {
  PAIRING ctx;
  BLST_ERROR ret;
  ...
  ret = PAIRING_Aggregate_PK_in_G1(&ctx, pk, signature, NULL, 0, msg, msg_len,
                                   aug, aug_len);
  if (ret != BLST_SUCCESS) return ret;
...
BLST_ERROR blst_core_verify_pk_in_g2(const POINTonE2_affine *pk,
  ...
  ret = PAIRING_Aggregate_PK_in_G2(&ctx, pk, signature, NULL, 0, msg, msg_len,
                                   aug, aug_len);
  if (ret != BLST_SUCCESS) return ret;
