...
void blst_fp_mul_by_3(vec384 ret, const vec384 a) { mul_by_3_fp(ret, a); }

void blst_fp_mul_by_8(vec384 ret, const vec384 a) { mul_by_8_fp(ret, a); }
...
void blst_fp_sqr(vec384 ret, const vec384 a) { sqr_fp(ret, a); }
...
void blst_fp_to(vec384 ret, const vec384 a) { mul_fp(ret, a, BLS12_381_RR); }

void blst_fp_from(vec384 ret, const vec384 a) { from_fp(ret, a); }
...
void blst_fp_from_uint32(vec384 ret, const unsigned int a[12]) {
  if (sizeof(limb_t) == 8) {
    int i;
    for (i = 0; i < 6; i++) ret[i] = a[2 * i] | ((limb_t)a[2 * i + 1] << 32);
...
void blst_fp2_mul_by_3(vec384x ret, const vec384x a) { mul_by_3_fp2(ret, a); }

void blst_fp2_mul_by_8(vec384x ret, const vec384x a) { mul_by_8_fp2(ret, a); }
...
void blst_fp2_sqr(vec384x ret, const vec384x a) { sqr_fp2(ret, a); }
...
void blst_p1_mult(POINTonE1 *out, const POINTonE1 *a, const limb_t *scalar,
                  size_t nbits) {
...
void blst_p2_mult(POINTonE2 *out, const POINTonE2 *a, const limb_t *scalar,
                  size_t nbits) {
...
void blst_scalar_from_uint32(vec256 ret, const unsigned int a[8]) {
  ...
  if (sizeof(limb_t) == 4) {
    ...
    int i;
    for (i = 0; i < 4; i++) ret[i] = a[2 * i] | ((limb_t)a[2 * i + 1] << 32);
  }
}
...
void blst_scalar_from_uint64(vec256 ret, const unsigned long long a[4]) {
  ...
  if (sizeof(limb_t) == 8 || is_endian.little) {
    if ((uptr_t)ret != (uptr_t)a) vec_copy(ret, a, sizeof(vec256));
  } else {
    int i;
    for (i = 0; i < 4; i++) {
      unsigned long long limb = a[i];
...
void blst_scalar_from_lendian(vec256 ret, const unsigned char a[32]) {
  ...
  if ((uptr_t)ret == (uptr_t)a && is_endian.little) return;
...
static void limbs_from_hexascii(limb_t *ret, size_t sz,
  ...
  while (len--) {
    limb <<= 4;
    limb |= nibble(*hex++);
    if (len % (2 * sizeof(limb_t)) == 0) ret[len / (2 * sizeof(limb_t))] = limb;
  }
}
