...
static void map_fp_times_Zz(vec384 map[], const vec384 isogeny_map[],
  ...
  while (n--) mul_fp(map[n], isogeny_map[n], Zz_powers[n]);
}
...
static void isogeny_map_to_E1(POINTonE1 *out, const POINTonE1 *p) {
   ...
   * ...
   */
  static const vec384 isogeny_map_x_num[] = {
      /*  (k_(1,*)<<384) % P  */
   ...
   */
  static const vec384 isogeny_map_x_den[] = {
      /*  (k_(2,*)<<384) % P  */
   ...
   * ...
   */
  static const vec384 isogeny_map_y_num[] = {
      /*  (k_(3,*)<<384) % P  */
   ...
   */
  static const vec384 isogeny_map_y_den[] = {
      /*  (k_(4,*)<<384) % P  */
...
#define sgn0_fp(a) (sgn0_pty_mont_384((a), BLS12_381_P, p0) & 1)
  ...
  e2 =
      recip_sqrt_fp(y1, tv4); /* y1 = tv4^c1          # (gx1*gxd^3)^((p-3)/4) */
  mul_fp(y1, y1, tv2);        /* y1 = y1 * tv2        # gx1*gxd*y1 */
  mul_fp(y2, y1,
         sqrt_minus_ZZZ); /* y2 = y1 * c2         # y2 = y1*sqrt(-Z^3) */
...
static void POINTonE1_dbl_n_add(POINTonE1 *out, size_t n, const POINTonE1 *p) {
  while (n--) POINTonE1_double(out, out);
...
static void POINTonE1_times_zz_minus_1_div_by_3(POINTonE1 *out,
                                                const POINTonE1 *in) {
  static const limb_t zz_minus_1_div_by_3[] = {TO_LIMB_T(0x0000000055555555),
                                               TO_LIMB_T(0x396c8c005555e156)};
  size_t n = 126 - 1;
  const POINTonE1 *dblin = in;

  while (n--) {
    POINTonE1_double(out, dblin);
    dblin = out;
    if (is_bit_set(zz_minus_1_div_by_3, n)) POINTonE1_add(out, out, in);
  }
}
#else
static void POINTonE1_times_zz_minus_1_div_by_3(POINTonE1 *out,
...
static void sigma(POINTonE1 *out, const POINTonE1 *in) {
  static const vec384 beta = {
      /* such that beta^3 - 1 = 0  */
