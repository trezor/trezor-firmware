...
#define POINT_MULT_SCALAR_W5_IMPL(ptype)                                       \
  static void ptype##_gather_booth_w5(                                         \
      ptype *restrict p, const ptype table[16], limb_t booth_idx) {            \
    size_t i;                                                                  \
    ...
    /* ~6% with -Os, ~2% with -O3 ... */                                       \
    for (i = 1; i <= 16; i++) ptype##_ccopy(p, table + i - 1, i == booth_idx); \
                                                                               \
  ...
  static void ptype##_precompute(ptype *row, const ptype *point) {             \
    ...
    ptype##_double(row + 16, row + 8);       /* row[16]=p*(8+8) */             \
                                                                               ...
                                                                               \
  static void ptype##s_mult_w5(ptype *ret, const ptype *points[],              \
                               size_t npoints, const limb_t *scalars[],        \
                               size_t bits, ptype table[][16]) {               \
    limb_t wmask, wval;                                                        \
    size_t i, window;                                                          \
    ptype temp[1];                                                             \
                                                                               \
    if (table == NULL) table = alloca(16 * sizeof(ptype) * npoints);           \
                                                                               \
    for (i = 0; i < npoints; i++) ptype##_precompute(table[i], points[i]);     \
                                                                               \
