...
#define DECLARE_VARIANT(prefix)                                                \
  ...
  extern int prefix##_sign(uint8_t *, size_t *, const uint8_t *, size_t,       \
                           const uint8_t *);                                   \
  extern int prefix##_verify(const uint8_t *, size_t, const uint8_t *, size_t, \
                             const uint8_t *);                                 \
...
#define VARIANT_ENTRY(prefix, n_val, pk_val, sig_val)                         \
  {prefix##_seed_keypair, prefix##_sign, prefix##_verify, prefix##_sig_bytes, \
   prefix##_pk_bytes,     pk_val,        sig_val,         n_val}
