...
#define NOISE_NXPSK2_HASHLEN 32
#define NOISE_NXPSK2_DHLEN 32

// Uncomment to enable initiator/responder functionality in the noise protocol
// implementation.
#define USE_NOISE_NXPSK2_INITIATOR
#define USE_NOISE_NXPSK2_RESPONDER

typedef struct {
  uint8_t key[NOISE_NXPSK2_HASHLEN];
  bool has_key;
  uint64_t nonce;
} noise_nxpsk2_cipher_state_t;

typedef struct {
  uint8_t handshake_hash[NOISE_NXPSK2_HASHLEN];
  uint8_t chaining_key[NOISE_NXPSK2_HASHLEN];
  noise_nxpsk2_cipher_state_t cipher_state;
} noise_nxpsk2_symmetric_state_t;

typedef struct {
  noise_nxpsk2_cipher_state_t send_cipher_state;
  noise_nxpsk2_cipher_state_t receive_cipher_state;
  uint8_t handshake_hash[NOISE_NXPSK2_HASHLEN];
} noise_nxpsk2_transport_state_t;

typedef struct {
  noise_nxpsk2_symmetric_state_t symmetric_state;
  uint8_t psk[NOISE_NXPSK2_DHLEN];

  bool has_ephemeral_private;
  uint8_t ephemeral_private[NOISE_NXPSK2_DHLEN];

  bool has_remote_ephemeral_public;
  uint8_t remote_ephemeral_public[NOISE_NXPSK2_DHLEN];

} noise_nxpsk2_handshake_state_t;

#ifdef USE_NOISE_NXPSK2_RESPONDER

// Random values are used for greater resilience agains glitching attacks.
typedef enum {
  NOISE_NXPSK2_RSPN_WAITING_FOR_REQUEST1 = 0x5091d95c,
  NOISE_NXPSK2_RSPN_READY_FOR_RESPONSE1 = 0x252d533a,
  NOISE_NXPSK2_RSPN_HANDSHAKE_COMPLETE = 0x54acac08
} noise_nxpsk2_responder_handshake_stage_t;

typedef struct {
  bool initialized;
  bool has_transport_state;
  noise_nxpsk2_responder_handshake_stage_t handshake_stage;
  noise_nxpsk2_handshake_state_t handshake_state;
  noise_nxpsk2_transport_state_t transport_state;
} noise_nxpsk2_responder_t;
...
// Random values are used for greater resilience agains glitching attacks.
typedef enum {
  NOISE_NXPSK2_INTR_READY_FOR_REQUEST1 = 0x24a23a5e,
  NOISE_NXPSK2_INTR_WAITING_FOR_RESPONSE1 = 0xa748a792,
  NOISE_NXPSK2_INTR_HANDSHAKE_COMPLETE = 0xf149f042
} noise_nxpsk2_initiator_handshake_stage_t;

typedef struct {
  bool initialized;
  bool has_transport_state;
  noise_nxpsk2_initiator_handshake_stage_t handshake_stage;
  noise_nxpsk2_handshake_state_t handshake_state;
  noise_nxpsk2_transport_state_t transport_state;
} noise_nxpsk2_initiator_t;

/**
 * The following set of functions implements the initiator side of the
 * Noise_NXpsk2_25519_AESGCM_SHA256 protocol and has to be used in the following
 * order:
 *
 * 1. noise_nxpsk2_initiator_init() - to initialize the initiator structure with
 * the pre-shared key
 * 2. noise_nxpsk2_initiator_create_request1() - to create the first message,
 * which includes the initiator's ephemeral public key and an encrypted payload
 * 3. noise_nxpsk2_initiator_handle_response1() - to handle the response from
 * the responder, which includes the responder's ephemeral public key, encrypted
 * static public key and encrypted payload, completing the handshake
 *
 * The initiator has no static key pair, so it is authenticated by the knowledge
 * of the pre-shared key only. The responder's static public key is not known in
 * advance, it is learned during the handshake and returned to the caller by
 * noise_nxpsk2_initiator_handle_response1().
 *
...
/**
 * @brief Initialize the initiator structure with the pre-shared key.
 *
...
/**
 * @brief Handle response1, the second and last handshake message.
 *
 ...
 * mixes in the pre-shared key, decrypts the response payload, and completes the
 * handshake by deriving the transport state.
 *
 * The responder is authenticated by the knowledge of the pre-shared key. Its
 * static public key is returned to the caller, who is responsible for deciding
 * whether it belongs to the expected party.
 *
 * In:    Incoming response format:
 * <ephemeral_public_key[32B]><encrypted_static_public_key[48B]><encrypted_payload[payload_size
 * + 16B]>
 * Out:   Responder's static public key and decrypted payload
 *
 * @param intr Pointer to the initiator structure
 * @param response Incoming response buffer
 * @param response_len Length of the incoming response buffer
 * @param remote_static_public Output buffer for the responder's static public
 * key (32 bytes)
 ...
 * order:
 *
 * 1. noise_nxpsk2_responder_init() - to initialize the responder structure with
 * the pre-shared key
 * 2. noise_nxpsk2_responder_handle_request1() - to handle the first message
 * from the initiator and extract the initiator's ephemeral public key
 * 3. noise_nxpsk2_responder_create_response1() - to create the response to the
 * first message, which includes the responder's ephemeral public key, encrypted
 * static public key and encrypted payload, completing the handshake
 *
 * The initiator has no static key pair in the NX pattern, so it is
 * authenticated by the knowledge of the pre-shared key only.
 *
 * Incorrect usage of the functions or incorrect message formats will lead to a
 * false status being returned and the caller should handle it accordingly (e.g.
 * by terminating the connection). After the handshake is complete, the caller
...
/**
 * @brief Initialize the responder structure with the pre-shared key.
 *
...
/**
 * @brief Create response1, the second and last handshake message.
 *
 * Generates the responder's ephemeral key pair, performs the ee DH operation,
 * encrypts and sends the responder's static public key, performs the es DH
 ...
 * @param static_public_key Static public key for the responder (32 bytes)
 * @param payload Plain payload to send (may be empty)
 * @param payload_size Length of the payload in bytes
