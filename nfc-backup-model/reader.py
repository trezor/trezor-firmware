...
class Reader:
    ...
    def connect(self):
        select_resp = ApduResponse.from_bytes(
            self._powered_card.handle_request(
                ApduRequest.from_header(commands.SELECT_APPLICATION, commands.TREZOR_APPLICATION_AID).to_bytes()
            )
        )
        ...
        hs1_resp = ApduResponse.from_bytes(
            self._powered_card.handle_request(
                ApduRequest.from_header(commands.TREZOR_HANDSHAKE_MESSAGE_1, initiator.create_request1()).to_bytes()
            )
        )
        ...
        hs2_resp = ApduResponse.from_bytes(
            self._powered_card.handle_request(
                ApduRequest.from_header(commands.TREZOR_HANDSHAKE_MESSAGE_2, initiator.create_request2()).to_bytes()
            )
        )
        assert hs2_resp.status == OK

        self._transport_state = initiator.get_transport_state()
        self._transport_state.receive_cipher_state.decrypt_with_ad(b"", hs2_resp.response)
    ...
    def _transcieve_encrypted(self, header, plaintext: bytes) -> bytes:
        ...
        encrypted = self._transport_state.send_cipher_state.encrypt_with_ad(header_bytes, plaintext)
        resp = ApduResponse.from_bytes(
            self._powered_card.handle_request(ApduRequest.from_header(header, encrypted).to_bytes())
        )
        assert resp.status == OK
        return self._transport_state.receive_cipher_state.decrypt_with_ad(b"", resp.response)
    ...
    def authenticate(self, pin: Pin, note: bytes) -> bytes:
        return self._transcieve_encrypted(commands.TREZOR_AUTHENTICATE, pickle.dumps((pin, note)))
