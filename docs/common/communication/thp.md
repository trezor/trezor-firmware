...
The purpose of this protocol is to establish a secure communication channel between a host application, such as Trezor Suite, and a Trezor device. When a given host establishes a secure channel with a Trezor for the first time, the two engage in a pairing procedure, which requires the user to complete one of the pairing methods listed in Section [TODO]. After a successful pairing, the host may request that the Trezor issue a pairing credential. The credential allows reestablishment of the secure channel without user interaction in the future.
...
- When the message HandshakeInitiationResp(*trezor_ephemeral_pubkey, encrypted_trezor_static_pubkey, tag*) is received, take the following actions:
    ...
    11. Search *credentials* for a pairs (*trezor_static_pubkey*, *credential*) such that *trezor_masked_static_pubkey* == X25519(SHA-256(*trezor_static_pubkey* || *trezor_ephemeral_pubkey*), *trezor_static_pubkey*).
        - If found, then do the following:
            - Set (*temp_host_static_privkey*, *temp_host_static_pubkey*) = (*host_static_privkey*, *host_static_pubkey*).
            - Set *state* = STATE_PAIRED.
            - Set *host_pairing_credential* = *credential*.
        - If not found, then do the following:
            - Set ~~(*temp_host_static_privkey*, *temp_host_static_pubkey*) = (X25519(0, *B*), 0).~~
                - Generate a new host_static_keypair randomly
            - Set *state* = STATE_UNPAIRED.
            - Set *host_pairing_credential* = *empty_string*. (TODO: should be none, i.e. the optional field *host_pairing_credential* in protobuf is omitted)
...
flowchart TD
  ...
  TP1 -- "in:&nbsp;SelectedMethod" --> selected_method
  ...
  TP3 -- "in:&nbsp;SelectedMethod" --> selected_method  
...
flowchart TD
  ...
  selected_method -- "[selected_method&nbsp;==&nbsp;CodeEntry and it WAS selected before]<br>out:&nbsp;PairingPreparationsFinished" --> TP3a
    selected_method -- "[selected_method&nbsp;==&nbsp;QrCode]<br>out:&nbsp;PairingPreparationsFinished" --> TP3b
      selected_method -- "[selected_method&nbsp;==&nbsp;Nfc]<br>out:&nbsp;PairingPreparationsFinished" --> TP3c
  TP3a -- "in:&nbsp;SelectedMethod" --> selected_method
  TP3b -- "in:&nbsp;SelectedMethod" --> selected_method  
  TP3c -- "in:&nbsp;SelectedMethod" --> selected_method  
  TP3a -- "in:&nbsp;CodeEntryCpaceHostTag <br>out:&nbsp;CodeEntrySecret" --> TC1
  TP3b -- "in:&nbsp;QrCodeTag<br>out:&nbsp;QrCodeSecret" --> TC1
  TP3c -- "in:&nbsp;NfcTagHost<br>out:&nbsp;NfcTagTrezor" --> TC1
```
...
- When the message StartPairingRequest(*host_name*) is received, take the following action:
    1. Display a dialog “Device XY wants to pair (TODO)” on the screen.
    2. Send ButtonRequest message.
    3. Wait for ButtonAck Response from the host.
    4. Wait for user interaction:
        1. User cancelled the dialog - end pairing, send ActionCancelled Failure, (TODO drop channel?)
        2. User confirmed the dialog - transition to state TP1

### State TP1

The behavior of Trezor in the state TP1is defined as follows:

- When the message SelectedMethod(selected_pairing_method) is received, transition to intermediate state “selected method”.

### State “selected method”

1. Read selected_pairing_method from received message SelectedMethod(selected_pairing_method)
...
- When the message SelectedMethod(selected_pairing_method) is received, transition to the intermediate state “selected method”.
