...
static const struct usb_string_descriptor winusb_string_descriptor = {
    .bLength = 0x12, .bDescriptorType = USB_DT_STRING, .wData = WINUSB_EXTRA_STRING};
...
static void winusb_set_config(usbd_device *usbd_dev, uint16_t wValue)
{
    ...
    usbd_register_control_callback(
        usbd_dev, USB_REQ_TYPE_VENDOR, USB_REQ_TYPE_TYPE, winusb_control_vendor_request);
}
...
void winusb_setup(usbd_device *usbd_dev, uint8_t interface)
{
    ...
    usbd_register_control_callback(
        usbd_dev, USB_REQ_TYPE_DEVICE, USB_REQ_TYPE_RECIPIENT, winusb_descriptor_request);
}
