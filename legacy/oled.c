...
void oledDrawCharUpdateCaret(int x, int y, char c, int font, int zoom, bool update_caret) {
...
int oledPartialStringWidth(const char *text, int font, int num) {
  if (!text) return 0;
...
void oledDrawPartialStringSize(int x, int y, const char* text, int font, int size, int num)
{
	if (!text) return;
	int l = 0;
	for (; *text && num; text++, num--) {
...
void oledDrawPartialString(int x, int y, const char* text, int font, int num)
{
	oledDrawPartialStringSize(x, y, text, font, 1, num);
}

void oledDrawStringCenterMultiline(int y, const char* text, int font, int maxwidth)
{
