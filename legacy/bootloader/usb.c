...
static void check_and_write_chunk(void)
{
    ...
    for (uint32_t i = offset / sizeof(uint32_t); i < chunk_pos / sizeof(uint32_t); i++) {
        flash_program_word(
            FLASH_FWHEADER_START + chunk_idx * FW_CHUNK_SIZE + i * sizeof(uint32_t), FW_CHUNK[i]);
    }
    flash_exit();
...
static secbool readprotobufint(const uint8_t **ptr, const uint8_t *end, uint32_t *result)
{
    *result = 0;
...
static int should_keep_storage(int old_was_signed, uint32_t fix_version_current)
{
...
static void rx_callback(usbd_device *dev, uint8_t ep)
{
    ...
    if (flash_state == STATE_READY || flash_state == STATE_OPEN) {
        ...
        if (msg_id == 0x0005) {  // WipeDevice message (id 5)
            layoutDialog(
                &bmp_icon_question, "Cancel", "Confirm", NULL, "Do you really want to",
                "wipe the device?", NULL, "All data will be lost.", NULL, NULL);
    ...
    if (flash_state == STATE_OPEN) {
        if (msg_id == 0x0006) {  // FirmwareErase message (id 6)
            ...
            if (firmware_present_new()) {
                layoutDialog(
                    &bmp_icon_question, "Abort", "Continue", NULL, "Install new", "firmware?", NULL,
                    "Never do this without", "your recovery card!", NULL);
            ...
            if (proceed) {
                ...
                if (firmware_present_new()) {
                    ...
                    old_was_signed = signatures_match(hdr, NULL) & check_firmware_hashes(hdr);
    ...
    if (flash_state == STATE_CHECK) {
        ...
        if (hash_check_ok) {
            ...
            for (int i = 3; i > 0; i--) {
                line[21] = '0' + i;
                layoutDialog(
                    &bmp_icon_ok, NULL, NULL, NULL, "New firmware", "successfully installed.", NULL,
                    "Your Trezor", line, NULL);
                delay(30000 * 1000);
            }
            scb_reset_system();
        } else {
            layoutDialog(
                &bmp_icon_warning, NULL, NULL, NULL, "Firmware installation", "aborted.", NULL,
                "You need to repeat", "the procedure with", "the correct firmware.");
            send_msg_failure(dev, 9);  // Failure_ProcessError
            shutdown();
        }
        return;
...
static const struct usb_device_capability_descriptor *capabilities_landing[] = {
    (const struct usb_device_capability_descriptor *)&webusb_platform_capability_descriptor_landing,
};
...
static const struct usb_bos_descriptor bos_descriptor_landing = {
    ...
    .bNumDeviceCaps = sizeof(capabilities_landing) / sizeof(capabilities_landing[0]),
    .capabilities = capabilities_landing};

static const struct usb_bos_descriptor bos_descriptor_no_landing = {
    .bLength = USB_DT_BOS_SIZE,
    .bDescriptorType = USB_DT_BOS,
    .bNumDeviceCaps = sizeof(capabilities_no_landing) / sizeof(capabilities_no_landing[0]),
...
static void usbInit(bool firmware_present)
{
    usbd_dev = usbd_init(
        ...
        sizeof(usb_strings) / sizeof(const char *), usbd_control_buffer,
        sizeof(usbd_control_buffer));
    usbd_register_set_config_callback(usbd_dev, set_config);
    usb21_setup(usbd_dev, firmware_present ? &bos_descriptor_no_landing : &bos_descriptor_landing);
