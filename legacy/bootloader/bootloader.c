...
void layoutFirmwareFingerprint(const uint8_t *hash)
{
    ...
    layoutDialog(
        &bmp_icon_question, "Abort", "Continue", "Compare fingerprints", str[0], str[1], str[2],
        str[3], NULL, NULL);
}
...
{
    layoutDialog(
        &bmp_icon_error, NULL, NULL, NULL, line1, line2, NULL, "Unplug your Trezor,",
...
{
    layoutDialog(
        &bmp_icon_ok, NULL, NULL, NULL, line1, line2, NULL, "You may now", "unplug your Trezor.",
        NULL);
}
...
#if PRODUCTION
    layoutDialog(
        &bmp_icon_warning, "Abort", "I'll take the risk", NULL, "WARNING!", NULL,
        "Unofficial firmware", "detected.", NULL, NULL);
...
static void bootloader_loop(void)
{
    ...
    if (firmware_present_new()) {
        ...
        oledDrawStringCenter(
            90, 50, VERSTR(VERSION_MAJOR) "." VERSTR(VERSION_MINOR) "." VERSTR(VERSION_PATCH),
            FONT_STANDARD);
    } else {
