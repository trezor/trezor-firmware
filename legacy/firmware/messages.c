...
_Static_assert(MSG_OUT_BUFFER_SIZE % USB_PACKET_SIZE == 0, "MSG_OUT_BUFFER_SIZE");
...
_Static_assert(MSG_DEBUG_OUT_BUFFER_SIZE % USB_PACKET_SIZE == 0, "MSG_DEBUG_OUT_BUFFER_SIZE");
...
static inline void msg_debug_out_append(uint8_t c)
{
    ...
    if (msg_debug_out_cur == USB_PACKET_SIZE) {
        msg_debug_out_cur = 0;
        msg_debug_out_end = (msg_debug_out_end + 1) % (MSG_DEBUG_OUT_BUFFER_SIZE / USB_PACKET_SIZE);
    }
}
...
void msg_process(
    char type, uint16_t msg_id, const pb_msgdesc_t *fields, uint8_t *msg_encoded,
...
void msg_read_common(char type, const uint8_t *buf, uint32_t len)
{
    ...
    if (read_state == READSTATE_IDLE) {
        ...
        msg_encoded_size = ((uint32_t)buf[5] << 24) + (buf[6] << 16) + (buf[7] << 8) + buf[8];
...
const uint8_t *msg_debug_out_data(void)
{
    ...
    msg_debug_out_start = (msg_debug_out_start + 1) % (MSG_DEBUG_OUT_BUFFER_SIZE / USB_PACKET_SIZE);
...
_Static_assert(USB_PACKET_SIZE >= MSG_HEADER_SIZE + Cancel_size, "msg_tiny too tiny");
_Static_assert(sizeof(msg_tiny) >= sizeof(Initialize), "msg_tiny too tiny");
_Static_assert(USB_PACKET_SIZE >= MSG_HEADER_SIZE + Initialize_size, "msg_tiny too tiny");
_Static_assert(sizeof(msg_tiny) >= sizeof(PassphraseAck), "msg_tiny too tiny");
_Static_assert(USB_PACKET_SIZE >= MSG_HEADER_SIZE + PassphraseAck_size, "msg_tiny too tiny");
_Static_assert(sizeof(msg_tiny) >= sizeof(ButtonAck), "msg_tiny too tiny");
_Static_assert(USB_PACKET_SIZE >= MSG_HEADER_SIZE + ButtonAck_size, "msg_tiny too tiny");
_Static_assert(sizeof(msg_tiny) >= sizeof(PinMatrixAck), "msg_tiny too tiny");
_Static_assert(USB_PACKET_SIZE >= MSG_HEADER_SIZE + PinMatrixAck_size, "msg_tiny too tiny");
#if DEBUG_LINK
_Static_assert(sizeof(msg_tiny) >= sizeof(DebugLinkDecision), "msg_tiny too tiny");
_Static_assert(USB_PACKET_SIZE >= MSG_HEADER_SIZE + DebugLinkDecision_size, "msg_tiny too tiny");
_Static_assert(sizeof(msg_tiny) >= sizeof(DebugLinkGetState), "msg_tiny too tiny");
_Static_assert(USB_PACKET_SIZE >= MSG_HEADER_SIZE + DebugLinkGetState_size, "msg_tiny too tiny");
#endif
uint16_t msg_tiny_id = 0xFFFF;

void msg_read_tiny(const uint8_t *buf, int len)
{
    if (len != USB_PACKET_SIZE || buf[0] != '?' || buf[1] != '#' || buf[2] != '#') {
    ...
    if (msg_size > sizeof(msg_tiny) / 2 || msg_size > (uint32_t)len - MSG_HEADER_SIZE) {
