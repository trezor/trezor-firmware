...
static HDNode *fsm_getDerivedNode(
    const char *curve, const uint32_t *address_n, size_t address_n_count, uint32_t *fingerprint)
{
...
static bool fsm_getSlip21Key(const char *path[], size_t path_count, uint8_t key[32])
{
...
static bool fsm_layoutAddress(
    const char *address, const char *desc, bool ignorecase, size_t prefixlen,
    const uint32_t *address_n, size_t address_n_count, bool address_is_account,
    const MultisigRedeemScriptType *multisig, int multisig_index, uint32_t multisig_xpub_magic,
    const CoinInfo *coin)
{
    ...
    for (;;) {
        switch (screen) {
            case 0: {  // show address
                ...
                layoutAddress(
                    display_addr, desc, false, ignorecase, address_n, address_n_count,
                    address_is_account);
                break;
            }
            case 1: {  // show QR code
                layoutAddress(
                    address, desc, true, ignorecase, address_n, address_n_count,
                    address_is_account);
                break;
            }
            default: {  // show XPUBs
                ...
                if (!node_ptr) {
                    ...
                    HDNode node;
                    if (!hdnode_from_xpub(
                            node_ptr->depth, node_ptr->child_num, node_ptr->chain_code.bytes,
                            node_ptr->public_key.bytes, coin->curve_name, &node)) {
                        strlcat(xpub, "ERROR", sizeof(xpub));
                    } else {
                        hdnode_serialize_public(
                            &node, node_ptr->fingerprint, multisig_xpub_magic, xpub, sizeof(xpub));
                    }
                }
...
static bool fsm_layoutPaginated(
    const char *description, const uint8_t *msg, uint32_t len, bool is_ascii)
{
    const char **str = NULL;
    ...
    do {
        ...
        layoutDialogSwipeEx(
            &bmp_icon_question, _("Cancel"), label, description, str[0], str[1], str[2], str[3],
            NULL, NULL, FONT_FIXED);
...
void fsm_msgRebootToBootloader(void)
{
    layoutDialogSwipe(
        ...
        _("restart device in"), _("bootloader mode?"), NULL, NULL, NULL);
...
{
    layoutDialogSwipe(
        &bmp_icon_warning, _("Abort"), _("Continue"), NULL, _("Wrong address path"),
        _("for selected coin."), NULL, _("Continue at your"), _("own risk!"), NULL);
    if (!protectButton(ButtonRequestType_ButtonRequest_UnknownDerivationPath, false)) {
        fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
        return false;
    }
    return true;
