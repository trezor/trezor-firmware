...
static bool fsm_nemCheckPath(uint32_t address_n_count, const uint32_t *address_n, uint8_t network)
{
...
void fsm_msgNEMGetAddress(NEMGetAddress *msg)
{
    ...
    HDNode *node =
        fsm_getDerivedNode(ED25519_KECCAK_NAME, msg->address_n, msg->address_n_count, NULL);
    if (!node) return;
    ...
    if (msg->has_show_display && msg->show_display) {
        ...
        if (!fsm_layoutAddress(
                resp->address, desc, true, 0, msg->address_n, msg->address_n_count, false, NULL, 0,
                0, NULL)) {
            return;
        }
    }
...
#define NEM_CHECK_PARAM_WHEN(b, s) CHECK_PARAM(!(b) || (reason = (s)) == NULL, reason)
    ...
    unsigned int provided = msg->has_transfer + msg->has_provision_namespace +
                            ...
                            msg->has_aggregate_modification + msg->has_importance_transfer;
    ...
    NEM_CHECK_PARAM_WHEN(
        msg->has_transfer, nem_validate_transfer(&msg->transfer, msg->transaction.network));
    NEM_CHECK_PARAM_WHEN(
        msg->has_provision_namespace,
        nem_validate_provision_namespace(&msg->provision_namespace, msg->transaction.network));
    NEM_CHECK_PARAM_WHEN(
        msg->has_mosaic_creation,
        nem_validate_mosaic_creation(&msg->mosaic_creation, msg->transaction.network));
    NEM_CHECK_PARAM_WHEN(msg->has_supply_change, nem_validate_supply_change(&msg->supply_change));
    NEM_CHECK_PARAM_WHEN(
        msg->has_aggregate_modification,
        nem_validate_aggregate_modification(&msg->aggregate_modification, !msg->has_multisig));
    NEM_CHECK_PARAM_WHEN(
        msg->has_importance_transfer, nem_validate_importance_transfer(&msg->importance_transfer));
    ...
    if (msg->has_multisig) {
        ...
        CHECK_PARAM(
            msg->transaction.network == msg->multisig.network,
    ...
    if (!fsm_nemCheckPath(
            msg->transaction.address_n_count, msg->transaction.address_n,
    ...
    HDNode *node = fsm_getDerivedNode(
        ED25519_KECCAK_NAME, msg->transaction.address_n, msg->transaction.address_n_count, NULL);
    if (!node) return;
    ...
    if (msg->has_aggregate_modification &&
        !nem_askAggregateModification(
            common, &msg->aggregate_modification, network, !msg->has_multisig)) {
        fsm_sendFailure(FailureType_Failure_ActionCancelled, _("Signing cancelled by user"));
        layoutHome();
        return;
    }
    ...
    nem_transaction_start(
        &context, &node->public_key[1], resp->data.bytes, sizeof(resp->data.bytes));
    ...
    if (msg->has_multisig) {
        ...
        if (msg->has_provision_namespace &&
            !nem_fsmProvisionNamespace(&inner, &msg->multisig, &msg->provision_namespace)) {
            layoutHome();
            return;
        }
        ...
        if (msg->has_aggregate_modification &&
            !nem_fsmAggregateModification(&inner, &msg->multisig, &msg->aggregate_modification)) {
            layoutHome();
            return;
        }

        if (msg->has_importance_transfer &&
            !nem_fsmImportanceTransfer(&inner, &msg->multisig, &msg->importance_transfer)) {
            layoutHome();
            return;
        }
        ...
        if (msg->has_provision_namespace &&
            !nem_fsmProvisionNamespace(&context, &msg->transaction, &msg->provision_namespace)) {
            layoutHome();
            return;
        }

        if (msg->has_mosaic_creation &&
            !nem_fsmMosaicCreation(&context, &msg->transaction, &msg->mosaic_creation)) {
            layoutHome();
            return;
        }

        if (msg->has_supply_change &&
            !nem_fsmSupplyChange(&context, &msg->transaction, &msg->supply_change)) {
            layoutHome();
            return;
        }

        if (msg->has_aggregate_modification &&
            !nem_fsmAggregateModification(
                &context, &msg->transaction, &msg->aggregate_modification)) {
            layoutHome();
            return;
        }

        if (msg->has_importance_transfer &&
            !nem_fsmImportanceTransfer(&context, &msg->transaction, &msg->importance_transfer)) {
            layoutHome();
            return;
        }
    }
...
void fsm_msgNEMDecryptMessage(NEMDecryptMessage *msg)
{
    ...
    layoutNEMDialog(
        &bmp_icon_question, _("Cancel"), _("Confirm"), _("Decrypt message"), _("Confirm address?"),
        address);
    ...
    bool ret = hdnode_nem_decrypt(
        node, msg->public_key.bytes, iv, salt, payload, size, resp->payload.bytes);
    if (!ret) {
