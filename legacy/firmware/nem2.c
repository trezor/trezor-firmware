...
const char *nem_validate_mosaic_creation(const NEMMosaicCreation *mosaic_creation, uint8_t network)
{
    ...
    if (mosaic_creation->definition.has_levy) {
        ...
        if (!nem_validate_address(mosaic_creation->definition.levy_address, network))
...
bool nem_askTransfer(
    const NEMTransactionCommon *common, const NEMTransfer *transfer, const char *desc)
{
    if (transfer->mosaics_count) {
        ...
        if (unknownMosaic) {
            layoutDialogSwipe(
                &bmp_icon_question, _("Cancel"), _("I take the risk"), _("Unknown Mosaics"),
                _("Divisibility and levy"), _("cannot be shown for"), _("unknown mosaics!"), NULL,
                NULL, NULL);
        ...
        for (size_t i = 0; i < transfer->mosaics_count; i++) {
            ...
            if (definitions[i]) {
                layoutNEMTransferMosaic(
                    definitions[i], mosaic->quantity, &multiplier, common->network);
            } else {
                layoutNEMTransferUnknownMosaic(
                    mosaic->namespace, mosaic->mosaic, mosaic->quantity, &multiplier);
            }
    ...
    if (transfer->has_payload) {
        layoutNEMTransferPayload(
            transfer->payload.bytes, transfer->payload.size, transfer->has_public_key);
        if (!protectButton(ButtonRequestType_ButtonRequest_ConfirmOutput, false)) {
            return false;
        }
    }
    ...
    layoutNEMDialog(
        &bmp_icon_question, _("Cancel"), _("Confirm"), desc, _("Confirm transfer to"),
        transfer->recipient);
...
bool nem_fsmTransfer(
    nem_transaction_ctx *context, const HDNode *node, const NEMTransactionCommon *common,
    const NEMTransfer *transfer)
{
    ...
    if (transfer->has_public_key) {
        ...
        bool ret =
            hdnode_nem_encrypt(node, transfer->public_key.bytes, iv, salt, payload, size, buffer);
    ...
    bool ret = nem_transaction_create_transfer(
        context, common->network, common->timestamp, NULL, common->fee, common->deadline,
        transfer->recipient, transfer->amount, payload, size, transfer->has_public_key,
        transfer->mosaics_count);

    if (!ret) {
        fsm_sendFailure(
            FailureType_Failure_ProcessError, _("Failed to create transfer transaction"));
        return false;
    }
    ...
    for (size_t i = 0; i < transfer->mosaics_count; i++) {
        ...
        ret = nem_transaction_write_mosaic(
            context, mosaic->namespace, mosaic->mosaic, mosaic->quantity);
    ...
    const char *desc)
{
    layoutDialogSwipe(
        ...
        provision_namespace->has_parent ? provision_namespace->parent : NULL, NULL, NULL);
    if (!protectButton(ButtonRequestType_ButtonRequest_ConfirmOutput, false)) {
        return false;
    }

    layoutNEMNetworkFee(
        desc, true, _("Confirm rental fee of"), provision_namespace->fee, _("and network fee of"),
        common->fee);
    if (!protectButton(ButtonRequestType_ButtonRequest_SignTx, false)) {
        return false;
    }
...
bool nem_askMosaicCreation(
    const NEMTransactionCommon *common, const NEMMosaicCreation *mosaic_creation, const char *desc,
    const char *address)
{
    layoutDialogSwipe(
        &bmp_icon_question, _("Cancel"), _("Next"), desc, _("Create mosaic"),
        mosaic_creation->definition.mosaic, _("under namespace"),
    ...
    bn_format_uint64(
        mosaic_creation->definition.supply, NULL, NULL, mosaic_creation->definition.divisibility,
        mosaic_creation->definition.divisibility, true, ',', str_out, sizeof(str_out));
    ...
    layoutDialogSwipe(
        ...
        mosaic_creation->definition.transferable ? _("transferable") : _("non-transferable"), NULL,
        NULL);
    if (!protectButton(ButtonRequestType_ButtonRequest_ConfirmOutput, false)) {
        return false;
    }
    ...
    if (mosaic_creation->definition.has_levy) {
        ...
        if (strcmp(address, mosaic_creation->definition.levy_address) == 0) {
            layoutDialogSwipe(
                &bmp_icon_question, _("Cancel"), _("Next"), _("Levy Recipient"),
                _("Levy will be paid to"), _("yourself"), NULL, NULL, NULL, NULL);
        } else {
            layoutNEMDialog(
                &bmp_icon_question, _("Cancel"), _("Next"), _("Levy Recipient"),
                _("Levy will be paid to"), mosaic_creation->definition.levy_address);
        }
    ...
    layoutNEMNetworkFee(
        desc, true, _("Confirm creation fee"), mosaic_creation->fee, _("and network fee of"),
        common->fee);
    if (!protectButton(ButtonRequestType_ButtonRequest_SignTx, false)) {
        return false;
    }
...
bool nem_fsmMosaicCreation(
    nem_transaction_ctx *context, const NEMTransactionCommon *common,
    const NEMMosaicCreation *mosaic_creation)
{
    return nem_transaction_create_mosaic_creation(
        context, common->network, common->timestamp, NULL, common->fee, common->deadline,
        mosaic_creation->definition.namespace, mosaic_creation->definition.mosaic,
        mosaic_creation->definition.description, mosaic_creation->definition.divisibility,
        mosaic_creation->definition.supply, mosaic_creation->definition.mutable_supply,
        mosaic_creation->definition.transferable,
        mosaic_creation->definition.has_levy ? mosaic_creation->definition.levy : 0,
        mosaic_creation->definition.fee, mosaic_creation->definition.levy_address,
        mosaic_creation->definition.levy_namespace, mosaic_creation->definition.levy_mosaic,
        mosaic_creation->sink, mosaic_creation->fee);
}

bool nem_askSupplyChange(
    const NEMTransactionCommon *common, const NEMMosaicSupplyChange *supply_change,
    const char *desc)
{
    layoutDialogSwipe(
        &bmp_icon_question, _("Cancel"), _("Next"), desc, _("Modify supply for"),
        supply_change->mosaic, _("under namespace"), supply_change->namespace, NULL, NULL);
    if (!protectButton(ButtonRequestType_ButtonRequest_ConfirmOutput, false)) {
        return false;
    }
    ...
    layoutDialogSwipe(
        ...
        supply_change->type == NEMSupplyChangeType_SupplyChange_Increase ? _("Increase supply by")
...
bool nem_fsmSupplyChange(
    nem_transaction_ctx *context, const NEMTransactionCommon *common,
    const NEMMosaicSupplyChange *supply_change)
{
    return nem_transaction_create_mosaic_supply_change(
        context, common->network, common->timestamp, NULL, common->fee, common->deadline,
        supply_change->namespace, supply_change->mosaic, supply_change->type, supply_change->delta);
}

bool nem_askAggregateModification(
    const NEMTransactionCommon *common, const NEMAggregateModification *aggregate_modification,
    const char *desc, bool creation)
{
    if (creation) {
        layoutDialogSwipe(
            &bmp_icon_question, _("Cancel"), _("Next"), desc, _("Convert account to"),
            _("multisig account?"), NULL, NULL, NULL, NULL);
        if (!protectButton(ButtonRequestType_ButtonRequest_ConfirmOutput, false)) {
            return false;
        }
    }
    ...
    if (relative_change) {
        char str_out[32] = {0};
        bn_format_uint64(
            relative_change < 0 ? -relative_change : relative_change, NULL, NULL, 0, 0, false, ',',
            str_out, sizeof(str_out));

        layoutDialogSwipe(
            &bmp_icon_question, _("Cancel"), _("Next"), desc,
            creation ? _("Set minimum")
                     : (relative_change < 0 ? _("Decrease minimum") : _("Increase minimum")),
            creation ? _("cosignatories to") : _("cosignatories by"), str_out, NULL, NULL, NULL);
        if (!protectButton(ButtonRequestType_ButtonRequest_ConfirmOutput, false)) {
            return false;
        }
    }
    ...
    const NEMAggregateModification *aggregate_modification)
{
    bool ret = nem_transaction_create_aggregate_modification(
        context, common->network, common->timestamp, NULL, common->fee, common->deadline,
        aggregate_modification->modifications_count, aggregate_modification->relative_change != 0);
    if (!ret) return false;
...
bool nem_askImportanceTransfer(
    const NEMTransactionCommon *common, const NEMImportanceTransfer *importance_transfer,
    const char *desc)
{
    layoutDialogSwipe(
        &bmp_icon_question, _("Cancel"), _("Next"), desc,
    ...
    const NEMImportanceTransfer *importance_transfer)
{
    return nem_transaction_create_importance_transfer(
        context, common->network, common->timestamp, NULL, common->fee, common->deadline,
        importance_transfer->mode, importance_transfer->public_key.bytes);
}
...
bool nem_askMultisig(const char *address, const char *desc, bool cosigning, uint64_t fee)
{
    layoutNEMDialog(
        &bmp_icon_question, _("Cancel"), _("Next"), desc,
        cosigning ? _("Cosign transaction for") : _("Initiate transaction for"), address);
    if (!protectButton(ButtonRequestType_ButtonRequest_ConfirmOutput, false)) {
        return false;
    }
    ...
    const nem_transaction_ctx *inner, bool cosigning)
{
    ...
    if (cosigning) {
        ...
        ret = nem_transaction_create_multisig(
            context, common->network, common->timestamp, NULL, common->fee, common->deadline,
            inner);
    }
    ...
    if (!ret) {
        fsm_sendFailure(
            FailureType_Failure_ProcessError, _("Failed to create multisig transaction"));
        return false;
    }
...
const NEMMosaicDefinition *nem_mosaicByName(
    const char *namespace, const char *mosaic, uint8_t network)
{
...
static inline size_t format_amount(
    const NEMMosaicDefinition *definition, const bignum256 *amnt, const bignum256 *multiplier,
    int divisor, char *str_out, size_t size)
{
    bignum256 val = {0};
    ...
    return bn_format(
        &val, NULL, definition && definition->has_ticker ? definition->ticker : NULL,
        definition && definition->has_divisibility ? definition->divisibility : 0, -divisor, false,
        ',', str_out, size);
}
    ...
    uint8_t network, char *str_out, size_t size)
{
    ...
    const NEMMosaicDefinition *mosaic =
        nem_mosaicByName(definition->levy_namespace, definition->levy_mosaic, network);
    ...
    switch (definition->levy) {
        ...
        case NEMMosaicLevy_MosaicLevy_Percentile:
            ...
            return format_amount(
                mosaic, &amnt, multiplier, NEM_LEVY_PERCENTILE_DIVISOR, str_out, size);
...
bool nem_path_check(
    uint32_t address_n_count, const uint32_t *address_n, uint8_t network, bool check_coin_type)
{
    bool valid = (address_n_count >= 3);
    valid = valid && (address_n[0] == (PATH_HARDENED | 44));
    valid = valid && (address_n[1] == (PATH_HARDENED | 43) || address_n[1] == (PATH_HARDENED | 1));
