...
void fsm_msgDebugLinkGetState(const DebugLinkGetState *msg)
{
    ...
    resp.has_mnemonic_secret = config_getMnemonicBytes(
        resp.mnemonic_secret.bytes, sizeof(resp.mnemonic_secret.bytes), &resp.mnemonic_secret.size);
...
void fsm_msgDebugLinkReseedRandom(const DebugLinkReseedRandom *msg)
{
    ...
    strlcpy(resp->message, "ReseedRandom not supported on hardware", sizeof(resp->message));
