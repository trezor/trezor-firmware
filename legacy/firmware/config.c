...
static secbool config_get_bytes(
    uint16_t key, uint8_t *dest, uint16_t dest_size, uint16_t *real_size)
{
    if (dest_size == 0) {
        return secfalse;
    }
...
#define OLD_STORAGE_SIZE(last_member) \
    (((offsetof(Storage, last_member) + pb_membersize(Storage, last_member)) + 3) & ~3)

    if (memcmp(FLASH_PTR(FLASH_META_START), &META_MAGIC_V10, sizeof(META_MAGIC_V10)) != 0 ||
        memcmp(
            FLASH_PTR(FLASH_META_START + FLASH_META_LEN), &CONFIG_MAGIC_V10,
    ...
    memcpy(
        config_uuid, FLASH_PTR(FLASH_META_START + FLASH_META_LEN + sizeof(CONFIG_MAGIC_V10)),
        sizeof(config_uuid));
    memcpy(
        &config,
        FLASH_PTR(
            FLASH_META_START + FLASH_META_LEN + sizeof(CONFIG_MAGIC_V10) + sizeof(config_uuid)),
        sizeof(config));
    ...
    if (config.has_pin) {
        size_t pin_len = MIN(strnlen(config.pin, sizeof(config.pin)), (size_t)MAX_PIN_LEN);
        storage_change_pin(
            PIN_EMPTY, PIN_EMPTY_LEN, (const uint8_t *)config.pin, pin_len, NULL, NULL);
    }
...
void config_init(void)
{
    ...
    if (sectrue != storage_get(KEY_UUID, config_uuid, sizeof(config_uuid), &len) ||
...
void session_clearCache(Session *session)
{
    ...
    memzero(&session->coinjoin_authorization, sizeof(session->coinjoin_authorization));
}
...
bool config_dumpNode(HDNodeType *node)
{
    ...
    uint16_t len = 0;
    if (sectrue != storage_get(KEY_NODE, &storageNode, sizeof(storageNode), &len) ||
...
const uint8_t *config_getSeed(void)
{
    ...
    if (config_getMnemonic(mnemonic, sizeof(mnemonic))) {
        ...
        if (passphrase[0] != 0) {
            layoutDialogSwipe(
                &bmp_icon_question, _("Cancel"), _("Confirm"), NULL, _("Access hidden wallet?"),
                NULL, _("Next screen will show"), _("the passphrase!"), NULL, NULL);
            if (!protectButton(ButtonRequestType_ButtonRequest_Other, false)) {
                memzero(mnemonic, sizeof(mnemonic));
                memzero(passphrase, sizeof(passphrase));
                fsm_sendFailure(FailureType_Failure_ActionCancelled, _("Passphrase dismissed"));
                layoutHome();
                return NULL;
            }
        ...
        char oldTiny = usbTiny(1);
        mnemonic_to_seed(
            mnemonic, passphrase, activeSessionCache->seed,
        ...
        fsm_sendFailure(FailureType_Failure_NotInitialized, _("Device not initialized"));
    }
...
bool config_setCoinJoinAuthorization(const AuthorizeCoinJoin *authorization)
{
    ...
    if (authorization != NULL) {
        activeSessionCache->authorization_type = MessageType_MessageType_AuthorizeCoinJoin;
        memcpy(
            &activeSessionCache->coinjoin_authorization, authorization, sizeof(AuthorizeCoinJoin));
    } else {
        activeSessionCache->authorization_type = 0;
        memzero(&activeSessionCache->coinjoin_authorization, sizeof(AuthorizeCoinJoin));
    }
...
const AuthorizeCoinJoin *config_getCoinJoinAuthorization(void)
{
    ...
    if (activeSessionCache->authorization_type != MessageType_MessageType_AuthorizeCoinJoin) {
        fsm_sendFailure(FailureType_Failure_InvalidSession, "Coinjoin not authorized");
        return NULL;
    }
...
{
    return hdnode_from_xprv(
        node->depth, node->child_num, node->chain_code.bytes, node->private_key.bytes, curve, out);
}
...
{
    char oldTiny = usbTiny(1);
    secbool ret = storage_unlock((const uint8_t *)pin, strnlen(pin, MAX_PIN_LEN), NULL);
    usbTiny(oldTiny);
    return sectrue == ret;
}
...
bool config_changePin(const char *old_pin, const char *new_pin)
{
    ...
    secbool ret = storage_change_pin(
        (const uint8_t *)old_pin, strnlen(old_pin, MAX_PIN_LEN), (const uint8_t *)new_pin,
        strnlen(new_pin, MAX_PIN_LEN), NULL, NULL);
    usbTiny(oldTiny);
...
{
    return sectrue == storage_has_wipe_code();
}
...
bool config_changeWipeCode(const char *pin, const char *wipe_code)
{
    ...
    secbool ret = storage_change_wipe_code(
        (const uint8_t *)pin, strnlen(pin, MAX_PIN_LEN), NULL, (const uint8_t *)wipe_code,
        strnlen(wipe_code, MAX_PIN_LEN));
    usbTiny(oldTiny);
    return sectrue == ret;
}
