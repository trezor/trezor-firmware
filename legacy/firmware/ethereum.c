...
_Static_assert(
    sizeof(signing_access_list) == sizeof(((EthereumSignTxEIP1559 *)NULL)->access_list),
    ...
    const EthereumAccessList access_list[8], uint32_t access_list_count)
{
    ...
    for (size_t i = 0; i < access_list_count; i++) {
        ...
        length +=
            rlp_calculate_length(address_length + rlp_calculate_length(keys_length, 0xff), 0xff);
    }
...
static void send_signature(void)
{
    ...
    if (eip1559) {
        hash_rlp_list_length(
            rlp_calculate_access_list_length(signing_access_list, signing_access_list_count));
        for (size_t i = 0; i < signing_access_list_count; i++) {
            ...
            uint32_t address_length = rlp_calculate_length(sizeof(address), address[0]);
            uint32_t keys_length = rlp_calculate_access_list_keys_length(
                signing_access_list[i].storage_keys, signing_access_list[i].storage_keys_count);

            hash_rlp_list_length(address_length + rlp_calculate_length(keys_length, 0xff));
            ...
            for (size_t j = 0; j < signing_access_list[i].storage_keys_count; j++) {
                hash_rlp_field(
                    signing_access_list[i].storage_keys[j].bytes,
 ...
 */
static void ethereumFormatAmount(
    const bignum256 *amnt, const EthereumTokenInfo *token, char *buf, int buflen)
{
    bignum256 bn1e9 = {0};
...
static void layoutEthereumConfirmTx(
    const uint8_t *to, uint32_t to_len, const uint8_t *value, uint32_t value_len,
    const EthereumTokenInfo *token)
{
    bignum256 val = {0};
    ...
    layoutDialogSwipe(
        &bmp_icon_question, _("Cancel"), _("Confirm"), NULL, _("Send"), amount, _to1, _to2, _to3,
        NULL);
}
...
static void layoutEthereumData(const uint8_t *data, uint32_t len, uint32_t total_len)
{
    ...
    layoutDialogSwipe(
        &bmp_icon_question, _("Cancel"), _("Confirm"), NULL, _("Transaction data:"), hexdata[0],
        hexdata[1], hexdata[2], summarystart, NULL);
}

static void layoutEthereumFee(
    const uint8_t *value, uint32_t value_len, const uint8_t *gas_price, uint32_t gas_price_len,
    const uint8_t *gas_limit, uint32_t gas_limit_len, bool is_token)
{
    ...
    layoutDialogSwipe(
        &bmp_icon_question, _("Cancel"), _("Confirm"), NULL, _("Really send"), tx_value,
        _("paying up to"), gas_value, _("for gas?"), NULL);
}

static void layoutEthereumFeeEIP1559(
    const char *description, const uint8_t *amount_bytes, uint32_t amount_len,
    const uint8_t *multiplier_bytes, uint32_t multiplier_len)
{
    ...
    layoutDialogSwipeWrapping(
        &bmp_icon_question, _("Cancel"), _("Confirm"), _("Confirm fee"), description, amount_str);
}
...
static bool ethereum_signing_init_common(struct signing_params *params)
{
    ...
    if (params->data_length > 0) {
        if (params->data_initial_chunk_size == 0) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("Data length provided, but no initial chunk"));
            return false;
        }
    ...
    struct signing_params *params, const EthereumTokenInfo *token)
{
    ...
    if (params->pubkeyhash_set && params->value_size == 0 && data_total == 68 &&
        ...
        memcmp(
            params->data_initial_chunk_bytes,
            "\xa9\x05\x9c\xbb\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 16) == 0) {
        params->token = token;
    }
}

static bool ethereum_signing_confirm_common(const struct signing_params *params)
{
    if (params->token != NULL) {
        layoutEthereumConfirmTx(
            params->data_initial_chunk_bytes + 16, 20, params->data_initial_chunk_bytes + 36, 32,
            params->token);
    } else {
        layoutEthereumConfirmTx(
            params->pubkeyhash, 20, params->value_bytes, params->value_size, NULL);
    }
    ...
    if (params->token == NULL && data_total > 0) {
        layoutEthereumData(
            params->data_initial_chunk_bytes, params->data_initial_chunk_size, data_total);
        if (!protectButton(ButtonRequestType_ButtonRequest_SignTx, false)) {
            return false;
        }
    }
    ...
    const EthereumSignTx *msg, const HDNode *node, const EthereumDefinitionsDecoded *defs)
{
    ...
    layoutEthereumFee(
        msg->value.bytes, msg->value.size, msg->gas_price.bytes, msg->gas_price.size,
        msg->gas_limit.bytes, msg->gas_limit.size, params.token != NULL);
    if (!protectButton(ButtonRequestType_ButtonRequest_SignTx, false)) {
        fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
        ethereum_signing_abort();
        return;
    ...
    rlp_length += rlp_calculate_length(params.pubkeyhash_set ? 20 : 0, params.pubkeyhash[0]);
    rlp_length += rlp_calculate_length(params.value_size, params.value_bytes[0]);
    rlp_length += rlp_calculate_length(data_total, params.data_initial_chunk_bytes[0]);
    if (tx_type) {
    ...
    const EthereumSignTxEIP1559 *msg, const HDNode *node, const EthereumDefinitionsDecoded *defs)
{
    ...
    layoutEthereumFeeEIP1559(
        _("Maximum fee per gas"), msg->max_gas_fee.bytes, msg->max_gas_fee.size, NULL, 0);
    if (!protectButton(ButtonRequestType_ButtonRequest_SignTx, false)) {
        fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
        ethereum_signing_abort();
        return;
    ...
    layoutEthereumFeeEIP1559(
        _("Priority fee per gas"), msg->max_priority_fee.bytes, msg->max_priority_fee.size, NULL,
        0);
    if (!protectButton(ButtonRequestType_ButtonRequest_SignTx, false)) {
        fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
        ethereum_signing_abort();
    ...
    layoutEthereumFeeEIP1559(
        _("Maximum fee"), msg->gas_limit.bytes, msg->gas_limit.size, msg->max_gas_fee.bytes,
    ...
    rlp_length += rlp_calculate_number_length(chain_id);
    rlp_length += rlp_calculate_length(msg->nonce.size, msg->nonce.bytes[0]);
    rlp_length += rlp_calculate_length(msg->max_priority_fee.size, msg->max_priority_fee.bytes[0]);
    rlp_length += rlp_calculate_length(msg->max_gas_fee.size, msg->max_gas_fee.bytes[0]);
    rlp_length += rlp_calculate_length(msg->gas_limit.size, msg->gas_limit.bytes[0]);
    rlp_length += rlp_calculate_length(params.pubkeyhash_set ? 20 : 0, params.pubkeyhash[0]);
    rlp_length += rlp_calculate_length(params.value_size, params.value_bytes[0]);
    rlp_length += rlp_calculate_length(data_total, params.data_initial_chunk_bytes[0]);
    ...
    rlp_length += rlp_calculate_length(
        rlp_calculate_access_list_length(msg->access_list, msg->access_list_count), 0xff);
    ...
    const EthereumSignMessage *msg, const HDNode *node, EthereumMessageSignature *resp)
{
    ...
    if (ecdsa_sign_digest(
            &secp256k1, node->private_key, hash, resp->signature.bytes, &v, ethereum_is_canonic) !=
        0) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Signing failed"));
        return;
    }
...
int ethereum_message_verify(const EthereumVerifyMessage *msg)
{
    ...
    if (ecdsa_recover_pub_from_sig(&secp256k1, pubkey, msg->signature.bytes, hash, v) != 0) {
        return 2;
    }
 ...
 */
static void ethereum_typed_hash(
    ...
    const EthereumSignTypedHash *msg, const HDNode *node, EthereumTypedDataSignature *resp)
{
    ...
    ethereum_typed_hash(
        msg->domain_separator_hash.bytes, msg->message_hash.bytes, msg->has_message_hash, hash);
...
static bool check_ethereum_slip44_unhardened(uint32_t slip44, const EthereumNetworkInfo *network)
{
...
static bool ethereum_path_check_bip44(
...
static bool ethereum_path_check_casa45(
    uint32_t address_n_count, const uint32_t *address_n, const EthereumNetworkInfo *network)
{
    ...
    const EthereumNetworkInfo *network)
{
    ...
    if (address_n[0] == (PATH_HARDENED | 44)) {
        return ethereum_path_check_bip44(address_n_count, address_n, pubkey_export, network);
    }
