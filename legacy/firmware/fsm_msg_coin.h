...
void fsm_msgGetPublicKey(const GetPublicKey *msg)
{
    ...
    node = fsm_getDerivedNode(curve, msg->address_n, msg->address_n_count, &fingerprint);
    if (!node) return;
    ...
    if (coin->xpub_magic && (script_type == InputScriptType_SPENDADDRESS ||
        ...
        hdnode_serialize_public(
            node, fingerprint, coin->xpub_magic, resp->xpub, sizeof(resp->xpub));
    } else if (
        coin->has_segwit && script_type == InputScriptType_SPENDP2SHWITNESS &&
        !msg->ignore_xpub_magic && coin->xpub_magic_segwit_p2sh) {
        hdnode_serialize_public(
            node, fingerprint, coin->xpub_magic_segwit_p2sh, resp->xpub, sizeof(resp->xpub));
    } else if (
        coin->has_segwit && script_type == InputScriptType_SPENDP2SHWITNESS &&
        ...
        coin->has_segwit && script_type == InputScriptType_SPENDWITNESS &&
        !msg->ignore_xpub_magic && coin->xpub_magic_segwit_native) {
        hdnode_serialize_public(
            node, fingerprint, coin->xpub_magic_segwit_native, resp->xpub, sizeof(resp->xpub));
    } else if (
        coin->has_segwit && script_type == InputScriptType_SPENDWITNESS && msg->ignore_xpub_magic &&
        coin->xpub_magic) {
        hdnode_serialize_public(
            node, fingerprint, coin->xpub_magic, resp->xpub, sizeof(resp->xpub));
    } else if (
        coin->has_taproot && script_type == InputScriptType_SPENDTAPROOT && coin->xpub_magic) {
        hdnode_serialize_public(
            node, fingerprint, coin->xpub_magic, resp->xpub, sizeof(resp->xpub));
    } else {
        fsm_sendFailure(
            FailureType_Failure_DataError, _("Invalid combination of coin and script_type"));
        layoutHome();
        return;
    }
...
static PathSchema fsm_getUnlockedSchema(MessageType message_type)
{
    ...
    if (authorization_type == MessageType_MessageType_AuthorizeCoinJoin) {
        ...
        if (authorization == NULL || authorization->address_n[0] != PATH_SLIP25_PURPOSE) {
            return SCHEMA_NONE;
        }
...
void fsm_msgSignTx(const SignTx *msg)
{
    ...
    CHECK_PARAM(
        (coin->decred || coin->overwintered) || !msg->has_expiry,
        _("Expiry not enabled on this coin."))
    CHECK_PARAM(coin->timestamp || !msg->has_timestamp, _("Timestamp not enabled on this coin."))
    ...
    const uint32_t *address_n, bool has_multisig, MessageType message_type, bool show_warning)
{
    ...
    if (coin_path_check(
            coin, script_type, address_n_count, address_n, has_multisig, unlock, true)) {
        return true;
    }

    if (config_getSafetyCheckLevel() == SafetyCheckLevel_Strict &&
        !coin_path_check(
            coin, script_type, address_n_count, address_n, has_multisig, unlock, false)) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Forbidden key path"));
        return false;
    }
...
void fsm_msgGetAddress(const GetAddress *msg)
{
    ...
    if (!fsm_checkCoinPath(
            coin, msg->script_type, msg->address_n_count, msg->address_n, msg->has_multisig,
            MessageType_MessageType_GetAddress, msg->show_display)) {
        layoutHome();
        return;
    }
    ...
    if (!compute_address(
            coin, msg->script_type, node, msg->has_multisig, &msg->multisig, address)) {
    ...
    if (msg->has_show_display && msg->show_display) {
        ...
        if (msg->has_multisig) {
            ...
            desc[15] = (n < 10) ? ' ' : ('0' + (n / 10));
            desc[16] = '0' + (n % 10);
            multisig_index = cryptoMultisigPubkeyIndex(coin, &(msg->multisig), node->public_key);
        } else {
        ...
        if (!fsm_layoutAddress(
                address, desc, is_cashaddr, is_cashaddr ? strlen(coin->cashaddr_prefix) + 1 : 0,
                msg->address_n, msg->address_n_count, false,
                msg->has_multisig ? &(msg->multisig) : NULL, multisig_index, multisig_xpub_magic,
                coin)) {
            return;
        }
    }
...
void fsm_msgSignMessage(const SignMessage *msg)
{
    ...
    if (!fsm_checkCoinPath(
            coin, msg->script_type, msg->address_n_count, msg->address_n, false,
    ...
    if (cryptoMessageSign(
            coin, node, msg->script_type, msg->no_script_type, msg->message.bytes,
...
void fsm_msgVerifyMessage(const VerifyMessage *msg)
{
    ...
    int result = cryptoMessageVerify(
        coin, msg->message.bytes, msg->message.size, msg->address, msg->signature.bytes);
    if (result == 0) {
        ...
        layoutDialogSwipe(
            &bmp_icon_ok, NULL, _("Continue"), NULL, NULL, _("The signature is valid."), NULL, NULL,
            NULL, NULL);
...
bool fsm_getOwnershipId(
    uint8_t *script_pubkey, size_t script_pubkey_size, uint8_t ownership_id[OWNERSHIP_ID_SIZE])
{
    const char *OWNERSHIP_ID_KEY_PATH[] = {"SLIP-0019", "Ownership identification key"};
    ...
    hmac_sha256(
        ownership_id_key, sizeof(ownership_id_key), script_pubkey, script_pubkey_size,
        ownership_id);
...
void fsm_msgGetOwnershipId(const GetOwnershipId *msg)
{
    ...
    if (!get_script_pubkey(
            coin, node, msg->has_multisig, &msg->multisig, msg->script_type, script_pubkey,
            &script_pubkey_size)) {
    ...
    if (!fsm_getOwnershipId(script_pubkey, script_pubkey_size, resp->ownership_id.bytes)) {
        return;
    }
...
void fsm_msgGetOwnershipProof(const GetOwnershipProof *msg)
{
    ...
    if (authorization_type == MessageType_MessageType_AuthorizeCoinJoin) {
        ...
        if (msg->address_n_count != authorization->address_n_count + BIP32_WALLET_DEPTH ||
            memcmp(
                msg->address_n, authorization->address_n,
            ...
            memcmp(msg->commitment_data.bytes + 1, authorization->coordinator, coordinator_len) !=
                0) {
    ...
    if (msg->has_multisig) {
        ...
        fsm_sendFailure(FailureType_Failure_DataError, _("Multisig not supported."));
        layoutHome();
        return;
    }
    ...
    if (msg->ownership_ids_count) {
        if (msg->ownership_ids_count != 1 || msg->ownership_ids[0].size != sizeof(ownership_id) ||
            memcmp(ownership_id, msg->ownership_ids[0].bytes, sizeof(ownership_id)) != 0) {
            fsm_sendFailure(FailureType_Failure_DataError, _("Invalid ownership identifier"));
            layoutHome();
            return;
        }
    }
    ...
    if (!authorization && msg->user_confirmation) {
        ...
        if (msg->has_commitment_data) {
            if (!fsm_layoutCommitmentData(msg->commitment_data.bytes, msg->commitment_data.size)) {
                fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
                layoutHome();
                return;
            }
    ...
    if (!get_ownership_proof(
            coin, msg->script_type, node, flags, ownership_id, script_pubkey, script_pubkey_size,
            msg->commitment_data.bytes, msg->commitment_data.size, resp)) {
...
void fsm_msgAuthorizeCoinJoin(const AuthorizeCoinJoin *msg)
{
    ...
    if (strnlen(msg->coordinator, sizeof(msg->coordinator)) > MAX_COORDINATOR_LEN) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Invalid coordinator name."));
        layoutHome();
        return;
    }
    ...
    if (msg->max_rounds < 1) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Invalid number of rounds."));
        layoutHome();
        return;
    }

    bool safety_checks_is_strict = (config_getSafetyCheckLevel() == SafetyCheckLevel_Strict);

    if (msg->max_rounds > MAX_ROUNDS && safety_checks_is_strict) {
        fsm_sendFailure(
            FailureType_Failure_DataError, _("The number of rounds is unexpectedly large."));
        layoutHome();
        return;
    }

    if (msg->max_coordinator_fee_rate > MAX_COORDINATOR_FEE_RATE && safety_checks_is_strict) {
        fsm_sendFailure(
            FailureType_Failure_DataError, _("The coordination fee rate is unexpectedly large."));
        layoutHome();
        return;
    }

    if (msg->max_fee_per_kvbyte > 10 * coin->maxfee_kb && safety_checks_is_strict) {
        fsm_sendFailure(
            FailureType_Failure_DataError, _("The fee per vbyte is unexpectedly large."));
        layoutHome();
        return;
    }

    if (msg->address_n_count == 0) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Empty path not allowed."));
        layoutHome();
        return;
    }
    ...
    if ((size_t)(msg->address_n_count + 2) > sizeof(msg->address_n) / sizeof(msg->address_n[0])) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Forbidden key path."));
        layoutHome();
        return;
    }
    ...
    if (!fsm_checkCoinPath(
            coin, msg->script_type, msg->address_n_count + 2, msg->address_n, false,
            MessageType_MessageType_AuthorizeCoinJoin, !path_warning_shown)) {
        layoutHome();
        return;
    }
...
void fsm_msgDoPreauthorized(const DoPreauthorized *msg)
{
    ...
    if (authorization_type == 0) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("No preauthorized operation"));
        layoutHome();
        return;
    }
...
void fsm_msgUnlockPath(const UnlockPath *msg)
{
    ...
    for (size_t i = 0; i < msg->address_n_count; ++i) {
        hmac_sha256_Update(&hctx, (const uint8_t *)&msg->address_n[i], sizeof(uint32_t));
    }
