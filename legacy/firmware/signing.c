...
static uint64_t orig_total_in, orig_external_in, orig_total_out, orig_change_out;
static uint32_t progress_step, progress_steps, progress_substep, progress_substeps,
    progress_midpoint, progress_update;
...
static void report_progress(bool force)
{
    ...
    if (progress_midpoint != 0) {
        if (progress_step < progress_midpoint) {
            ...
            progress = (500 * progress_step + 500 * progress_substep / progress_substeps) /
                       progress_midpoint;
        } else {
...
static bool assert_progress_finished(void)
{
    ...
    if (progress_step != progress_steps) {
        snprintf(
            line, sizeof(line), "%s finished at %" PRIu32 "/%" PRIu32, progress_label,
            progress_step, progress_steps);
    } else if (progress_substep != 0) {
        snprintf(
            line, sizeof(line), "%s finished at substep %" PRIu32, progress_label,
            progress_substep);
    } else {
        return true;
    }
...
static void init_signing_progress(void)
{
    ...
    if (!(coin->force_bip143 || coin->overwintered || coin->decred)) {
        ...
        if (is_replacement) {
            ...
            progress_steps += (orig_info.inputs_count - orig_info.segwit_count - external_count) *
...
void phase1_request_orig_input(void)
{
    if (!is_replacement) {
        ...
        if (memcmp(input.orig_hash.bytes, orig_hash, sizeof(orig_hash)) != 0) {
            fsm_sendFailure(
                FailureType_Failure_ProcessError, _("Only one original transaction is allowed."));
            signing_abort();
            return;
        }
        ...
        if (input.orig_index >= orig_info.inputs_count) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("Not enough inputs in original transaction."));
            signing_abort();
            return;
        }
...
void phase2_request_orig_input(void)
{
    if (idx1 < orig_info.inputs_count) {
        ...
        if (memcmp(hash, orig_info.hash_inputs_check, 32) != 0) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("Transaction has changed during signing"));
            signing_abort();
            return;
        }
...
static bool extract_input_multisig_fp(TxInfo *tx_info, const TxInputType *txinput)
{
    if (txinput->has_multisig && !tx_info->multisig_fp_mismatch) {
        uint8_t h[32] = {0};
        if (cryptoMultisigFingerprint(&txinput->multisig, h) == 0) {
            fsm_sendFailure(
                FailureType_Failure_ProcessError, _("Error computing multisig fingerprint"));
            signing_abort();
            return false;
        }
...
bool check_change_multisig_fp(const TxInfo *tx_info, const TxOutputType *txoutput)
{
    uint8_t h[32] = {0};
...
bool check_change_script_type(const TxInfo *tx_info, const TxOutputType *toutput)
{
    InputScriptType input_script_type = 0;
    if (!change_output_to_input_script_type(toutput->script_type, &input_script_type)) {
        return false;
    }
...
void extract_input_bip32_path(TxInfo *tx_info, const TxInputType *tinput)
{
    ...
    if (tx_info->in_address_n_count == 0) {
        ...
        memcpy(
            tx_info->in_address_n, tinput->address_n,
...
bool check_change_bip32_path(const TxInfo *tx_info, const TxOutputType *toutput)
{
    ...
    return (
        count >= BIP32_WALLET_DEPTH && count == tx_info->in_address_n_count &&
        0 == memcmp(
                 tx_info->in_address_n, toutput->address_n,
                 (count - BIP32_WALLET_DEPTH) * sizeof(uint32_t)) &&
...
static bool fill_input_script_sig(TxInputType *tinput)
{
    if (hdnode_fill_public_key(&node) != 0) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Failed to derive public key."));
        signing_abort();
        return false;
    }
    ...
    if (tinput->has_multisig) {
        tinput->script_sig.size =
            compile_script_multisig(coin, &(tinput->multisig), tinput->script_sig.bytes);
    } else {  // SPENDADDRESS
        uint8_t hash[20] = {0};
...
{
    if (!get_script_pubkey(
            coin, &node, in->has_multisig, &in->multisig, in->script_type, in->script_pubkey.bytes,
            &in->script_pubkey.size)) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Failed to derive scriptPubKey"));
        signing_abort();
        return false;
    }
...
static bool validate_path(
    InputScriptType script_type, pb_size_t address_n_count, const uint32_t *address_n,
    bool has_multisig)
{
    if (is_coinjoin == sectrue) {
        // Check whether the authorization matches the parameters of the input.
        if (address_n_count != coinjoin_authorization.address_n_count + BIP32_WALLET_DEPTH ||
            memcmp(
                address_n, coinjoin_authorization.address_n,
                sizeof(uint32_t) * coinjoin_authorization.address_n_count) != 0 ||
        ...
        if (!coin_path_check(
                coin, script_type, address_n_count, address_n, has_multisig, unlocked_schema,
                true)) {
...
{
    return validate_path(txi->script_type, txi->address_n_count, txi->address_n, txi->has_multisig);
}

static bool derive_node(
    InputScriptType script_type, pb_size_t address_n_count, const uint32_t *address_n,
    bool has_multisig)
{
    if (!coin_path_check(
            coin, script_type, address_n_count, address_n, has_multisig, unlocked_schema, false) &&
...
{
    return derive_node(txi->script_type, txi->address_n_count, txi->address_n, txi->has_multisig);
}

static bool tx_info_init(
    TxInfo *tx_info, uint32_t inputs_count, uint32_t outputs_count, uint32_t version,
    uint32_t lock_time, bool has_expiry, uint32_t expiry, bool has_branch_id, uint32_t branch_id,
    bool has_version_group_id, uint32_t version_group_id, bool has_timestamp, uint32_t timestamp)
{
    if (!coin->overwintered) {
        if (has_version_group_id) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("Version group ID not enabled on this coin."));
            signing_abort();
            return false;
        }
        if (has_branch_id) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("Branch ID not enabled on this coin."));
            signing_abort();
            return false;
        }
    }
    ...
    if (!coin->timestamp && has_timestamp) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Timestamp not enabled on this coin."));
        signing_abort();
        return false;
    }

    if (!coin->decred && !coin->overwintered && has_expiry) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Expiry not enabled on this coin."));
        signing_abort();
        return false;
    }
    ...
    if (coin->timestamp) {
        if (!has_timestamp || !timestamp) {
            fsm_sendFailure(FailureType_Failure_DataError, _("Timestamp must be set."));
            signing_abort();
            return false;
        }
    ...
    if (coin->overwintered) {
        if (!has_version_group_id) {
            fsm_sendFailure(FailureType_Failure_DataError, _("Version group ID must be set."));
            signing_abort();
            return false;
        }

        if (!has_branch_id) {
            fsm_sendFailure(FailureType_Failure_DataError, _("Branch ID must be set."));
            signing_abort();
            return false;
        }

        if (tx_info->version != 4 && tx_info->version != 5) {
            fsm_sendFailure(FailureType_Failure_DataError, _("Unsupported transaction version."));
            signing_abort();
            return false;
        }
    ...
    if (coin->overwintered) {
        if (tx_info->version == 5) {
            // ZIP-244
            hasher_InitParam(
                &tx_info->hasher_prevouts, HASHER_BLAKE2B_PERSONAL, "ZTxIdPrevoutHash", 16);
            hasher_InitParam(
                &tx_info->hasher_amounts, HASHER_BLAKE2B_PERSONAL, "ZTxTrAmountsHash", 16);
            hasher_InitParam(
                &tx_info->hasher_scriptpubkeys, HASHER_BLAKE2B_PERSONAL, "ZTxTrScriptsHash", 16);
            hasher_InitParam(
                &tx_info->hasher_sequences, HASHER_BLAKE2B_PERSONAL, "ZTxIdSequencHash", 16);
            hasher_InitParam(
                &tx_info->hasher_outputs, HASHER_BLAKE2B_PERSONAL, "ZTxIdOutputsHash", 16);
        } else {
            // ZIP-243
            hasher_InitParam(
                &tx_info->hasher_prevouts, HASHER_BLAKE2B_PERSONAL, "ZcashPrevoutHash", 16);
            hasher_InitParam(
                &tx_info->hasher_sequences, HASHER_BLAKE2B_PERSONAL, "ZcashSequencHash", 16);
            hasher_InitParam(
                &tx_info->hasher_outputs, HASHER_BLAKE2B_PERSONAL, "ZcashOutputsHash", 16);
        }
    } else
#endif
    {
...
static bool init_coinjoin(const SignTx *msg, const AuthorizeCoinJoin *authorization)
{
    if (!msg->has_coinjoin_request) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Missing coinjoin request."));
        signing_abort();
        return false;
    }

    if (strcmp(coin->coin_name, authorization->coin_name) != 0) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Unauthorized operation."));
    }

    memcpy(&coinjoin_authorization, authorization, sizeof(coinjoin_authorization));
...
void signing_init(
    const SignTx *msg, const CoinInfo *_coin, const HDNode *_root,
    const AuthorizeCoinJoin *authorization, PathSchema unlock)
{
    ...
    if (!tx_info_init(
            &info, msg->inputs_count, msg->outputs_count, msg->version, msg->lock_time,
            msg->has_expiry, msg->expiry, msg->has_branch_id, msg->branch_id,
            msg->has_version_group_id, msg->version_group_id, msg->has_timestamp, msg->timestamp)) {
        return;
    }

    uint32_t size = TXSIZE_HEADER + TXSIZE_FOOTER + ser_length_size(info.inputs_count) +
    ...
    tx_init(
        &to, info.inputs_count, info.outputs_count, info.version, info.lock_time, info.expiry,
        branch_id, 0, coin->curve->hasher_sign, coin->overwintered, info.version_group_id,
        info.timestamp);
    ...
    if (coin->decred) {
        ...
        tx_init(
            &ti, info.inputs_count, info.outputs_count, info.version, info.lock_time, info.expiry,
            branch_id, 0, coin->curve->hasher_sign, coin->overwintered, info.version_group_id,
            info.timestamp);
...
static bool signing_validate_input(const TxInputType *txinput)
{
    if (txinput->prev_hash.size != 32) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Provided prev_hash is invalid."));
        signing_abort();
        return false;
    }

    if (txinput->has_multisig && !is_multisig_input_script_type(txinput->script_type)) {
        fsm_sendFailure(
            FailureType_Failure_DataError, _("Multisig field provided but not expected."));
        signing_abort();
        return false;
    }

    if (!txinput->has_multisig && txinput->script_type == InputScriptType_SPENDMULTISIG) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Multisig details required."));
        signing_abort();
        return false;
    }
    ...
    if (is_internal_input_script_type(txinput->script_type)) {
        if (txinput->address_n_count == 0) {
            fsm_sendFailure(FailureType_Failure_DataError, _("Missing address_n field."));
            signing_abort();
            return false;
        }
        ...
        if (txinput->has_ownership_proof) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("Ownership proof provided but not expected."));
            signing_abort();
            return false;
        }
        ...
        if (txinput->address_n_count != 0) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("Input's address_n provided but not expected."));
            signing_abort();
            return false;
        }
        ...
        if (!txinput->has_script_pubkey) {
            ...
            fsm_sendFailure(FailureType_Failure_DataError, _("Missing script_pubkey field."));
            signing_abort();
            return false;
        }
    } else {
        fsm_sendFailure(FailureType_Failure_DataError, _("Unsupported script type."));
        signing_abort();
        return false;
    }
    ...
    if (txinput->script_type == InputScriptType_SPENDTAPROOT && !coin->has_taproot) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Taproot not enabled on this coin."));
        signing_abort();
        return false;
    }
    ...
    if (txinput->has_commitment_data && !txinput->has_ownership_proof) {
        fsm_sendFailure(
            FailureType_Failure_DataError, _("commitment_data field provided but not expected."));
        signing_abort();
        return false;
    }
    ...
    if (txinput->has_orig_hash) {
        if (!txinput->has_orig_index) {
            fsm_sendFailure(FailureType_Failure_DataError, _("Missing orig_index field."));
            signing_abort();
            return false;
        }

        if (txinput->orig_hash.size != 32) {
            fsm_sendFailure(FailureType_Failure_DataError, _("Provided orig_hash is invalid."));
            signing_abort();
            return false;
        }
    }
...
{
    if (txoutput->has_multisig && !is_multisig_output_script_type(txoutput->script_type)) {
        fsm_sendFailure(
            FailureType_Failure_DataError, _("Multisig field provided but not expected."));
        signing_abort();
        return false;
    ...
    if (txoutput->address_n_count > 0 && !is_change_output_script_type(txoutput->script_type)) {
        fsm_sendFailure(
            FailureType_Failure_DataError, _("Output's address_n provided but not expected."));
        signing_abort();
        return false;
    }

    if (txoutput->script_type == OutputScriptType_PAYTOOPRETURN) {
        if (txoutput->has_address || (txoutput->address_n_count > 0) || txoutput->has_multisig) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("OP_RETURN output with address or multisig"));
            signing_abort();
            return false;
        }
        if (txoutput->amount != 0) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("OP_RETURN output with non-zero amount"));
            signing_abort();
            return false;
        }
    } else {
        ...
        if (txoutput->has_address && txoutput->address_n_count > 0) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("Both address and address_n provided."));
            signing_abort();
            return false;
    ...
    if (txoutput->script_type == OutputScriptType_PAYTOTAPROOT && !coin->has_taproot) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Taproot not enabled on this coin."));
        signing_abort();
        return false;
    }
    ...
    if (txoutput->has_orig_hash) {
        ...
        if (txoutput->orig_hash.size != 32) {
            fsm_sendFailure(FailureType_Failure_ProcessError, _("Encountered invalid orig_hash"));
            signing_abort();
            return false;
        }
    }
...
static bool tx_info_add_input(TxInfo *tx_info, const TxInputType *txinput)
{
    ...
    tx_script_hash(
        &tx_info->hasher_scriptpubkeys, txinput->script_pubkey.size, txinput->script_pubkey.bytes);
...
static bool tx_info_check_input(TxInfo *tx_info, TxInputType *tinput)
{
    ...
    if (!tx_info->multisig_fp_mismatch) {
        ...
        uint8_t h[32] = {0};
        if (!tinput->has_multisig || cryptoMultisigFingerprint(&(tinput->multisig), h) == 0 ||
            memcmp(tx_info->multisig_fp, h, 32) != 0) {
            result = false;
        }
    }
    ...
    if (tx_info->in_address_n_count != BIP32_NOCHANGEALLOWED) {
        ...
        if (count < 2 || count != tx_info->in_address_n_count ||
            0 != memcmp(tx_info->in_address_n, tinput->address_n, (count - 2) * sizeof(uint32_t))) {
            result = false;
        }
    }
    ...
    if (result == false) {
        fsm_sendFailure(
            FailureType_Failure_ProcessError, _("Transaction has changed during signing"));
        signing_abort();
    }
    return result;
}

static bool tx_info_add_output(TxInfo *tx_info, const TxOutputBinType *tx_bin_output)
{
...
static void tx_info_finish(TxInfo *tx_info)
{
    ...
    if (coin->curve->hasher_sign == HASHER_SHA2D) {
        hasher_Raw(
            HASHER_SHA2, tx_info->hash_prevouts, sizeof(tx_info->hash_prevouts),
            tx_info->hash_prevouts143);
        hasher_Raw(
            HASHER_SHA2, tx_info->hash_sequences, sizeof(tx_info->hash_sequences),
            tx_info->hash_sequence143);
        hasher_Raw(
            HASHER_SHA2, tx_info->hash_outputs, sizeof(tx_info->hash_outputs),
            tx_info->hash_outputs143);
    } else {
        memcpy(tx_info->hash_prevouts143, tx_info->hash_prevouts, sizeof(tx_info->hash_prevouts));
        memcpy(tx_info->hash_sequence143, tx_info->hash_sequences, sizeof(tx_info->hash_sequences));
        memcpy(tx_info->hash_outputs143, tx_info->hash_outputs, sizeof(tx_info->hash_outputs));
    }
...
static bool signing_add_input(TxInputType *txinput)
{
    ...
    if (!tx_input_check_hash(&info.hasher_check, txinput)) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Failed to hash input"));
        signing_abort();
        return false;
    }
...
static bool compile_output(TxOutputType *in, TxOutputBinType *out, bool needs_confirm)
{
    ...
    if (in->script_type == OutputScriptType_PAYTOOPRETURN) {
        ...
        if (in->amount != 0 || in->has_address || (in->address_n_count > 0) || in->has_multisig) {
        ...
        if (needs_confirm) {
            if (in->op_return_data.size >= 8 &&
                memcmp(in->op_return_data.bytes, "omni", 4) == 0) {  // OMNI transaction
        ...
        op_return_to_script_pubkey(
            in->op_return_data.bytes, in->op_return_data.size, out->script_pubkey.bytes,
    ...
    if (in->address_n_count > 0) {
        ...
        if (!change_output_to_input_script_type(in->script_type, &input_script_type)) {
            fsm_sendFailure(FailureType_Failure_ProcessError, _("Failed to compile output"));
            signing_abort();
            return false;
        }
        if (!derive_node(input_script_type, in->address_n_count, in->address_n, in->has_multisig)) {
            return false;
        }
        if (hdnode_fill_public_key(&node) != 0 ||
            !compute_address(
                coin, input_script_type, &node, in->has_multisig, &in->multisig, in->address)) {
            fsm_sendFailure(FailureType_Failure_ProcessError, _("Failed to compile output"));
            signing_abort();
            return false;
        }
    ...
    if (!address_to_script_pubkey(
            coin, in->address, out->script_pubkey.bytes, &out->script_pubkey.size)) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Failed to compile output"));
        signing_abort();
        return false;
    }
...
static bool is_change_output(const TxInfo *tx_info, const TxOutputType *txoutput)
{
...
static bool signing_add_output(TxOutputType *txoutput)
{
    ...
    if (txoutput->address_n_count != 0) {
        ...
        if (!change_output_to_input_script_type(txoutput->script_type, &script_type)) {
            fsm_sendFailure(FailureType_Failure_DataError, _("Unsupported script type."));
            signing_abort();
            return false;
        }
        ...
        if (!validate_path(
                script_type, txoutput->address_n_count, txoutput->address_n,
    ...
    if (coin->decred) {
        if (serialize) {
            ...
            resp.serialized.has_serialized_tx = true;
            resp.serialized.serialized_tx.size =
                tx_serialize_output(&to, &bin_output, resp.serialized.serialized_tx.bytes);
        }
...
static bool save_signature(TxInputType *txinput)
{
    ...
    if (txinput->script_type == InputScriptType_SPENDTAPROOT) {
        ...
        if (bytes[size - 1] != SIGHASH_ALL || ecdsa_sig_from_der(bytes, size - 1, sig) != 0) {
            fsm_sendFailure(FailureType_Failure_DataError, _("Unsupported signature script."));
            signing_abort();
            return false;
        }
...
static bool signing_add_orig_input(TxInputType *orig_input)
{
    ...
    if (orig_input->script_type != InputScriptType_EXTERNAL) {
        // External inputs should have scriptPubKey set by the host.
        if (!input_derive_node(orig_input) || !fill_input_script_pubkey(orig_input)) {
            return false;
        }
    }
    ...
    if (orig_input->prev_hash.size != input.prev_hash.size ||
        memcmp(orig_input->prev_hash.bytes, input.prev_hash.bytes, input.prev_hash.size) != 0 ||
        orig_input->prev_index != input.prev_index || orig_input->amount != input.amount ||
        orig_input->script_type != input.script_type ||
        orig_input->script_pubkey.size != input.script_pubkey.size ||
        memcmp(
            orig_input->script_pubkey.bytes, input.script_pubkey.bytes, input.script_pubkey.size) !=
            0) {
        fsm_sendFailure(
            FailureType_Failure_ProcessError, _("Original input does not match current input."));
        signing_abort();
        return false;
    }
...
static bool signing_add_orig_output(TxOutputType *orig_output)
{
    ...
    if (idx2 != output.orig_index) {
        ...
        if (bin_output.script_pubkey.size != orig_bin_output.script_pubkey.size ||
            memcmp(
                bin_output.script_pubkey.bytes, orig_bin_output.script_pubkey.bytes,
                bin_output.script_pubkey.size) != 0) {
            fsm_sendFailure(FailureType_Failure_ProcessError, _("Not an original output."));
            signing_abort();
            return false;
        }
        ...
        if (!is_change) {
            if (output.amount < orig_output->amount) {
                ...
                for (int page = 0; page < 2; ++page) {
                    layoutConfirmModifyOutput(coin, amount_unit, &output, orig_output, page);
                    if (!protectButton(ButtonRequestType_ButtonRequest_ConfirmOutput, false)) {
                        fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
                        signing_abort();
                        return false;
...
static bool payment_confirm_tx(void)
{
    ...
    if (is_replacement) {
        ...
        if (external_in != orig_external_in) {
            fsm_sendFailure(
                FailureType_Failure_ProcessError, _("Adding external inputs is not supported."));
            signing_abort();
            return false;
        }
        ...
        if (total_out - change_out - external_in >
            ...
            fsm_sendFailure(
                FailureType_Failure_ProcessError, _("Invalid replacement transaction."));
            signing_abort();
            return false;
        }

        // Replacement transactions must not change the effective nLockTime.
        uint32_t effective_lock_time = info.min_sequence == SEQUENCE_FINAL ? 0 : info.lock_time;
        ...
        // last confirmation
        layoutConfirmTx(coin, amount_unit, total_in, external_in, total_out, change_out, tx_weight);
        if (!protectButton(ButtonRequestType_ButtonRequest_SignTx, false)) {
            fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
            signing_abort();
            return false;
...
static bool coinjoin_confirm_tx(void)
{
    ...
    if (has_unverified_external_input) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Unverifiable external input."));
        signing_abort();
        return false;
    }
    ...
    uint64_t our_coordination_fee =
        MIN(coinjoin_request.fee_rate, coinjoin_authorization.max_coordinator_fee_rate) *
    ...
    uint64_t max_fee_per_output = MAX_OUTPUT_WEIGHT * mining_fee / (tx_weight - tx_base_weight);
    ...
    uint64_t min_allowed_output_amount_plus_fee =
        MIN(coinjoin_request.min_registrable_amount, MIN_REGISTRABLE_OUTPUT_AMOUNT) +
        max_fee_per_output;

    if (our_fees > our_coordination_fee + our_max_mining_fee + min_allowed_output_amount_plus_fee) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Total fee over threshold."));
        signing_abort();
        return false;
    }

    if (coinjoin_authorization.max_rounds < 1) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Exceeded number of coinjoin rounds."));
        signing_abort();
        return false;
    }
...
static void signing_hash_bip143(const TxInfo *tx_info, const TxInputType *txinput, uint8_t *hash)
{
...
static void signing_hash_bip341(
    const TxInfo *tx_info, uint32_t i, uint8_t sighash_type, uint8_t *hash)
{
...
#if !BITCOIN_ONLY
static void signing_hash_zip243(const TxInfo *tx_info, const TxInputType *txinput, uint8_t *hash)
{
    uint32_t hash_type = signing_hash_type(txinput);
    ...
    // 2. nVersionGroupId
    hasher_Update(&hasher_preimage, (const uint8_t *)&tx_info->version_group_id, 4);
    // 3. hashPrevouts
...
#if !BITCOIN_ONLY
static void signing_hash_zip244(const TxInfo *tx_info, const TxInputType *txinput, uint8_t *hash)
{
    Hasher hasher = {0};
    ...
    tx_script_hash(&hasher, txinput->script_pubkey.size, txinput->script_pubkey.bytes);
    ...
    hasher_Update(&hasher, tx_info->hash_prevouts, sizeof(tx_info->hash_prevouts));
    ...
    hasher_Update(&hasher, tx_info->hash_scriptpubkeys, sizeof(tx_info->hash_scriptpubkeys));
    // S.2e: sequence_sig_digest (32-byte hash)
    hasher_Update(&hasher, tx_info->hash_sequences, sizeof(tx_info->hash_sequences));
    ...
    memcpy(personal + 12, &tx_info->branch_id, 4);
    hasher_InitParam(&hasher, HASHER_BLAKE2B_PERSONAL, personal, sizeof(personal));
    // S.1: header_digest (32-byte hash output)
    hasher_Update(&hasher, tx_info->hash_header, sizeof(tx_info->hash_header));
    // S.2: transparent_sig_digest (32-byte hash output)
    hasher_Update(&hasher, transparent_sig_digest, sizeof(transparent_sig_digest));
...
static bool signing_verify_orig_nonlegacy_input(TxInputType *orig_input)
{
    ...
    if (orig_input->script_type == InputScriptType_SPENDTAPROOT) {
        ...
        valid = (zkp_bip340_tweak_public_key(node.public_key + 1, NULL, output_public_key) == 0) &&
        ...
        valid = ecdsa_verify_digest(coin->curve->params, node.public_key, sig, hash) == 0;
    }
...
#endif
        tx_init(
            &ti, orig_info.inputs_count, orig_info.outputs_count, orig_info.version,
            orig_info.lock_time, orig_info.expiry, branch_id, 0, coin->curve->hasher_sign,
            coin->overwintered, orig_info.version_group_id, orig_info.timestamp);
...
static bool signing_check_orig_tx(void)
{
    ...
    if (memcmp(hash, orig_hash, sizeof(orig_hash)) != 0) {
        ...
        // the provided path.
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Invalid original TXID."));
        signing_abort();
        return false;
    }
...
{
    if (!is_replacement || memcmp(output.orig_hash.bytes, orig_hash, sizeof(orig_hash)) != 0) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Unknown original transaction."));
        signing_abort();
        return;
    }
    ...
    if (output.orig_index >= orig_info.outputs_count) {
        fsm_sendFailure(
            FailureType_Failure_ProcessError, _("Not enough outputs in original transaction."));
        signing_abort();
        return;
    }
...
#if !BITCOIN_ONLY
static void signing_hash_decred(
    const TxInputType *txinput, const uint8_t *hash_witness, uint8_t *hash)
{
    uint32_t hash_type = signing_hash_type(txinput);
    Hasher hasher_preimage = {0};
    hasher_Init(&hasher_preimage, coin->curve->hasher_sign);
...
static bool signing_sign_ecdsa(
    TxInputType *txinput, const uint8_t *private_key, const uint8_t *public_key,
    const uint8_t *hash)
{
    resp.has_serialized = true;
    ...
    if (!tx_sign_ecdsa(
            coin->curve->params, private_key, hash, resp.serialized.signature.bytes,
    ...
    if (txinput->has_multisig) {
        ...
        if (pubkey_idx < 0) {
            fsm_sendFailure(
                FailureType_Failure_DataError, _("Pubkey not found in multisig script"));
            signing_abort();
            return false;
        }
        memcpy(
            txinput->multisig.signatures[pubkey_idx].bytes, resp.serialized.signature.bytes,
            resp.serialized.signature.size);
        ...
        if (txinput->script_sig.size == 0) {
            fsm_sendFailure(
                FailureType_Failure_ProcessError, _("Failed to serialize multisig script"));
            signing_abort();
            return false;
        }
    } else {  // SPENDADDRESS
        txinput->script_sig.size = serialize_script_sig(
            resp.serialized.signature.bytes, resp.serialized.signature.size, public_key, 33,
            sighash, txinput->script_sig.bytes);
    }
    return true;
}

static bool signing_sign_bip340(const uint8_t *private_key, const uint8_t *hash)
{
    resp.has_serialized = true;
    resp.serialized.has_signature_index = true;
    resp.serialized.signature_index = idx1;
    ...
    if (!tx_sign_bip340(
            private_key, hash, resp.serialized.signature.bytes, &resp.serialized.signature.size)) {
        fsm_sendFailure(FailureType_Failure_ProcessError, _("Signing failed"));
        signing_abort();
        return false;
    }
...
static bool signing_sign_segwit_input(TxInputType *txinput)
{
    ...
    if (is_external_input(idx1) != (txinput->script_type == InputScriptType_EXTERNAL)) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Transaction has changed during signing"));
        signing_abort();
        return false;
    }
    ...
    if (txinput->script_type == InputScriptType_SPENDTAPROOT) {
        ...
        if (!input_validate_path(txinput) || !tx_info_check_input(&info, txinput) ||
            !input_derive_node(txinput) || !signing_sign_bip340(node.private_key, hash)) {
            return false;
        }
        ...
        if (!signing_sign_ecdsa(txinput, node.private_key, node.public_key, hash)) return false;
        ...
        if (serialize) {
            ...
            if (txinput->has_multisig) {
                ...
                for (uint32_t i = 0; i < txinput->multisig.signatures_count; i++) {
                    ...
                    nwitnesses++;
                    txinput->multisig.signatures[i].bytes[txinput->multisig.signatures[i].size] =
                        sighash;
                    r += tx_serialize_script(
                        txinput->multisig.signatures[i].size + 1,
                ...
                r += compile_script_multisig(
                    coin, &txinput->multisig, resp.serialized.serialized_tx.bytes + r);
        ...
        if (serialize) {
            ...
            resp.serialized.has_serialized_tx = true;
            if (txinput->script_type == InputScriptType_EXTERNAL && txinput->has_witness) {
                // fill in the provided witness
                memcpy(
                    resp.serialized.serialized_tx.bytes, txinput->witness.bytes,
...
static bool signing_sign_decred_input(TxInputType *txinput)
{
    ...
    if (serialize) {
        ...
        resp.serialized.has_serialized_tx = true;
        resp.serialized.serialized_tx.size =
            tx_serialize_decred_witness(&to, txinput, resp.serialized.serialized_tx.bytes);
    }
    return true;
}
...
void signing_txack(TransactionType *tx)
{
    ...
    switch (signing_stage) {
        case STAGE_REQUEST_1_INPUT:
            progress_step++;
            if (!signing_validate_input(&tx->inputs[0]) || !signing_add_input(&tx->inputs[0])) {
                return;
            }
            ...
            if (tx->inputs[0].script_type == InputScriptType_SPENDMULTISIG ||
                ...
                if (tx->inputs[0].has_ownership_proof) {
                    ...
                    if (!fsm_getOwnershipId(
                            tx->inputs[0].script_pubkey.bytes, tx->inputs[0].script_pubkey.size,
                            ownership_id)) {
                        signing_abort();
                        return;
                    }
                    ...
                    if (!tx_input_verify_nonownership(coin, tx->inputs, ownership_id)) {
                        fsm_sendFailure(
                            FailureType_Failure_DataError, _("Invalid external input."));
                        signing_abort();
                        return;
                    }
                    ...
                    if (config_getSafetyCheckLevel() == SafetyCheckLevel_Strict) {
                        fsm_sendFailure(
                            FailureType_Failure_ProcessError, _("Unverifiable external input."));
                        signing_abort();
                        return;
                    }
                }
        ...
        case STAGE_REQUEST_1_ORIG_META:
            if (!tx_info_init(
                    &orig_info, tx->inputs_cnt, tx->outputs_cnt, tx->version, tx->lock_time,
                    tx->has_expiry, tx->expiry, tx->has_branch_id, tx->branch_id,
                    tx->has_version_group_id, tx->version_group_id, tx->has_timestamp,
                    tx->timestamp)) {
                return;
            }
            ...
            if (coin->decred) {
                fsm_sendFailure(
                    FailureType_Failure_DataError, _("Replacement transactions not supported"));
                signing_abort();
                return;
            }
            ...
            if (!coin->extra_data && tx->extra_data_len > 0) {
                fsm_sendFailure(
                    FailureType_Failure_DataError, _("Extra data not enabled on this coin."));
                signing_abort();
                return;
            }
            ...
            tx_init(
                &tp, tx->inputs_cnt, tx->outputs_cnt, tx->version, tx->lock_time, tx->expiry,
                tx->branch_id, tx->extra_data_len, coin->curve->hasher_sign, coin->overwintered,
                tx->version_group_id, tx->timestamp);

            phase1_request_orig_input();
            return;
        case STAGE_REQUEST_1_ORIG_INPUT:
            if (!signing_validate_input(tx->inputs) || !signing_add_orig_input(tx->inputs)) {
                return;
            }
        ...
        case STAGE_REQUEST_2_OUTPUT:
            progress_step++;
            if (!signing_validate_output(&tx->outputs[0]) || !signing_add_output(&tx->outputs[0])) {
                return;
            }
        ...
        case STAGE_REQUEST_2_ORIG_OUTPUT:
            if (!signing_validate_output(tx->outputs) || !signing_add_orig_output(tx->outputs)) {
                return;
            }
        ...
        case STAGE_REQUEST_2_ORIG_EXTRADATA:
            ...
            if (!tx_serialize_extra_data_hash(&tp, tx->extra_data.bytes, tx->extra_data.size)) {
                fsm_sendFailure(
                    FailureType_Failure_ProcessError, _("Failed to serialize extra data"));
                signing_abort();
                return;
            }
            if (tp.extra_data_received < tp.extra_data_len) {
                ...
                send_req_2_orig_extradata(
                    tp.extra_data_received, MIN(1024, tp.extra_data_len - tp.extra_data_received));
            } else {
                phase1_finish();
            }
            return;
        ...
        case STAGE_REQUEST_3_PREV_META:
            ...
            if (!coin->decred && !coin->overwintered && tx->has_expiry) {
                fsm_sendFailure(
                    FailureType_Failure_DataError, _("Expiry not enabled on this coin."));
                signing_abort();
                return;
            }
            if (!coin->timestamp && tx->has_timestamp) {
                fsm_sendFailure(
                    FailureType_Failure_DataError, _("Timestamp not enabled on this coin."));
                signing_abort();
                return;
            }
        ...
        case STAGE_REQUEST_3_PREV_OUTPUT:
            ...
            if (idx2 == input.prev_index) {
                if (input.amount != tx->bin_outputs[0].amount) {
                    fsm_sendFailure(FailureType_Failure_DataError, _("Invalid amount specified"));
                    signing_abort();
                    return;
                }
                if (input.script_pubkey.size != tx->bin_outputs[0].script_pubkey.size ||
                    memcmp(
                        input.script_pubkey.bytes, tx->bin_outputs[0].script_pubkey.bytes,
                        input.script_pubkey.size) != 0) {
                    fsm_sendFailure(
                        FailureType_Failure_DataError, _("Input does not match scriptPubKey"));
                    signing_abort();
                    return;
                }
#if !BITCOIN_ONLY
                if (coin->decred && tx->bin_outputs[0].decred_script_version > 0) {
                    fsm_sendFailure(
                        FailureType_Failure_DataError, _("Decred script version does "
        ...
        case STAGE_REQUEST_3_PREV_EXTRADATA:
            ...
            if (tp.extra_data_received < tp.extra_data_len) {  // still some data remaining
        ...
        case STAGE_REQUEST_3_ORIG_INPUT:
            if (!signing_validate_input(tx->inputs) || !signing_hash_orig_input(tx->inputs)) {
                return;
            }
            progress_step++;
        ...
        case STAGE_REQUEST_3_ORIG_OUTPUT:
            if (!signing_validate_output(tx->outputs) || !signing_hash_orig_output(tx->outputs)) {
                return;
            }
            progress_step++;
        ...
        case STAGE_REQUEST_4_INPUT:
            ...
            if (idx2 == 0) {
                tx_init(
                    &ti, info.inputs_count, info.outputs_count, info.version, info.lock_time,
                    info.expiry, tx->branch_id, 0, coin->curve->hasher_sign, coin->overwintered,
            ...
            if (idx2 == idx1) {
                if (!tx_info_check_input(&info, &tx->inputs[0]) ||
                    !input_derive_node(&tx->inputs[0]) || !fill_input_script_sig(&tx->inputs[0])) {
                    return;
                }
                memcpy(&input, &tx->inputs[0], sizeof(input));
        ...
        case STAGE_REQUEST_4_OUTPUT:
            ...
            if (idx2 < info.outputs_count - 1) {
                ...
                if (!tx_info_check_outputs_hash(&info) || !signing_sign_legacy_input()) {
                    return;
                }
                signatures++;
                // since this took a longer time, update progress
        ...
        case STAGE_REQUEST_NONLEGACY_INPUT:
            ...
            if (tx->inputs[0].script_type == InputScriptType_SPENDMULTISIG ||
                ...
                if (!signing_sign_ecdsa(&tx->inputs[0], node.private_key, node.public_key, hash))
                    return;
                signatures++;
                // since this took a longer time, update progress
                report_progress(true);
                ...
                if (!compile_script_multisig_hash(
                        coin, &tx->inputs[0].multisig, tx->inputs[0].script_sig.bytes + 3)) {
                    fsm_sendFailure(FailureType_Failure_ProcessError, _("Failed to compile input"));
                    signing_abort();
                    return;
                }
            ...
            if (serialize) {
                ...
                resp.serialized.has_serialized_tx = true;
                resp.serialized.serialized_tx.size =
                    tx_serialize_input(&to, &tx->inputs[0], resp.serialized.serialized_tx.bytes);
            }
            phase2_request_next_input(false);
            return;
        ...
        case STAGE_REQUEST_DECRED_WITNESS:
            ...
            if (idx1 == 0) {
                // witness
                tx_init(
                    &to, info.inputs_count, info.outputs_count, info.version, info.lock_time,
                    info.expiry, tx->branch_id, 0, coin->curve->hasher_sign, coin->overwintered,
                    info.version_group_id, info.timestamp);
                to.is_decred = true;
            }
            ...
            ti.is_decred = true;
            if (!tx_info_check_input(&info, &tx->inputs[0]) || !input_derive_node(&tx->inputs[0]) ||
            ...
            for (idx2 = 0; idx2 < info.inputs_count; idx2++) {
                ...
                if (!r) {
                    fsm_sendFailure(
                        FailureType_Failure_ProcessError, _("Failed to serialize input"));
                    signing_abort();
                    return;
                }
            }
