...
static bool parse_encoded_definition(
    struct EncodedDefinition *const result, const pb_size_t size, const pb_byte_t *bytes)
{
    // format version + definition type + data version + payload length + payload
    // (at least 1B) + proof length + sigmask + signature
    if (size < (FORMAT_VERSION_LENGTH + 1 + 4 + 2 + 1 + 1 + 1 + sizeof(ed25519_signature))) {
        return false;
    }
    ...
    if (size != (cursor - bytes) + result->proof_length * sizeof(proof_entry) + 1 +
...
static bool decode_definition(
    const pb_size_t size, const pb_byte_t *bytes, const EthereumDefinitionType expected_type,
    void *definition)
{
    ...
    if (memcmp(FORMAT_VERSION, parsed_def.format_version, FORMAT_VERSION_LENGTH)) {
        goto err;
    }
    ...
    if (!cryptoCosiVerify(
            parsed_def.signature, hash, sizeof(hash), SIGNATURE_THRESHOLD, DEFS_PUBLIC_KEYS,
            DEFS_PUBLIC_KEYS_COUNT, parsed_def.sigmask)
#if DEBUG_LINK
        && !cryptoCosiVerify(
               parsed_def.signature, hash, sizeof(hash), SIGNATURE_THRESHOLD, DEFS_PUBLIC_KEYS_DEV,
               DEFS_PUBLIC_KEYS_COUNT, parsed_def.sigmask)
#endif
    ) {
        // invalid signature
    ...
    const pb_msgdesc_t *fields =
        (expected_type == EthereumDefinitionType_NETWORK ? EthereumNetworkInfo_fields
                                                         : EthereumTokenInfo_fields);
    pb_istream_t stream = pb_istream_from_buffer(parsed_def.payload, parsed_def.payload_length);
...
static const EthereumNetworkInfo *get_network(
    const EncodedNetwork *encoded_network, const uint64_t chain_id, const uint32_t slip44)
{
    ...
    if (!decode_definition(
            encoded_network->size, encoded_network->bytes, EthereumDefinitionType_NETWORK,
            &decoded_network)) {
    ...
    if (chain_id != CHAIN_ID_UNKNOWN && decoded_network.chain_id != chain_id) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Network definition mismatch"));
        return NULL;
    }
...
static const EthereumTokenInfo *get_token(
    const EncodedToken *encoded_token, const uint64_t chain_id, const char *address)
{
    ...
    // try to get built-in definition
    const EthereumTokenInfo *token = ethereum_token_by_address(chain_id, address_bytes.bytes);
    ...
    if (!decode_definition(
            encoded_token->size, encoded_token->bytes, EthereumDefinitionType_TOKEN,
            &decoded_token)) {
        // error already sent by decode_definition
        return NULL;
    }

    if (decoded_token.chain_id != chain_id || memcmp(
                                                  decoded_token.address.bytes, address_bytes.bytes,
    ...
    const uint64_t chain_id, const uint32_t slip44, const char *token_address)
{
    ...
    const EthereumNetworkInfo *network = get_network(encoded_network, chain_id, slip44);
    if (network == NULL) {
    ...
    if (!is_unknown_network(network) && token_address != NULL) {
        const EthereumTokenInfo *token = get_token(encoded_token, network->chain_id, token_address);
        if (token == NULL) {
