...
static bool fsm_ethereumCheckPath(
...
static const EthereumDefinitionsDecoded *get_definitions(
    bool has_definitions, const EthereumDefinitions *definitions, uint64_t chain_id, const char *to)
{
    ...
    return ethereum_get_definitions(encoded_network, encoded_token, chain_id, SLIP44_UNKNOWN, to);
}

static const EthereumNetworkInfo *get_network_definition_only(
    bool has_encoded_network, const EncodedNetwork *encoded_network, const uint32_t slip44)
{
...
void fsm_msgEthereumGetPublicKey(const EthereumGetPublicKey *msg)
{
    ...
    if (!msg->address_n_count ||
        (msg->address_n[0] != (44 | PATH_HARDENED) && msg->address_n[0] != (45 | PATH_HARDENED))) {
        fsm_sendFailure(FailureType_Failure_DataError, _("Invalid path for EthereumGetPublicKey"));
        layoutHome();
        return;
    }
...
void fsm_msgEthereumSignTx(const EthereumSignTx *msg)
{
    ...
    const EthereumDefinitionsDecoded *defs = get_definitions(
        msg->has_definitions, &msg->definitions, msg->chain_id, msg->has_to ? msg->to : NULL);

    if (!defs ||
        !fsm_ethereumCheckPath(msg->address_n_count, msg->address_n, false, defs->network)) {
        layoutHome();
        return;
    }
...
void fsm_msgEthereumGetAddress(const EthereumGetAddress *msg)
{
    ...
    uint32_t slip44 =
        (msg->address_n_count > 1) ? (msg->address_n[1] & PATH_UNHARDEN_MASK) : SLIP44_UNKNOWN;
    ...
    if (!network || !fsm_ethereumCheckPath(msg->address_n_count, msg->address_n, false, network)) {
        layoutHome();
        return;
    }
    ...
    if (msg->has_show_display && msg->show_display) {
        ...
        if (!fsm_layoutAddress(
                resp->address, desc, false, 0, msg->address_n, msg->address_n_count, true, NULL, 0,
                0, NULL)) {
            return;
        }
    }
...
void fsm_msgEthereumSignTypedHash(const EthereumSignTypedHash *msg)
{
    ...
    layoutConfirmHash(
        &bmp_icon_warning, _("EIP-712 domain hash"), msg->domain_separator_hash.bytes, 32);
    if (!protectButton(ButtonRequestType_ButtonRequest_Other, false)) {
        fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
        layoutHome();
        return;
    ...
    if (msg->has_message_hash) {
        layoutConfirmHash(
            &bmp_icon_warning, _("EIP-712 message hash"), msg->message_hash.bytes, 32);
        if (!protectButton(ButtonRequestType_ButtonRequest_Other, false)) {
            fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
            layoutHome();
            return;
