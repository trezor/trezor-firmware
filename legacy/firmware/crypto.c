...
static void cryptoMessageHash(
    const CoinInfo *coin, const uint8_t *message, size_t message_len,
    uint8_t hash[HASHER_DIGEST_LENGTH])
{
    Hasher hasher = {0};
    hasher_Init(&hasher, coin->curve->hasher_sign);
    hasher_Update(
        &hasher, (const uint8_t *)coin->signed_message_header, strlen(coin->signed_message_header));
    uint8_t varint[5] = {0};
...
static InputScriptType address_to_script_type(const CoinInfo *coin, const char *address)
{
    ...
    addr_raw_len =
        base58_decode_check(address, coin->curve->hasher_base58, addr_raw, sizeof(addr_raw));
    ...
    if (script_type == InputScriptType_SPENDADDRESS) {
...
#endif
        {
            len = base58_decode_check(
                address, coin->curve->hasher_base58, addr_raw, MAX_ADDR_RAW_SIZE);
        }
        ecdsa_get_address_raw(
            pubkey, coin->address_type, coin->curve->hasher_pubkey, recovered_raw);
        ...
        ecdsa_get_address_segwit_p2sh_raw(
            pubkey, coin->address_type_p2sh, coin->curve->hasher_pubkey, recovered_raw);
        if (memcmp(recovered_raw, addr_raw, len) != 0 ||
...
static bool check_cointype(const CoinInfo *coin, uint32_t slip44, bool full)
{
    ...
    if (!full) {
        ...
        if (slip44 == SLIP44_BITCOIN && coin->has_fork_id && coin->coin_type != SLIP44_TESTNET) {
            return true;
        }
    }
#endif
    ...
    const uint32_t *address_n, bool has_multisig, PathSchema unlock, bool full_check)
{
    ...
    if (address_n[0] == PATH_HARDENED + 45) {
        if (address_n_count == 4) {
            ...
            // m/45'/coin_type/account/[0-1000000]/change/address_index
            valid = valid && check_cointype(coin, PATH_HARDENED | address_n[1], full_check);
            valid = valid && ((address_n[1] & PATH_HARDENED) == (address_n[2] & PATH_HARDENED));
            valid = valid && ((address_n[2] & PATH_UNHARDEN_MASK) <= PATH_MAX_ACCOUNT);
            valid = valid && (address_n[3] <= 1000000);
    ...
    if (address_n[0] == PATH_SLIP25_PURPOSE) {
        ...
        valid = valid && (address_n[4] <= PATH_MAX_CHANGE);
        valid = valid && ((unlock == SCHEMA_SLIP25_TAPROOT) ||
