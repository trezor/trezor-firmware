...
bool stellar_signingInit(const StellarSignTx *msg)
{
    ...
    sha256_Raw(
        (uint8_t *)msg->network_passphrase, strnlen(msg->network_passphrase, 1024), network_hash);
    ...
    memcpy(
        &(stellar_activeTx.signing_pubkey), node->public_key + 1,
    ...
    switch (msg->memo_type) {
        ...
        case StellarMemoType_TEXT:
            stellar_hashupdate_string(
                (unsigned char *)&(msg->memo_text), strnlen(msg->memo_text, 28));
            break;
        // ID (8 bytes, uint64)
        ...
        case StellarMemoType_RETURN:
            ...
            break;
        default:
            fsm_sendFailure(FailureType_Failure_DataError, _("Stellar invalid memo type"));
            return false;
    }
    ...
    if (strncmp("Public Global Stellar Network ; September 2015", msg->network_passphrase, 1024) ==
        0) {
...
bool stellar_confirmSourceAccount(bool has_source_account, const char *str_account)
{
    ...
    stellar_layoutTransactionDialog(
        _("Op src account OK?"), NULL, str_addr_rows[0], str_addr_rows[1], str_addr_rows[2]);
...
bool stellar_confirmCreateAccountOp(const StellarCreateAccountOp *msg)
{
    ...
    stellar_layoutTransactionDialog(
        _("Create account: "), str_addr_rows[0], str_addr_rows[1], str_addr_rows[2],
        str_amount_line);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
...
bool stellar_confirmPaymentOp(const StellarPaymentOp *msg)
{
    ...
    stellar_layoutTransactionDialog(
        str_pay_amount, str_asset_row, str_to, str_addr_rows[1], str_addr_rows[2]);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
    }
...
bool stellar_confirmPathPaymentStrictReceiveOp(const StellarPathPaymentStrictReceiveOp *msg)
{
    if (!stellar_signing) return false;
    ...
    */
    stellar_layoutTransactionDialog(
        str_pay_amount, str_dest_asset, str_to, str_dest_rows[1], str_dest_rows[2]);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
    }
    ...
    stellar_layoutTransactionDialog(
        str_source_amount, str_send_asset, _("This is the max"), _("amount debited from your"),
        _("account."));
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
...
bool stellar_confirmPathPaymentStrictSendOp(const StellarPathPaymentStrictSendOp *msg)
{
    if (!stellar_signing) return false;
    ...
    */
    stellar_layoutTransactionDialog(
        _("Path Pay at least"), str_pay_amount, str_dest_asset, str_to, str_dest_rows[1]);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
    }
    ...
    char str_source_amount[32] = {0};
    char str_source_number[32] = {0};
    stellar_format_stroops(msg->send_amount, str_source_number, sizeof(str_source_number));

    strlcpy(str_source_amount, _("Pay Using "), sizeof(str_source_amount));
    strlcat(str_source_amount, str_source_number, sizeof(str_source_amount));

    stellar_layoutTransactionDialog(
        str_dest_rows[2], str_source_amount, str_send_asset, _("This is the amount debited"),
        _("from your account."));
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
...
bool stellar_confirmManageBuyOfferOp(const StellarManageBuyOfferOp *msg)
{
    ...
    stellar_format_stroops(msg->amount, str_buying_amount, sizeof(str_buying_amount));
    ...
    */
    ...
    stellar_layoutTransactionDialog(
        str_offer, str_buying, str_buying_asset, str_selling, str_selling_asset);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
    }
...
bool stellar_confirmManageSellOfferOp(const StellarManageSellOfferOp *msg)
{
    ...
    */
    ...
    stellar_layoutTransactionDialog(
        str_offer, str_selling, str_selling_asset, str_buying, str_buying_asset);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
    }
...
bool stellar_confirmCreatePassiveSellOfferOp(const StellarCreatePassiveSellOfferOp *msg)
{
    if (!stellar_signing) return false;
...
bool stellar_confirmSetOptionsOp(const StellarSetOptionsOp *msg)
{
    ...
    if (msg->has_inflation_destination_account) {
        ...
        if (!stellar_getAddressBytes(
                msg->inflation_destination_account, inflation_destination_account_bytes)) {
        ...
        stellar_layoutTransactionDialog(
            str_title, NULL, str_addr_rows[0], str_addr_rows[1], str_addr_rows[2]);
        if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
            stellar_signingFail(_("User canceled"));
            return false;
        }
    ...
    if (msg->has_clear_flags) {
        ...
        stellar_layoutTransactionDialog(str_title, rows[0], rows[1], rows[2], rows[3]);
        if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
            stellar_signingFail(_("User canceled"));
            return false;
        }
    ...
    if (msg->has_home_domain) {
        ...
        stellar_hashupdate_string(
            (unsigned char *)&(msg->home_domain), strnlen(msg->home_domain, 32));
    }
    ...
    if (msg->has_signer_type) {
        ...
        switch (msg->signer_type) {
            case StellarSignerType_ACCOUNT:
                ...
                stellar_layoutTransactionDialog(
                    str_title, str_weight_row, str_addr_rows[0], str_addr_rows[1],
                    str_addr_rows[2]);
                if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
                    stellar_signingFail(_("User canceled"));
                    return false;
            ...
            case StellarSignerType_HASH:
                str_signer_type = (msg->signer_type == 1) ? _("pre-auth hash") : _("hash(x)");
                needs_hash_confirm = true;
                strlcat(str_title, str_signer_type, sizeof(str_title));

                stellar_layoutTransactionDialog(
                    str_title, str_weight_row, NULL, _("(confirm hash on next"), _("screen)"));
                if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
                    stellar_signingFail(_("User canceled"));
                    return false;
                }
        ...
        if (needs_hash_confirm) {
            ...
            stellar_layoutTransactionDialog(_("Confirm Hash"), rows[0], rows[1], rows[2], rows[3]);
            if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
                stellar_signingFail(_("User canceled"));
                return false;
            }
...
bool stellar_confirmChangeTrustOp(const StellarChangeTrustOp *msg)
{
    ...
    stellar_layoutTransactionDialog(
        str_title, str_amount_row, str_addr_rows[0], str_addr_rows[1], str_addr_rows[2]);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
    }
...
bool stellar_confirmAllowTrustOp(const StellarAllowTrustOp *msg)
{
    ...
    stellar_layoutTransactionDialog(
        str_title, str_asset_row, str_by, str_trustor_rows[1], str_trustor_rows[2]);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
    }
...
bool stellar_confirmAccountMergeOp(const StellarAccountMergeOp *msg)
{
    ...
    stellar_layoutTransactionDialog(
        _("Merge Account"), _("All XLM will be sent to:"), str_destination_rows[0],
        str_destination_rows[1], str_destination_rows[2]);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
    }
...
bool stellar_confirmManageDataOp(const StellarManageDataOp *msg)
{
    ...
    stellar_layoutTransactionDialog(
        str_title, str_key_lines[0], str_key_lines[1], str_key_lines[2], str_key_lines[3]);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
    }
    ...
    if (msg->has_value) {
        ...
        const char **str_hash_lines =
            split_message((const uint8_t *)str_hash_digest, sizeof(str_hash_digest), 16);

        stellar_layoutTransactionDialog(
            str_title, str_hash_lines[0], str_hash_lines[1], str_hash_lines[2], str_hash_lines[3]);
        if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
            stellar_signingFail(_("User canceled"));
            return false;
        }
...
bool stellar_confirmBumpSequenceOp(const StellarBumpSequenceOp *msg)
{
    ...
    stellar_layoutTransactionDialog(
        _("Bump Sequence"), _("Set sequence to:"), str_bump_to, NULL, NULL);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return false;
    }
...
{
    const HDNode *node =
        stellar_deriveNode(stellar_activeTx.address_n, stellar_activeTx.address_n_count);
    if (!node) {
 ...
 *  ALPHA12EXAMP (G0987)
 */
void stellar_format_asset(const StellarAsset *asset, char *str_formatted, size_t len)
{
...
bool stellar_validateAddress(const char *str_address)
{
    ...
    uint8_t *ret = base32_decode(
        str_address, STELLAR_ADDRESS_SIZE, decoded, sizeof(decoded), BASE32_ALPHABET_RFC4648);
    valid = (ret != NULL);
...
void stellar_layoutTransactionSummary(const StellarSignTx *msg)
{
    ...
    stellar_layoutTransactionDialog(
        str_lines[0], _("Signing with:"), str_addr_rows[0], str_addr_rows[1], str_addr_rows[2]);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return;
    }
    ...
    // source account.
    if (memcmp(source_bytes, stellar_activeTx.signing_pubkey, STELLAR_KEY_SIZE) != 0) {
        const char **str_source_rows = stellar_lineBreakAddress(source_bytes);

        stellar_layoutTransactionDialog(
            _("Tx source account OK?"), NULL, str_source_rows[0], str_source_rows[1],
            str_source_rows[2]);
        if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
            stellar_signingFail(_("User canceled"));
            return;
    ...
    stellar_layoutTransactionDialog(
        str_lines[0], str_lines[1], str_lines[2], str_lines[3], str_lines[4]);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return;
    }
    ...
    stellar_layoutTransactionDialog(
        _("Confirm Time Bounds"), str_lines[0], str_lines[1], str_lines[2], str_lines[3]);
    if (!protectButton(ButtonRequestType_ButtonRequest_ProtectCall, false)) {
        stellar_signingFail(_("User canceled"));
        return;
    }
 ...
 */
void stellar_layoutSigningDialog(
    const char *line1, const char *line2, const char *line3, const char *line4, const char *line5,
    uint32_t *address_n, size_t address_n_count, const char *warning, bool is_final_step)
{
...
bool stellar_path_check(uint32_t address_n_count, const uint32_t *address_n)
{
    ...
    valid = valid && (address_n[1] == (PATH_HARDENED | 148) || address_n[1] == (PATH_HARDENED | 1));
