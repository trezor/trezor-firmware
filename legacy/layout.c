...
void layoutButtonNo(const char *btnNo, const BITMAP *icon)
{
    ...
    oledInvert(
        0, OLED_HEIGHT - 9, icon_width + oledStringWidth(btnNo, FONT_STANDARD) + 4,
        OLED_HEIGHT - 1);
}
...
void layoutButtonYes(const char *btnYes, const BITMAP *icon)
{
    ...
    oledDrawStringRight(OLED_WIDTH - icon_width - 3, OLED_HEIGHT - 8, btnYes, FONT_STANDARD);
    oledInvert(
        OLED_WIDTH - oledStringWidth(btnYes, FONT_STANDARD) - icon_width - 4, OLED_HEIGHT - 9,
