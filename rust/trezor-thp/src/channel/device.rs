...
pub struct ChannelIdAllocator {
    // Next value, not guaranteed to be valid channel id.
    counter: AtomicU16,
}

// SAFETY: The only member is Sync, state is always valid between accesses.
// unsafe impl Sync for ChannelIdAllocator {}
...
impl ChannelIdAllocator {
    ...
    /// If the argument is not in the valid range, closest valid value is returned.
    pub const fn new_from(first_channel_id: u16) -> Self {
        Self {
            counter: AtomicU16::new(first_channel_id),
        }
    }
