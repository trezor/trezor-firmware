...
impl<R: Role, B: Backend> Channel<R, B> {
    ...
    fn raw_in_ext(
        ...
        if self.sync.is_ack_piggybacking_allowed() {
            ...
            // XXX self.send_ack = None; // trezorlib is not ready for this
    ...
    fn handle_packet(
        ...
        if cid != self.channel_id {
            log::warn!("[{}] Invalid channel {}, ignoring.", self.channel_id, cid);
        ...
        if cb.is_ack() {
            ...
            match &self.state {
                ...
                ChannelState::Sending { .. } => {
                    self.handle_init_while_sending(cb.sync_bits(), packet_buffer, receive_buffer)?;
                }
            ...
            let cb = u8::from(cb);
            log::warn!("[{}] Unexpected control byte {}.", self.channel_id, cb);
            return Err(Error::malformed_data());
        }
    ...
    fn handle_init(&mut self, packet_buffer: &[u8], receive_buffer: &mut [u8]) -> Result<()> {
        let sync_bits = SyncBits::try_from(packet_buffer)?;
        if !self.sync.receive_start(sync_bits) {
    ...
    fn handle_init_while_sending(
        &mut self,
        sb: SyncBits,
        packet_buffer: &[u8],
        receive_buffer: &mut [u8],
    ) -> Result<()> {
        let ChannelState::Sending { fragmenter, retry } = &self.state else {
            return Err(Error::unexpected_input());
        };
        // Assuming only one side is sending at a time even though spec does not mandate it.
        if !(fragmenter.is_done() || *retry > 0) {
            log::error!("[{}] Got reply before finishing sending.", self.channel_id);
        }

        let seq_bit_valid = self.sync.receive_start(sb);
        if self.sync.is_ack_piggybacking_allowed() {
            if seq_bit_valid {
                // Normal piggybacking case.
                self.handle_init(packet_buffer, receive_buffer)
            } else {
                // Our ACK got lost and will be resent later FIXME.
                Err(Error::malformed_data())
            }
        } else if seq_bit_valid {
            // No piggybacking + misbehaving peer or bit error - ignore.
            Err(Error::malformed_data())
        } else {
            // No piggybacking, invalid SEQ.
    ...
    fn handle_last_packet(&mut self, receive_buffer: &mut [u8]) -> Result<PacketInResult> {
        ...
        if reassembler.is_done() {
            ...
            if self.sync.is_ack_piggybacking_allowed() && !self.sync.can_send() {
                self.sync.send_mark_delivered(*sync_bits);
                if self.sync.can_send() {
                    ack_received = true;
                } else {
                    log::warn!("[{}] Unexpected ACK bit.", self.channel_id);
                }
            }
        }
...
impl<R: Role, B: Backend> ChannelIO for Channel<R, B> {
    ...
    fn message_out<'a>(&mut self, receive_buffer: &'a mut [u8]) -> Result<(u8, u16, &'a [u8])> {
        ...
        let receive_buffer = match self.noise()?.decrypt(receive_buffer) {
            ...
            Err(e) => {
                if R::is_host() {
                    log::error!("[{}] Decryption failed.", self.channel_id);
                    self.state = ChannelState::Failed { error: None };
                } else {
                    log::error!(
