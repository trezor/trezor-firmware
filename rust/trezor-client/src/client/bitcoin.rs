...
impl Trezor {
    ...
    pub fn register_policy(
        ...
        descriptor: String,
        network: Network,
    ) -> Result<protos::MiniscriptDescriptor> {
        let mut req = protos::MiniscriptDescriptor::new();
        req.set_coin_name(utils::coin_name(network)?);
        req.set_name(name);
        req.set_descriptor(descriptor);
        self.call(req.clone(), Box::new(|_, _m: protos::Success| Ok(req)))?.interact()
    }
    ...
    pub fn get_address(
        ...
        network: Network,
        show_display: bool,
        policy: Option<protos::MiniscriptDescriptor>,
    ) -> Result<Address> {
    ...
    pub fn sign_tx(
        ...
        let mut progress =
            SignTxProgress::new(self, req, root_fingerprint, derivations);
