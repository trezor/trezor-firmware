...
impl Trezor {
    ...
    pub fn register_policy(
        ...
        descriptor: String,
        network: Network,
    ) -> Result<protos::MiniscriptPolicy> {
        let mut policy = protos::MiniscriptPolicy::new();
        policy.set_descriptor(descriptor);

        let mut req = protos::MiniscriptRegisterPolicy::new();
        req.set_coin_name(utils::coin_name(network)?);
        req.set_name(name);
        req.policy = Some(policy.clone()).into();
        self.call(req, Box::new(|_, _m: protos::Success| Ok(policy)))?.interact()
    }
    ...
    pub fn get_address(
        ...
        network: Network,
        show_display: bool,
        policy: Option<protos::MiniscriptPolicy>,
    ) -> Result<Address> {
    ...
    pub fn sign_tx(
        ...
        if policy.is_some() {
            // tx serialization is not supported with Miniscript
            req.serialize = Some(false);
        }

        let req = self.call(req, Box::new(move |_, m: protos::TxRequest| Ok(m)))?.interact()?;

        let mut progress = SignTxProgress::new(self, req, &policy, root_fingerprint, derivations);
