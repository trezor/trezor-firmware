...
mod tests {
    ...
    fn test_miniscript_register_policy() {
        ...
        let registered = with_auto_approve(|| {
            emulator.apply_settings(None, None, None, None, Some(true))?;
            emulator.register_policy(
                "test_miniscript_register_policy".to_owned(),
                format!(
                    "wsh(or_d(pk({}/<0;1>/*),and_v(v:pkh({}/<0;1>/*),older(1))))",
        ...
        let addr = emulator
            .get_address(&path, InputScriptType::SPENDWITNESS, network, false, Some(registered))
            .unwrap();
    ...
    fn test_miniscript_sign_psbt_simple() {
        ...
        let registered = with_auto_approve(|| {
            emulator.apply_settings(None, None, None, None, Some(true))?;
            emulator.register_policy(
                "test_miniscript_sign_psbt_simple".to_owned(),
                format!(
                    "wsh(or_d(pk({}/<0;1>/*),and_v(v:pkh({}/<0;1>/*),older(52596))))",
    ...
    fn test_miniscript_sign_psbt_expand_primary() {
        ...
        let registered = with_auto_approve(|| {
            emulator.apply_settings(None, None, None, None, Some(true))?;
            emulator.register_policy(
                "test_miniscript_sign_psbt_expand_primary".to_owned(),
                format!(
                    "wsh(or_i(and_v(v:thresh(2,pkh({0}/<2;3>/*),a:pkh({1}/<2;3>/*),a:pkh({2}/<0;1>/*)),older(1)),and_v(v:pk({0}/<0;1>/*),pk({1}/<0;1>/*))))#asx72vh2",
                    ...
                    "[beda3749/48'/1'/0'/2']tpubDF9MY7ez2hYzCEi8T3msePgjy1eeZA1tVqeeN3mTPHcUagWbGTcBfj3qNyBCA3DgmrzTFyApcK5pBBrcmh2VYv1UaVdnwozX7WsJYi3M5z8",
                ),
                network,
            )
        }).unwrap();
    ...
    fn test_miniscript_sign_psbt_expand_recovery() {
        ...
        let registered = with_auto_approve(|| {
            emulator.apply_settings(None, None, None, None, Some(true))?;
            emulator.register_policy(
                "test_miniscript_sign_psbt_expand_recovery".to_owned(),
                format!(
                    "wsh(or_i(and_v(v:thresh(2,pkh({0}/<2;3>/*),a:pkh({1}/<2;3>/*),a:pkh({2}/<0;1>/*)),older(1)),and_v(v:pk({0}/<0;1>/*),pk({1}/<0;1>/*))))#asx72vh2",
                    "[72758bc3/48'/1'/0'/2']tpubDFEgneqafaLtGYbwidefdMyBZ3mxyoqyQZPPqjZ7AVKjwxSzDgRhgobENPu8Fn8gkJJtgKaFa6fbS9AuccZSeJqqXGy2x9BCgiJ6By3qRAJ",
                    "[5c9e228d/48'/1'/0'/2']tpubDEGquuorgFNbDrg8vepq1HnaV2mgQu9TcSBgBYfXw4AX8VMgkWqvkxHNuJmiah8iVnA3Hgj4cSvaGAXEnq814yC6hMEreckLsd7zyLL3o76",
