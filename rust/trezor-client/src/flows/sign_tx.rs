...
use bitcoin::{bip32, hashes::sha256d, psbt, Network, Transaction};
use protos::{
...
pub struct SignTxProgress<'a> {
    ...
    derivations: HashMap<bitcoin::secp256k1::PublicKey, bip32::DerivationPath>,
}
...
impl<'a> SignTxProgress<'a> {
    ...
    pub fn new(
        ...
        SignTxProgress { client, req, registered, fingerprint, derivations }
    }
    ...
    pub fn ack_psbt(&mut self, psbt: &psbt::Psbt, network: Network) -> Result<()> {
        ...
        let ack = match self.req.request_type() {
            TxRequestType::TXINPUT => self.ack_input_request(psbt),
            TxRequestType::TXOUTPUT => self.ack_output_request(psbt, network),
            TxRequestType::TXMETA => self.ack_meta_request(psbt),
        ...
        self.req = self.client.call(ack, Box::new(|_, m: protos::TxRequest| Ok(m)))?.interact()?;
        Ok(())
    }

    /// Fulfill a TxRequest for TXINPUT.
    fn ack_input_request(&self, psbt: &psbt::Psbt) -> Result<protos::TxAck> {
        let req = &self.req;
        ...
        if !req.details.has_tx_hash() {
            ...
            let our_derivations: Vec<_> = psbt_input
                ...
                .filter_map(|(pubkey, (fpr, derivation))| {
                    (*fpr == self.fingerprint && self.derivations.get(pubkey) == Some(derivation))
                        .then_some(derivation)
                })
                .collect();

            // If there is exactly 1 HD keypath known, we can provide it.
            data_input.address_n = match our_derivations.len() {
                1 => our_derivations[0].to_u32_vec(),
                0 => return Err(Error::InvalidPsbt("No keypath found".to_owned())),
                _ => return Err(Error::InvalidPsbt("Too many keypaths".to_owned())),
            };

            // Since we know the keypath, we probably have to sign it.  So update script_type.
            let script_type = if self.registered.is_some() {
                data_input.registered = self.registered.clone().into();
                // override if spending from a registered Miniscript policy
    ...
    fn ack_output_request(&self, psbt: &psbt::Psbt, network: Network) -> Result<protos::TxAck> {
        let req = &self.req;
        if req.details.is_none() || !req.details.has_request_index() {
            return Err(Error::MalformedTxRequest(req.clone()));
        }

        // For outputs, the Trezor only needs bin_outputs to be set for dependent txs and full
        // outputs for the signing tx.
        let mut txdata = TransactionType::new();
        if req.details.has_tx_hash() {
    ...
    fn ack_meta_request(&self, psbt: &psbt::Psbt) -> Result<protos::TxAck> {
        let req = &self.req;
