...
_MSG_HEADER_V1 = '>BBHL'  # wire type, data length
...
def serialize_message_header(data, msg_type, msg_len):
    ustruct.pack_into(_MSG_HEADER_V1, data, REP_MARKER_V1_LEN, _MSG_HEADER_MAGIC, _MSG_HEADER_MAGIC, msg_type, msg_len)
...
'''
    ...
    while data_len > 0:
        ...
        if data_len > 0:
            data = yield # First next record

    target.throw(EOFError())

def encode_wire_v1_message(msg_type, msg_data, target):
    report = memoryview(bytearray(64)) # Maximum report length
    report[0] = REP_MARKER_V1 # Put report marker
