...
def identify_r_int_debug_plot(time, vbat, ibat, r_int,
    ...
    fig.suptitle("R_int identification on switching load profile " + (": " + name if name is not None else ""), fontsize=FIG_FONT_SIZE)
...
def identify_ocv_curve_debug_plot(time, vbat, ibat, ocv, ocv_curve_indeces,
                                  name=None):

    fig, ax = plt.subplots(2,1)
    fig_general_config(fig)

    fig.suptitle("ocv curve identification" + (": " + name if name is not None else ""), fontsize=FIG_FONT_SIZE)
