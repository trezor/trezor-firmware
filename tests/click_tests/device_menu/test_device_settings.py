...
LED_TITLE = TR.led__title
BRIGHTNESS_TITLE = TR.brightness__title
HAPTIC_FEEDBACK_TITLE = TR.haptic_feedback__title
...
def test_device_settings_uninitialized(device_handler: "BackgroundDeviceHandler"):
    ...
    with pytest.raises(ValueError, match=f"'{TR.words__security}' is not in"):
        ...
        assert LED_TITLE not in items
        assert BRIGHTNESS_TITLE not in items
        assert HAPTIC_FEEDBACK_TITLE not in items
...
def test_toggle_led(device_handler: "BackgroundDeviceHandler"):
    ...
    # Toggle LED setting
    items = debug.read_layout().vertical_menu_content()
    assert LED_TITLE in items
    led_idx = items.index(LED_TITLE)
...
def test_toggle_haptic(device_handler: "BackgroundDeviceHandler"):
    ...
    items = debug.read_layout().vertical_menu_content()
    assert HAPTIC_FEEDBACK_TITLE in items
    haptic_idx = items.index(HAPTIC_FEEDBACK_TITLE)
...
def test_brightness(device_handler: "BackgroundDeviceHandler"):
    ...
    items = debug.read_layout().vertical_menu_content()
    assert BRIGHTNESS_TITLE in items
    brightness_idx = items.index(BRIGHTNESS_TITLE)
