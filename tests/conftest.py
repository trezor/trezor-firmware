...
def _raw_client(request: pytest.FixtureRequest) -> Client:
    ...
    if request.session.config.getoption("control_emulators"):
        ...
        interact = os.environ.get("INTERACT") == "1"
        timeout = os.environ.get("TREZOR_DEFAULT_READ_TIMEOUT")
        if timeout is not None:
            if interact:
                raise ValueError(
                    "Cannot set both INTERACT and TREZOR_DEFAULT_READ_TIMEOUT"
                )

            protocol._DEFAULT_READ_TIMEOUT = float(timeout)
