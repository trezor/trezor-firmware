...
def available_commands(client: ProdtestClient) -> set[str]:
    ...
    expose.
    """
    resp = client.command_ok(ProdtestCommand(Cmd.HELP))
    ...
    def _factory(
        *, tropic_model_configfile: str | Path | None = None
        ...
        try:
            ...
            created_dirs.append(str(emu.profile_dir))
            emu.start()
...
def client(
    ...
    with emu:
        ...
        yield client
        client.close()
    shutil.rmtree(emu.profile_dir, ignore_errors=True)
