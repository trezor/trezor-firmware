...
def test_write_and_read_back(prodtest_client: ProdtestClient) -> None:
    """Data written with mem-write should be returned verbatim by mem-read."""
    hexdata = "DEADBEEF01020304"
    prodtest_client.command_ok(ProdtestCommand(Cmd.PRODTEST_MEM_WRITE, hexdata))

    resp = prodtest_client.command_ok(ProdtestCommand(Cmd.PRODTEST_MEM_READ))
