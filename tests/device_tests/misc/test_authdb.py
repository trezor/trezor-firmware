...
def test_set_root_returns_incremented_counter(session: Session) -> None:
    tree = _make_tree()
    counter1, identifier1, *_rest1 = authdb.set_root(session, tree.get_root_hash())
    counter2, identifier2, *_rest2 = authdb.set_root(session, tree.get_root_hash())
...
def test_identifier_is_32_bytes(session: Session) -> None:
    tree = _make_tree()
    _counter, identifier, *_rest = authdb.set_root(session, tree.get_root_hash())
...
def test_identifier_consistent_across_calls(session: Session) -> None:
    ...
    tree = _make_tree()
    _, id1, *_r1 = authdb.set_root(session, tree.get_root_hash())
    _, id2, *_r2 = authdb.set_root(session, tree.get_root_hash())
    proof = tree.get_proof(b"alice")
    _, _, _, id3 = authdb.lookup(
        session, address=b"alice", value=b"data_alice", counter=tree.get_counter(b"alice"), proof=proof
    )
    assert id1 == id2 == id3
...
def test_lookup_valid_proof(session: Session, address: bytes, value: bytes) -> None:
    ...
    valid, membership, counter, identifier = authdb.lookup(
        session, address=address, value=value, counter=tree.get_counter(address), proof=proof
    )
...
def test_lookup_invalid_wrong_value(session: Session) -> None:
    ...
    valid, _membership, _counter, _identifier = authdb.lookup(
        session, address=b"alice", value=b"WRONG", counter=tree.get_counter(b"alice"), proof=proof
    )
    assert valid is False


@pytest.mark.models("core")
def test_lookup_invalid_wrong_counter(session: Session) -> None:
    tree = _make_tree()
    authdb.set_root(session, tree.get_root_hash())

    proof = tree.get_proof(b"alice")
    valid, _membership, _counter, _identifier = authdb.lookup(
        session, address=b"alice", value=b"data_alice", counter=tree.get_counter(b"alice") + 1, proof=proof
    )
    assert valid is False
...
def test_lookup_invalid_wrong_address(session: Session) -> None:
    ...
    proof = tree.get_proof(b"alice")
    valid, _membership, _counter, _identifier = authdb.lookup(
        session, address=b"bob", value=b"data_alice", counter=tree.get_counter(b"alice"), proof=proof
    )
    assert valid is False
...
def test_verify_proof_host_matches_device(session: Session) -> None:
    ...
    for address, value in ENTRIES.items():
        ...
        host_result = AuthDbTree.verify_proof(address, counter, value, proof, root)
        device_valid, _, _, _ = authdb.lookup(
            session, address=address, value=value, counter=counter, proof=proof
        )
...
def test_nonmembership_proof_valid(session: Session) -> None:
    ...
    target = b"zara"
    proof, w_addr, w_counter, w_val = tree.get_nonmembership_proof(target)
...
def test_nonmembership_proof_member_returns_false(session: Session) -> None:
    ...
    valid, _membership, _counter, _identifier = authdb.lookup(
        ...
        witness_counter=tree.get_counter(b"alice"),
    )
    assert valid is False
...
def test_nonmembership_host_matches_device(session: Session) -> None:
    ...
    host = AuthDbTree.verify_nonmembership(target, proof, w_addr, w_counter, w_val, root)
...
def test_update_leaf_insert(session: Session) -> None:
    ...
    counter0, root0, _, _mac0, _auth_mac0 = authdb.update_leaf(
        session, address=b"alice", old_value=b"", new_value=b"data_alice", new_counter=1, proof=[]
    )
...
@pytest.mark.models("core")
def test_update_leaf_update_rejects_wrong_new_counter(session: Session) -> None:
    """new_counter must be exactly old_counter + 1 -- device-enforced (leaf
    counter, docs/authdb-sync-proposal.md Part 1)."""
    tree = AuthDbTree()
    tree.insert(b"alice", b"data_alice")
    authdb.set_root(session, tree.get_root_hash())
    ...
    with pytest.raises(TrezorFailure):
        authdb.update_leaf(
            ...
            new_value=b"data_alice_v2",
            old_counter=old_counter,
            new_counter=old_counter + 2,  # skips a version
            proof=proof,
        )
...
def test_update_leaf_delete_to_empty_returns_incremented_counter(session: Session) -> None:
    ...
    # clear_root() now PRESERVES the counter (see storage/authdb.py's
    # EMPTY_ROOT sentinel), so the counter continues from counter1 rather
    # than resetting to 0/1 -- a behavior change from the old
    # record-deleting clear_root(), asserted explicitly here.
    counter2, root2, _identifier3, _mac2, _auth_mac2 = authdb.update_leaf(
        session, address=b"bob", old_value=b"", new_value=b"data_bob", new_counter=1, proof=[]
    )
...
def test_update_leaf_wrong_old_value_rejected(session: Session) -> None:
    ...
    with pytest.raises(TrezorFailure):
        authdb.update_leaf(
            ...
            new_value=b"data_alice_v2",
            old_counter=tree.get_counter(b"alice"),
            new_counter=tree.get_counter(b"alice") + 1,
            proof=proof,
        )
...
def test_approve_then_update_leaf_with_mac(session: Session) -> None:
    ...
    approve_resp = session.call(
        messages.AuthDbApprove(address=address, new_value=value),
    ...
    counter, new_root, identifier, _mac, _auth_mac = authdb.update_leaf(
        ...
        device_id=approve_resp.wallet_id,
    )
    assert new_root is not None
    assert identifier == approve_resp.wallet_id
...
def test_set_device_id_switches_identity(session: Session) -> None:
    ...
    valid, membership, _counter, ident_b = authdb.lookup(
        session, address=b"alice", value=b"data_alice", counter=1, proof=[]
    )
    assert ident_b == id_b
    assert membership is True
...
# ---------------------------------------------------------------------------
# AuthDbSetRoot -- mandatory MAC, debug zero-MAC bypass, verified path
# ---------------------------------------------------------------------------
...
def test_set_root_wrong_device_id_rejected(session: Session) -> None:
    ...
    fail even with a well-formed-looking (but garbage) non-zero MAC."""
    ...
    with pytest.raises(TrezorFailure):
        authdb.set_root(
            session,
            tree.get_root_hash(),
            mac=b"\x11" * 32,  # non-zero: takes the verified path, not the debug bypass
            device_id=wrong_id,
            counter=1,
        )
...
def test_set_root_forged_mac_rejected(session: Session) -> None:
    """Correct device_id/counter but a forged MAC must still be rejected."""
...
@pytest.mark.models("core")
def test_set_root_zero_mac_allowed_in_debug(session: Session) -> None:
    """mac is mandatory on the wire, but the all-zero sentinel is accepted as
    valid ONLY on debug builds -- a plain unauthenticated root injection.
    Device tests always run against a debug build. Regression guard: an
    earlier revision made this handler unconditionally require a real
    mac+device_id+counter, which broke every bare-ish set_root() call in
    this file (see core/src/apps/authdb/set_root.py)."""
    tree = AuthDbTree()
    tree.insert(b"alice", b"data_alice")

    counter, wallet_id, new_root, applied_count, last_applied_sequence, root_mac = authdb.set_root(
        session, tree.get_root_hash()
    )
    assert counter > 0
    assert wallet_id is not None
    assert new_root == tree.get_root_hash()
    assert applied_count == 0  # no operations supplied
    assert last_applied_sequence == 0


@pytest.mark.models("core")
def test_set_root_verified_path_requires_higher_counter(session: Session) -> None:
    """The verified (non-zero mac) path is anti-rollback, same rule as
    AuthDbFastForwardRoot: counter must be strictly greater than current."""
    wallet_id = sha256(b"set-root-anti-rollback").digest()
    authdb.set_device_id(session, wallet_id)

    tree = AuthDbTree()
    tree.insert(b"alice", b"data_alice")
    # Bootstrap via the debug zero-mac path so there's a real counter to
    # roll back against.
    counter0, *_r = authdb.set_root(session, tree.get_root_hash())

    with pytest.raises(TrezorFailure):
        authdb.set_root(
            session,
            tree.get_root_hash(),
            mac=b"\x22" * 32,  # forged, but device_id check passes first only if counter check passes too
            device_id=wallet_id,
            counter=counter0,  # not strictly greater -- must be rejected regardless of mac
        )
...
def test_update_leaf_mac_bound_to_wrong_value(session: Session) -> None:
    ...
    _counter0, _root0, identifier, _mac0, auth_mac_for_v1 = authdb.update_leaf(
        session, address=address, old_value=b"", new_value=b"data_alice_v1", new_counter=1, proof=[]
    )
...
def test_update_leaf_replayed_old_root_proof_rejected(session: Session) -> None:
    ...
    nm_proof, w_addr, w_counter, w_val = tree.get_nonmembership_proof(b"bob")
    authdb.update_leaf(
        session,
        address=b"bob",
        old_value=b"",
...
def test_lookup_rejects_malformed_proof_element_length(session: Session) -> None:
    ...
    valid, _membership, _counter, _identifier = authdb.lookup(
        session, address=b"alice", value=b"data_alice", counter=1, proof=truncated
    )
    assert valid is False
...
def test_cache_not_isolated_across_device_id_switch(session: Session) -> None:
    ...
    TestAuthDbStorageIsolation.test_cache_not_isolated_per_wallet in
...
def _rebase(op: "authdb.OfflineOperation", proof, w_addr=None, w_counter=None, w_val=None):
    """Build a RebasedOperation forwarding an OfflineOperation's signed fields
    byte-for-byte (value-for-value), the way a real rebase engine must (see
    review notes)."""
...
def test_offline_sync_full_lifecycle(session: Session) -> None:
    ...
    seq1, mac1, identifier = authdb.queue_offline_operation(
        session, address=b"alice", old_value=b"", new_value=b"data_alice", new_counter=1
    )
    assert seq1 == 1
    ...
    applied_count, new_root, counter, last_applied, ident2, root_mac = authdb.apply_offline_operations(
        session, [rebased1]
    )
...
def test_offline_sync_conflict_stops_batch_without_losing_it(session: Session) -> None:
    """Two offline edits both based on the same old_value/old_counter (a real
    conflict): the first commits, the second must be rejected and stay
    queued for conflict resolution -- and GC must not delete it."""
    authdb.queue_offline_operation(session, address=b"alice", old_value=b"", new_value=b"v1", new_counter=1)
    authdb.queue_offline_operation(
        session, address=b"alice", old_value=b"v1", new_value=b"v2", old_counter=1, new_counter=2
    )
    authdb.queue_offline_operation(
        session, address=b"alice", old_value=b"v1", new_value=b"v3", old_counter=1, new_counter=2
    )
    ...
    applied_count, new_root, _counter, last_applied, _id, _root_mac = authdb.apply_offline_operations(
        session, rebased
    )
...
def test_offline_sync_altered_value_after_queuing_rejected(session: Session) -> None:
    ...
    authdb.queue_offline_operation(
        session, address=b"alice", old_value=b"", new_value=b"approved", new_counter=1
    )
    _root, _counter, _id, ops = authdb.get_offline_operations(session)
    op = ops[0]
...
def test_offline_sync_sequence_gap_rejected(session: Session) -> None:
    ...
    _seq2, _mac2, _id = authdb.queue_offline_operation(
        session, address=b"bob", old_value=b"", new_value=b"v2", new_counter=1
    )
    _root, _counter, _id, ops = authdb.get_offline_operations(session)
    op2 = ops[1]
...
def test_offline_queue_capacity_enforced(session: Session) -> None:
    for n in range(64):  # MAX_OFFLINE_QUEUE_ENTRIES
        authdb.queue_offline_operation(
            session, address=b"addr-%d" % n, old_value=b"", new_value=b"val-%d" % n, new_counter=1
        )

    with pytest.raises(TrezorFailure):
        authdb.queue_offline_operation(
            session, address=b"overflow", old_value=b"", new_value=b"val", new_counter=1
        )


@pytest.mark.models("core")
def test_offline_sync_delete_to_empty_tree(session: Session) -> None:
    """Regression guard: apply_offline_operations() must commit root/counter/
    last_applied_sequence atomically on a delete-to-empty transition (see
    storage/authdb.py's commit_applied_operation())."""
    authdb.queue_offline_operation(session, address=b"alice", old_value=b"", new_value=b"v1", new_counter=1)
    _root, counter0, _id, ops = authdb.get_offline_operations(session)

    applied_count, new_root, counter1, last_applied, _id, root_mac1 = authdb.apply_offline_operations(
        session, [_rebase(ops[0], proof=[])]
    )
    assert applied_count == 1
    assert new_root is not None
    assert counter1 == counter0 + 1
    assert root_mac1 is not None

    delete_seq, delete_mac, _id = authdb.queue_offline_operation(
        session, address=b"alice", old_value=b"v1", new_value=b"", old_counter=1, new_counter=0
    )
    _root2, _counter2, _id2, ops2 = authdb.get_offline_operations(session)
    delete_op = [op for op in ops2 if op.sequence == delete_seq][0]

    applied_count2, new_root2, counter2, last_applied2, _id3, root_mac2 = authdb.apply_offline_operations(
    ...
    assert root_mac2 is None  # new_root is None -> no root to endorse
...
# ---------------------------------------------------------------------------
# AuthDbSetRoot's embedded `operations` replay (install + replay in one call)
# ---------------------------------------------------------------------------

@pytest.mark.models("core")
def test_set_root_replays_operations_in_same_call(session: Session) -> None:
    """The literal point of AuthDbSetRoot absorbing AuthDbApplyOfflineOperations:
    install a root (here via the debug zero-mac bypass) and replay this
    wallet's own pending queue on top of it, in one round trip."""
    authdb.queue_offline_operation(session, address=b"alice", old_value=b"", new_value=b"v1", new_counter=1)
    _root, _counter, _id, ops = authdb.get_offline_operations(session)
    ...
    rebased = [_rebase(op, proof=[]) for op in ops]
    counter, wallet_id, new_root, applied_count, last_applied_sequence, root_mac = authdb.set_root(
        session, tree.get_root_hash(), operations=rebased
    )
    # NOTE: the root passed to set_root() here is irrelevant to the replay
    # result other than as the debug-bypass install target *before* replay
    # runs -- replay always operates against whatever the device's current
    # root actually is post-install, which for this single-op INIT batch
    # ends up as tree's root regardless of what raw root value was installed
    # first (INIT requires an empty tree, which set_root's debug path here
    # doesn't touch since it targets a fresh identity).
    assert applied_count == 1
    assert new_root == tree.get_root_hash()
    assert last_applied_sequence == 1
    assert root_mac is not None
    assert wallet_id is not None
...
@pytest.mark.models("core")
def test_set_root_with_no_operations_is_install_only(session: Session) -> None:
    tree = AuthDbTree()
    tree.insert(b"alice", b"data_alice")
