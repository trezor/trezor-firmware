...
def test_evolu_get_node(client: Client):
    ...
    node = evolu.get_node(client.get_session(), proof=proof, index=0)
...
def test_evolu_get_node_invalid_proof(client: Client):
    ...
    with pytest.raises(
        ...
        evolu.get_node(client.get_session(), proof=invalid_proof, index=0)
...
def test_evolu_get_node_no_proof(client: Client):
    with pytest.raises(
        ...
        evolu.get_node(client.get_session(), proof=b"", index=0)
...
def test_evolu_get_node_none_proof(client: Client):
    with pytest.raises(
        ...
        evolu.get_node(client.get_session(), proof=None, index=0)  # type: ignore
