...
def test_pin_menu_cancel_setup(session: Session):
    def cancel_pin_setup_input_flow():
        ...
        if session.client.layout_type is LayoutType.Delizia:
            # enter context menu
            debug.click(client.debug.screen_buttons.menu())
            debug.synchronize_at("VerticalMenu")
            # click "Cancel PIN setup"
            debug.click(client.debug.screen_buttons.vertical_menu_items()[0])
            debug.synchronize_at("Paragraphs")
            # swipe through info screen
            debug.swipe_up()
            debug.synchronize_at("PromptScreen")
            # tap to confirm
            debug.click(client.debug.screen_buttons.tap_to_confirm())
