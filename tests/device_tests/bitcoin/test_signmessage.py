...
VECTORS_LENIENCY_FORBIDDEN = (  # path, script_type
    ...
    # The same holds deeper down: PATTERN_CASA_UNHARDENED is unhardened
    # throughout, so its account level is withheld even though its leaves sign
    pytest.param("m/49/0/0", S.SPENDP2SHWITNESS, id="casa_unhardened_account"),
    # Above the export point
    pytest.param("m/44h/0h", S.SPENDADDRESS, id="bip44_coin_type"),
    # Below the export point, but not a leaf: the grant is the node itself,
    # not its subtree
    pytest.param("m/44h/0h/0h/0", S.SPENDADDRESS, id="bip44_change"),
    # The export point of the right shape, but under the wrong coin_type
    pytest.param("m/44h/1h/0h", S.SPENDADDRESS, id="bip44_account-testnet"),
    # A message cannot be signed with SPENDTAPROOT, so no taproot export point
    # is granted, not even for a script type that could sign
    pytest.param("m/86h/0h/0h", S.SPENDADDRESS, id="bip86_account"),
)
