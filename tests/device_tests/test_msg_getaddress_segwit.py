...
class TestMsgGetaddressSegwit(common.TrezorTest):
    ...
    def test_show_multisig_3(self):
        ...
        for i in [1, 2, 3]:
            self.assertEqual(self.client.get_address("Testnet", self.client.expand_path("999'/1'/"+str(i)+"'/2/0"),
                                                     False, multisig1, script_type=proto_types.SPENDP2SHWITNESS),
                             '2N2MxyAfifVhb3AMagisxaj3uij8bfXqf4Y');
            self.assertEqual(self.client.get_address("Testnet", self.client.expand_path("999'/1'/"+str(i)+"'/2/1"),
