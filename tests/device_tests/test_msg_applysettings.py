...
def test_apply_homescreen_caesar_toif_good(session: Session):
    with session.test_ctx as client:
        _set_expected_responses(client, homescreen=CAESAR_HOMESCREEN)
        device.apply_settings(session, homescreen=CAESAR_HOMESCREEN)
...
@pytest.mark.models("safe3")
def test_apply_homescreen_caesar_toif_single_message(session: Session):
    with session.test_ctx as client:
        _set_expected_responses(client)
        session.call(messages.ApplySettings(homescreen=CAESAR_HOMESCREEN))
...
@pytest.mark.models("safe3")
def test_apply_homescreen_caesar_toif_wrong_prefix(session: Session):
    with pytest.raises(exceptions.TrezorFailure), session.test_ctx as client:
        BAD_CAESAR_HOMESCREEN = bytearray(CAESAR_HOMESCREEN)
        BAD_CAESAR_HOMESCREEN[0] = ord("X")
        _set_expected_responses(client)
        session.call(messages.ApplySettings(homescreen=bytes(BAD_CAESAR_HOMESCREEN)))
        session.refresh_features()

        # Revert to default settings
        client.set_expected_responses(EXPECTED_RESPONSES_NOPIN)
        session.call(messages.ApplySettings(homescreen_length=0))
        session.refresh_features()


@pytest.mark.models("safe3")
@pytest.mark.setup_client(pin=None)  # so that "PIN NOT SET" is shown in the header
def test_apply_homescreen_caesar_toif_with_notification(session: Session):
    with session.test_ctx:
        device.apply_settings(session, homescreen=CAESAR_HOMESCREEN)


@pytest.mark.models("safe3")
def test_apply_homescreen_caesar_toif_with_long_label(session: Session):
    with session.test_ctx as client:
        _set_expected_responses(client, homescreen=CAESAR_HOMESCREEN)
        device.apply_settings(session, homescreen=CAESAR_HOMESCREEN)
...
@pytest.mark.models("safe3")
def test_apply_homescreen_caesar_toif_wrong_size(session: Session):
    # 64x64 img
...
@pytest.mark.models("safe3")
def test_apply_homescreen_caesar_upload_jpeg_fail(session: Session):
...
@pytest.mark.models("safe3")
def test_apply_homescreen_caesar_upload_t1_fail(session: Session):
    with pytest.raises(exceptions.TrezorFailure), session.test_ctx as client:
        _set_expected_responses(client)
...
def test_set_brightness(session: Session, value: int | None, should_raise: bool):
    with session.test_ctx as client:
        ...
        with pytest.raises(exceptions.TrezorFailure) if should_raise else nullcontext():
            device.set_brightness(session, value)
