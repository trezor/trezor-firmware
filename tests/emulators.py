...
BINDIR = ROOT / "tests" / "emulators-new"
...
def gen_from_model(model: str) -> str:
    if model in LEGACY:
        return "legacy"
    if model in CORE:
        return "core"
...
def filename_from_tag(gen: str, model: str, tag: str) -> Path:
...
class EmulatorWrapper:
    ...
    def __init__(
        ...
        if tag is not None and model is not None:
            executable = filename_from_tag(gen, model, tag)
        else:
