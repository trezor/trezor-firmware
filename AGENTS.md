...
- Device/integration tests (pytest against a running emulator): `make -C core test_emu`.
  ...
  - The emulator stays in the foreground — run it in the background or a second terminal, and stop it with
    `pkill -f firmware-emu` (beware: `pgrep -f`/`pkill -f` patterns can also match your own shell's command line).
...
- T3W1 (Safe 7) needs the Tropic model server, but `core/emu.py` starts it automatically when the built
  emulator has tropic support — no manual step needed. Start it manually
