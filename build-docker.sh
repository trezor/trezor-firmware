...
function help_and_die() {
  ...
  echo "  --skip-extapps - do not build the external apps (${EXTAPPS[*]}); built by default alongside a"
  echo "                   'firmware' target, for models in {${EXTAPP_MODELS[*]}}, and only for the non-bitcoin-only variant"
...
# External apps, and the models that support them. Modularization only
# exists in the universal (non-bitcoin-only) firmware, so an extapp is built
# whenever 'firmware' is built for one of these models in that variant (see
# the analogous TREZOR_MODEL/BITCOIN_ONLY condition around XTASK_BUILD_OPTS
# += --apps in core/Makefile).
EXTAPPS=(tron)
EXTAPP_MODELS=(T3W1)
...
for TREZOR_MODEL in ${MODELS[@]}; do
  ...
  for BITCOIN_ONLY in ${VARIANTS[@]}; do
    ...
    if [[ "$MAKE_TARGETS" == *build_firmware* ]]; then
      ...
      # Modularization only exists in the universal firmware, for the models
      # in EXTAPP_MODELS (see the EXTAPP_MODELS_TO_BUILD comment above).
      if [ "$OPT_BUILD_EXTAPPS" -eq 1 ] && [ "$BITCOIN_ONLY" = "0" ] \
          && echo "${EXTAPP_MODELS[@]}" | grep -qw "$TREZOR_MODEL"; then
        EXTAPP_MODELS_TO_BUILD+=("$TREZOR_MODEL")
      fi
    fi
...
# build extapps, for every model that got a qualifying firmware build above
if [ ${#EXTAPP_MODELS_TO_BUILD[@]} -gt 0 ]; then
  for TREZOR_MODEL in "${EXTAPP_MODELS_TO_BUILD[@]}"; do
    MODEL_LOWER="$(echo "$TREZOR_MODEL" | tr '[:upper:]' '[:lower:]')"

    SCRIPT_NAME=".build_extapps_${TREZOR_MODEL}.sh"
    cat <<EOF > "build/$SCRIPT_NAME"
      # DO NOT MODIFY!
      # this file was generated by ${BASH_SOURCE[0]}
      # variant: extapps build TREZOR_MODEL=$TREZOR_MODEL
      set -e -o pipefail
      cd /reproducible-build/trezor-firmware
      $GIT_CLEAN_REPO
      rm -rf /build/*
      for app in ${EXTAPPS[@]}; do
        uv run make extapp_build_firmware \\
            EXTAPP=\$app EXTAPP_MODEL=$MODEL_LOWER EXTAPP_LANG=en
        sha256sum sdk/apps/target/artifacts/$MODEL_LOWER/\$app.elf | cut -d' ' -f1 \\
            > sdk/apps/target/artifacts/$MODEL_LOWER/\$app.elf.fingerprint
      done
      # copy the artifacts, without the "latest" symlink
      cp -v sdk/apps/target/artifacts/$MODEL_LOWER/*.* /build/
      chown -R $USER:$GROUP /build
EOF

    echo
    echo ">>> DOCKER RUN extapps TREZOR_MODEL=$TREZOR_MODEL PRODUCTION=$PRODUCTION"
    echo "    (apps: ${EXTAPPS[@]})"
    echo
    ...
    $DOCKER run \
      ...
      --rm \
      -v "$DIR:/local" \
      -v "$DIR/build/extapps-$TREZOR_MODEL":/build:z \
      --env PRODUCTION="$PRODUCTION" \
      --init \
      "$SNAPSHOT_NAME" \
      /nix/var/nix/profiles/default/bin/nix-shell --run "bash /local/build/$SCRIPT_NAME"
...
# Extapp fingerprints (if built) use a plain sha256 of the app image and are not
# part of the labeled fingerprints file.
for file in build/extapps-*/*.elf.fingerprint; do
  if [ -f "$file" ]; then
    if [ -z "$EXTAPP_HEADER_SHOWN" ]; then
      echo
      echo "extapp fingerprints:"
      EXTAPP_HEADER_SHOWN=1
    fi
