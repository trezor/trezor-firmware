...
def sign_tx(
    ...
    while response.remaining_len > 0:
        data_length = len(response.serialized_tx)
        response = client.call(messages.CardanoSignedTxAck(received_data_len=len(response.serialized_tx)))
        result += response.serialized_tx
