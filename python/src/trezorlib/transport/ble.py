...
class BleTransport(ProtocolBasedTransport):
    ...
    def read_chunk(self, timeout: float | None = None) -> bytes:
        ...
        chunk = self.ble().read(timeout)

        if chunk is None:
            raise Timeout("Timeout reading BLE packet")
...
class BleAsync:
    ...
    async def read(self, timeout: float | None = None):
        ...
        if timeout is not None:
            # Wait for ready(self.tx) to complete with a timeout.
            try:
                await asyncio.wait_for(ready(self.tx), timeout)
