...
from .. import btc, messages, models, tools
...
if TYPE_CHECKING:
    ...
    from ..messages import ButtonRequest
...
def common_fixtures_dir() -> Path:
    """Return the common fixtures directory.

    Searches in the following order:
    1. Environment variable TREZOR_COMMON_FIXTURES_DIR (explicit override)
    2. Legacy location relative to firmware repo root
    """

    env_dir = os.environ.get("TREZOR_COMMON_FIXTURES_DIR")
    if env_dir:
        print(f"Using common fixtures directory from environment variable: {env_dir}")
        return Path(env_dir)


    print("root is", ROOT)  # Just to make sure the path is correct when running tests


    # Legacy: common fixtures living in the firmware repo
    legacy = ROOT / "common" / "tests" / "fixtures"
    if legacy.is_dir():
        return legacy

    raise FileNotFoundError(
        "Common fixtures directory not found. Set TREZOR_COMMON_FIXTURES_DIR environment variable."
    )
...
def parametrize_using_common_fixtures(*paths: str) -> "MarkDecorator":
    ...
    print(common_fixtures_dir())  # Just to make sure the path is correct when running tests
    for path in paths:
...
def click_info_button_delizia_eckhart(debug: "DebugLink") -> None:
