...
def apply_settings(
    ...
    """Change some settings on the device, for example label or homescreen.
...
def change_pin(client: "TrezorClient", remove: bool = False) -> "MessageType":
    """Change PIN on the device.

    Args:
        client: TrezorClient instance
        remove: Whether to remove the PIN. False mean setting a new PIN.
...
def change_wipe_code(client: "TrezorClient", remove: bool = False) -> "MessageType":
    """Change wipe-code on the device.
...
def wipe(client: "TrezorClient") -> "MessageType":
    """Wipe the device - delete all sensitive data and settings.
...
def recover(
    ...
    """Start recovery workflow asking user for specific words of mnemonic.

    Used to recover device safely even on untrusted computer.
...
def reset(
    ...
    Args:
        client: TrezorClient instance
        display_random: Display entropy generated by the device before asking for additional entropy
        ...
        backup_type: Type of backup to be made
